...
import {
  E2A12_PROTOCOL_HASH,
  E2A12_PROTOCOL_VERSION,
  assertE2A12ProtocolFrozen,
  deriveE2A12ProtocolHash,
  e2a12HeldOutCases,
  e2a12HeldOutProtocolSnapshot,
  type E2A12TopicDialogueCase
} from "./e2a12-v8-held-out-protocol";

export const E2A12_ARTIFACT_ROOT = path.join(
  process.cwd(),
  ".data",
  "e2a12-v8-held-out-canary"
);
export const E2A12_E2A11_RUN_ID =
  "e2a11_20260719230059_c2965f3b" as const;
export const E2A12_EVALUATOR_VERSION =
  "e2a12-v8-production-equivalent-runtime-canary-v1" as const;
export const E2A12_OVERLAP_POLICY_VERSION =
  "e2a12-held-out-normalized-similarity-v1" as const;
...
const RUNNER_LOCK_PATH = path.join(E2A12_ARTIFACT_ROOT, ".runner.lock");

export type E2A12Budget = {
  maximum_cases: 12;
  maximum_initial_generation_calls: 12;
  maximum_regeneration_calls: 12;
  maximum_total_generation_calls: 24;
  maximum_input_tokens: 350000;
  maximum_output_tokens: 60000;
  maximum_estimated_cost_usd: 15;
  maximum_regenerations_per_case: 1;
  provider_case_concurrency: 1;
};

type SafeArtifactPaths = ReturnType<typeof artifactPaths>;
type E2A12CaseResult = {
  case_id: string;
  case_number: number;
...
function assertArtifactSafe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a12_artifact_secret_or_hidden_reasoning_detected");
  }
}
...
export function e2a12ProtectedArtifactSnapshot() {
  const inherited = e2a11ProtectedArtifactSnapshot();
  const trackedGroups = {
  ...
  return {
    snapshot_version: "e2a12-protected-artifact-snapshot-v1",
...
export function analyzeE2A12ProtocolOverlap() {
  ...
  return {
    overlap_policy_version: E2A12_OVERLAP_POLICY_VERSION,
...
export function validateE2A12HeldOutProtocol() {
  const cases = e2a12HeldOutCases();
  ...
  const checks = {
    ...
    tenth_turn_count_2: tenth.length === 2,
    ...
    stress_variants_2:
      cases.filter((entry) => entry.held_out_stress_variant).length === 2,
    protocol_schema_valid: cases.every((entry) => Boolean(entry.dialogue_input)),
    overlap_passed: analyzeE2A12ProtocolOverlap().passed
  };
  return {
    protocol_version: E2A12_PROTOCOL_VERSION,
    derived_protocol_hash: deriveE2A12ProtocolHash(),
    frozen_protocol_hash: E2A12_PROTOCOL_HASH,
    checks,
...
function positiveInteger(
  ...
  if (!Number.isInteger(value) || value <= 0 || value > cap) {
    throw new Error(`e2a12_invalid_budget:${name}`);
  }
  return value;
}
...
export function resolveE2A12Budget(
  env: Readonly<Record<string, string | undefined>> = process.env
): E2A12Budget {
  return {
    maximum_cases: positiveInteger(
      env, "EVAL_E2A12_MAX_CASES", 12, 12
    ) as 12,
    maximum_initial_generation_calls: positiveInteger(
      env, "EVAL_E2A12_MAX_INITIAL_CALLS", 12, 12
    ) as 12,
    maximum_regeneration_calls: positiveInteger(
      env, "EVAL_E2A12_MAX_REGENERATION_CALLS", 12, 12
    ) as 12,
    maximum_total_generation_calls: positiveInteger(
      env, "EVAL_E2A12_MAX_TOTAL_CALLS", 24, 24
    ) as 24,
    maximum_input_tokens: positiveInteger(
      env, "EVAL_E2A12_MAX_INPUT_TOKENS", 350_000, 350_000
    ) as 350000,
    maximum_output_tokens: positiveInteger(
      env, "EVAL_E2A12_MAX_OUTPUT_TOKENS", 60_000, 60_000
    ) as 60000,
    maximum_estimated_cost_usd: positiveNumber(
      env, "EVAL_E2A12_MAX_COST_USD", 15, 15
    ) as 15,
    maximum_regenerations_per_case: 1,
    provider_case_concurrency: 1
  };
...
function duplicateRunnerPids() {
  try {
    return execFileSync("ps", ["-axo", "pid=,command="], {
      ...
      if (!/formative-evaluation-e2a12-(?:live|preflight)/u.test(line)) return [];
...
function activeBundle() {
  if (!existsSync(ACTIVE_BUNDLE_PATH)) {
    throw new Error("e2a12_active_approval_bundle_missing");
  }
...
export async function inspectE2A12Preflight(input: {
  ...
  const protocol = validateE2A12HeldOutProtocol();
  const overlap = analyzeE2A12ProtocolOverlap();
  const protectedArtifacts = e2a12ProtectedArtifactSnapshot();
  const compilationPath = path.join(
    os.tmpdir(),
    `e2a12-compilation-${randomBytes(5).toString("hex")}.json`
  );
  const compilation = await compileE2A11CandidateRequestsNoNetwork(
    compilationPath
  );
  rmSync(compilationPath, { force: true });
  const budget = resolveE2A12Budget();
  ...
  if (duplicatePids.length > 0) blockers.push("duplicate_e2a12_runner_active");
  const root = input.artifactRoot ?? E2A12_ARTIFACT_ROOT;
  if (input.requireLiveEnvironment && existsSync(path.join(root, "latest-run.json"))) {
    blockers.push("e2a12_live_canary_already_exists");
  }
  if (input.requireLiveEnvironment && existsSync(RUNNER_LOCK_PATH)) {
    blockers.push("e2a12_runner_lock_exists");
  }
  ...
  if (input.requireLiveEnvironment) {
    if (process.env.EVAL_E2A12_LIVE_PROVIDER !== "1") {
      blockers.push("live_e2a12_opt_in_missing");
    }
...
export async function createE2A12Fixture(
  prisma: PrismaClient,
  runId: string,
  cases = e2a12HeldOutCases()
): Promise<Fixture> {
  const user = await prisma.user.create({
    data: {
      user_id: `e2a12_${runId}`,
      user_id_normalized: `e2a12_${runId}`.toLocaleLowerCase(),
      display_name: "E2A12 synthetic student",
      role: "student"
    }
  });
  const teacher = await prisma.user.create({
    data: {
      user_id: `e2a12_teacher_${runId}`,
      user_id_normalized: `e2a12_teacher_${runId}`.toLocaleLowerCase(),
      display_name: "E2A12 synthetic teacher",
      role: "teacher_researcher"
    }
  });
  const assessment = await prisma.assessment.create({
    data: {
      assessment_public_id: `e2a12_asmt_${runId}`,
      title: "E2A12 isolated held-out runtime fixture",
      status: "draft",
  ...
  const concept = await prisma.conceptUnit.create({
    data: {
      concept_unit_public_id: `e2a12_cu_${runId}`,
  ...
  for (const testCase of cases) {
    const session = await prisma.assessmentSession.create({
      data: {
        session_public_id: `e2a12_${runId}_${testCase.case_number}`,
        user_db_id: user.id,
        assessment_db_id: assessment.id,
...
export async function cleanupE2A12Fixture(
  prisma: PrismaClient,
  fixture: Fixture
) {
  ...
  await prisma.user.deleteMany({
    where: { user_id: { in: [
      `e2a12_${fixture.run_id}`,
      `e2a12_teacher_${fixture.run_id}`
    ] } }
  });
}

function requestForE2A12Case(testCase: E2A12TopicDialogueCase) {
  return requestForCase(testCase);
}

function repairInstructionsForE2A12(input: {
  testCase: E2A12TopicDialogueCase;
  ...
  if (input.testCase.selected_mode === "remain_in_dialogue") {
    if (!input.testCase.selected_operation) {
      throw new Error("e2a12_operation_repair_missing_operation");
    }
...
function estimatedInputTokens(testCase: E2A12TopicDialogueCase) {
  const request = requestForE2A12Case(testCase);
  return Math.ceil(
    `${request.instructions}\n${JSON.stringify(request.provider_input)}`.length / 3
  );
}

function aggregateAttempts(results: E2A12CaseResult[]) {
...
function assertBudgetBeforeDispatch(input: {
  budget: E2A12Budget;
  completedResults: E2A12CaseResult[];
  ...
  if (input.attemptIndex === 1 && input.completedResults.length + 1 >
    ...
    throw new Error("e2a12_case_budget_exceeded");
  }
  ...
  if (costs.length > 0 && costs.every((cost) => cost !== null)) {
    ...
    if (priorCost + reserve > input.budget.maximum_estimated_cost_usd) {
      throw new Error("e2a12_cost_budget_insufficient");
    }
  }
}
...
function artifactPaths(runDir: string) {
  return {
    ...
    protocol: path.join(runDir, "held-out-protocol.json"),
    protocolHash: path.join(runDir, "held-out-protocol.sha256"),
  ...
  const effective = await input.prisma.operationalAgentEffectiveResult.findUnique({
    where: {
      invocation_key_effective_result_version: {
        invocation_key: `e2a12:${input.fixtureSession.session_public_id}`,
...
async function executeCases(input: {
  ...
  const results: E2A12CaseResult[] = [];
  let stopReason: string | null = null;
  for (const testCase of input.cases) {
    const fixtureSession = input.fixture.sessions.get(testCase.case_id);
    if (!fixtureSession) {
      throw new Error(`e2a12_fixture_session_missing:${testCase.case_id}`);
    }
    const request = requestForE2A12Case(testCase);
    ...
    try {
      const execution = await executeTopicDialogueCandidateRuntimeV2({
        ...
        invocation_key: `e2a12:${fixtureSession.session_public_id}`,
        ...
        invoke_provider: async ({ attempt_index, prior_validation }) => {
          ...
          >({
            ...
            metadata: {
              evaluation: "e2a12_v8_held_out_runtime_canary",
              case_id: testCase.case_id,
              ...
              protocol_hash_prefix: E2A12_PROTOCOL_HASH.slice(0, 12)
            }
          });
      ...
      const result: E2A12CaseResult = {
        ...
        audit_projection_passed:
          ...
          execution.audit_projection.protocol_hash === E2A12_PROTOCOL_HASH,
      ...
      stopReason = error instanceof Error
        ? error.message
        : `e2a12_case_execution_failed:${testCase.case_id}`;
...
function hardEvidenceValid(result: E2A12CaseResult) {
...
function allHardReasons(results: E2A12CaseResult[]) {
...
function buildSummary(input: {
  results: E2A12CaseResult[];
  stopReason: string | null;
  budget: E2A12Budget;
  protectedBefore: ReturnType<typeof e2a12ProtectedArtifactSnapshot>;
  protectedAfter: ReturnType<typeof e2a12ProtectedArtifactSnapshot>;
  ...
  const baseGate = input.results.length === 12 &&
    initialAttempts.length === 12 &&
  ...
  const finalStatus = incomplete
    ? "v8_canary_incomplete" as const
    : baseGate && regenerationCount <= 2
      ? "v8_canary_automated_pass_pending_human_review" as const
      : baseGate && regenerationCount > 2
        ? "v8_canary_failed_stability_threshold" as const
        : "v8_canary_failed" as const;
  const findingCount = (dimension: string) => attempts.filter((attempt) =>
    attempt.pedagogical_rubric.some((flag) => flag.dimension === dimension)
  ).length;
  return {
    summary_version: "e2a12-v8-held-out-runtime-canary-summary-v1",
    final_status: finalStatus,
    automated_canary_passed:
      finalStatus === "v8_canary_automated_pass_pending_human_review",
...
function buildHumanReviewPacket(input: {
  ...
  return {
    packet_version: "e2a12-v8-held-out-human-review-packet-v1",
    review_target: "v8_fresh_held_out_runtime_effective_output",
    protocol_hash: E2A12_PROTOCOL_HASH,
    candidate_hash: E2A11_CANDIDATE_HASH,
...
export function validateE2A12Artifacts(runDir: string) {
...
function newRunId() {
  const timestamp = new Date().toISOString().replace(/[-:TZ.]/gu, "")
    .slice(0, 14);
  return `e2a12_${timestamp}_${randomBytes(4).toString("hex")}`;
}

function acquireRunnerLock(live: boolean, runId: string) {
  if (!live) return () => undefined;
  mkdirSync(E2A12_ARTIFACT_ROOT, { recursive: true });
...
export async function executeE2A12Canary(input: {
  live: boolean;
  provider?: LlmProvider;
  artifactRoot?: string;
  runId?: string;
  ...
  const preflight = await inspectE2A12Preflight({
    ...
    requireCleanTrackedTree: input.live && !input.skipCleanTreeForTest,
    artifactRoot: input.artifactRoot
  });
  if (!preflight.passed) {
    throw new Error(`e2a12_preflight_failed:${preflight.blockers.join(",")}`);
  }
  if (input.live) assertE2A12ProtocolFrozen();
  const runId = input.runId ?? newRunId();
  const root = input.artifactRoot ?? E2A12_ARTIFACT_ROOT;
  const runDir = path.join(root, runId);
  if (existsSync(runDir)) throw new Error("e2a12_run_already_exists");
  mkdirSync(runDir, { recursive: true });
  const releaseLock = acquireRunnerLock(input.live, runId);
  const paths = artifactPaths(runDir);
  const protectedBefore = e2a12ProtectedArtifactSnapshot();
  const buildInfo = resolveApplicationBuildInfo(input.live ? {} : {
    artifactPath: path.join(
      os.tmpdir(),
      `e2a12-no-live-build-info-${randomBytes(4).toString("hex")}.json`
    ),
  ...
  const manifestBase = {
    manifest_version: "e2a12-v8-held-out-runtime-canary-manifest-v1",
    run_id: runId,
    ...
    evaluator_version: E2A12_EVALUATOR_VERSION,
    approved_v2_hash: candidate.approved_v2_hash,
    failed_v4_hash: E2A5_FAILED_V4_HASH,
    failed_v5_hash: E2A6_CANDIDATE_HASH,
    failed_v6_hash: E2A7_CANDIDATE_HASH,
  ...
  writeJson(paths.protocol, e2a12HeldOutProtocolSnapshot());
  writeFileSync(paths.protocolHash, `${E2A12_PROTOCOL_HASH}\n`, "utf8");
  ...
  if (!compilation.artifact.all_17_roles_compile ||
    compilation.artifact.network_request_count !== 0) {
    releaseLock();
    throw new Error("e2a12_request_compilation_gate_failed");
  }
  ...
  try {
    fixture = await createE2A12Fixture(prisma, runId, cases);
    ...
    if (fixture) {
      try {
        await cleanupE2A12Fixture(prisma, fixture);
  ...
  const protectedAfter = e2a12ProtectedArtifactSnapshot();
  ...
  const artifactValidation = validateE2A12Artifacts(runDir);
  if (!artifactValidation.passed) {
    throw new Error("e2a12_artifact_validation_failed");
  }
...
export async function executeLiveE2A12Canary() {
  ...
  return withResolvedOpenAICredential(
    ...
    () => executeE2A12Canary({ live: true })
  );
}

export function loadE2A12Canary(
  runId?: string,
  artifactRoot = E2A12_ARTIFACT_ROOT
) {
  const latestPath = path.join(artifactRoot, "latest-run.json");
  if (!runId && !existsSync(latestPath)) {
    throw new Error("e2a12_latest_run_missing");
  }
  const latest = runId ? {
    run_id: runId,
  ...
  if (!existsSync(runDir)) throw new Error("e2a12_run_missing");
  return {
    latest,
    ...
    artifact_validation: validateE2A12Artifacts(runDir)
  };
}

export function temporaryE2A12ArtifactRoot() {
  return path.join(
    os.tmpdir(),
    `e2a12-v8-canary-${randomBytes(5).toString("hex")}`
  );
}
