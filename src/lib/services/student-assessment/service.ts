...
function isCompletedAssessmentSession(session: {
  status: string;
  current_phase: string;
  completed_at: Date | null;
}) {
    ...
    run: async () => {
      ...
      const structuredPayload = {
        ...
        previous_response: { selected_option: response.selected_option, reasoning_text: response.reasoning_text, confidence_rating: response.confidence_rating,
          no_tempting_option: previousTemptingEvidence?.no_tempting_option ?? null,
...
export async function endStudentAssessmentAttempt(input: {
  student_user_db_id: string;
  session_public_id: string;
  reason?: string | null;
}) {
