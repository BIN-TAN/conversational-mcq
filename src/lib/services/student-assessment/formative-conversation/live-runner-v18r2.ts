...
Return outcome=continue_conversation with profile_transition_recommendation=null when you are not
recommending a validated profile change and another formative student turn remains available.
Observation evidence belongs in evidence_observations and remains non-authoritative; do not copy
...
For a terminal transition, every evidence ID used by a field, claim disposition, or transition
rationale must also appear in profile_transition_recommendation.canonical_evidence_ids. That field
means only canonical student evidence supporting the proposed transition; it is not a generic log of
evidence noticed during the turn. Evidence observations remain separately persisted. Use only IDs from
the supplied eligible catalog and keep all references within this conversation's evidence scope.
Free-text evidence descriptions do not substitute for IDs.

The top-level outcome and profile_transition_recommendation.proposed_outcome must agree for terminal
states. The profile_transition_recommendation is the authoritative profile change record.
...
export const FORMATIVE_CONVERSATION_V18R2_PROMPT_HASH = createHash("sha256")
  .update(FORMATIVE_CONVERSATION_V18R2_INSTRUCTIONS)
  .digest("hex");
...
function logicalExecutionFromTransport(input: {
  ...
  const result = input.transport.last_result as StructuredAgentResult<FormativeConversationV18R2AgentOutput> | null;
  if (!result) {
    return {
...
export function buildFormativeConversationV18R2ProductionRequest(input: {
...
export function createLiveFormativeConversationV18R2AgentRunner(): FormativeConversationV18R2AgentRunner {
  try {
    ...
    return {
      identity: {
        agent_name: FORMATIVE_CONVERSATION_AGENT_NAME,
        agent_version: "formative-conversation-runtime-v4",
      ...
      async execute({ invocation_key, context }) {
        ...
        if (!accepted.valid || !accepted.output) {
          throw new Error(
            "formative_conversation_v18r2_accepted_candidate_projection_invalid"
          );
        }
