...
export function buildFormativeConversationV5RuntimeInputs(
  ...
  if (!firstCompiledCase) {
    throw new Error(
      "formative_conversation_v5_v3_compiled_cases_missing"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v5_v3_runtime_input_compilation_failed"
    );
  }
  return {
    subjects,
...
async function assertLivePreflight(
  ...
  if (
    process.env
      .FORMATIVE_CONVERSATION_V5_V3_LIVE_EVALUATION_ENABLED !==
    "true"
  ) {
    throw new Error(
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v3_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v3_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v3_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
      await writeJson(failurePath, {
        artifact_version:
          "formative-conversation-v5-v3-predispatch-failure-v1",
        runtime_candidate_hash: loaded.runtime_candidate_hash,
        evaluation_protocol_hash: loaded.protocol_hash,
        compiled_plan_hash:
          loaded.compiled_plan.compiled_plan_hash,
        ...
        failure_code:
          error instanceof Error
            ...
            : "formative_conversation_v5_v3_predispatch_failed",
