...
export const FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V5_LIVE_EVALUATION_ENABLED",
  "FORMATIVE_CONVERSATION_V5_V5_DATABASE_CONNECTION_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V5_DATABASE_IDENTITY_MATCHED"
] as const;
...
function parseInjectedEnvironmentKeys(env: NodeJS.ProcessEnv) {
  const value =
    env.FORMATIVE_CONVERSATION_V5_V5_INJECTED_ENVIRONMENT_KEYS;
  assertCondition(
    configured(value),
...
function requireInjectedEnvironment(env: NodeJS.ProcessEnv) {
  const injected = parseInjectedEnvironmentKeys(env);
  for (const name of FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT) {
    assertCondition(
...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const { env } = input;
  const injected = requireInjectedEnvironment(env);
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V5_ENVIRONMENT_SOURCE;
  ...
  requireExactString(
    env,
    "FORMATIVE_CONVERSATION_V5_V5_LAUNCHER_VERSION",
    FORMATIVE_CONVERSATION_V5_LAUNCHER_VERSION
  );
  requireExactString(
    env,
    "FORMATIVE_CONVERSATION_V5_V5_LAUNCH_MECHANISM",
    FORMATIVE_CONVERSATION_V5_LAUNCH_MECHANISM
  );
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V5_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  const parsedEnv = safeParseServerEnv(env);
  assertCondition(
    parsedEnv.success,
    "formative_conversation_v5_server_environment_invalid"
  );
  const serverEnv = parsedEnv.data;
  ...
  assertCondition(
    serverEnv.OPERATIONAL_AGENT_INTEGRATION_EVAL_EVIDENCE_REQUIRED ===
      true,
    "formative_conversation_v5_evaluation_evidence_requirement_disabled"
  );
  requireExactBoolean(
    env,
    "FORMATIVE_CONVERSATION_V5_V5_LIVE_EVALUATION_ENABLED",
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V5_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
    for (const sourceName of [
      "FORMATIVE_CONVERSATION_V5_V5_SOURCE_APPROVAL_BUNDLE_PATH",
      "FORMATIVE_CONVERSATION_V5_V5_SOURCE_APPROVED_MANIFEST_PATH",
      "FORMATIVE_CONVERSATION_V5_V5_SOURCE_APPROVAL_EVIDENCE_PATH"
    ]) {
      assertCondition(
  ...
  const safeSnapshot = {
    ...
    required_environment_key_count:
      FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT.length,
    ...
    rollback_runtime_hash:
      active.record.rollback.approved_runtime_hash,
    runtime_assertions: {
      provider: serverEnv.LLM_PROVIDER,
      live_calls_enabled: serverEnv.LLM_LIVE_CALLS_ENABLED,
      operational_agent_mode: serverEnv.OPERATIONAL_AGENT_MODE,
      ...
      live_evaluation_gate_enabled: true
    },
    secret_presence: {
      database_url: true,
      session_secret: true,
      openai_credential: true,
