...
export const FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V6_LIVE_EVALUATION_ENABLED",
...
function parseInjectedEnvironmentKeys(env: NodeJS.ProcessEnv) {
  const value =
    env.FORMATIVE_CONVERSATION_V5_V6_INJECTED_ENVIRONMENT_KEYS;
  assertCondition(
    configured(value),
...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V6_ENVIRONMENT_SOURCE;
  ...
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V6_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V6_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
