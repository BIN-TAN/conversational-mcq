...
async function writeLiveArtifacts(input: {
  ...
  const configuredStagingBase =
    process.env.FORMATIVE_CONVERSATION_V5_V11_STAGING_BASE_ROOT;
  if (!configuredStagingBase) {
    throw new Error(
      "formative_conversation_v11_staging_base_not_configured"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v11_staging_base_invalid"
    );
  }
  ...
  await write("persistence-diagnostics.json", {
    artifact_version:
      "formative-conversation-v11-persistence-diagnostics-v1",
    provider_run_id: input.provider_run_id,
  ...
  const finalized =
    await createFormativeConversationV11FinalizedManifest({
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v11_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v11_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v11_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
      await writeJson(failurePath, {
        artifact_version:
          "formative-conversation-v5-v11-predispatch-failure-v1",
        runtime_candidate_hash: loaded.runtime_candidate_hash,
        evaluation_protocol_hash: loaded.protocol_hash,
        compiled_plan_hash:
          loaded.compiled_plan.compiled_plan_hash,
