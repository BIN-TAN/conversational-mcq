...
export const FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V11_LIVE_EVALUATION_ENABLED",
  "FORMATIVE_CONVERSATION_V5_V11_DATABASE_CONNECTION_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V11_DATABASE_IDENTITY_MATCHED"
] as const;

export const FORMATIVE_CONVERSATION_V11_DATABASE_CANARY_REQUIRED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V11_CANARY_SESSION_SECRET_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V11_REMOTE_DATABASE_CANARY_ENABLED"
] as const;
...
function parseInjectedEnvironmentKeys(env: NodeJS.ProcessEnv) {
  const value =
    env.FORMATIVE_CONVERSATION_V5_V11_INJECTED_ENVIRONMENT_KEYS;
  assertCondition(
    configured(value),
...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const requiredEnvironment = noProviderDatabaseCanary
    ? FORMATIVE_CONVERSATION_V11_DATABASE_CANARY_REQUIRED_ENVIRONMENT
  ...
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V11_ENVIRONMENT_SOURCE;
  ...
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V11_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  if (noProviderDatabaseCanary) {
    ...
    assertCondition(
      ...
      "formative_conversation_v11_canary_provider_credential_injected"
    );
    requireExactString(
      env,
      "FORMATIVE_CONVERSATION_V5_V11_CANARY_SESSION_SECRET_SOURCE",
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V11_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
