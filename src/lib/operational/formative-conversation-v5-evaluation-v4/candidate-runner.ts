...
class EvaluationBudgetLedger {
  ...
  beginLogicalCall(context: FormativeConversationAgentInput) {
    const reservation = this.assertCanBeginLogicalCall(context);
...
export function createFormativeConversationV5CandidateRunner(input: {
  ...
  let firstGenerationRequestAuthorized = false;
  let firstGenerationRequestAuthorization:
    | Promise<void>
    | null = null;
  const authorizeFirstGenerationRequest = async () => {
    if (firstGenerationRequestAuthorized) {
      return;
    }
    firstGenerationRequestAuthorization ??=
      input.before_first_generation_request();
    await firstGenerationRequestAuthorization;
    firstGenerationRequestAuthorized = true;
  };
  ...
  const runner: FormativeConversationAgentRunner = {
    ...
    async execute({ invocation_key, context }) {
      budgetLedger.assertCanBeginLogicalCall(context);
      await authorizeFirstGenerationRequest();
      budgetLedger.beginLogicalCall(context);
      ...
      > = {
        ...
        schema_name:
          FORMATIVE_CONVERSATION_AGENT_CONTRACT_VERSION,
        client_request_id: `${logicalCallId}:request`,
        timeout_ms:
      ...
      try {
        ...
        const result = transport.accepted_result;
        ...
        const firstAttempt = transport.attempt_traces[0];
        const lastAttempt = transport.attempt_traces.at(-1);
        const startedAt = firstAttempt
          ? new Date(firstAttempt.started_at)
          : new Date();
        const completedAt = lastAttempt
        ...
        return {
          ...
          retry_count: transport.transport_retry_count,
  ...
  return {
    ...
    dispatch_state: () => ({
      first_generation_request_authorized:
        firstGenerationRequestAuthorized
    }),
    complete() {
