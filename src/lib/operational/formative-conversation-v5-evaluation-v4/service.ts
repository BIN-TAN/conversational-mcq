...
export function buildFormativeConversationV5RuntimeInputs(
  ...
  if (!firstCompiledCase) {
    throw new Error(
      "formative_conversation_v5_v4_compiled_cases_missing"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v5_v4_runtime_input_compilation_failed"
    );
  }
  return {
    subjects,
...
export async function compileFormativeConversationV5PreDispatch() {
  ...
  const exportReadiness = await getResearchExportReadiness();
  if (
...
async function assertLivePreflight(
  ...
  if (
    process.env
      .FORMATIVE_CONVERSATION_V5_V4_LIVE_EVALUATION_ENABLED !==
    "true"
  ) {
    throw new Error(
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const preDispatch = await assertLivePreflight(options);
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v4_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v4_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v4_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
      await writeJson(failurePath, {
        artifact_version:
          "formative-conversation-v5-v4-predispatch-failure-v1",
        runtime_candidate_hash: loaded.runtime_candidate_hash,
        evaluation_protocol_hash: loaded.protocol_hash,
        compiled_plan_hash:
          loaded.compiled_plan.compiled_plan_hash,
        ...
        failure_code:
          error instanceof Error
            ...
            : "formative_conversation_v5_v4_predispatch_failed",
