...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V7_ENVIRONMENT_SOURCE;
  ...
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V7_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V7_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
    for (const sourceName of [
      "FORMATIVE_CONVERSATION_V5_V7_SOURCE_APPROVAL_BUNDLE_PATH",
      "FORMATIVE_CONVERSATION_V5_V7_SOURCE_APPROVED_MANIFEST_PATH",
      "FORMATIVE_CONVERSATION_V5_V7_SOURCE_APPROVAL_EVIDENCE_PATH"
    ]) {
      assertCondition(
