...
const CandidateIdentitySchema = z
  .object({
    artifact_version: z.literal(
      "formative-conversation-v5-executable-candidate-identity-v1"
    ),
...
const SourceConfigurationSchema = z
  .object({
    artifact_version: z.literal(
      "formative-conversation-v5-executable-source-configuration-v1"
    ),
    captured_from_application_git_commit: z
      .string()
      .regex(/^[a-f0-9]{40}$/),
    ...
    required_environment: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])),
...
export function assertFormativeConversationV5Isolation(
  ...
  assertCondition(
    protocol.isolation.classroom_collision_forbidden &&
      ...
      new Set(
        fixtures.map((fixture) => fixture.execution_persona_id)
...
function verifyFixtureCallGraph(fixture: FormativeConversationV5Fixture) {
  ...
  assertCondition(
    fixture.call_graph.student_message_count === fixture.student_messages.length,
...
export function loadFormativeConversationV5EvaluationPackage() {
  ...
  const sourceConfiguration = SourceConfigurationSchema.parse(
    readJson(FORMATIVE_CONVERSATION_V5_SOURCE_CONFIGURATION_PATH)
  );
  const candidateIdentity = CandidateIdentitySchema.parse(
...
export function buildFormativeConversationV5EvaluationPlan() {
  ...
  return {
    plan_version: "formative-conversation-v5-evaluation-plan-v1",
    mode: "plan",
    provider_calls: 0,
    network_requests: 0,
    candidate: {
    ...
    fixtures: {
      ...
      ordered_cases: loaded.fixtures.map((fixture) => ({
        case_id: fixture.case_id,
        fixture_hash: fixture.fixture_hash,
        ...
        student_message_count: fixture.student_messages.length,
        call_graph: fixture.call_graph.logical_calls
      }))
    },
    ...
    required_live_command: [
      ...
      "npm run operational:formative-conversation-v5-evaluate --",
      "--mode=live",
