...
import {
  FormativeConversationV18R2PersistedProfileSnapshotSchema
...
import {
  createFormativeConversationV18R2FinalizedManifest as createFormativeConversationV18FinalizedManifest,
  FORMATIVE_CONVERSATION_V18R2_ATTESTATION_FILENAME as FORMATIVE_CONVERSATION_V18_ATTESTATION_FILENAME,
  FORMATIVE_CONVERSATION_V18R2_MANIFEST_FILENAME as FORMATIVE_CONVERSATION_V18_MANIFEST_FILENAME,
  FORMATIVE_CONVERSATION_V18R2_STAGING_ROOT as FORMATIVE_CONVERSATION_V18_STAGING_ROOT
} from "./security-release";
import {
  writeFormativeConversationV18R2DispatchCheckpoint as writeFormativeConversationV18DispatchCheckpoint
...
type FormativeConversationV5PreDispatchDependencies = {
  ...
  verify_deployed_provenance?: () => ReturnType<
    typeof verifyFormativeConversationV18R2DeployedProvenance
  >;
};
...
): FormativeConversationV18R2DeployedIdentity {
...
function verifyCurrentFormativeConversationV18R2DeployedSource(input: {
  loaded: ReturnType<typeof loadFormativeConversationV18EvaluationPackage>;
  expected_deployed_git_sha: string;
}) {
  const identity = deployedIdentity(input.loaded);
  return verifyFormativeConversationV18R2DeployedProvenance({
    mode:
      process.env.FORMATIVE_CONVERSATION_V5_V18R2_PROVENANCE_MODE ?? "",
...
export async function compileFormativeConversationV5PreDispatch(
  ...
  const committedSource = dependencies.verify_deployed_provenance
    ...
    : verifyCurrentFormativeConversationV18R2DeployedSource({
        loaded,
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v18r2_live_execution_not_prepared"
    );
  }
...
export async function writeFormativeConversationV5PlanArtifact() {
  ...
  const localSource =
    verifyFormativeConversationV18R2LocalCommittedSource({
      source_paths: FORMATIVE_CONVERSATION_V18R2_COMMITTED_SOURCE_PATHS
    });
  const plan = buildFormativeConversationV18EvaluationPlan();
...
async function caseEvidenceArtifact(input: {
  ...
  const endToEndSnapshot = endToEndTransition
    ? FormativeConversationV18R2PersistedProfileSnapshotSchema.safeParse(
...
async function writeLiveArtifacts(input: {
  ...
  >;
  ledger: ReturnType<
    typeof createFormativeConversationV18R2CandidateRunner
  >["ledger"];
  ...
  const configuredStagingBase =
    process.env.FORMATIVE_CONVERSATION_V5_V18R2_STAGING_BASE_ROOT;
  ...
  await write("provenance-manifest.json", {
    artifact_version:
      "formative-conversation-v18r2-deployed-provenance-manifest-v1",
    source_provider_run_id: input.provider_run_id,
    derived_evaluation_id: input.derived_evaluation_id,
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v18r2_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v18r2_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
        ...
        identity: {
          ...
          execution_authorization_identity_hash: stableHash({
            authorization_version:
              "formative-conversation-v18r2-exact-live-authorization-v1",
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v18r2_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
