...
export const FORMATIVE_CONVERSATION_V18R2_DEPLOYED_PROVENANCE_VERSION =
  "deployment-source-provenance-v1";
export const FORMATIVE_CONVERSATION_V18R2_DEPLOYED_PROVENANCE_MODE =
  "render_deployed_artifact";

const Sha256 = /^[a-f0-9]{64}$/u;
const GitSha = /^[a-f0-9]{40}$/u;

export type FormativeConversationV18R2DeployedIdentity = {
...
] as const satisfies readonly (keyof FormativeConversationV18R2DeployedIdentity)[];

export function verifyFormativeConversationV18R2DeployedProvenance(input: {
  mode: string;
  render_git_commit?: string | null;
  operator_authorized_git_sha?: string | null;
  expected_identity: FormativeConversationV18R2DeployedIdentity;
  deployed_identity: FormativeConversationV18R2DeployedIdentity;
}) {
  if (input.mode !== FORMATIVE_CONVERSATION_V18R2_DEPLOYED_PROVENANCE_MODE) {
    throw new Error("formative_conversation_v18r2_deployment_mode_unsupported");
  }
  ...
  if (!deploymentReported) {
    throw new Error("formative_conversation_v18r2_render_git_commit_missing");
  }
  if (!GitSha.test(deploymentReported)) {
    throw new Error("formative_conversation_v18r2_render_git_commit_malformed");
  }
  if (!operatorAuthorized) {
    throw new Error("formative_conversation_v18r2_expected_git_sha_missing");
  }
  if (!GitSha.test(operatorAuthorized)) {
    throw new Error("formative_conversation_v18r2_expected_git_sha_malformed");
  }
  if (deploymentReported !== operatorAuthorized) {
    throw new Error("formative_conversation_v18r2_deployed_git_sha_mismatch");
  }
  for (const field of identityFields) {
    if (
      ...
      throw new Error(`formative_conversation_v18r2_deployed_identity_mismatch:${field}`);
    }
  }
  const hashable = {
    version: FORMATIVE_CONVERSATION_V18R2_DEPLOYED_PROVENANCE_VERSION,
    mode: FORMATIVE_CONVERSATION_V18R2_DEPLOYED_PROVENANCE_MODE,
