...
export const FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V13_LIVE_EVALUATION_ENABLED",
...
export const FORMATIVE_CONVERSATION_V13_DATABASE_CANARY_REQUIRED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V13_CANARY_SESSION_SECRET_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V13_REMOTE_DATABASE_CANARY_ENABLED"
] as const;
...
function parseInjectedEnvironmentKeys(env: NodeJS.ProcessEnv) {
  const value =
    env.FORMATIVE_CONVERSATION_V5_V13_INJECTED_ENVIRONMENT_KEYS;
  assertCondition(
    configured(value),
...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const requiredEnvironment = noProviderDatabaseCanary
    ? FORMATIVE_CONVERSATION_V13_DATABASE_CANARY_REQUIRED_ENVIRONMENT
  ...
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V13_ENVIRONMENT_SOURCE;
  ...
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V13_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  if (noProviderDatabaseCanary) {
    ...
    assertCondition(
      ...
      "formative_conversation_v13_canary_provider_credential_injected"
    );
    requireExactString(
      env,
      "FORMATIVE_CONVERSATION_V5_V13_CANARY_SESSION_SECRET_SOURCE",
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V13_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
