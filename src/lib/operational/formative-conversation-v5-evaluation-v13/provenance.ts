...
export const FORMATIVE_CONVERSATION_V13_RUN_PROVENANCE_VERSION =
...
const DEFAULT_COMMITTED_SOURCE_PATHS = [
  FORMATIVE_CONVERSATION_V5_EVALUATION_ROOT,
  "src/lib/operational/formative-conversation-v5-evaluation-v13",
  "prisma/operational-formative-conversation-v5-v13-evaluate.ts",
  "prisma/operational-formative-conversation-v5-v13-fixture-materialize.ts",
...
export type FormativeConversationV13CommittedSourceVerification = {
  version: typeof FORMATIVE_CONVERSATION_V13_RUN_PROVENANCE_VERSION;
...
export function verifyFormativeConversationV13CommittedSource(input?: {
  ...
  if (!SHA40.test(sourceCommit)) {
    throw new Error("formative_conversation_v13_source_commit_invalid");
  }
  const sourcePaths = input?.source_paths ?? DEFAULT_COMMITTED_SOURCE_PATHS;
  const files = packageFiles(workspaceRoot, sourcePaths);
  if (files.length === 0) {
    throw new Error("formative_conversation_v13_tracked_package_missing");
  }
  const verifiedFiles = files.map((relativePath) => {
    ...
    if (!current.equals(committed)) {
      throw new Error(
        `formative_conversation_v13_tracked_source_changed:${relativePath}`
      );
    }
  ...
  if (deployedCommit !== null && !SHA40.test(deployedCommit)) {
    throw new Error("formative_conversation_v13_deployed_commit_invalid");
  }
  const payload = {
    version: FORMATIVE_CONVERSATION_V13_RUN_PROVENANCE_VERSION,
  ...
  } satisfies FormativeConversationV13CommittedSourceVerification;
}

export function writeFormativeConversationV13RunScopedProvenance(input?: {
  workspace_root?: string;
  deployed_commit?: string | null;
  source_paths?: readonly string[];
}) {
  const workspaceRoot = path.resolve(input?.workspace_root ?? process.cwd());
  const provenance = verifyFormativeConversationV13CommittedSource(input);
  ...
  const outputPath = path.join(
    provenanceRoot,
    `v13-source-${Date.now()}-${randomUUID().slice(0, 8)}.json`
  );
