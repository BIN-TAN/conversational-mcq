...
} from "../formative-conversation-v5-evaluation-v18/candidate-runner";
import { summarizeFormativeConversationV18EvaluationAccounting } from "../formative-conversation-v5-evaluation-v18/evaluation-accounting";
import {
...
import {
  createFormativeConversationV18R1FinalizedManifest as createFormativeConversationV18FinalizedManifest,
  FORMATIVE_CONVERSATION_V18R1_ATTESTATION_FILENAME as FORMATIVE_CONVERSATION_V18_ATTESTATION_FILENAME,
  FORMATIVE_CONVERSATION_V18R1_MANIFEST_FILENAME as FORMATIVE_CONVERSATION_V18_MANIFEST_FILENAME,
  FORMATIVE_CONVERSATION_V18R1_STAGING_ROOT as FORMATIVE_CONVERSATION_V18_STAGING_ROOT
} from "./security-release";
import {
  writeFormativeConversationV18R1DispatchCheckpoint as writeFormativeConversationV18DispatchCheckpoint
...
type FormativeConversationV5PreDispatchDependencies = {
  ...
  verify_deployed_provenance?: () => ReturnType<
...
function deployedIdentity(
  loaded: ReturnType<typeof loadFormativeConversationV18EvaluationPackage>
): FormativeConversationV18R1DeployedIdentity {
  return loaded.deployed_artifact_identity;
}

function frozenIdentity(
  loaded: ReturnType<typeof loadFormativeConversationV18EvaluationPackage>
): FormativeConversationV18R1DeployedIdentity {
  const identity = loaded.candidate_identity as Record<string, unknown>;
  return {
    ...
    deployed_source_closure_hash: String(
      identity.deployed_source_closure_hash ?? ""
    ),
    security_wrapper_hash: loaded.security_wrapper_hash,
    formative_prompt_hash: String(identity.formative_prompt_hash ?? ""),
    profiling_prompt_hash: String(identity.profiling_prompt_hash ?? ""),
    canonical_evidence_identity_implementation_hash: String(
      identity.canonical_evidence_identity_implementation_hash ?? ""
    ),
    misconception_claim_identity_implementation_hash: String(
      identity.misconception_claim_identity_implementation_hash ?? ""
    )
  };
}

function verifyCurrentFormativeConversationV18R1DeployedSource(input: {
  loaded: ReturnType<typeof loadFormativeConversationV18EvaluationPackage>;
  expected_deployed_git_sha: string;
}) {
  const identity = deployedIdentity(input.loaded);
  return verifyFormativeConversationV18R1DeployedProvenance({
    mode:
      process.env.FORMATIVE_CONVERSATION_V5_V18R1_PROVENANCE_MODE ?? "",
    render_git_commit: process.env.RENDER_GIT_COMMIT,
    operator_authorized_git_sha: input.expected_deployed_git_sha,
    expected_identity: frozenIdentity(input.loaded),
...
export async function compileFormativeConversationV5PreDispatch(
  ...
  options?: { expected_deployed_git_sha?: string }
) {
  ...
  const committedSource = dependencies.verify_deployed_provenance
    ? dependencies.verify_deployed_provenance()
    : verifyCurrentFormativeConversationV18R1DeployedSource({
        loaded,
        expected_deployed_git_sha:
          options?.expected_deployed_git_sha ?? ""
      });
  if (
    loaded.candidate_manifest.live_execution_prepared !== true ||
    loaded.protocol.live_execution_prepared !== true ||
    loaded.authorization_package.live_execution_prepared !== true
  ) {
    throw new Error(
      "formative_conversation_v18r1_live_execution_not_prepared"
    );
  }
  ...
  return {
    ...
    readiness: {
      ...
      deployed_provenance: {
        source_commit_sha: committedSource.source_commit_sha,
        deployment_reported_commit_sha:
          committedSource.deployment_reported_commit_sha,
        operator_authorized_commit_sha:
          committedSource.operator_authorized_commit_sha,
        deployed_artifact_identity_status:
          committedSource.deployed_artifact_identity_status,
        provenance_hash: committedSource.provenance_hash,
        git_invocations: committedSource.git_invocations
      },
      live_environment: liveEnvironment
    },
    dispatch_boundary: {
...
export async function writeFormativeConversationV5PlanArtifact() {
  ...
  const localSource =
    verifyFormativeConversationV18R1LocalCommittedSource({
      source_paths: FORMATIVE_CONVERSATION_V18R1_COMMITTED_SOURCE_PATHS
    });
  const plan = buildFormativeConversationV18EvaluationPlan();
  ...
  const artifact = {
    ...
    runtime_input_compilation: {
      status: "compiled",
      case_count: environmentPreflight.runtime_inputs.subjects.length,
      execution_subject_ids:
        environmentPreflight.runtime_inputs.subjects.map(
          (subject) => subject.subject_id
        ),
      assessment_item_count:
        environmentPreflight.runtime_inputs.assessment_definition
          .administered_items.length,
      frozen_profile_count: Object.keys(
        environmentPreflight.runtime_inputs.frozen_initial_profiles
      ).length,
      production_profiling_called:
        false,
      production_profiling_subject_count:
        environmentPreflight.runtime_inputs.production_profiling_subject_ids.length,
      profiling_contract_canary_count: 3,
      profiling_contract_canaries_execute_real_provider_calls_in_live_mode:
        true,
      runtime_input_hash:
        environmentPreflight.runtime_inputs.runtime_input_hash
    },
    pre_deployment_validation: {
      local_committed_source: localSource,
      live_environment: environmentPreflight.live_environment
    },
    dispatch_boundary: {
      status: "plan_only_stopped_before_deployed_provenance",
      checkpoint_created: false,
      provider_calls: 0,
      provider_auth_network_requests: 0,
      database_writes: 0
...
export async function assertFormativeConversationV5LivePreflight(
  ...
  const preDispatch = await compileFormativeConversationV5PreDispatch(
    dependencies,
    { expected_deployed_git_sha: options.expected_deployed_git_sha }
  );
...
async function writeLiveArtifacts(input: {
  ...
  const configuredStagingBase =
    process.env.FORMATIVE_CONVERSATION_V5_V18R1_STAGING_BASE_ROOT;
  ...
  await write("provenance-manifest.json", {
    artifact_version:
      "formative-conversation-v18r1-deployed-provenance-manifest-v1",
    source_provider_run_id: input.provider_run_id,
    derived_evaluation_id: input.derived_evaluation_id,
    ...
    source_commit_sha: input.deployed_provenance.source_commit_sha,
    deployment_reported_commit_sha:
      input.deployed_provenance.deployment_reported_commit_sha,
    operator_authorized_commit_sha:
      input.deployed_provenance.operator_authorized_commit_sha,
    deployed_artifact_provenance_hash:
      input.deployed_provenance.provenance_hash,
    deployed_artifact_identity_status:
      input.deployed_provenance.deployed_artifact_identity_status
  });
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v18r1_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v18r1_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
    ...
    await writeFormativeConversationV18DispatchCheckpoint({
        ...
        identity: {
          ...
          provenance_contract_hash:
            loaded.provenance_contract_hash,
          source_commit_sha:
            preDispatch.readiness.deployed_provenance.source_commit_sha,
          deployment_reported_commit_sha:
            preDispatch.readiness.deployed_provenance
              .deployment_reported_commit_sha,
          operator_authorized_commit_sha:
            preDispatch.readiness.deployed_provenance
              .operator_authorized_commit_sha,
          deployed_artifact_identity_status:
            preDispatch.readiness.deployed_provenance
              .deployed_artifact_identity_status,
          deployed_artifact_provenance_hash:
            preDispatch.readiness.deployed_provenance.provenance_hash,
          execution_authorization_identity_hash: stableHash({
            authorization_version:
              "formative-conversation-v18r1-exact-live-authorization-v1",
            authorization_text: options.authorization
          })
        }
      });
  ...
  try {
    ...
    const artifacts = await writeLiveArtifacts({
      ...
      deployed_provenance: preDispatch.readiness.deployed_provenance,
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v18r1_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
