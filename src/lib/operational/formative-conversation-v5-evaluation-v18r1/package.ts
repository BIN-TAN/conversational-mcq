...
} from "@/lib/operational/formative-conversation-v18r1/candidate";
...
import { FORMATIVE_CONVERSATION_V18R1_RUNNER_SOURCE_PATHS } from "./provenance";
...
export function loadFormativeConversationV18EvaluationPackage() {
  ...
  assertCondition(
    ...
    "formative_conversation_v18r1_runtime_source_identity_mismatch"
  );
  ...
  const generatedArtifacts = {
    ...
    immutable_v18_reference: FORMATIVE_CONVERSATION_V5_IMMUTABLE_V18_REFERENCE_PATH,
  ...
  for (const [name, artifactPath] of Object.entries(generatedArtifacts)) {
    assertCondition(
      ...
      `formative_conversation_v18r1_generated_artifact_hash_mismatch:${name}`
    );
  }
  ...
  assertCondition(
    runtimeManifest.runtime_candidate_hash === runtimeCandidateHash &&
      ...
      candidateIdentity.immutable_v18_reference_hash ===
      ...
      candidateManifest.source_configuration_hash ===
        sourceConfigurationHash &&
      candidateManifest.immutable_v18_reference_hash ===
        immutableV18ReferenceHash &&
      ...
      protocol.immutable_v18_reference_hash ===
        immutableV18ReferenceHash &&
  ...
  const materializerSource = {
    path: "prisma/operational-formative-conversation-v18r1-materialize.ts",
  ...
  assertCondition(
    ...
    "formative_conversation_v18r1_deployed_source_closure_mismatch"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v18r1_prompt_or_claim_identity_mismatch"
  );
  ...
  return {
    ...
    // The unchanged V18 candidate runner's structural package type retains
    // this historical property name. V18R1 persists only immutable-v18-reference.
