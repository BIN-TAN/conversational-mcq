...
export const FORMATIVE_CONVERSATION_V18R1_DISPATCH_CHECKPOINT_VERSION =
  "formative-conversation-v18r1-dispatch-checkpoint-v1";

export type FormativeConversationV18R1DispatchIdentity = {
...
export async function writeFormativeConversationV18R1DispatchCheckpoint(input: {
  dispatch_root: string;
  identity: FormativeConversationV18R1DispatchIdentity;
}) {
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v18r1_dispatch_commit_identity_mismatch"
    );
  }
  ...
  const payload = {
    checkpoint_version:
      FORMATIVE_CONVERSATION_V18R1_DISPATCH_CHECKPOINT_VERSION,
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "formative_conversation_v18r1_protocol_already_dispatched"
      );
    }
    throw error;
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v18r1_dispatch_checkpoint_identity_mismatch"
    );
  }
  return {
