...
export const FORMATIVE_CONVERSATION_V18R1_LOCAL_PROVENANCE_VERSION =
  "formative-conversation-v18r1-local-committed-source-v1";
...
export function assertFormativeConversationV18R1MaterializationReproducible(
  firstHash: string,
  secondHash: string
) {
  if (
    ...
    throw new Error(
      "formative_conversation_v18r1_materialization_not_reproducible"
    );
  }
  return true as const;
}

export function verifyFormativeConversationV18R1LocalCommittedSource(input: {
  ...
  if (!/^[a-f0-9]{40}$/u.test(head) || originMain !== head) {
    throw new Error("formative_conversation_v18r1_local_head_origin_mismatch");
  }
  if (input.expected_head && input.expected_head !== head) {
    throw new Error("formative_conversation_v18r1_local_expected_head_mismatch");
  }
  ...
  if (
    ...
    throw new Error("formative_conversation_v18r1_local_source_closure_missing");
  }
  ...
  const verifiedFiles = files.map((relativePath) => {
    ...
    if (!current.equals(committedBytes)) {
      throw new Error(
        `formative_conversation_v18r1_local_source_changed:${relativePath}`
      );
    }
  ...
  if (status !== "") {
    throw new Error("formative_conversation_v18r1_local_working_tree_not_clean");
  }
  const hashable = {
    version: FORMATIVE_CONVERSATION_V18R1_LOCAL_PROVENANCE_VERSION,
