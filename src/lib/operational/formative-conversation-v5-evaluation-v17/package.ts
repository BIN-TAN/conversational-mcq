...
import { formativeConversationV17SecurityWrapperFingerprint } from "./security-release";
import {
  assertFormativeConversationV17CompilationParity,
  compileFormativeConversationV17ExecutionPlan
} from "./compiler";
import {
  ...
  FORMATIVE_CONVERSATION_V5_IMMUTABLE_V16_REFERENCE_PATH,
...
export type FormativeConversationV17CandidateConfiguration = {
  roles: {
...
export function formativeConversationV17FileSha256(relativePath: string) {
  return createHash("sha256")
    .update(readFileSync(absolute(relativePath)))
    .digest("hex");
}

export function formativeConversationV17FixtureHash(
...
export function formativeConversationV17AggregateFixtureHash(
  fixtures: readonly FormativeConversationV17Fixture[]
) {
  return stableHash(
...
export function loadFormativeConversationV17EvaluationPackage() {
  ...
  const immutableV16Reference = readJson(FORMATIVE_CONVERSATION_V5_IMMUTABLE_V16_REFERENCE_PATH);
  ...
  const committedCompiledPlan = FormativeConversationV17CompiledPlanSchema.parse(
  ...
  assertCondition(
    ...
    "formative_conversation_v17_security_wrapper_hash_mismatch"
  );
  assertEmbeddedHash(
    sourceConfiguration,
    "source_configuration_hash",
    "formative_conversation_v17_source_configuration_hash_mismatch"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v17_governance_artifact_invalid"
  );
  assertCondition(
    immutableV16Reference.source_revision ===
      "formative-conversation-host-v5-executable-v16" &&
      immutableV16Reference.local_artifact_policy ===
        "hash_reference_only_not_required_for_v17_live_readiness" &&
      immutableV16Reference.required_for_v17_live_readiness === false,
    "formative_conversation_v17_immutable_v16_reference_invalid"
  );
  const offlineReplayFixtures = immutableV16Reference.offline_replay_fixtures;
  assertCondition(
    Array.isArray(offlineReplayFixtures) && offlineReplayFixtures.length === 3,
    "formative_conversation_v17_offline_replay_inventory_invalid"
  );
  for (const entry of offlineReplayFixtures) {
    const replay = asRecord(entry);
    const replayPath = String(replay.path ?? "");
    assertCondition(
      replayPath.startsWith(`${FORMATIVE_CONVERSATION_V5_EVALUATION_ROOT}/offline-replay-fixtures/`) &&
        /^[a-f0-9]{64}$/u.test(String(replay.file_sha256 ?? "")) &&
        formativeConversationV17FileSha256(replayPath) === replay.file_sha256,
      "formative_conversation_v17_offline_replay_identity_invalid"
    );
  }

  const fixtureReferences = fixtureManifest.fixtures;
  assertCondition(
    Array.isArray(fixtureReferences) && fixtureReferences.length === 11,
    "formative_conversation_v17_fixture_inventory_invalid"
  );
  const fixtures = fixtureReferences.map((reference: JsonRecord, index: number) => {
    const fixturePath = String(reference.path ?? "");
    const fixture = FormativeConversationV17FixtureSchema.parse(readJson(fixturePath));
    assertCondition(
      fixture.case_id === FORMATIVE_CONVERSATION_V5_CASE_ORDER[index] &&
        ...
        formativeConversationV17FixtureHash(fixture) === fixture.fixture_hash &&
        formativeConversationV17FileSha256(fixturePath) === reference.file_sha256,
      `${fixture.case_id}:formative_conversation_v17_fixture_identity_mismatch`
    );
    return fixture;
  });
  const aggregateFixtureHash = formativeConversationV17AggregateFixtureHash(fixtures);
  assertCondition(
    aggregateFixtureHash === fixtureManifest.aggregate_fixture_hash &&
      fixtureManifest.fixture_manifest_hash === fixtureManifestHash,
    "formative_conversation_v17_aggregate_fixture_hash_mismatch"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v17_operational_configuration_missing"
  );
  const candidate = sourceCandidate as FormativeConversationV17CandidateConfiguration;
  assertCondition(
    candidate.roles.student_profiling_agent.model_name === "gpt-5.6-terra" &&
      ...
      candidate.roles.formative_conversation_agent.max_output_tokens === 3_500,
    "formative_conversation_v17_role_configuration_invalid"
  );
  const budget = FormativeConversationV17BudgetSchema.parse(protocol.budget);
  const compiled = compileFormativeConversationV17ExecutionPlan({
  ...
  return {
    ...
    immutable_v16_reference: immutableV16Reference,
...
export type FormativeConversationV17Package = ReturnType<
  typeof loadFormativeConversationV17EvaluationPackage
>;

export function exactFormativeConversationV17LiveAuthorization(
...
export function verifyFormativeConversationV17Governance(
  loaded: FormativeConversationV17Package
) {
  const active = resolveActiveOperationalApproval();
  assertCondition(
    active?.kind === "derived_approval",
    "formative_conversation_v17_active_approval_unavailable"
  );
  assertCondition(
    ...
    "formative_conversation_v17_active_or_rollback_identity_changed"
  );
  return {
...
export function buildFormativeConversationV17EvaluationPlan() {
  const loaded = loadFormativeConversationV17EvaluationPackage();
  const governance = verifyFormativeConversationV17Governance(loaded);
  return {
    plan_version: "formative-conversation-v17-evaluation-plan-v1",
    mode: "plan",
    provider_calls: 0,
