...
export const FORMATIVE_CONVERSATION_V17_DISPATCH_CHECKPOINT_VERSION =
  "formative-conversation-v17-dispatch-checkpoint-v1";

export type FormativeConversationV17DispatchIdentity = {
...
export async function writeFormativeConversationV17DispatchCheckpoint(input: {
  dispatch_root: string;
  identity: FormativeConversationV17DispatchIdentity;
}) {
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v17_dispatch_checkpoint_identity_mismatch"
    );
  }
  return {
