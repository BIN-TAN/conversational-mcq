...
export const FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V17_CANONICAL_SERVICE_NAME",
  "FORMATIVE_CONVERSATION_V5_V17_MIGRATIONS_CURRENT",
  "FORMATIVE_CONVERSATION_V5_V17_LIVE_EVALUATION_ENABLED",
  "FORMATIVE_CONVERSATION_V5_V17_DATABASE_CONNECTION_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V17_DATABASE_IDENTITY_MATCHED",
  "FORMATIVE_CONVERSATION_V5_V17_CANONICAL_LAUNCHER_VALIDATED",
  "FORMATIVE_CONVERSATION_V5_V17_CANONICAL_LOADER_VERSION"
] as const;

export const FORMATIVE_CONVERSATION_V17_DATABASE_CANARY_REQUIRED_ENVIRONMENT = [
  ...
  "OPERATIONAL_AGENT_INTEGRATION_EVAL_EVIDENCE_REQUIRED",
  "FORMATIVE_CONVERSATION_V5_V17_CANARY_SESSION_SECRET_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V17_REMOTE_DATABASE_CANARY_ENABLED"
] as const;
...
function parseInjectedEnvironmentKeys(env: NodeJS.ProcessEnv) {
  const value =
    env.FORMATIVE_CONVERSATION_V5_V17_INJECTED_ENVIRONMENT_KEYS;
  assertCondition(
    configured(value),
...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const requiredEnvironment = noProviderDatabaseCanary
    ? FORMATIVE_CONVERSATION_V17_DATABASE_CANARY_REQUIRED_ENVIRONMENT
  ...
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V17_ENVIRONMENT_SOURCE;
  ...
  requireExactString(
    env,
    "FORMATIVE_CONVERSATION_V5_V17_LAUNCHER_VERSION",
    FORMATIVE_CONVERSATION_V5_LAUNCHER_VERSION
  );
  requireExactString(
    env,
    "FORMATIVE_CONVERSATION_V5_V17_LAUNCH_MECHANISM",
  ...
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V17_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  assertCondition(
    env.FORMATIVE_CONVERSATION_V5_V17_CANONICAL_SERVICE_NAME !==
      "conversational-mcq-staging",
    "formative_conversation_v17_deprecated_service_forbidden"
  );
  requireExactBoolean(
    env,
    "FORMATIVE_CONVERSATION_V5_V17_MIGRATIONS_CURRENT",
  ...
  if (noProviderDatabaseCanary) {
    ...
    assertCondition(
      ...
      "formative_conversation_v17_canary_provider_credential_injected"
    );
    requireExactString(
      env,
      "FORMATIVE_CONVERSATION_V5_V17_CANARY_SESSION_SECRET_SOURCE",
  ...
  const profilingRole = input.candidate.roles.student_profiling_agent;
  const policy = input.candidate.runtime_policy;
  assertCondition(
    role && profilingRole && policy,
  ...
  requireExactString(env, "OPENAI_MODEL_PROFILING", profilingRole.model_name);
  requireExactString(
    env,
    "OPENAI_REASONING_EFFORT_PROFILING",
    profilingRole.reasoning_effort
  );
  requireExactNumber(
    env,
    "OPENAI_MAX_OUTPUT_TOKENS_PROFILING",
    profilingRole.max_output_tokens
  );
  requireExactBoolean(
    env,
    "FORMATIVE_CONVERSATION_LIVE_CALLS_ENABLED",
  ...
  const activeProfilingRole = approvedCandidateRoleConfigResolution(
    active.manifest,
    "student_profiling_agent"
  );
  assertCondition(
  ...
  assertCondition(
    activeProfilingRole.config.model_name === profilingRole.model_name &&
      activeProfilingRole.config.reasoning_effort ===
        profilingRole.reasoning_effort &&
      activeProfilingRole.config.max_output_tokens ===
        profilingRole.max_output_tokens,
    "formative_conversation_v17_active_profiling_role_unapproved"
  );
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V17_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
    for (const sourceName of [
      "FORMATIVE_CONVERSATION_V5_V17_SOURCE_APPROVAL_BUNDLE_PATH",
      "FORMATIVE_CONVERSATION_V5_V17_SOURCE_APPROVED_MANIFEST_PATH",
      "FORMATIVE_CONVERSATION_V5_V17_SOURCE_APPROVAL_EVIDENCE_PATH"
    ]) {
      assertCondition(
  ...
  const safeSnapshot = {
    ...
    profiling_role: {
      approved_role: activeProfilingRole.approved_role,
      compatibility_used: activeProfilingRole.compatibility_used,
      live_calls_approved: true
    },
    inactive_candidate: {
    ...
    runtime_assertions: {
      ...
      profiling_model_name: profilingRole.model_name,
      profiling_reasoning_effort: profilingRole.reasoning_effort,
      profiling_max_output_tokens: profilingRole.max_output_tokens,
...
export function resolveFormativeConversationV5ApplicationReadiness(
  ...
  const profilingRoleConfig = resolveOpenAIModelConfigForRole(
    "student_profiling_agent"
  );
  const roleLiveCalls =
    resolveOperationalRoleLiveCallsEnabled(
  ...
  assertCondition(
    profilingRoleConfig.model_name ===
      environment.runtime_assertions.profiling_model_name &&
      profilingRoleConfig.reasoning_effort ===
        environment.runtime_assertions.profiling_reasoning_effort &&
      profilingRoleConfig.max_output_tokens ===
        environment.runtime_assertions.profiling_max_output_tokens,
    "formative_conversation_v17_application_profiling_role_resolution_mismatch"
  );
  assertCondition(
    roleLiveCalls &&
  ...
  return {
    ...
    application_readiness: {
      ...
      student_profiling_role_resolved: true,
