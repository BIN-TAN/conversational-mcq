...
export type FormativeConversationV5EvaluationLedger = {
  ledger_version: "formative-conversation-v5-evaluation-ledger-v2";
  evaluation_id: string;
  started_at: string;
  completed_at: string | null;
  logical_calls_used: number;
  base_calls_started: number;
...
class EvaluationBudgetLedger {
  ...
  constructor(
    ...
    this.value = {
      ledger_version:
        "formative-conversation-v5-evaluation-ledger-v2",
      evaluation_id: evaluationId,
      started_at: now.toISOString(),
      completed_at: null,
      logical_calls_used: 0,
      base_calls_started: 0,
      base_calls_completed: 0,
  ...
  beginLogicalCall(
    ...
    if (kind === "primary") {
      this.value.base_calls_started += 1;
    } else {
  ...
  completeLogicalCall(kind: "primary" | "semantic_regeneration") {
    if (kind === "primary") {
      this.value.base_calls_completed += 1;
    } else {
      ...
      const semanticExecution =
        await executeFormativeConversationWithSemanticRegeneration({
          ...
          async execute_logical_generation({ sequence, kind, request }) {
            ...
            budgetLedger.beginLogicalCall(request, kind);
            ...
            try {
              ...
              budgetLedger.completeLogicalCall(kind);
              return {
                result,
                logical_call_id: logicalCallId,
