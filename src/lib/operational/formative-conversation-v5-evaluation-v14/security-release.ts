...
export const FORMATIVE_CONVERSATION_V14_CONTROL_SCHEMA_VERSION =
  "formative-conversation-v13-artifact-control-v1" as const;
export const FORMATIVE_CONVERSATION_V14_ARTIFACT_MANIFEST_VERSION =
  "formative-conversation-v13-finalized-artifact-manifest-v1" as const;
export const FORMATIVE_CONVERSATION_V14_PREVENTIVE_SCANNER_VERSION =
  "formative-conversation-v13-preventive-artifact-scanner-v1" as const;
export const FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_VERSION =
  "formative-conversation-v13-preventive-scan-attestation-v1" as const;
export const FORMATIVE_CONVERSATION_V14_RELEASE_POLICY_VERSION =
  "formative-conversation-v13-owner-only-atomic-release-v1" as const;
export const FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_VERSION =
  "formative-conversation-v13-security-wrapper-v1" as const;

export const FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_SOURCE_PATHS = [
  "scripts/operational-formative-conversation-v5-v14-process-local-runner.mjs",
  "scripts/operational-formative-conversation-v5-v14-launcher.mjs",
  "prisma/operational-formative-conversation-v5-v14-evaluate.ts",
  "src/lib/operational/formative-conversation-v5-evaluation-v14/security-release.ts",
  "src/lib/operational/formative-conversation-v5-evaluation-v14/service.ts"
] as const;

export function formativeConversationV14SecurityWrapperFingerprint(
  fileSha256ByPath: Readonly<Record<string, string>>
) {
  const files = FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_SOURCE_PATHS.map(
    (sourcePath) => ({
      path: sourcePath,
  ...
  return stableHash({
    wrapper_version: FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_VERSION,
    control_schema_version: FORMATIVE_CONVERSATION_V14_CONTROL_SCHEMA_VERSION,
    artifact_manifest_version:
      FORMATIVE_CONVERSATION_V14_ARTIFACT_MANIFEST_VERSION,
    scanner_version: FORMATIVE_CONVERSATION_V14_PREVENTIVE_SCANNER_VERSION,
    release_policy_version: FORMATIVE_CONVERSATION_V14_RELEASE_POLICY_VERSION,
    attestation_version: FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_VERSION,
    attestation_schema_fingerprint:
      FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_SCHEMA_FINGERPRINT,
    files
  });
}

export const FORMATIVE_CONVERSATION_V14_DATA_ROOT =
  ".data/operational-formative-conversation-v5-evaluation-v14";
export const FORMATIVE_CONVERSATION_V14_STAGING_ROOT =
  `${FORMATIVE_CONVERSATION_V14_DATA_ROOT}/staging`;
export const FORMATIVE_CONVERSATION_V14_RELEASE_ROOT =
  `${FORMATIVE_CONVERSATION_V14_DATA_ROOT}/runs`;
export const FORMATIVE_CONVERSATION_V14_FAILURE_ROOT =
  `${FORMATIVE_CONVERSATION_V14_DATA_ROOT}/quarantine/failures`;
export const FORMATIVE_CONVERSATION_V14_ATTESTATION_FILENAME =
  "preventive-scan-attestation.json";
export const FORMATIVE_CONVERSATION_V14_MANIFEST_FILENAME =
...
export const FormativeConversationV14ArtifactManifestSchema = z
  .object({
...
export type FormativeConversationV14ArtifactManifest = z.infer<
  typeof FormativeConversationV14ArtifactManifestSchema
>;

export const FormativeConversationV14ControlPayloadSchema = z
  .object({
    schema_version: z.literal(
      FORMATIVE_CONVERSATION_V14_CONTROL_SCHEMA_VERSION
    ),
    ...
    provider_run_id: z.string().regex(/^fcv5v14_provider_[a-z0-9_]+$/),
    derived_evaluation_id: z.string().regex(/^fcv5v14_derived_[a-z0-9_]+$/)
  })
  .strict();

export type FormativeConversationV14ControlPayload = z.infer<
  typeof FormativeConversationV14ControlPayloadSchema
>;

const ScanAttestationPayloadSchema = z
  .object({
    attestation_version: z.literal(
      FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_VERSION
    ),
    scanner_version: z.literal(
      FORMATIVE_CONVERSATION_V14_PREVENTIVE_SCANNER_VERSION
    ),
    release_policy_version: z.literal(
      FORMATIVE_CONVERSATION_V14_RELEASE_POLICY_VERSION
    ),
    evaluation_revision: z.literal(
      "formative-conversation-host-v5-executable-v14"
    ),
...
export const FormativeConversationV14ScanAttestationSchema = z
  .object({
...
export const FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_SCHEMA_FINGERPRINT =
  stableHash({
    version: FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_VERSION,
    fields: [
      "scanner_version",
...
export type FormativeConversationV14PreventiveReleaseReport = {
  status: "released";
  scanner_version: typeof FORMATIVE_CONVERSATION_V14_PREVENTIVE_SCANNER_VERSION;
  release_policy_version: typeof FORMATIVE_CONVERSATION_V14_RELEASE_POLICY_VERSION;
...
export class FormativeConversationV14PreventiveReleaseError extends Error {
  readonly failure_record_path: string | null;

  constructor(code: string, failureRecordPath: string | null = null) {
    super(code);
    this.name = "FormativeConversationV14PreventiveReleaseError";
...
function errorCode(error: unknown) {
  if (error instanceof z.ZodError) {
    return "formative_conversation_v14_schema_validation_failed";
  }
  const code =
    error instanceof Error ? error.message.split(":", 1)[0] : "";
  return /^formative_conversation_v14_[a-z0-9_]+$/.test(code)
    ? code
    : "formative_conversation_v14_preventive_release_failed";
}
...
async function assertOwnerOnlyDirectory(directoryPath: string) {
  ...
  if (
    ...
    throw new Error("formative_conversation_v14_owner_only_directory_invalid");
  }
}
...
async function hardenOwnerOnlyTree(root: string) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      throw new Error("formative_conversation_v14_symlink_detected");
    }
    ...
    if (!entry.isFile()) {
      throw new Error("formative_conversation_v14_special_file_detected");
    }
...
async function assertNoSymlinkComponents(
  ...
  if (
    ...
    throw new Error("formative_conversation_v14_path_traversal_detected");
  }
...
function scanContent(content: Buffer, exactSecrets: readonly string[]): ScanCounts {
  if (
    ...
    throw new Error("formative_conversation_v14_exact_secrets_unavailable");
  }
  let exactMatches = 0;
...
function assertSafeZipEntryName(entryName: string) {
  ...
  if (
    ...
    throw new Error("formative_conversation_v14_zip_path_traversal_detected");
  }
}
...
async function zipEntries(content: Buffer) {
  ...
  for (const entryName of Object.keys(archive.files).sort()) {
    ...
    if (zipEntryIsSymlink(entry)) {
      throw new Error("formative_conversation_v14_zip_symlink_detected");
    }
...
export async function createFormativeConversationV14ControlChannel() {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "fcv5-v14-control-")
  );
...
export async function assertFormativeConversationV14LiveControlEnvironment(input: {
  workspace_root: string;
  env: NodeJS.ProcessEnv;
}) {
  const controlPath = input.env.FORMATIVE_CONVERSATION_V5_V14_CONTROL_PATH;
  const controlNonce = input.env.FORMATIVE_CONVERSATION_V5_V14_CONTROL_NONCE;
  const stagingBase =
    input.env.FORMATIVE_CONVERSATION_V5_V14_STAGING_BASE_ROOT;
  if (!controlPath || !controlNonce || !stagingBase) {
    throw new Error("formative_conversation_v14_control_channel_not_configured");
  }
  if (!/^[a-f0-9]{32}$/.test(controlNonce)) {
    throw new Error("formative_conversation_v14_control_nonce_invalid");
  }
  const workspaceRoot = path.resolve(input.workspace_root);
  const stagingBoundary = path.resolve(
    workspaceRoot,
    FORMATIVE_CONVERSATION_V14_STAGING_ROOT
  );
  ...
  if (
    ...
    throw new Error("formative_conversation_v14_control_environment_path_invalid");
  }
  ...
  if ((await realpath(resolvedStagingBase)) !== resolvedStagingBase) {
    throw new Error("formative_conversation_v14_staging_root_not_canonical");
  }
  try {
    await lstat(resolvedControlPath);
    throw new Error("formative_conversation_v14_control_payload_duplicate");
  } catch (error) {
    if (
      error instanceof Error &&
      (error.message === "formative_conversation_v14_control_payload_duplicate" ||
        !("code" in error) ||
        error.code !== "ENOENT")
    ) {
      throw error;
...
export async function writeFormativeConversationV14ControlPayload(input: {
  control_path: string;
  payload: FormativeConversationV14ControlPayload;
}) {
  const payload = FormativeConversationV14ControlPayloadSchema.parse(
    input.payload
  );
...
export async function readFormativeConversationV14ControlPayload(input: {
  control_path: string;
  ...
  try {
    ...
    if (
      ...
      throw new Error("formative_conversation_v14_control_file_invalid");
    }
    raw = await readFile(input.control_path, "utf8");
  } catch (error) {
    if (
      error instanceof Error &&
      (error.message === "formative_conversation_v14_control_file_invalid" ||
        ("code" in error && error.code !== "ENOENT"))
    ) {
      throw error;
    }
    throw new Error("formative_conversation_v14_control_payload_missing");
  }
  ...
  try {
    ...
    throw new Error("formative_conversation_v14_control_payload_malformed");
  }
  const payload = FormativeConversationV14ControlPayloadSchema.safeParse(parsed);
  if (!payload.success) {
    throw new Error("formative_conversation_v14_control_payload_malformed");
  }
  if (payload.data.control_nonce !== input.expected_nonce) {
    throw new Error("formative_conversation_v14_control_payload_conflicting");
  }
  return payload.data;
}

export async function createFormativeConversationV14FinalizedManifest(input: {
  staging_root: string;
  package_id: string;
}) {
  ...
  const manifestPath = path.join(
    input.staging_root,
    FORMATIVE_CONVERSATION_V14_MANIFEST_FILENAME
  );
  ...
  if (files.includes(manifestPath) || files.includes(attestationPath)) {
    throw new Error("formative_conversation_v14_package_already_finalized");
  }
  ...
  const manifest: FormativeConversationV14ArtifactManifest = {
    artifact_version:
      FORMATIVE_CONVERSATION_V14_ARTIFACT_MANIFEST_VERSION,
    evaluation_revision:
      "formative-conversation-host-v5-executable-v14",
  ...
  FormativeConversationV14ArtifactManifestSchema.parse(manifest);
...
export async function writeFormativeConversationV14SafeFailureRecord(input: {
  workspace_root: string;
  failure_code: string;
  ...
  const failureRoot = path.resolve(
    ...
    FORMATIVE_CONVERSATION_V14_FAILURE_ROOT
  );
  await mkdir(failureRoot, { recursive: true, mode: 0o700 });
  await chmod(failureRoot, 0o700);
  const failurePath = path.join(
    failureRoot,
    `v14-security-failure-${input.control_nonce}.json`
  );
  try {
    await writeOwnerOnlyJson(failurePath, {
      artifact_version:
        "formative-conversation-v14-safe-security-failure-v1",
      status: "blocked",
...
export function scanFormativeConversationV14BufferedOutputs(input: {
  ...
  if (exactMatches > 0 || genericMatches > 0) {
    throw new Error("formative_conversation_v14_secret_match_detected");
  }
  return {
...
export function assertFormativeConversationV14ScanCoverage(input: {
  expected_regular_file_count: number;
  actual_regular_file_count: number;
  expected_zip_file_count: number;
  actual_zip_file_count: number;
  ...
  if (expectedTotal > 0 && actualTotal === 0) {
    throw new Error("formative_conversation_v14_zero_scan_coverage");
  }
  if (
    ...
    throw new Error("formative_conversation_v14_scan_coverage_mismatch");
  }
}

export type FormativeConversationV14ReleaseHooks = {
...
export async function releaseFormativeConversationV14Artifacts(input: {
  workspace_root: string;
  control: FormativeConversationV14ControlPayload;
  exact_secret_values: string[];
  buffered_outputs: readonly string[];
  clear_exact_secrets: () => void | Promise<void>;
  hooks?: FormativeConversationV14ReleaseHooks;
}): Promise<FormativeConversationV14PreventiveReleaseReport> {
  const control = FormativeConversationV14ControlPayloadSchema.parse(
    input.control
  );
  const workspaceRoot = path.resolve(input.workspace_root);
  const expectedDataRoot = path.resolve(
    workspaceRoot,
    FORMATIVE_CONVERSATION_V14_DATA_ROOT
  );
  ...
  const expectedReleaseRoot = path.resolve(
    workspaceRoot,
    FORMATIVE_CONVERSATION_V14_RELEASE_ROOT
  );
  ...
  try {
    if (
      ...
      throw new Error("formative_conversation_v14_control_path_not_canonical");
    }
    if (
      ...
      throw new Error("formative_conversation_v14_control_path_invalid");
    }
    ...
    try {
      ...
      throw new Error("formative_conversation_v14_release_target_exists");
    } catch (error) {
      if (
        error instanceof Error &&
        (error.message === "formative_conversation_v14_release_target_exists" ||
          !("code" in error) ||
          error.code !== "ENOENT")
      ) {
        throw error;
    ...
    if (initialManifestHash !== control.artifact_manifest_sha256) {
      throw new Error("formative_conversation_v14_manifest_control_hash_mismatch");
    }
    let manifestValue: unknown;
    try {
      manifestValue = JSON.parse(manifestContent.toString("utf8"));
    } catch {
      throw new Error("formative_conversation_v14_manifest_malformed");
    }
    const manifest = FormativeConversationV14ArtifactManifestSchema.parse(
      manifestValue
    );
    if (
      ...
      throw new Error("formative_conversation_v14_manifest_identity_mismatch");
    }
    const artifactPaths = manifest.artifacts.map((entry) => entry.path);
    if (new Set(artifactPaths).size !== artifactPaths.length) {
      throw new Error("formative_conversation_v14_manifest_duplicate_artifact");
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v14_manifest_count_mismatch");
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v14_unexpected_or_missing_artifact");
    }
    ...
    for (const artifact of manifest.artifacts) {
      ...
      if (!metadata.isFile()) {
        throw new Error("formative_conversation_v14_manifest_artifact_missing");
      }
      const content = await readFile(artifactPath);
      if (content.byteLength !== artifact.bytes || sha256(content) !== artifact.sha256) {
        throw new Error("formative_conversation_v14_artifact_hash_mismatch");
      }
      accumulate(content);
      ...
      if (
        ...
        throw new Error("formative_conversation_v14_zip_entry_coverage_mismatch");
      }
    ...
    const outputScan = scanFormativeConversationV14BufferedOutputs({
      exact_secret_values: input.exact_secret_values,
      buffered_outputs: input.buffered_outputs
    });
    assertFormativeConversationV14ScanCoverage({
    ...
    if ((await sha256File(manifestPath)) !== initialManifestHash) {
      throw new Error("formative_conversation_v14_manifest_mutated_during_scan");
    }
    for (const artifact of manifest.artifacts) {
      if (
        ...
        throw new Error("formative_conversation_v14_artifact_mutated_during_scan");
      }
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v14_artifact_created_after_enumeration");
    }
    ...
    const attestation = FormativeConversationV14ScanAttestationSchema.parse({
    ...
    if (
      ...
      throw new Error("formative_conversation_v14_attestation_secret_match_detected");
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v14_final_scan_coverage_mismatch");
    }
    assertFormativeConversationV14ScanCoverage({
      expected_regular_file_count: manifest.expected_regular_file_count,
    ...
    if (!secretsCleared) {
      throw new Error("formative_conversation_v14_secret_clear_failed");
    }
    ...
    if (!secretsCleared || !attestationComplete) {
      throw new Error("formative_conversation_v14_release_boundary_violation");
    }
    ...
    failureRecordPath = await writeFormativeConversationV14SafeFailureRecord({
      workspace_root: workspaceRoot,
      failure_code: errorCode(error),
      control_nonce: control.control_nonce
    });
    throw new FormativeConversationV14PreventiveReleaseError(
      errorCode(error),
      failureRecordPath
    );
  }
...
export async function removeFormativeConversationV14ControlChannel(input: {
  control_path: string;
