...
import { FORMATIVE_CONVERSATION_CANDIDATE_ACCEPTANCE_VERSION } from "@/lib/services/student-assessment/formative-conversation/candidate-validation";
import {
  FORMATIVE_CONVERSATION_V14_ADVERSARIAL_MATRIX_HASH,
  FORMATIVE_CONVERSATION_V14_ADVERSARIAL_MATRIX_VERSION
} from "./adversarial-matrix";
import {
...
import { OPENAI_RESPONSES_ADAPTER_VERSION } from "@/lib/llm/providers/openai-responses-provider";
...
import {
  ...
  FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_SOURCE_PATHS,
  FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_VERSION,
  formativeConversationV14SecurityWrapperFingerprint
} from "./security-release";
import {
  FORMATIVE_CONVERSATION_V14_RUN_PROVENANCE_VERSION
} from "./provenance";
import { FORMATIVE_CONVERSATION_V14_EVALUATION_ACCOUNTING_VERSION } from "./evaluation-accounting";
import {
...
import {
  ...
  FORMATIVE_CONVERSATION_V5_V14_REMOTE_CANARY_EVIDENCE_PATH,
  ...
  FormativeConversationV5BlockedV11LauncherFailureSchema,
  ...
  FormativeConversationV5FailedV4PreDispatchEvidenceSchema,
  FormativeConversationV5FixtureManifestSchema,
  FormativeConversationV5FixtureSchema,
  FormativeConversationV5LiveEnvironmentContractSchema,
...
} from "../formative-conversation-v5-evaluation-v9/remote-database-canary";

const RUNTIME_CANDIDATE_MANIFEST_PATH =
  "config/operational-candidates/formative-conversation-host-v5-executable-v14/runtime-candidate-manifest.json";
...
export function assertFormativeConversationV5RuntimeFingerprint(
  ...
  assertCondition(
    FORMATIVE_CONVERSATION_PROMPT_VERSION === identity.prompt_version &&
      ...
      FORMATIVE_CONVERSATION_PROFILE_TRANSITION_VERSION ===
        identity.profile_transition_version &&
      FORMATIVE_CONVERSATION_PROFILE_TRANSITION_VALIDATOR_VERSION ===
        identity.profile_transition_validator_version &&
      FORMATIVE_CONVERSATION_TRANSITION_EVIDENCE_CLOSURE_VERSION ===
        identity.transition_evidence_closure_version &&
      FORMATIVE_CONVERSATION_V14_EVALUATION_ACCOUNTING_VERSION ===
        identity.evaluation_accounting_version &&
      FORMATIVE_CONVERSATION_STUDENT_OUTPUT_FORMAT_VERSION ===
        identity.student_output_format_version &&
      PROVIDER_FAILURE_TAXONOMY_VERSION ===
        identity.provider_failure_taxonomy_version &&
      PROVIDER_TRANSPORT_RETRY_POLICY_VERSION ===
        identity.provider_transport_retry_policy_version &&
      PROVIDER_REQUEST_TRACING_POLICY_VERSION ===
      ...
      FORMATIVE_CONVERSATION_V14_CONTROL_SCHEMA_VERSION ===
        identity.artifact_control_channel_version &&
      FORMATIVE_CONVERSATION_V14_RELEASE_POLICY_VERSION ===
        identity.artifact_release_policy_version &&
      FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_VERSION ===
        identity.scan_attestation_version &&
      FORMATIVE_CONVERSATION_V14_SCAN_ATTESTATION_SCHEMA_FINGERPRINT ===
        identity.scan_attestation_schema_fingerprint &&
      FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_VERSION ===
        identity.security_wrapper_version &&
      FORMATIVE_CONVERSATION_V14_RUN_PROVENANCE_VERSION ===
        identity.run_scoped_provenance_version &&
      FORMATIVE_CONVERSATION_CANDIDATE_ACCEPTANCE_VERSION ===
        identity.candidate_acceptance_version &&
      FORMATIVE_CONVERSATION_V14_ADVERSARIAL_MATRIX_VERSION ===
        identity.adversarial_matrix_version &&
      FORMATIVE_CONVERSATION_V14_ADVERSARIAL_MATRIX_HASH ===
        identity.adversarial_matrix_hash &&
      FORMATIVE_CONVERSATION_SEMANTIC_REGENERATION_POLICY_VERSION ===
        identity.semantic_regeneration_policy_version &&
      FORMATIVE_CONVERSATION_SEMANTIC_REGENERATION_INSTRUCTION_VERSION ===
        identity.semantic_regeneration_instruction_version &&
      FORMATIVE_CONVERSATION_SEMANTIC_REGENERATION_INSTRUCTION_HASH ===
        identity.semantic_regeneration_instruction_hash &&
      FORMATIVE_CONVERSATION_SEMANTIC_REGENERATION_ACCOUNTING_VERSION ===
        identity.semantic_regeneration_accounting_version &&
      FORMATIVE_CONVERSATION_SAFE_INVALID_OUTPUT_EVIDENCE_VERSION ===
        identity.safe_invalid_output_evidence_version &&
      FORMATIVE_CONVERSATION_V9_REMOTE_DATABASE_CANARY_VERSION ===
...
export function assertFormativeConversationV5RunnerBinding(
  ...
  assertCondition(
    runnerVersion ===
      "formative-conversation-v5-protocol-runner-v14",
...
function verifyFixtureCallGraph(fixture: FormativeConversationV5Fixture) {
  ...
  assertCondition(
      ...
      fixture.call_graph.maximum_logical_calls === expectedCalls + 1 &&
...
export function loadFormativeConversationV5EvaluationPackage() {
  ...
  const liveEnvironmentContract =
    FormativeConversationV5LiveEnvironmentContractSchema.parse(
      readJson(
        FORMATIVE_CONVERSATION_V5_LIVE_ENVIRONMENT_CONTRACT_PATH
      )
    );
  ...
  const failedV9FailureAnalysis = readJson(
    FORMATIVE_CONVERSATION_V5_V9_FAILURE_ANALYSIS_PATH
  );
  ...
  const v11LauncherFailureAnalysis =
    FormativeConversationV5BlockedV11LauncherFailureSchema.parse(
      readJson(
        FORMATIVE_CONVERSATION_V5_V11_LAUNCHER_FAILURE_ANALYSIS_PATH
      )
    );
  ...
  const sourceCandidate = readCandidateOperationalModelConfig(
    RUNTIME_CANDIDATE_MANIFEST_PATH
  );
  assertCondition(
    protocol.artifact_contract.v11_launcher_failure_analysis_sha256 ===
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V11_LAUNCHER_FAILURE_ANALYSIS_PATH
      ) &&
      v11LauncherFailureAnalysis.evaluation_protocol_hash ===
        "b4053264bcc2caf72a7e77ff34ae1f90be17fdebb0a0857f09c98a15c293e6b5" &&
      v11LauncherFailureAnalysis.dispatch_checkpoint_created === false &&
      v11LauncherFailureAnalysis.provider_calls === 0,
    "formative_conversation_v14_v11_launcher_failure_evidence_mismatch"
  );
  ...
  const securityWrapperFingerprint =
    formativeConversationV14SecurityWrapperFingerprint(
      Object.fromEntries(
        FORMATIVE_CONVERSATION_V14_SECURITY_WRAPPER_SOURCE_PATHS.map(
          (sourcePath) => [sourcePath, formativeConversationV5FileSha256(sourcePath)]
        )
      )
    );
  ...
  assertCondition(
    runtimeCandidateHash === candidateManifest.runtime_candidate_hash &&
      ...
      candidateManifest.runtime_behavior_changed &&
      candidateManifest.runtime_candidate_hash_unchanged === false &&
      candidateIdentity.runtime_candidate_hash_unchanged === false &&
      runtimeCandidateHash !==
        protocol.failed_v5_execution.runtime_candidate_hash &&
      runtimeCandidateHash !==
        protocol.failed_v9_execution.runtime_candidate_hash &&
      runtimeCandidateHash !== candidateManifest.frozen_v10_runtime_candidate_hash &&
      candidateManifest.frozen_v12_runtime_candidate_hash ===
  ...
  assertCondition(
    protocol.live_environment_contract_hash ===
      ...
      candidateIdentity.live_environment_contract_hash ===
        liveEnvironmentContractHash &&
      candidateIdentity.live_environment_contract_sha256 ===
        formativeConversationV5FileSha256(
          FORMATIVE_CONVERSATION_V5_LIVE_ENVIRONMENT_CONTRACT_PATH
        ) &&
      liveEnvironmentContract.contract_version ===
        FORMATIVE_CONVERSATION_V5_LIVE_ENVIRONMENT_CONTRACT_VERSION &&
      liveEnvironmentContract.launcher.version ===
        FORMATIVE_CONVERSATION_V5_LAUNCHER_VERSION &&
      liveEnvironmentContract.launcher.mechanism ===
        FORMATIVE_CONVERSATION_V5_LAUNCH_MECHANISM &&
      JSON.stringify(
        liveEnvironmentContract.required_injected_environment
      ) ===
        JSON.stringify(
          FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT
        ) &&
      JSON.stringify(liveEnvironmentContract.secret_environment) ===
        JSON.stringify(FORMATIVE_CONVERSATION_V5_SECRET_ENVIRONMENT),
    "formative_conversation_v5_live_environment_contract_mismatch"
  );
  assertCondition(
    formativeConversationV5FileSha256(
      FORMATIVE_CONVERSATION_V5_V4_FAILURE_EVIDENCE_PATH
    ) === protocol.failed_v4_pre_dispatch.evidence_sha256 &&
      protocol.failed_v4_pre_dispatch.source_commit_sha ===
        failedV4PreDispatch.source_commit_sha &&
      protocol.failed_v4_pre_dispatch.runtime_candidate_hash ===
        failedV4PreDispatch.runtime_candidate_hash &&
      protocol.failed_v4_pre_dispatch.protocol_hash ===
        failedV4PreDispatch.protocol_hash &&
      protocol.failed_v4_pre_dispatch.source_plan_artifacts.every(
        (reference, index) =>
          reference.path ===
            failedV4PreDispatch.source_plan_artifacts[index]?.path &&
          reference.sha256 ===
            failedV4PreDispatch.source_plan_artifacts[index]?.sha256
      ) &&
      candidateIdentity.failed_v4_protocol_hash ===
        failedV4PreDispatch.protocol_hash &&
      candidateIdentity.failed_v4_pre_dispatch_evidence_sha256 ===
        protocol.failed_v4_pre_dispatch.evidence_sha256 &&
      sourceConfiguration.preserved_governance
        .failed_v4_protocol_hash ===
        failedV4PreDispatch.protocol_hash &&
      sourceConfiguration.preserved_governance
        .failed_v4_source_commit_sha ===
        failedV4PreDispatch.source_commit_sha &&
      sourceConfiguration.preserved_governance
  ...
  assertCondition(
    protocol.failed_v9_execution.immutable_evidence.every(
      (entry) => formativeConversationV5FileSha256(entry.path) === entry.sha256
    ) &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V9_FAILURE_ANALYSIS_PATH
      ) === protocol.failed_v9_execution.failure_analysis_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V9_HUMAN_REVIEW_ADVISORY_PATH
      ) === protocol.failed_v9_execution.human_review_advisory_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V9_CASE4_SAFE_FAILURE_PATH
      ) === protocol.failed_v9_execution.case4_safe_failure_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V9_CASE6_SAFE_FAILURE_PATH
      ) === protocol.failed_v9_execution.case6_safe_failure_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V9_CASE7_TRANSITION_AUDIT_PATH
      ) === protocol.failed_v9_execution.case7_transition_audit_sha256 &&
      (failedV9FailureAnalysis as Record<string, unknown>).execution_status ===
        "completed_failed" &&
      (failedV9FailureAnalysis as Record<string, unknown>)
        .preserved_immutable === true &&
      candidateIdentity.failed_v9_runtime_candidate_hash ===
        protocol.failed_v9_execution.runtime_candidate_hash &&
      candidateIdentity.failed_v9_protocol_hash ===
        protocol.failed_v9_execution.protocol_hash &&
      sourceConfiguration.preserved_governance
        .failed_v9_runtime_candidate_hash ===
        protocol.failed_v9_execution.runtime_candidate_hash &&
      sourceConfiguration.preserved_governance.failed_v9_protocol_hash ===
        protocol.failed_v9_execution.protocol_hash,
    "formative_conversation_v5_v9_failure_evidence_changed"
  );
  assertCondition(
    formativeConversationV5FileSha256(
      FORMATIVE_CONVERSATION_V5_V14_REMOTE_CANARY_EVIDENCE_PATH
  ...
  assertCondition(
    failedV4PreDispatch.source_plan_artifacts.every(
      (reference) =>
        !existsSync(absolute(reference.path)) ||
        formativeConversationV5FileSha256(reference.path) ===
          reference.sha256
    ),
    "formative_conversation_v5_v4_source_plan_artifact_changed"
  );
  assertCondition(
    formativeConversationV5FileSha256(
      protocol.failed_v5_execution.dispatch_checkpoint_path
    ) ===
      protocol.failed_v5_execution.dispatch_checkpoint_sha256 &&
      formativeConversationV5FileSha256(
        protocol.failed_v5_execution.source_provider_run_path
      ) ===
        protocol.failed_v5_execution.source_provider_run_sha256 &&
      formativeConversationV5FileSha256(
        protocol.failed_v5_execution.derived_evaluation_path
      ) ===
        protocol.failed_v5_execution.derived_evaluation_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V5_FAILURE_ANALYSIS_PATH
      ) ===
        protocol.failed_v5_execution.failure_analysis_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V5_HUMAN_REVIEW_ADVISORY_PATH
      ) ===
        protocol.failed_v5_execution.human_review_advisory_sha256 &&
      candidateIdentity.failed_v5_protocol_hash ===
        protocol.failed_v5_execution.protocol_hash &&
      candidateIdentity.failed_v5_checkpoint_sha256 ===
        protocol.failed_v5_execution.dispatch_checkpoint_sha256 &&
      candidateIdentity.failed_v5_source_provider_run_sha256 ===
        protocol.failed_v5_execution.source_provider_run_sha256 &&
      candidateIdentity.failed_v5_derived_evaluation_sha256 ===
        protocol.failed_v5_execution.derived_evaluation_sha256 &&
      candidateIdentity.failed_v5_failure_analysis_sha256 ===
        protocol.failed_v5_execution.failure_analysis_sha256 &&
      candidateIdentity.failed_v5_human_review_advisory_sha256 ===
        protocol.failed_v5_execution.human_review_advisory_sha256 &&
      candidateManifest.failed_v5_protocol_hash ===
        protocol.failed_v5_execution.protocol_hash &&
      sourceConfiguration.preserved_governance
        .failed_v5_protocol_hash ===
        protocol.failed_v5_execution.protocol_hash &&
      sourceConfiguration.preserved_governance
    ...
    "formative_conversation_v5_v5_evidence_changed"
  );
  assertCondition(
    protocol.failed_v6_execution.dispatch_checkpoint_path ===
        FORMATIVE_CONVERSATION_V5_V6_DISPATCH_PATH &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V6_DISPATCH_PATH
      ) ===
        protocol.failed_v6_execution.dispatch_checkpoint_sha256 &&
      protocol.failed_v6_execution.source_provider_run_path ===
        `${FORMATIVE_CONVERSATION_V5_V6_RUN_ROOT}/source-provider-run.json` &&
      formativeConversationV5FileSha256(
        protocol.failed_v6_execution.source_provider_run_path
      ) ===
        protocol.failed_v6_execution.source_provider_run_sha256 &&
      protocol.failed_v6_execution.derived_evaluation_path ===
        `${FORMATIVE_CONVERSATION_V5_V6_RUN_ROOT}/derived-evaluation.json` &&
      formativeConversationV5FileSha256(
        protocol.failed_v6_execution.derived_evaluation_path
      ) ===
        protocol.failed_v6_execution.derived_evaluation_sha256 &&
      protocol.failed_v6_execution.human_review_package_path ===
        `${FORMATIVE_CONVERSATION_V5_V6_RUN_ROOT}/human-review-package.json` &&
      formativeConversationV5FileSha256(
        protocol.failed_v6_execution.human_review_package_path
      ) ===
        protocol.failed_v6_execution.human_review_package_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V6_FAILURE_ANALYSIS_PATH
      ) ===
        protocol.failed_v6_execution.failure_analysis_sha256 &&
      formativeConversationV5FileSha256(
        FORMATIVE_CONVERSATION_V5_V6_HUMAN_REVIEW_ADVISORY_PATH
      ) ===
        protocol.failed_v6_execution.human_review_advisory_sha256 &&
      candidateIdentity.failed_v6_protocol_hash ===
        protocol.failed_v6_execution.protocol_hash &&
      candidateIdentity.failed_v6_checkpoint_sha256 ===
        protocol.failed_v6_execution.dispatch_checkpoint_sha256 &&
      candidateIdentity.failed_v6_source_provider_run_sha256 ===
        protocol.failed_v6_execution.source_provider_run_sha256 &&
      candidateIdentity.failed_v6_derived_evaluation_sha256 ===
        protocol.failed_v6_execution.derived_evaluation_sha256 &&
      candidateIdentity.failed_v6_human_review_package_sha256 ===
        protocol.failed_v6_execution.human_review_package_sha256 &&
      candidateIdentity.failed_v6_failure_analysis_sha256 ===
        protocol.failed_v6_execution.failure_analysis_sha256 &&
      candidateIdentity.failed_v6_human_review_advisory_sha256 ===
        protocol.failed_v6_execution.human_review_advisory_sha256 &&
      candidateManifest.failed_v6_protocol_hash ===
        protocol.failed_v6_execution.protocol_hash &&
      sourceConfiguration.preserved_governance
        .failed_v6_protocol_hash ===
        protocol.failed_v6_execution.protocol_hash &&
      sourceConfiguration.preserved_governance
        .failed_v6_source_commit_sha ===
        protocol.failed_v6_execution.frozen_commit &&
      sourceConfiguration.preserved_governance
        .failed_v6_provider_run_id ===
        protocol.failed_v6_execution.provider_run_id &&
      sourceConfiguration.preserved_governance
        .failed_v6_derived_evaluation_id ===
        protocol.failed_v6_execution.derived_evaluation_id &&
      sourceConfiguration.preserved_governance
    ...
    "formative_conversation_v5_v6_evidence_changed"
  );
  assertCondition(
  ...
  assertCondition(
    candidateManifest.compiled_execution_plan_path ===
      ...
      protocol.compiled_execution_plan_path ===
        FORMATIVE_CONVERSATION_V5_COMPILED_PLAN_PATH,
    "formative_conversation_v5_v5_compiled_plan_path_mismatch"
  );
  assertCondition(
  ...
  assertCondition(
    protocol.failed_v3_execution.protocol_hash ===
        candidateManifest.failed_v3_protocol_hash &&
      protocol.failed_v3_execution.protocol_hash ===
        sourceConfiguration.preserved_governance
          .failed_v3_protocol_hash &&
      protocol.failed_v3_execution.protocol_hash ===
        candidateIdentity.failed_v3_protocol_hash &&
      protocol.failed_v3_execution.dispatch_checkpoint_sha256 ===
        sourceConfiguration.preserved_governance
          .failed_v3_dispatch_checkpoint_sha256 &&
      protocol.failed_v3_execution.dispatch_checkpoint_sha256 ===
        candidateIdentity.failed_v3_checkpoint_sha256 &&
      protocol.failed_v3_execution.source_provider_run_sha256 ===
        candidateIdentity.failed_v3_source_provider_run_sha256 &&
      protocol.failed_v3_execution.derived_evaluation_sha256 ===
        candidateIdentity.failed_v3_derived_evaluation_sha256,
    "formative_conversation_v5_v3_failure_evidence_mismatch"
  );
  assertCondition(
    formativeConversationV5FileSha256(
      FORMATIVE_CONVERSATION_V5_V3_DISPATCH_PATH
    ...
    "formative_conversation_v5_v3_immutable_artifact_changed"
  );
  assertCondition(
    candidateIdentity.candidate_revision_manifest_hash ===
      ...
      candidateIdentity.source_application_git_commit_location ===
        sourceConfiguration.source_commit_capture &&
      candidateIdentity.security_wrapper_fingerprint ===
        securityWrapperFingerprint &&
      candidateIdentity.v10_security_wrapper_failure_analysis_sha256 ===
        formativeConversationV5FileSha256(
          FORMATIVE_CONVERSATION_V5_V10_SECURITY_FAILURE_ANALYSIS_PATH
        ),
  ...
  return {
    ...
    failed_v4_pre_dispatch: failedV4PreDispatch,
    aggregate_fixture_hash: aggregateFixtureHash,
    fixtures,
...
export function buildFormativeConversationV5EvaluationPlan() {
  ...
  return {
    plan_version: "formative-conversation-v5-evaluation-plan-v3",
    mode: "plan",
    provider_calls: 0,
    ...
    live_environment: {
      ...
      contract_file_sha256:
        loaded.live_environment_contract_file_sha256,
      launcher: loaded.live_environment_contract.launcher,
      required_injected_environment:
        loaded.live_environment_contract
          .required_injected_environment,
    ...
    required_live_command: [
      "node --import tsx scripts/operational-formative-conversation-v5-v14-process-local-runner.mjs",
      '--env-fifo "$FORMATIVE_CONVERSATION_V14_ENV_FIFO" --',
      "--mode=live",
