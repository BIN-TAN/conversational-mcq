...
import { summarizeFormativeConversationV14EvaluationAccounting } from "./evaluation-accounting";
import {
...
async function writeLiveArtifacts(input: {
  ...
  const configuredStagingBase =
    process.env.FORMATIVE_CONVERSATION_V5_V14_STAGING_BASE_ROOT;
  if (!configuredStagingBase) {
    throw new Error(
      "formative_conversation_v14_staging_base_not_configured"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v14_staging_base_invalid"
    );
  }
  ...
  await write("persistence-diagnostics.json", {
    artifact_version:
      "formative-conversation-v14-persistence-diagnostics-v1",
    provider_run_id: input.provider_run_id,
  ...
  const finalized =
    await createFormativeConversationV14FinalizedManifest({
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v14_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v14_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v14_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
      await writeJson(failurePath, {
        artifact_version:
          "formative-conversation-v5-v14-predispatch-failure-v1",
        runtime_candidate_hash: loaded.runtime_candidate_hash,
        evaluation_protocol_hash: loaded.protocol_hash,
        compiled_plan_hash:
          loaded.compiled_plan.compiled_plan_hash,
