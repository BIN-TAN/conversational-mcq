...
export const FORMATIVE_CONVERSATION_V5_REQUIRED_INJECTED_ENVIRONMENT = [
  ...
  "FORMATIVE_CONVERSATION_V5_V18_MIGRATIONS_CURRENT",
  "FORMATIVE_CONVERSATION_V5_V18_LIVE_EVALUATION_ENABLED",
  "FORMATIVE_CONVERSATION_V5_V18_DATABASE_CONNECTION_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V18_DATABASE_IDENTITY_MATCHED",
  "FORMATIVE_CONVERSATION_V5_V18_CANONICAL_LAUNCHER_VALIDATED",
  "FORMATIVE_CONVERSATION_V5_V18_CANONICAL_LOADER_VERSION"
] as const;

export const FORMATIVE_CONVERSATION_V18_DATABASE_CANARY_REQUIRED_ENVIRONMENT = [
  ...
  "OPERATIONAL_AGENT_INTEGRATION_EVAL_EVIDENCE_REQUIRED",
  "FORMATIVE_CONVERSATION_V5_V18_CANARY_SESSION_SECRET_SOURCE",
  "FORMATIVE_CONVERSATION_V5_V18_REMOTE_DATABASE_CANARY_ENABLED"
] as const;
...
export function validateFormativeConversationV5LiveEnvironment(
  ...
  const requiredEnvironment = noProviderDatabaseCanary
    ? FORMATIVE_CONVERSATION_V18_DATABASE_CANARY_REQUIRED_ENVIRONMENT
  ...
  const environmentSource =
    env.FORMATIVE_CONVERSATION_V5_V18_ENVIRONMENT_SOURCE;
  ...
  requireExactString(
    env,
    "FORMATIVE_CONVERSATION_V5_V18_LAUNCHER_VERSION",
    FORMATIVE_CONVERSATION_V5_LAUNCHER_VERSION
  );
  requireExactString(
    env,
    "FORMATIVE_CONVERSATION_V5_V18_LAUNCH_MECHANISM",
  ...
  const databaseConnectionSource =
    env.FORMATIVE_CONVERSATION_V5_V18_DATABASE_CONNECTION_SOURCE;
  assertCondition(
  ...
  assertCondition(
    env.FORMATIVE_CONVERSATION_V5_V18_CANONICAL_SERVICE_NAME !==
      "conversational-mcq-staging",
    "formative_conversation_v18_deprecated_service_forbidden"
  );
  requireExactBoolean(
    env,
    "FORMATIVE_CONVERSATION_V5_V18_MIGRATIONS_CURRENT",
  ...
  if (noProviderDatabaseCanary) {
    ...
    assertCondition(
      ...
      "formative_conversation_v18_canary_provider_credential_injected"
    );
    requireExactString(
      env,
      "FORMATIVE_CONVERSATION_V5_V18_CANARY_SESSION_SECRET_SOURCE",
  ...
  assertCondition(
    activeProfilingRole.approved_role === "student_profiling_agent",
    "formative_conversation_v18_active_profiling_role_unavailable"
  );
  ...
  const projectionUsed =
    env.FORMATIVE_CONVERSATION_V5_V18_APPROVAL_PATH_PROJECTION_USED ===
    "true";
  if (projectionUsed) {
    for (const sourceName of [
      "FORMATIVE_CONVERSATION_V5_V18_SOURCE_APPROVAL_BUNDLE_PATH",
      "FORMATIVE_CONVERSATION_V5_V18_SOURCE_APPROVED_MANIFEST_PATH",
      "FORMATIVE_CONVERSATION_V5_V18_SOURCE_APPROVAL_EVIDENCE_PATH"
    ]) {
      assertCondition(
  ...
  const safeSnapshot = {
    ...
    inactive_candidate_role_configuration: {
      formative_conversation: role,
      student_profiling: profilingRole,
      active_bundle_configuration_not_substituted: true
    },
    profiling_role: {
...
export function resolveFormativeConversationV5ApplicationReadiness(
  ...
  return {
    ...
    application_readiness: {
      ...
      inactive_candidate_formative_conversation_role_resolved: true,
      inactive_candidate_student_profiling_role_resolved: true,
      active_approval_resolved_separately: true,
