...
import { validateFormativeConversationV18CandidateAcceptance } from "@/lib/services/student-assessment/formative-conversation/candidate-validation-v18";
import {
...
import {
  buildFormativeConversationV18ProductionRequest,
...
export type FormativeConversationV18EvaluationLedger = {
  ledger_version: "formative-conversation-v18-shared-evaluation-ledger-v2";
...
export type FormativeConversationV18ProfilingExecution = {
  ...
  provider_execution_audit: {
    audit_version: "formative-conversation-v18-profiling-provider-audit-v2";
...
class EvaluationBudgetLedger {
  ...
  private assertWallClock() {
    if (Date.now() - this.startedAtMs > this.budget.maximum_wall_clock_duration_ms) {
      throw new Error("formative_conversation_v18_wall_clock_budget_exceeded");
    }
  }
  ...
  beginLogicalCall(input: {
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_budget_reservation_exceeded");
    }
  ...
  recordResult(result: StructuredAgentResult<unknown>) {
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_actual_token_budget_exceeded");
    }
  }
  ...
  return {
    ...
    input: {
      ...
      semantic_regeneration: {
        policy_version: "formative-conversation-v18-profiling-semantic-regeneration-v1",
    ...
    metadata: {
      ...
      semantic_regeneration_policy:
        "formative-conversation-v18-profiling-semantic-regeneration-v1",
      semantic_regeneration_attempt: "1"
    }
  } satisfies StructuredAgentRequest<unknown, AgentOutputByName["student_profiling_agent"]>;
}

export function createFormativeConversationV18CandidateRunner(input: {
  ...
  if (!profilingConfig || !formativeConfig) {
    throw new Error(
      "formative_conversation_v18_candidate_role_configuration_missing"
    );
  }
  ...
  async function executeLogical<TOutput>(call: {
    ...
    try {
      ...
      if (!result) {
        throw new Error(
          `formative_conversation_v18_provider_${transport.status}`
        );
      }
    ...
    const baseRequest = buildProductionAgentRequest({
      ...
      metadata: {
        ...
        evaluation_runner_version:
          "formative-conversation-v5-protocol-runner-v18",
  ...
  async function prepareLiveInitialProfile(profileInput: {
    ...
    const invocationKey = `${built.agent_invocation_key}:v18:${input.evaluation_id}`;
    ...
    if (existing) {
      throw new Error(
        "formative_conversation_v18_profiling_invocation_not_replayable"
      );
    }
    ...
    try {
      ...
      if (!validation?.valid || !validation.output) {
        ...
        throw new Error(
          `formative_conversation_v18_profiling_semantic_rejected:${
            validation?.issues.join(",") ?? "profiling_result_missing"
          }`
        );
      }
  ...
  const formativeRunner: FormativeConversationV18AgentRunner = {
    identity: {
      ...
      const semanticExecution = await executeFormativeConversationV18({
        ...
        async execute_logical_generation({ sequence, kind, request }) {
          const execution = await executeLogical({
            ...
            accept_result: (result) =>
              result.status === "completed" &&
              FormativeConversationV18AgentOutputSchema.safeParse(
                result.parsed_output
              ).success
          });
          return execution satisfies FormativeConversationV18LogicalGenerationExecution;
        }
      });
      ledger.recordSemanticAcceptance();
