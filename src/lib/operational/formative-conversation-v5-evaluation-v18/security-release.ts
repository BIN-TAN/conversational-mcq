...
export const FORMATIVE_CONVERSATION_V18_CONTROL_SCHEMA_VERSION =
  "formative-conversation-v18-artifact-control-v1" as const;
export const FORMATIVE_CONVERSATION_V18_ARTIFACT_MANIFEST_VERSION =
  "formative-conversation-v18-finalized-artifact-manifest-v1" as const;
export const FORMATIVE_CONVERSATION_V18_PREVENTIVE_SCANNER_VERSION =
  "formative-conversation-v18-preventive-artifact-scanner-v1" as const;
export const FORMATIVE_CONVERSATION_V18_SCAN_ATTESTATION_VERSION =
  "formative-conversation-v18-preventive-scan-attestation-v1" as const;
export const FORMATIVE_CONVERSATION_V18_RELEASE_POLICY_VERSION =
  "formative-conversation-v18-owner-only-atomic-release-v1" as const;
export const FORMATIVE_CONVERSATION_V18_SECURITY_WRAPPER_VERSION =
  "formative-conversation-v18-security-wrapper-v1" as const;

export const FORMATIVE_CONVERSATION_V18_SECURITY_WRAPPER_SOURCE_PATHS = [
...
export function formativeConversationV18SecurityWrapperFingerprint(
  ...
  const files = FORMATIVE_CONVERSATION_V18_SECURITY_WRAPPER_SOURCE_PATHS.map(
    (sourcePath) => ({
      path: sourcePath,
  ...
  return stableHash({
    wrapper_version: FORMATIVE_CONVERSATION_V18_SECURITY_WRAPPER_VERSION,
    control_schema_version: FORMATIVE_CONVERSATION_V18_CONTROL_SCHEMA_VERSION,
    artifact_manifest_version:
      FORMATIVE_CONVERSATION_V18_ARTIFACT_MANIFEST_VERSION,
    scanner_version: FORMATIVE_CONVERSATION_V18_PREVENTIVE_SCANNER_VERSION,
    release_policy_version: FORMATIVE_CONVERSATION_V18_RELEASE_POLICY_VERSION,
    attestation_version: FORMATIVE_CONVERSATION_V18_SCAN_ATTESTATION_VERSION,
    attestation_schema_fingerprint:
      FORMATIVE_CONVERSATION_V18_SCAN_ATTESTATION_SCHEMA_FINGERPRINT,
    files
  });
}

export const FORMATIVE_CONVERSATION_V18_DATA_ROOT =
  ".data/operational-formative-conversation-v5-evaluation-v18";
export const FORMATIVE_CONVERSATION_V18_STAGING_ROOT =
  `${FORMATIVE_CONVERSATION_V18_DATA_ROOT}/staging`;
export const FORMATIVE_CONVERSATION_V18_RELEASE_ROOT =
  `${FORMATIVE_CONVERSATION_V18_DATA_ROOT}/runs`;
export const FORMATIVE_CONVERSATION_V18_FAILURE_ROOT =
  `${FORMATIVE_CONVERSATION_V18_DATA_ROOT}/quarantine/failures`;
export const FORMATIVE_CONVERSATION_V18_ATTESTATION_FILENAME =
  "artifact-scan-attestation.json";
export const FORMATIVE_CONVERSATION_V18_MANIFEST_FILENAME =
...
export const FormativeConversationV18ArtifactManifestSchema = z
  .object({
    artifact_version: z.literal(
      FORMATIVE_CONVERSATION_V18_ARTIFACT_MANIFEST_VERSION
    ),
    evaluation_revision: z.literal(
      "formative-conversation-host-v5-executable-v18"
    ),
    package_id: z.string().regex(/^fcv5v18_[a-z0-9_]+$/),
...
export type FormativeConversationV18ArtifactManifest = z.infer<
  typeof FormativeConversationV18ArtifactManifestSchema
>;

export const FormativeConversationV18ControlPayloadSchema = z
  .object({
    ...
    provider_run_id: z.string().regex(/^fcv5v18_provider_[a-z0-9_]+$/),
    derived_evaluation_id: z.string().regex(/^fcv5v18_derived_[a-z0-9_]+$/)
  })
  .strict();

export type FormativeConversationV18ControlPayload = z.infer<
  typeof FormativeConversationV18ControlPayloadSchema
>;

const ScanAttestationPayloadSchema = z
  .object({
    attestation_version: z.literal(
      FORMATIVE_CONVERSATION_V18_SCAN_ATTESTATION_VERSION
    ),
    scanner_version: z.literal(
      FORMATIVE_CONVERSATION_V18_PREVENTIVE_SCANNER_VERSION
    ),
    release_policy_version: z.literal(
      FORMATIVE_CONVERSATION_V18_RELEASE_POLICY_VERSION
    ),
    evaluation_revision: z.literal(
      "formative-conversation-host-v5-executable-v18"
    ),
...
export const FormativeConversationV18ScanAttestationSchema = z
  .object({
...
export const FORMATIVE_CONVERSATION_V18_SCAN_ATTESTATION_SCHEMA_FINGERPRINT =
  stableHash({
    version: FORMATIVE_CONVERSATION_V18_SCAN_ATTESTATION_VERSION,
    fields: [
      "scanner_version",
...
export type FormativeConversationV18PreventiveReleaseReport = {
  status: "released";
  scanner_version: typeof FORMATIVE_CONVERSATION_V18_PREVENTIVE_SCANNER_VERSION;
  release_policy_version: typeof FORMATIVE_CONVERSATION_V18_RELEASE_POLICY_VERSION;
...
export class FormativeConversationV18PreventiveReleaseError extends Error {
  readonly failure_record_path: string | null;

  constructor(code: string, failureRecordPath: string | null = null) {
    super(code);
    this.name = "FormativeConversationV18PreventiveReleaseError";
...
function errorCode(error: unknown) {
  if (error instanceof z.ZodError) {
    return "formative_conversation_v18_schema_validation_failed";
  }
  const code =
    error instanceof Error ? error.message.split(":", 1)[0] : "";
  return /^formative_conversation_v18_[a-z0-9_]+$/.test(code)
    ? code
    : "formative_conversation_v18_preventive_release_failed";
}
...
async function assertOwnerOnlyDirectory(directoryPath: string) {
  ...
  if (
    ...
    throw new Error("formative_conversation_v18_owner_only_directory_invalid");
  }
}
...
async function hardenOwnerOnlyTree(root: string) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      throw new Error("formative_conversation_v18_symlink_detected");
    }
    ...
    if (!entry.isFile()) {
      throw new Error("formative_conversation_v18_special_file_detected");
    }
...
async function assertNoSymlinkComponents(
  ...
  if (
    ...
    throw new Error("formative_conversation_v18_path_traversal_detected");
  }
...
function scanContent(content: Buffer, exactSecrets: readonly string[]): ScanCounts {
  if (
    ...
    throw new Error("formative_conversation_v18_exact_secrets_unavailable");
  }
  let exactMatches = 0;
...
function assertSafeZipEntryName(entryName: string) {
  ...
  if (
    ...
    throw new Error("formative_conversation_v18_zip_path_traversal_detected");
  }
}
...
async function zipEntries(content: Buffer) {
  ...
  for (const entryName of Object.keys(archive.files).sort()) {
    ...
    if (zipEntryIsSymlink(entry)) {
      throw new Error("formative_conversation_v18_zip_symlink_detected");
    }
...
export async function createFormativeConversationV18ControlChannel() {
...
export async function assertFormativeConversationV18LiveControlEnvironment(input: {
  workspace_root: string;
  env: NodeJS.ProcessEnv;
}) {
  const controlPath = input.env.FORMATIVE_CONVERSATION_V5_V18_CONTROL_PATH;
  const controlNonce = input.env.FORMATIVE_CONVERSATION_V5_V18_CONTROL_NONCE;
  const stagingBase =
    input.env.FORMATIVE_CONVERSATION_V5_V18_STAGING_BASE_ROOT;
  if (!controlPath || !controlNonce || !stagingBase) {
    throw new Error("formative_conversation_v18_control_channel_not_configured");
  }
  if (!/^[a-f0-9]{32}$/.test(controlNonce)) {
    throw new Error("formative_conversation_v18_control_nonce_invalid");
  }
  ...
  if (
    ...
    throw new Error("formative_conversation_v18_control_environment_path_invalid");
  }
  ...
  if ((await realpath(resolvedStagingBase)) !== resolvedStagingBase) {
    throw new Error("formative_conversation_v18_staging_root_not_canonical");
  }
  try {
    await lstat(resolvedControlPath);
    throw new Error("formative_conversation_v18_control_payload_duplicate");
  } catch (error) {
    if (
      error instanceof Error &&
      (error.message === "formative_conversation_v18_control_payload_duplicate" ||
        !("code" in error) ||
        error.code !== "ENOENT")
    ) {
      throw error;
...
export async function writeFormativeConversationV18ControlPayload(input: {
  control_path: string;
  payload: FormativeConversationV18ControlPayload;
}) {
  const payload = FormativeConversationV18ControlPayloadSchema.parse(
    input.payload
  );
...
export async function readFormativeConversationV18ControlPayload(input: {
  control_path: string;
  ...
  try {
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_control_file_invalid");
    }
    raw = await readFile(input.control_path, "utf8");
  } catch (error) {
    if (
      error instanceof Error &&
      (error.message === "formative_conversation_v18_control_file_invalid" ||
        ("code" in error && error.code !== "ENOENT"))
    ) {
      throw error;
    }
    throw new Error("formative_conversation_v18_control_payload_missing");
  }
  ...
  try {
    ...
    throw new Error("formative_conversation_v18_control_payload_malformed");
  }
  const payload = FormativeConversationV18ControlPayloadSchema.safeParse(parsed);
  if (!payload.success) {
    throw new Error("formative_conversation_v18_control_payload_malformed");
  }
  if (payload.data.control_nonce !== input.expected_nonce) {
    throw new Error("formative_conversation_v18_control_payload_conflicting");
  }
  return payload.data;
}

export async function createFormativeConversationV18FinalizedManifest(input: {
  staging_root: string;
  package_id: string;
}) {
  ...
  if (files.includes(manifestPath) || files.includes(attestationPath)) {
    throw new Error("formative_conversation_v18_package_already_finalized");
  }
  ...
  const manifest: FormativeConversationV18ArtifactManifest = {
    artifact_version:
      FORMATIVE_CONVERSATION_V18_ARTIFACT_MANIFEST_VERSION,
    evaluation_revision:
      "formative-conversation-host-v5-executable-v18",
  ...
  FormativeConversationV18ArtifactManifestSchema.parse(manifest);
...
export async function writeFormativeConversationV18SafeFailureRecord(input: {
  workspace_root: string;
  failure_code: string;
  ...
  const failureRoot = path.resolve(
    ...
    FORMATIVE_CONVERSATION_V18_FAILURE_ROOT
  );
  ...
  try {
    await writeOwnerOnlyJson(failurePath, {
      artifact_version:
        "formative-conversation-v18-safe-security-failure-v1",
      status: "blocked",
...
export function scanFormativeConversationV18BufferedOutputs(input: {
  ...
  if (exactMatches > 0 || genericMatches > 0) {
    throw new Error("formative_conversation_v18_secret_match_detected");
  }
  return {
...
export function assertFormativeConversationV18ScanCoverage(input: {
  expected_regular_file_count: number;
  actual_regular_file_count: number;
  expected_zip_file_count: number;
  actual_zip_file_count: number;
  ...
  if (expectedTotal > 0 && actualTotal === 0) {
    throw new Error("formative_conversation_v18_zero_scan_coverage");
  }
  if (
    ...
    throw new Error("formative_conversation_v18_scan_coverage_mismatch");
  }
}

export type FormativeConversationV18ReleaseHooks = {
...
export async function releaseFormativeConversationV18Artifacts(input: {
  workspace_root: string;
  control: FormativeConversationV18ControlPayload;
  exact_secret_values: string[];
  buffered_outputs: readonly string[];
  clear_exact_secrets: () => void | Promise<void>;
  hooks?: FormativeConversationV18ReleaseHooks;
}): Promise<FormativeConversationV18PreventiveReleaseReport> {
  const control = FormativeConversationV18ControlPayloadSchema.parse(
    input.control
  );
  const workspaceRoot = path.resolve(input.workspace_root);
  ...
  try {
    if (
      ...
      throw new Error("formative_conversation_v18_control_path_not_canonical");
    }
    if (
      ...
      throw new Error("formative_conversation_v18_control_path_invalid");
    }
    ...
    try {
      ...
      throw new Error("formative_conversation_v18_release_target_exists");
    } catch (error) {
      if (
        error instanceof Error &&
        (error.message === "formative_conversation_v18_release_target_exists" ||
          !("code" in error) ||
          error.code !== "ENOENT")
      ) {
        throw error;
    ...
    if (initialManifestHash !== control.artifact_manifest_sha256) {
      throw new Error("formative_conversation_v18_manifest_control_hash_mismatch");
    }
    let manifestValue: unknown;
    try {
      manifestValue = JSON.parse(manifestContent.toString("utf8"));
    } catch {
      throw new Error("formative_conversation_v18_manifest_malformed");
    }
    const manifest = FormativeConversationV18ArtifactManifestSchema.parse(
      manifestValue
    );
    if (
      ...
      throw new Error("formative_conversation_v18_manifest_identity_mismatch");
    }
    const artifactPaths = manifest.artifacts.map((entry) => entry.path);
    if (new Set(artifactPaths).size !== artifactPaths.length) {
      throw new Error("formative_conversation_v18_manifest_duplicate_artifact");
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_manifest_count_mismatch");
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_unexpected_or_missing_artifact");
    }
    ...
    for (const artifact of manifest.artifacts) {
      ...
      if (!metadata.isFile()) {
        throw new Error("formative_conversation_v18_manifest_artifact_missing");
      }
      const content = await readFile(artifactPath);
      if (content.byteLength !== artifact.bytes || sha256(content) !== artifact.sha256) {
        throw new Error("formative_conversation_v18_artifact_hash_mismatch");
      }
      accumulate(content);
      ...
      if (
        ...
        throw new Error("formative_conversation_v18_zip_entry_coverage_mismatch");
      }
    ...
    const outputScan = scanFormativeConversationV18BufferedOutputs({
    ...
    if ((await sha256File(manifestPath)) !== initialManifestHash) {
      throw new Error("formative_conversation_v18_manifest_mutated_during_scan");
    }
    for (const artifact of manifest.artifacts) {
      if (
        ...
        throw new Error("formative_conversation_v18_artifact_mutated_during_scan");
      }
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_artifact_created_after_enumeration");
    }
    ...
    const attestation = FormativeConversationV18ScanAttestationSchema.parse({
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_attestation_secret_match_detected");
    }
    ...
    if (
      ...
      throw new Error("formative_conversation_v18_final_scan_coverage_mismatch");
    }
    assertFormativeConversationV18ScanCoverage({
      expected_regular_file_count: manifest.expected_regular_file_count,
    ...
    if (!secretsCleared) {
      throw new Error("formative_conversation_v18_secret_clear_failed");
    }
    ...
    if (!secretsCleared || !attestationComplete) {
      throw new Error("formative_conversation_v18_release_boundary_violation");
    }
    ...
    failureRecordPath = await writeFormativeConversationV18SafeFailureRecord({
      workspace_root: workspaceRoot,
      failure_code: errorCode(error),
      control_nonce: control.control_nonce
    });
    throw new FormativeConversationV18PreventiveReleaseError(
      errorCode(error),
      failureRecordPath
    );
  }
...
export async function removeFormativeConversationV18ControlChannel(input: {
  control_path: string;
