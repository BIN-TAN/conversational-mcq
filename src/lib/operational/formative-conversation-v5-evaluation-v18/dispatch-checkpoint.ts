...
export const FORMATIVE_CONVERSATION_V18_DISPATCH_CHECKPOINT_VERSION =
  "formative-conversation-v18-dispatch-checkpoint-v1";

export type FormativeConversationV18DispatchIdentity = {
...
export async function writeFormativeConversationV18DispatchCheckpoint(input: {
  dispatch_root: string;
  identity: FormativeConversationV18DispatchIdentity;
}) {
  ...
  const payload = {
    checkpoint_version:
      FORMATIVE_CONVERSATION_V18_DISPATCH_CHECKPOINT_VERSION,
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "formative_conversation_v18_protocol_already_dispatched"
      );
    }
    throw error;
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v18_dispatch_checkpoint_identity_mismatch"
    );
  }
  return {
