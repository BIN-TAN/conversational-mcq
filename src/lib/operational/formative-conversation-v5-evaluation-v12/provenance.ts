...
export const FORMATIVE_CONVERSATION_V12_RUN_PROVENANCE_VERSION =
  "formative-conversation-v12-run-scoped-committed-source-provenance-v1" as const;
...
const DEFAULT_COMMITTED_SOURCE_PATHS = [
  FORMATIVE_CONVERSATION_V5_EVALUATION_ROOT,
  "src/lib/operational/formative-conversation-v5-evaluation-v12",
  "prisma/operational-formative-conversation-v5-v12-evaluate.ts",
  "prisma/operational-formative-conversation-v5-v12-fixture-materialize.ts",
...
export type FormativeConversationV12CommittedSourceVerification = {
  version: typeof FORMATIVE_CONVERSATION_V12_RUN_PROVENANCE_VERSION;
...
export function verifyFormativeConversationV12CommittedSource(input?: {
  ...
  if (!SHA40.test(sourceCommit)) {
    throw new Error("formative_conversation_v12_source_commit_invalid");
  }
  const sourcePaths = input?.source_paths ?? DEFAULT_COMMITTED_SOURCE_PATHS;
  const files = packageFiles(workspaceRoot, sourcePaths);
  if (files.length === 0) {
    throw new Error("formative_conversation_v12_tracked_package_missing");
  }
  const verifiedFiles = files.map((relativePath) => {
    ...
    if (!current.equals(committed)) {
      throw new Error(
        `formative_conversation_v12_tracked_source_changed:${relativePath}`
      );
    }
  ...
  if (deployedCommit !== null && !SHA40.test(deployedCommit)) {
    throw new Error("formative_conversation_v12_deployed_commit_invalid");
  }
  const payload = {
    version: FORMATIVE_CONVERSATION_V12_RUN_PROVENANCE_VERSION,
  ...
  } satisfies FormativeConversationV12CommittedSourceVerification;
}

export function writeFormativeConversationV12RunScopedProvenance(input?: {
  workspace_root?: string;
  deployed_commit?: string | null;
  source_paths?: readonly string[];
}) {
  const workspaceRoot = path.resolve(input?.workspace_root ?? process.cwd());
  const provenance = verifyFormativeConversationV12CommittedSource(input);
  ...
  const outputPath = path.join(
    provenanceRoot,
    `v12-source-${Date.now()}-${randomUUID().slice(0, 8)}.json`
  );
