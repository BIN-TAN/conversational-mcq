...
import {
  ...
  FORMATIVE_CONVERSATION_V5_IMMUTABLE_V15_REPLAY_REFERENCE_PATH,
  FORMATIVE_CONVERSATION_V5_INHERITED_V15_REFERENCE_PATH,
...
export function loadFormativeConversationV5EvaluationPackage() {
  ...
  const inheritedV15Reference = readJson(
    FORMATIVE_CONVERSATION_V5_INHERITED_V15_REFERENCE_PATH
  );
  const immutableV15ReplayReference = readJson(
    FORMATIVE_CONVERSATION_V5_IMMUTABLE_V15_REPLAY_REFERENCE_PATH
  );
  ...
  const protocolHash = assertEmbeddedHash(
    protocol,
    "protocol_hash",
    "formative_conversation_v16_protocol_hash_mismatch"
  );
  const fixtureManifestHash = assertEmbeddedHash(
    fixtureManifest,
    "fixture_manifest_hash",
    "formative_conversation_v16_fixture_manifest_hash_mismatch"
  );
  const liveEnvironmentContractHash = assertEmbeddedHash(
    liveEnvironmentContract,
    "environment_contract_hash",
    "formative_conversation_v16_environment_contract_hash_mismatch"
  );
  const dispatchCheckpointContractHash = assertEmbeddedHash(
    dispatchCheckpointContract,
    "dispatch_checkpoint_contract_hash",
    "formative_conversation_v16_dispatch_contract_hash_mismatch"
  );
  const candidateRevisionManifestHash = assertEmbeddedHash(
    candidateManifest,
    "candidate_manifest_hash",
    "formative_conversation_v16_candidate_manifest_hash_mismatch"
  );
  assertEmbeddedHash(
    sourceConfiguration,
    "source_configuration_hash",
    "formative_conversation_v16_source_configuration_hash_mismatch"
  );
  const inheritedV15ReferenceHash = stableHash(inheritedV15Reference);
  const immutableV15ReplayReferenceHash = stableHash(
    immutableV15ReplayReference
  );
  assertCondition(
    ...
    ) === inheritedV15Reference.v15_immutable_snapshot_hash &&
      candidateManifest.inherited_v15_reference_hash ===
        inheritedV15ReferenceHash &&
      candidateManifest.immutable_v15_replay_reference_hash ===
        immutableV15ReplayReferenceHash,
    "formative_conversation_v16_inherited_v15_reference_mismatch"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v16_fixture_inventory_invalid"
  );
  const fixtures = fixtureReferences.map((entry, index) => {
    assertCondition(
      entry !== null && typeof entry === "object" && !Array.isArray(entry),
      "formative_conversation_v16_fixture_reference_invalid"
    );
    ...
    assertCondition(
      ...
      `${fixture.case_id}:formative_conversation_v16_fixture_identity_mismatch`
    );
    return fixture;
  });
  ...
  assertCondition(
    ...
    "formative_conversation_v16_aggregate_fixture_hash_mismatch"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v16_operational_configuration_missing"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v16_candidate_role_configuration_invalid"
  );
  ...
  assertCondition(
    ...
    "formative_conversation_v16_governance_boundary_invalid"
  );
  assertCondition(
    candidateIdentity.runtime_candidate_hash === runtimeCandidateHash &&
      ...
      candidateIdentity.dispatch_checkpoint_contract_hash ===
        dispatchCheckpointContractHash &&
      candidateIdentity.inherited_v15_reference_hash ===
        inheritedV15ReferenceHash &&
      candidateIdentity.immutable_v15_replay_reference_hash ===
        immutableV15ReplayReferenceHash,
    "formative_conversation_v16_candidate_identity_mismatch"
  );
  assertCondition(
    ...
    "formative_conversation_v16_authorization_package_mismatch"
  );
  ...
  return {
    ...
    inherited_v15_reference: inheritedV15Reference,
    inherited_v15_reference_hash: inheritedV15ReferenceHash,
    immutable_v15_replay_reference: immutableV15ReplayReference,
...
export function verifyFormativeConversationV5Governance(
  ...
  assertCondition(
    ...
    "formative_conversation_v16_active_approval_unavailable"
  );
  assertCondition(
    ...
    "formative_conversation_v16_active_or_rollback_identity_changed"
  );
  return {
...
export function buildFormativeConversationV5EvaluationPlan() {
  ...
  return {
    plan_version: "formative-conversation-v16-evaluation-plan-v1",
    mode: "plan",
    provider_calls: 0,
