...
export const FORMATIVE_CONVERSATION_V16_DISPATCH_CHECKPOINT_VERSION =
  "formative-conversation-v16-dispatch-checkpoint-v1";

export type FormativeConversationV16DispatchIdentity = {
...
export async function writeFormativeConversationV16DispatchCheckpoint(input: {
  dispatch_root: string;
  identity: FormativeConversationV16DispatchIdentity;
}) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "formative_conversation_v16_protocol_already_dispatched"
      );
    }
    throw error;
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v16_dispatch_checkpoint_identity_mismatch"
    );
  }
  return {
