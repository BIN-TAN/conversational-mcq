...
import { summarizeFormativeConversationV15EvaluationAccounting } from "./evaluation-accounting";
import {
...
import {
  createFormativeConversationV15FinalizedManifest,
  FORMATIVE_CONVERSATION_V15_STAGING_ROOT
} from "./security-release";
import {
  writeFormativeConversationV15DispatchCheckpoint
...
type FormativeConversationV5PreDispatchDependencies = {
  ...
  get_migration_readiness?: typeof getFormativeConversationV15MigrationReadiness;
};
...
export async function getFormativeConversationV15MigrationReadiness() {
...
export async function compileFormativeConversationV5PreDispatch(
  ...
  const migrationReadiness = await (
    ...
    getFormativeConversationV15MigrationReadiness
  )();
  if (
  ...
  if (!migrationReadiness.ready) {
    throw new Error(
      "formative_conversation_v15_database_migrations_not_current"
    );
  }
...
async function writeLiveArtifacts(input: {
  ...
  const configuredStagingBase =
    process.env.FORMATIVE_CONVERSATION_V5_V15_STAGING_BASE_ROOT;
  if (!configuredStagingBase) {
    throw new Error(
      "formative_conversation_v15_staging_base_not_configured"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v15_staging_base_invalid"
    );
  }
  ...
  const executionAccounting =
    summarizeFormativeConversationV15EvaluationAccounting({
  ...
  await write("persistence-diagnostics.json", {
    artifact_version:
      "formative-conversation-v15-persistence-diagnostics-v1",
    provider_run_id: input.provider_run_id,
  ...
  const finalized =
    await createFormativeConversationV15FinalizedManifest({
...
export async function executeFormativeConversationV5LiveEvaluation(
  ...
  const { loaded } = preDispatch;
  const providerRunId = `fcv5v15_provider_${timestampId()}_${randomUUID().slice(0, 8)}`;
  const derivedEvaluationId = `fcv5v15_derived_${timestampId()}_${randomUUID().slice(0, 8)}`;
  let checkpointCreated = false;
  const writeDispatchCheckpoint = async () => {
    await writeFormativeConversationV15DispatchCheckpoint({
    ...
    if (!checkpointCreated) {
      const failurePath = safeRelative(
        path.join(
          ...
          `fcv5v15_predispatch_${timestampId()}_${randomUUID().slice(0, 8)}.json`
        )
      );
      await writeJson(failurePath, {
        artifact_version:
          "formative-conversation-v5-v15-predispatch-failure-v1",
        runtime_candidate_hash: loaded.runtime_candidate_hash,
        evaluation_protocol_hash: loaded.protocol_hash,
        compiled_plan_hash:
          loaded.compiled_plan.compiled_plan_hash,
