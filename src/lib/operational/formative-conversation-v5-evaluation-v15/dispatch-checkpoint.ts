...
export const FORMATIVE_CONVERSATION_V15_DISPATCH_CHECKPOINT_VERSION =
  "formative-conversation-v15-dispatch-checkpoint-v1";

export type FormativeConversationV15DispatchIdentity = {
...
export async function writeFormativeConversationV15DispatchCheckpoint(input: {
  dispatch_root: string;
  identity: FormativeConversationV15DispatchIdentity;
}) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "formative_conversation_v15_protocol_already_dispatched"
      );
    }
    throw error;
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v15_dispatch_checkpoint_identity_mismatch"
    );
  }
  return {
