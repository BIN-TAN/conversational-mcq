...
import { ArrowUpDown, Eye, Mail, Search, Upload, UserPlus } from "lucide-react";
      ...
      <section className="rounded-lg border border-line bg-white p-4 shadow-soft">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link className="inline-flex min-h-10 items-center gap-2 rounded-md border border-line bg-white px-4 py-2 text-sm font-semibold text-ink hover:border-accent"
              href={`/teacher/students/invitations${selectedUserIds.size ? `?${new URLSearchParams([...selectedUserIds].map(id => ["student", id])).toString()}` : ""}`}>
              <Mail className="h-4 w-4" aria-hidden="true" />Prepare login emails
            </Link>
            <Link
