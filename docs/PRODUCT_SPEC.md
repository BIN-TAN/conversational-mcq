...
The authoring conversation accepts bounded PDF, DOCX, PNG, JPEG, and WebP course-material attachments as well as pasted text. DOCX text is extracted locally through the hardened document parser. PDFs and images are supplied transiently to the teacher-authoring model as labeled multimodal inputs. Original binary files are not stored in the application database; the teacher-only authoring record retains a content hash, safe file metadata, bounded extracted Word text where available, a validated educational-content summary, limitations, and the source AgentCall reference. Uploaded material is untrusted source content and must never be projected into student-facing agent contexts.

The authoring conversation and item generation are separate operations. The conversation shapes the blueprint and records focused design questions and changes. The blueprint's cognitive-demand mix uses four teacher-facing bands: foundational (remembering, understanding, and applying), analyzing, evaluating, and creating. Only after the teacher opens review mode and requests generation may the item authoring assistant use the saved blueprint to generate draft MCQ candidates. Detailed item sets are generated through small resumable structured-output chunks and merged only after the complete saved blueprint contract passes. Generated keys remain proposals. Generated candidates must enter the same teacher review flow as imported items: the teacher reviews and edits the stem and options, confirms the answer key, reviews reasoning and distractor notes, selects candidates, and imports them as drafts. The assistant cannot publish, include, or administer an item automatically.
