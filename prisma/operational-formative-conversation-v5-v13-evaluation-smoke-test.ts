...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v13/candidate-runner";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v13/package";
import {
  writeFormativeConversationV5DispatchCheckpoint
} from "../src/lib/operational/formative-conversation-v5-evaluation-v13/service";
import {
...
async function main() {
  ...
  const v5Source = [
    ...
    "src/lib/operational/formative-conversation-v5-evaluation-v13/contracts.ts"
  ]
  ...
  assert(
    ...
    "V13 must preserve safe invalid opening and semantic-regeneration evidence without raw provider envelopes."
  );
  assert(
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v13/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v13_provider_smoke",
      derived_evaluation_id: "fcv5v13_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v13_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v13_derived_smoke_rerun",
  ...
  assert(
    loaded.runtime_candidate_hash !==
      ...
      v13Authorization.includes(
        "formative-conversation-host-v5-executable-v13"
      ) &&
      v13Authorization.includes(loaded.runtime_candidate_hash) &&
      v13Authorization.includes(loaded.protocol_hash),
    "V13 must preserve model and prompt behavior while binding the corrected validation runtime and protocol identities."
  );
  assert(
  ...
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v13/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v13/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v13 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
