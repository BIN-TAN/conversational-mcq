...
import {
  ...
  writeFormativeConversationV13RunScopedProvenance
...
function committedSourceFixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "fcv5-v13-provenance-"));
  const sources = [
    "config/operational-candidates/formative-conversation-host-v5-executable-v13/spec.json",
    "src/lib/operational/formative-conversation-v5-evaluation-v13/runtime.ts"
  ];
  ...
  git(root, ["init", "-q"]);
  git(root, ["config", "user.name", "V13 Provenance Test"]);
  git(root, ["config", "user.email", "v13-provenance@example.invalid"]);
  git(root, ["add", "."]);
...
function testCommittedSourceVerification() {
  ...
  try {
    ...
    const verified = verifyFormativeConversationV13CommittedSource({
    ...
    const written = writeFormativeConversationV13RunScopedProvenance({
      workspace_root: fixture.root,
    ...
    assert.throws(
      ...
      /formative_conversation_v13_tracked_source_changed/
    );
  } finally {
...
function testMaterializationIsVerificationOnly() {
  ...
  const v13Before = fileInventory(v13Root);
  ...
  assertInventoryEqual(v13Before, fileInventory(v13Root), "V13 candidate package");
...
function main() {
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed",
        suite: "formative-conversation-v13-provenance",
