...
const runPublicId = `fcv5_v8_transition_smoke_${Date.now()}`;

function loadFixture(caseId: (typeof caseIds)[number]) {
  return FormativeConversationV5FixtureSchema.parse(
    JSON.parse(
      readFileSync(
        `config/operational-candidates/formative-conversation-host-v5-executable-v8/fixtures/${caseId}.json`,
        "utf8"
      )
    )
  );
...
function createRunner() {
  ...
  const outcomeByCase = new Map<
    ...
    | null
  >([
    ["fcv5_03_direct_answer_handling", null],
  ...
  const runner: FormativeConversationAgentRunner = {
    ...
    async execute({ context }) {
      ...
      const immutable = immutableV7Transcripts.get(
        state.fixture.case_id
      );
      const output = immutable
        ? exactV7Output({
            immutable,
            callIndex: state.call_count - 1,
            context
          })
        : isFinalCall && outcome
      ...
      JSON.stringify(
        {
          ...
          deterministic_context_replays: [
            "case3_conversation_creation",
            "case5_exact_v7_largely_improved",
            "case6_largely_improved",
            "case7_exact_v7_teacher_assistance",
            "case8_exact_v7_largely_improved",
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v8_transition_runtime_smoke_failed",
      provider_calls: 0,
      network_requests: 0
    })
  );
