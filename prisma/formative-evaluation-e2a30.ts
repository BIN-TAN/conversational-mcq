...
const VERSION = "e2a30-thermal-physics-transport-autonomous-live-v1";
const E2A29B_RUN = "e2a29b_20260722T145005Z_78aead78";
const E2A29B_ROOT = path.join(
  process.cwd(), ".data", "e2a29b-nonconceptual-profile-consistency",
  E2A29B_RUN
);
const E2A29A_RUN = "e2a29a_20260722144006_a6d11876";
const E2A29A_ROOT = path.join(
  process.cwd(), ".data", "e2a29a-provider-infrastructure-reconciliation",
  E2A29A_RUN
);
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a30-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a30-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.30 evaluation-only constraints:",
...
function frozenArtifactPath(name: string) {
  return path.join(E2A29B_ROOT, name);
}

function buildE2A30FrozenProtocol(): Protocol {
  const sourcePath = frozenArtifactPath("e2a30-frozen-protocol.json");
  const frozen = readJson<FrozenProtocolArtifact>(sourcePath);
  const expectedFileSha = readFileSync(
    frozenArtifactPath("e2a30-frozen-protocol.sha256"), "utf8"
  ).trim();
  ...
  if (expectedFileSha !== PROTOCOL_HASH ||
      ...
      frozen.domain !== "introductory_thermal_physics" ||
      frozen.active_distractor.option_label !== "B" ||
    ...
    throw new Error("e2a30_frozen_protocol_file_sha_mismatch");
  }
  const expectedObjectives = new Map<number, string>([
    [1, "initial_misconception"],
    [2, "copied_or_echoed_tutor_wording"],
    [3, "partial_improvement"],
    [4, "correct_heat_transfer_mechanism_with_explicit_wrong_B_temperature_conclusion"],
    [5, "natural_clarification"],
    [6, "independent_coherent_rejection"]
  ]);
  ...
  return {
    ...
    session_count: 1,
    session: {
      session_id: "E2A30-THERMAL-PHYSICS",
      design: "held_out_thermal_physics_transport_recovery_anchor_contradiction",
      academic_domain: frozen.domain,
      concept: frozen.concept,
      target_evidence_contract: {
        item_id: "thermal_sensation_temperature_item_1",
        distractor_option: frozen.active_distractor.option_label,
        distractor_claim: frozen.active_distractor.claim,
        required_relationship:
          "Metal and wood left in the same room can have the same temperature even when the metal feels colder.",
        required_mechanism:
          "Metal transfers thermal energy away from the hand faster than wood, so skin cools faster even when both objects began at the same temperature.",
        prohibited_contradiction: frozen.active_distractor.claim,
        required_anchor_stance: "rejects_distractor"
      },
      student_profile: {
        ...
        trajectory:
          "lower-temperature intuition to heat-transfer reasoning with a deliberate mechanism/conclusion contradiction"
      },
      frozen_student_trajectory: [
        turn(1, {
          no_live_fixture_message:
            "The metal feels colder, so it must be at a lower temperature than the wood. I would choose option B.",
          semantic_envelope: ["misconception"], tutor_expected: true
        }),
        turn(2, {
          no_live_fixture_message:
            "The metal moves heat away from my hand faster, so I think that is what happens.",
        ...
        turn(3, {
          no_live_fixture_message:
            "Maybe the metal takes thermal energy from my hand faster, but it still seems like it starts colder than the wood.",
          semantic_envelope: ["misconception", "partial"], tutor_expected: true
        }),
        turn(4, {
          no_live_fixture_message:
            "The metal transfers thermal energy away from my hand faster, and that faster heat transfer makes my skin feel colder. Option B still seems right because the metal must have started at a lower temperature.",
          semantic_envelope: ["misconception", "partial"], tutor_expected: true,
          simulator_instruction:
            "Explain the faster thermal-energy transfer from the hand accurately, then end with the explicit conclusion that option B still seems right because the metal began colder.",
          required_anchor_application: "explicit",
          ...
          required_tutor_goal:
            "reconcile faster thermal-energy transfer with the explicit lower-temperature conclusion and endorsement of option B"
        }),
        turn(5, {
          no_live_fixture_message:
            "So can both objects start at the same temperature even though the metal cools my hand faster?",
          semantic_envelope: ["insufficient", "partial"], tutor_expected: true
        }),
        turn(6, {
          no_live_fixture_message:
            "The metal and wood can start at the same room temperature. Metal transfers thermal energy away from my hand faster, so my skin cools faster, but that feeling does not prove the metal started colder. I reject option B.",
          semantic_envelope: ["sound"], tutor_expected: false,
          simulator_instruction:
            "Independently distinguish object temperature from thermal-energy transfer rate, explain the cold sensation, and explicitly reject option B.",
          required_anchor_application: "explicit",
          required_anchor_stance: "rejects_distractor",
          required_anchor_consistency:
            "consistent_with_conceptual_reasoning"
...
function frozenArtifactContract() {
  return readJson<JsonObject>(
    frozenArtifactPath("e2a30-artifact-contract.json")
  );
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a30_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a30_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a30_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a30_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a30_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a30_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a30_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a30_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a30.ts",
...
function protectedEvidenceIdentity() {
  ...
  return {
    snapshot_version: "e2a30-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    identity_version: "e2a30-composite-runtime-identity-v1",
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a30-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A30FrozenProtocol();
  const e2a29bSummary = readJson<{ status?: string;
    provider_calls_made?: number; network_requests_made?: number;
    protected_evidence_unchanged?: boolean;
    candidate_integrity_passed?: boolean; e2a30_executed?: boolean;
    e2a30_protocol_hash?: string; e2a30_artifact_contract_hash?: string }>(
    path.join(E2A29B_ROOT, "summary.json")
  );
  const e2a29aSummary = readJson<{ passed?: boolean;
    provider_calls_made?: number; network_requests_made?: number }>(
    path.join(E2A29A_ROOT, "summary.json")
  );
  const overlap = readJson<{ passed?: boolean;
    previous_analysis_passed?: boolean; execution_authorized?: boolean }>(
    frozenArtifactPath("e2a30-held-out-overlap-analysis.json")
  );
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    maximum?: Record<string, number> }>(frozenArtifactPath("e2a30-budget.json"));
  ...
  if (e2a29bSummary.status !==
      "e2a29b_transport_and_consistency_commit_gate_passed_e2a30_ready" ||
      e2a29bSummary.provider_calls_made !== 0 ||
      e2a29bSummary.network_requests_made !== 0 ||
      e2a29bSummary.protected_evidence_unchanged !== true ||
      e2a29bSummary.candidate_integrity_passed !== true ||
      e2a29bSummary.e2a30_executed !== false ||
      e2a29bSummary.e2a30_protocol_hash !== PROTOCOL_HASH ||
      e2a29bSummary.e2a30_artifact_contract_hash !==
        ARTIFACT_CONTRACT_HASH) {
    blockers.push("e2a29b_authoritative_artifacts_invalid");
  }
  if (e2a29aSummary.passed !== true ||
      e2a29aSummary.provider_calls_made !== 0 ||
      e2a29aSummary.network_requests_made !== 0) {
    blockers.push("e2a29a_transport_artifacts_invalid");
  }
  if (protocol.protocol_hash !== PROTOCOL_HASH) {
    blockers.push("protocol_hash_mismatch");
  }
  if (overlap.passed !== true || overlap.previous_analysis_passed !== true ||
      overlap.execution_authorized !== false) {
  ...
  if (artifactContract.artifact_contract_version !==
      ...
      !Array.isArray(artifactContract.every_logical_generation_call_requires) ||
      !Array.isArray(artifactContract.every_adapter_attempt_requires) ||
      artifactContract.no_chain_of_thought !== true ||
      artifactContract.no_secrets_or_environment_values !== true ||
  ...
  const frozenMaximum = frozenBudget.maximum ? {
    ...
    initial_tutor_calls: frozenBudget.maximum.initial_tutor_calls,
    tutor_regenerations: frozenBudget.maximum.tutor_semantic_regenerations,
  ...
  if (pids("[f]ormative-evaluation-e2a30").length > 0) {
    blockers.push("duplicate_e2a30_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a30_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a30_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A30 !== "1") blockers.push("live_opt_in_missing");
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a30_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a30_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a30_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a30_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a30-one-session-authorization",
    ...
    "--confirm-stop-after-e2a30"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a30_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a30_dispatch_checkpoint_missing");
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  const activeAnchorId =
    `${source.item_id}:option:${source.distractor_option}`;
  return TargetEvidenceContractV5Schema.parse({
    ...
    anchor_resolution_criteria: [
      `Apply the temperature and heat-transfer distinction to option ${source.distractor_option} and coherently reject the lower-temperature claim.`
    ],
    anchor_contradiction_criteria: [
      `Endorsing option ${source.distractor_option} conflicts with explaining that the objects can begin at the same temperature while metal transfers thermal energy from the hand faster.`
    ],
    ...
    active_anchor_alias_contract: buildActiveAnchorAliasContract({
      active_anchor_id: activeAnchorId,
      option_label: source.distractor_option,
      option_text: source.distractor_claim,
      accepted_paraphrases: [
        "the bulb uses up current",
        "less current leaves the bulb than enters it",
        "the bulb takes some current away"
      ]
    })
  });
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a30_session_${session.session_id}`,
    scenario_version: "e2a30-frozen-student-trajectory-v1",
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a30_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a30_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a30_${session.session_id}`,
    ...
    source_activity_context: {
      ...
      profile_condition: "synthetic_frozen_e2a30_trajectory",
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a30_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a30_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a30",
    source_diagnostic_purpose: "reasoning_boundary_repair",
    ...
    response_summary: contradictory
      ? `${session.session_id} turn ${turn.turn} supplies an accurate thermal-energy-transfer mechanism but explicitly endorses the lower-temperature claim, creating a mechanism/conclusion conflict.`
...
function noLiveTutorOutput(input: AutonomousPedagogyInput) {
  ...
  const strategies = [
    "contrast object temperature with thermal-energy transfer rate",
    "trace energy transfer from the hand into each material",
    "ask for a one-sentence link between sensation and transfer rate",
    "separate cold sensation from lower initial temperature",
    "reconcile the stated option conclusion with the heat-transfer mechanism"
  ];
  const studentTasks = [
    ...
    "Put the current-and-energy mechanism and your final judgment side by side. Do they support the same conclusion? Explain which part you would keep or change."
  ];
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a30_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a30_no_live_evaluator_case_missing");
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        throw new Error(`e2a30_no_live_tutor_fixture_invalid:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`);
      }
    } else {
      throw new Error(`e2a30_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a30_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a30_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a30_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a30_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a30:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a30_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a30_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a30_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a30_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a30_canary_failed_rate_limit_retry_exhausted"
          : "e2a30_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a30_canary_failed_provider_nonretryable_request"
        : `e2a30_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      if (endpoint) {
        ...
        throw new Error("e2a30_unnecessary_turn_after_endpoint");
      }
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation.passed) {
        throw new Error(
          `e2a30_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation.issues.join("|")}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a30_canary_failed_evaluator_contract");
          }
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a30_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a30_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a30_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a30_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a30_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a30_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a30_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a30_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a30_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a30_idempotency_replay_failed");
      }
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a30_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a30_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a30_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error("e2a30_anchor_stance_misclassified");
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a30_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a30_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
      ...
      )) throw new Error("e2a30_anchor_interpretation_failure");
      ...
      )) throw new Error("e2a30_genuine_sound_false_negative");
      if (!humanProfileMatches(
        ...
        throw new Error(
          expectedProfile(turn).includes("sound")
            ? "e2a30_genuine_sound_false_negative"
            : result.latest_profile.reasoning_quality === "sound"
              ? "e2a30_genuine_false_sound"
              : `e2a30_profile_semantically_outside_allowed_envelope:turn_${turn.turn}:${result.latest_profile.reasoning_quality}:${turn.semantic_envelope.join("|")}`
        );
      }
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a30_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a30_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a30_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a30_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a30_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a30_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a30-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  if (burden.metric_version !== "e2a30-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  const frozenProtocol = readJson<FrozenProtocolArtifact>(
    frozenArtifactPath("e2a30-frozen-protocol.json")
  );
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a30-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a30-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "frozen-protocol.sha256"), {
    ...
    verified: (() => {
      return stableHash(frozenProtocol) === PROTOCOL_HASH;
    })()
  });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a30-thermal-physics-isolated-fixture-v1",
    synthetic_only: true,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a30_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a30-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a30-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a30-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a30-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a30-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a30-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a30-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a30-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a30-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a30-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a30_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a30_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a30-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A30FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a30_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a30_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a30_no_live_network_detected");
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a30_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a30_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a30-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A30FrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a30_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a30_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a30_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a30_confirmation_missing:")) {
    throw new Error("e2a30_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a30_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a30_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a30_run_not_found");
...
function auditRun(run?: string) {
  ...
  return {
    audit_version: "e2a30-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a30.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a30_runner_failed");
  process.exitCode = 1;
});
