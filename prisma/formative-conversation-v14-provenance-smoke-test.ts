...
import {
  ...
  writeFormativeConversationV14RunScopedProvenance
...
function committedSourceFixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "fcv5-v14-provenance-"));
  const sources = [
    "config/operational-candidates/formative-conversation-host-v5-executable-v14/spec.json",
    "src/lib/operational/formative-conversation-v5-evaluation-v14/runtime.ts"
  ];
  ...
  git(root, ["init", "-q"]);
  git(root, ["config", "user.name", "V14 Provenance Test"]);
  git(root, ["config", "user.email", "v14-provenance@example.invalid"]);
  git(root, ["add", "."]);
...
function testCommittedSourceVerification() {
  ...
  try {
    ...
    const verified = verifyFormativeConversationV14CommittedSource({
    ...
    const written = writeFormativeConversationV14RunScopedProvenance({
      workspace_root: fixture.root,
    ...
    assert.throws(
      ...
      /formative_conversation_v14_tracked_source_changed/
    );
  } finally {
...
function testMaterializationIsVerificationOnly() {
  ...
  const v14Before = fileInventory(v14Root);
  ...
  assertInventoryEqual(v14Before, fileInventory(v14Root), "V14 candidate package");
...
function main() {
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed",
        suite: "formative-conversation-v14-provenance",
