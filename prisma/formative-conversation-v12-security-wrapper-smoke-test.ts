...
import {
  ...
  assertFormativeConversationV12LiveControlEnvironment,
  createFormativeConversationV12ControlChannel,
  createFormativeConversationV12FinalizedManifest,
  ...
  readFormativeConversationV12ControlPayload,
  releaseFormativeConversationV12Artifacts,
  removeFormativeConversationV12ControlChannel,
  type FormativeConversationV12ControlPayload,
  writeFormativeConversationV12ControlPayload
...
async function createFixture(input?: {
  ...
  const workspaceRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "fcv5-v12-security-"))
  );
  const dataRoot = path.join(workspaceRoot, FORMATIVE_CONVERSATION_V12_DATA_ROOT);
  ...
  const packageId = `fcv5v12_provider_security_${nonce}`;
  ...
  const finalized = await createFormativeConversationV12FinalizedManifest({
    staging_root: stagingRoot,
    package_id: packageId
  });
  const control: FormativeConversationV12ControlPayload = {
    schema_version: FORMATIVE_CONVERSATION_V12_CONTROL_SCHEMA_VERSION,
    record_type: "finalized_artifact_package",
    evaluation_revision: "formative-conversation-host-v5-executable-v12",
    control_nonce: nonce,
    mode: "live",
    staging_root: stagingRoot,
    ...
    derived_evaluation_id: `fcv5v12_derived_security_${nonce}`
  };
  const exactSecret = `v12-exact-secret-${nonce}`;
  return {
    workspaceRoot,
    dataRoot,
    stagingBoundary,
...
async function release(
  ...
  input?: {
    outputs?: string[];
    secrets?: string[];
    hooks?: Parameters<typeof releaseFormativeConversationV12Artifacts>[0]["hooks"];
  }
) {
  const secrets = input?.secrets ?? [fixture.exactSecret];
  return releaseFormativeConversationV12Artifacts({
...
function failureRecords(fixture: Fixture) {
  const root = path.join(fixture.workspaceRoot, FORMATIVE_CONVERSATION_V12_FAILURE_ROOT);
...
async function testControlChannelValidation() {
  const channel = await createFormativeConversationV12ControlChannel();
  ...
  try {
    await expectReject(
      ...
      "formative_conversation_v12_control_payload_missing"
    );
    ...
    await expectReject(
      ...
      "formative_conversation_v12_control_payload_malformed"
    );
    ...
    await expectReject(
      ...
      "formative_conversation_v12_control_payload_duplicate"
    );
    unlinkSync(channel.control_path);
    await writeFormativeConversationV12ControlPayload({
      control_path: channel.control_path,
      payload
    });
    const read = await readFormativeConversationV12ControlPayload({
      control_path: channel.control_path,
      expected_nonce: channel.control_nonce
    });
    ...
    await expectReject(
      ...
      "formative_conversation_v12_control_payload_conflicting"
    );
    await expectReject(
      () =>
        writeFormativeConversationV12ControlPayload({
    ...
    await removeFormativeConversationV12ControlChannel({
      control_path: channel.control_path,
...
async function testLiveControlEnvironmentBoundary() {
  ...
  try {
    await expectReject(
      ...
      "formative_conversation_v12_control_channel_not_configured"
    );
    const env: NodeJS.ProcessEnv = {
      NODE_ENV: "test",
      FORMATIVE_CONVERSATION_V5_V12_CONTROL_PATH: controlPath,
      FORMATIVE_CONVERSATION_V5_V12_CONTROL_NONCE: fixture.nonce,
      FORMATIVE_CONVERSATION_V5_V12_STAGING_BASE_ROOT: fixture.stagingBase
    };
    const validated =
      await assertFormativeConversationV12LiveControlEnvironment({
        workspace_root: fixture.workspaceRoot,
        env
      });
...
async function testContainmentAndSymlinks() {
  ...
  try {
    ...
    await expectReject(
      () => release(traversal),
      "formative_conversation_v12_control_path_not_canonical"
    );
  ...
  try {
    ...
    await expectReject(
      () => release(symlink),
      "formative_conversation_v12_symlink_detected"
    );
...
async function testManifestCoverageFailures() {
  ...
  try {
    ...
    await expectReject(
      () => release(missing),
      "formative_conversation_v12_unexpected_or_missing_artifact"
    );
  ...
  assert.throws(
    ...
    /formative_conversation_v12_zero_scan_coverage/
  );
  ...
  try {
    ...
    await expectReject(
      () => release(zipCoverage),
      "formative_conversation_v12_zip_entry_coverage_mismatch"
    );
  } finally {
    cleanup(zipCoverage);
  }
...
async function testSecretDetection() {
  for (const location of ["regular", "zip"] as const) {
    const secret = `v12-leak-${randomUUID().replaceAll("-", "")}`;
    const fixture = await createFixture(
      location === "regular"
    ...
    try {
      await expectReject(
        ...
        "formative_conversation_v12_secret_match_detected"
      );
...
async function testMutationAndFinalizationOrdering() {
  ...
  try {
    await expectReject(
      ...
      "formative_conversation_v12_artifact_created_after_enumeration"
    );
  ...
  try {
    await expectReject(
      ...
      "formative_conversation_v12_manifest_mutated_during_scan"
    );
  } finally {
    cleanup(mutated);
  }
  ...
  try {
    await expectReject(
      ...
      "formative_conversation_v12_exact_secrets_unavailable"
    );
  } finally {
...
async function testSuccessfulAtomicReleaseAndAttestation() {
  ...
  try {
    ...
    const attestationText = readFileSync(
      path.join(fixture.releaseRoot, FORMATIVE_CONVERSATION_V12_ATTESTATION_FILENAME),
      "utf8"
    );
...
async function main() {
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed",
        suite: "formative-conversation-v12-security-wrapper",
