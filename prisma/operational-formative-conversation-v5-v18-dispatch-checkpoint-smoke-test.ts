...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18/dispatch-checkpoint";

const hash = (character: string) => character.repeat(64);
const identity = {
  provider_run_id: "fcv5v18_provider_test",
  derived_evaluation_id: "fcv5v18_derived_test",
...
async function main() {
const root = await mkdtemp(path.join(tmpdir(), "fcv18-dispatch-"));
try {
  const first = await writeFormativeConversationV18DispatchCheckpoint({
    dispatch_root: root,
    identity
  });
