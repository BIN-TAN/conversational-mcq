...
import {
  E2A46_ARTIFACT_NAMES,
  E2A46_ARTIFACT_ROOT,
  inspectE2A46FreezeRun,
  latestE2A46FreezeRunDirectory,
  makeE2A46FreezeRunId,
  writeE2A46FreezeArtifacts
} from "../src/lib/evaluation/formative/e2a46-pilot-operational-readiness-protocol";

let networkRequestCount = 0;
globalThis.fetch = async () => {
  networkRequestCount += 1;
  throw new Error("e2a46_network_request_prohibited");
};
...
function execute(runDirectory: string) {
  ...
  const result = writeE2A46FreezeArtifacts({
    runDirectory,
    networkRequestCount: networkRequestCount - before
  });
  assert(
    networkRequestCount === before,
    "e2a46_provider_call_guard_detected_network_request"
  );
  return result;
}
...
function runSmoke(suite: string) {
  const runDirectory = mkdtempSync(
    path.join(tmpdir(), "e2a46-pilot-readiness-")
  );
  try {
    ...
    const checks: Record<string, boolean> = {
      ...
      onboarding: suites.onboarding.passed,
      "workflow-readiness": suites.workflow_readiness.passed,
      "failure-recovery": suites.failure_recovery.passed,
      privacy: suites.privacy.passed,
      monitoring: suites.monitoring.passed,
      "audit-preservation": suites.audit_preservation.passed,
      "teacher-student-boundary":
        suites.teacher_student_boundary.passed,
      "data-collection": suites.data_collection.passed,
      "readiness-decision": suites.readiness_decision.passed,
    ...
    assert(suite in checks, `e2a46_unknown_smoke_suite:${suite}`);
    assert(checks[suite], `e2a46_${suite}_smoke_failed`);
    console.log(JSON.stringify({
      ...
      operational_readiness_decision:
        result.summary.operational_readiness_decision,
...
function main() {
    ...
    const runDirectory = runIndex >= 0
      ? path.join(
          ...
          process.argv[runIndex + 1] ??
            "missing_e2a46_run_identifier"
        )
      : latestE2A46FreezeRunDirectory();
    console.log(JSON.stringify(
      inspectE2A46FreezeRun(runDirectory),
      null,
      2
    ));
    return;
  ...
  throw new Error(`e2a46_unknown_command:${command}`);
}
