...
const identity = {
  provider_run_id: "fcv5v16_provider_test",
  derived_evaluation_id: "fcv5v16_derived_test",
...
async function main() {
const root = await mkdtemp(path.join(tmpdir(), "fcv16-dispatch-"));
try {
  const first = await writeFormativeConversationV16DispatchCheckpoint({
    dispatch_root: root,
    identity
  });
