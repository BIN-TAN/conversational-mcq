...
function installIsolatedEnvironment() {
  ...
  assert(
    ...
    "The V18R2 runtime smoke must use local PostgreSQL."
  );
  ...
  try {
    ...
    for (const childTest of childTests) {
      ...
      assert.equal(
        child.status,
        0,
        child.error?.message ?? `The V18R2 database smoke failed: ${childTest}`
      );
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v18r2_runtime_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0,
      generation_network_requests: 0,
      dispatch_checkpoints: 0
