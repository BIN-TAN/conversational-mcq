...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v4/candidate-runner";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v4/package";
import {
...
async function main() {
  assert(packagePathsExist(), "v4 package paths must exist.");
  ...
  assert(
    ...
    "The v4 inventory must contain exactly eight fixtures."
  );
  assert(
    JSON.stringify(
  ...
  const v4Source = [
    ...
    "src/lib/operational/formative-conversation-v5-evaluation-v4/contracts.ts"
  ]
    .map((filePath) => readFileSync(filePath, "utf8"))
    .join("\n");
  assert(
    !v4Source.includes("SyntheticStudentPersonaSchema") &&
      !v4Source.includes(
        "runSyntheticStudentResearchValidation"
      ),
    "v4 must not compile or execute cases through the shared persona schema."
  );
  assert(
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v4/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  const tempRoot = await mkdtemp(
    path.join(tmpdir(), "fcv5-v4-checkpoint-")
  );
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v4_provider_smoke",
      derived_evaluation_id: "fcv5v4_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v4_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v4_derived_smoke_rerun",
  ...
  const v4Authorization =
    exactFormativeConversationV5LiveAuthorization(loaded);
  assert(
    loaded.runtime_candidate_hash !==
      "eb10a75438b00706857a9b11d4d799d82cd27b251a34ff1943459c9060222267" &&
      loaded.protocol_hash !==
        "13186b2e5dc54486864b216292afd02774c832340a548c77585886a53c7642b5" &&
      v4Authorization.includes(
        "formative-conversation-host-v5-executable-v4"
      ) &&
      v4Authorization.includes(loaded.runtime_candidate_hash) &&
      v4Authorization.includes(loaded.protocol_hash),
    "v4 must have a new runtime identity, protocol, and exact authorization."
  );
  assert(
  ...
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v4/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v4/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v4 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ...
          : "formative_conversation_v5_v4_smoke_failed",
      provider_calls: 0,
