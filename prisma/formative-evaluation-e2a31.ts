...
const VERSION = "e2a31-ecology-held-out-autonomous-live-v1";
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a31-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a31-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const ARTIFACT_CONTRACT_HASH =
  "509d099d13a978d372e38ab19ed12b260fe7a12bf45ff2ffb57a0dd653b9d493";
...
const COMPILED_EVALUATOR_V5_REQUEST_HASH =
  "d3f0f85d1bf79335235bb317ab05c95a52e41a07121eae1b3e0963ebe7dea8b0";
const OVERLAP_ANALYSIS_HASH =
  "074dd68633ae0fff44c1a3be23b1b480a4556c9da056cf97b56d6a3417486301";
const BUDGET_HASH =
  "f50274fa2cca81ebdba77decac244fecda1c64fea6a0acbe03013cf8a656fc22";
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.31 evaluation-only constraints:",
...
function buildE2A31FrozenProtocol(): Protocol {
  ...
  if (expectedFileSha !== PROTOCOL_HASH ||
    ...
    throw new Error("e2a31_frozen_protocol_file_sha_mismatch");
  }
...
function frozenCompositeRuntimeIdentity() {
  ...
  if (recorded !== FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      ...
      identity.e2a31_protocol_hash !== PROTOCOL_HASH ||
      identity.candidate_configuration_hash !== CANDIDATE_HASH ||
      identity.candidate_file_sha256 !== CANDIDATE_FILE_SHA ||
      identity.approved_v2_hash !== APPROVED_HASH) {
    throw new Error("e2a31_frozen_composite_runtime_identity_mismatch");
  }
  return identity;
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a31_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a31_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a31_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a31_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a31_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a31_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a31_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a31_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a31.ts",
...
function protectedEvidenceIdentity() {
  ...
  return {
    snapshot_version: "e2a31-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    identity_version: "e2a31-composite-runtime-identity-v1",
    ...
    frozen_e2a31a_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    frozen_e2a31a_protocol_hash: PROTOCOL_HASH,
    ...
    frozen_e2a31a_compiled_evaluator_v5_request_hash:
      COMPILED_EVALUATOR_V5_REQUEST_HASH,
    frozen_e2a31a_overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    frozen_e2a31a_artifact_contract_hash: ARTIFACT_CONTRACT_HASH,
    frozen_e2a31a_budget_hash: BUDGET_HASH,
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a31-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A31FrozenProtocol();
  const freezeSummary = readJson<{ status?: string; passed?: boolean;
    protocol_hash?: string; composite_runtime_identity_hash?: string;
    target_evidence_contract_hash?: string; ecology_alias_contract_hash?: string;
    evaluator_v5_request_compiled?: boolean; old_evaluator_contract_count?: number;
    overlap_analysis_passed?: boolean; exact_scenario_previously_seen?: boolean;
    candidate_integrity_passed?: boolean;
    protected_source_integrity_passed?: boolean;
    e2a31_executed?: boolean; provider_calls_made?: number;
    network_requests_made?: number; candidate_approved?: boolean;
    candidate_activated?: boolean }>(frozenArtifactPath("summary.json"));
  const freezeValidation = readJson<{ passed?: boolean;
    required_artifact_count?: number; missing_artifacts?: unknown[];
    unexpected_artifacts?: unknown[]; unsafe_artifacts?: unknown[];
    semantic_issues?: unknown[]; provider_calls_made?: number;
  ...
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    maximum?: Record<string, number> }>(frozenArtifactPath("budget.json"));
  ...
  const frozenProviderGuard = readJson<{ passed?: boolean;
    execution_authorized?: boolean; provider_executor_constructed?: boolean;
    provider_calls_made?: number; network_requests_made?: number }>(
  ...
  if (freezeSummary.status !==
      "e2a31a_ecology_protocol_frozen_not_authorized_not_executed" ||
      freezeSummary.passed !== true ||
      freezeSummary.protocol_hash !== PROTOCOL_HASH ||
      freezeSummary.composite_runtime_identity_hash !==
        FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      freezeSummary.target_evidence_contract_hash !==
        TARGET_EVIDENCE_CONTRACT_HASH ||
      freezeSummary.ecology_alias_contract_hash !==
        ECOLOGY_ALIAS_CONTRACT_HASH ||
      freezeSummary.evaluator_v5_request_compiled !== true ||
      freezeSummary.old_evaluator_contract_count !== 0 ||
      freezeSummary.overlap_analysis_passed !== true ||
      freezeSummary.exact_scenario_previously_seen !== false ||
      freezeSummary.candidate_integrity_passed !== true ||
      freezeSummary.protected_source_integrity_passed !== true ||
      freezeSummary.e2a31_executed !== false ||
      freezeSummary.provider_calls_made !== 0 ||
      freezeSummary.network_requests_made !== 0 ||
      freezeSummary.candidate_approved !== false ||
      freezeSummary.candidate_activated !== false) {
    blockers.push("e2a31a_authoritative_freeze_invalid");
  }
  if (freezeValidation.passed !== true ||
      freezeValidation.required_artifact_count !== 17 ||
      freezeValidation.missing_artifacts?.length !== 0 ||
      freezeValidation.unexpected_artifacts?.length !== 0 ||
      freezeValidation.unsafe_artifacts?.length !== 0 ||
      freezeValidation.semantic_issues?.length !== 0 ||
      freezeValidation.provider_calls_made !== 0 ||
      freezeValidation.network_requests_made !== 0) {
    blockers.push("e2a31a_artifact_validation_invalid");
  }
  ...
  if (artifactContract.artifact_contract_version !==
      ...
      !Array.isArray(artifactContract.evaluator_turn_requires) ||
  ...
  if (stableHash(compiledEvaluatorRequest) !==
        COMPILED_EVALUATOR_V5_REQUEST_HASH ||
  ...
  if (frozenProviderGuard.passed !== true ||
      frozenProviderGuard.execution_authorized !== false ||
      frozenProviderGuard.provider_executor_constructed !== false ||
  ...
  if (pids("[f]ormative-evaluation-e2a31").length > 0) {
    blockers.push("duplicate_e2a31_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a31_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a31_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A31 !== "1") blockers.push("live_opt_in_missing");
  ...
  return {
    version: "e2a31-live-preflight-v1",
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a31_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a31_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a31_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a31_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a31-one-session-authorization",
    ...
    "--confirm-stop-after-e2a31"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a31_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a31_dispatch_checkpoint_missing");
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  if (stableHash(contract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(contract.active_anchor_alias_contract) !==
        ECOLOGY_ALIAS_CONTRACT_HASH) {
    throw new Error("e2a31_target_evidence_contract_hash_mismatch");
  }
  return contract;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a31_session_${session.session_id}`,
    scenario_version: "e2a31-frozen-student-trajectory-v1",
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a31_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a31_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a31_${session.session_id}`,
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a31_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a31_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a31",
    source_diagnostic_purpose: "reasoning_boundary_repair",
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a31_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a31_no_live_evaluator_case_missing");
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        throw new Error(`e2a31_no_live_tutor_fixture_invalid:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`);
      }
    } else {
      throw new Error(`e2a31_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a31_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a31_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a31_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a31_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a31:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a31_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a31_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a31_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a31_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a31_canary_failed_rate_limit_retry_exhausted"
          : "e2a31_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a31_canary_failed_provider_nonretryable_request"
        : `e2a31_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      if (endpoint) {
        ...
        throw new Error("e2a31_unnecessary_turn_after_endpoint");
      }
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation.passed) {
        throw new Error(
          `e2a31_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation.issues.join("|")}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a31_canary_failed_evaluator_contract");
          }
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a31_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a31_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a31_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a31_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a31_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a31_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a31_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a31_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a31_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a31_idempotency_replay_failed");
      }
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a31_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a31_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a31_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error("e2a31_anchor_stance_misclassified");
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a31_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a31_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 3 && (
      ...
      )) throw new Error("e2a31_anchor_interpretation_failure");
      ...
      )) throw new Error("e2a31_genuine_sound_false_negative");
      if (!humanProfileMatches(
        ...
        throw new Error(
          expectedProfile(turn).includes("sound")
            ? "e2a31_genuine_sound_false_negative"
            : result.latest_profile.reasoning_quality === "sound"
              ? "e2a31_genuine_false_sound"
              : `e2a31_profile_semantically_outside_allowed_envelope:turn_${turn.turn}:${result.latest_profile.reasoning_quality}:${turn.semantic_envelope.join("|")}`
        );
      }
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a31_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a31_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a31_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a31_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a31_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a31_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a31-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  if (burden.metric_version !== "e2a31-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  const frozenBudget = readJson<JsonObject>(frozenArtifactPath("budget.json"));
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a31-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a31-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a31-ecology-isolated-fixture-v1",
    synthetic_only: true,
  ...
  writeJson(path.join(runDir, "overlap-analysis-binding.json"), {
    binding_version: "e2a31-overlap-analysis-binding-v1",
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a31_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "budget-ledger.json"), {
    ledger_version: "e2a31-budget-ledger-v1",
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a31-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a31-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a31-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a31-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a31-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a31-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a31-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a31-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a31-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a31-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a31_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a31_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a31-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A31FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a31_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a31_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a31_no_live_network_detected");
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a31_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a31_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a31-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A31FrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a31_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a31_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a31_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a31_confirmation_missing:")) {
    throw new Error("e2a31_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a31_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a31_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a31_run_not_found");
...
function auditRun(run?: string) {
  ...
  return {
    audit_version: "e2a31-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a31.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a31_runner_failed");
  process.exitCode = 1;
});
