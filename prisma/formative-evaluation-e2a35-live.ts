...
const VERSION = "e2a35-sampling-bias-self-correction-live-v1";
const E2A35_FREEZE_RUN = "e2a35_20260724T220053666_90a459a1";
const E2A35_FREEZE_ROOT = path.join(
  process.cwd(), ".data",
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a35-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a35-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const ARTIFACT_CONTRACT_HASH =
  "125125d45aeb7ae534b1d92b70a9c7bd82e3b0c7a0a6ec91f9ed97f08b937793";
const TARGET_EVIDENCE_CONTRACT_HASH =
  "e2963a503625c2bb5b655e65cabd8a738d2c6a27873e3dd040e686b3fc74fa11";
const SAMPLING_BIAS_ALIAS_CONTRACT_HASH =
  "0967667ce7f1cfddba8e4988e599393dc040cfbabd5c79c18b502d02f5377dfb";
const CANONICAL_ANCHOR_CONTRACT_HASH =
  "88dae9c3a8d4e2e35a306a832554d234ca35b28d0c68f7a86722725588454f52";
const TRAJECTORY_ENVELOPE_HASH =
  "6b2cbe8831ab81681bcbdedfce677ee6d7a3c04b77f3395154648597a2723f8e";
const COMPILED_EVALUATOR_V5_REQUEST_HASH =
  "f88d6d6d7ddd1bf3bc218d2191ce60cf3931df11689e480364611c11701ba050";
const COMPILED_EVALUATOR_V5_ARTIFACT_SHA =
  "6f20e0361161e29cd177bf8f7793eb30199fec9165ef1229c6496df4cc3e1da0";
const OVERLAP_ANALYSIS_HASH =
  "df5e7a6f33a073ce4ddb28f1e57e0a62abfea344c7af51855d8ce934d9befc24";
const BUDGET_HASH =
  "9919cf9a9e0c3dcc9e835366a9bb5ff43d34c95d93cdd4495f88b564952c39ae";
const STANCE_EVIDENCE_CONTRACT_HASH =
  "a94ff4b200a33a6677c2f31428e52181292c3a87fea75c8e016387f00bcf3539";
const SELF_CORRECTION_INTENT_CONTRACT_HASH =
  "d58956617bb6f632b0cabd5e21f2069be267a42363e7ab618784bbcbd576cb74";
...
const ARTIFACT_NAMES = [
  ...
  "sampling-bias-target-evidence-contract.json",
  "sampling-bias-alias-contract.json",
  ...
  "sampling-bias-alias-resolution-results.jsonl",
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.35 evaluation-only constraints:",
  ...
  "Keep the response inside the frozen volunteer-sampling-bias scenario.",
...
type FrozenProtocolArtifact = {
  ...
  contract_versions: {
    ...
    anchor_stance_scope: string;
    self_correction_intent: string;
  ...
  contract_hashes: {
    ...
    self_correction_intent: string;
    compiled_evaluator_v5_request: string;
    trajectory_envelope: string;
  ...
  conversational_state_policy: Record<string, boolean>;
  deterministic_gate_results: Record<string, boolean>;
  execution_boundary: {
  ...
  budget: {
    ...
    provider_concurrency: 1;
    provider_transport_retry_policy_version: string;
  };
...
function frozenArtifactPath(name: string) {
  return path.join(E2A35_FREEZE_ROOT, name);
}

function frozenProtocolArtifact() {
  const protocol = readJson<FrozenProtocolArtifact>(
    frozenArtifactPath("frozen-protocol.json")
  );
  const body: Partial<FrozenProtocolArtifact> = { ...protocol };
  ...
  if (recordedHash !== PROTOCOL_HASH ||
      ...
      protocol.protocol_version !==
        "e2a35-sampling-bias-self-correction-canary-v1" ||
      protocol.status !==
        "e2a35_protocol_frozen_not_authorized_not_executed" ||
      protocol.held_out_domain !== "research_methods" ||
      protocol.held_out_topic !== "sampling_bias_and_self_selection" ||
      protocol.execution_boundary.execution_authorized !== false ||
      protocol.execution_boundary.live_execution_performed !== false ||
      protocol.execution_boundary.provider_calls_made !== 0 ||
      protocol.execution_boundary.network_requests_made !== 0 ||
      protocol.contract_versions.evaluator !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
      protocol.contract_versions.anchor_stance_scope !==
        ANCHOR_STANCE_SCOPE_RESOLUTION_VERSION ||
      protocol.contract_versions.self_correction_intent !==
        SELF_CORRECTION_INTENT_VERSION ||
      protocol.contract_versions.trajectory_envelope !==
        TRAJECTORY_ENVELOPE_VERSION ||
      protocol.contract_hashes.target_evidence !==
        TARGET_EVIDENCE_CONTRACT_HASH ||
      protocol.contract_hashes.canonical_anchor !==
        CANONICAL_ANCHOR_CONTRACT_HASH ||
      protocol.contract_hashes.anchor_stance !==
        STANCE_EVIDENCE_CONTRACT_HASH ||
      protocol.contract_hashes.self_correction_intent !==
        SELF_CORRECTION_INTENT_CONTRACT_HASH ||
      protocol.contract_hashes.compiled_evaluator_v5_request !==
        COMPILED_EVALUATOR_V5_REQUEST_HASH ||
      protocol.contract_hashes.trajectory_envelope !==
        TRAJECTORY_ENVELOPE_HASH ||
      protocol.contract_hashes.budget !== BUDGET_HASH ||
      protocol.contract_hashes.artifact_contract !==
        ARTIFACT_CONTRACT_HASH ||
      Object.values(protocol.deterministic_gate_results).some(
        (passed) => passed !== true
      )) {
    throw new Error("e2a35_frozen_protocol_mismatch");
  }
  return protocol;
}

function buildE2A35FrozenProtocol(): Protocol {
  ...
  const selfCorrectionContract = SelfCorrectionIntentContractV1Schema.parse(
    readJson(frozenArtifactPath("self-correction-intent-contract.json"))
  );
  ...
  if (frozen.protocol_hash !== PROTOCOL_HASH ||
      ...
      frozen.scenario.active_distractor !== targetContract.distractor_claim ||
      stableHash(targetContract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(targetContract.active_anchor_alias_contract) !==
        SAMPLING_BIAS_ALIAS_CONTRACT_HASH ||
      stableHash(selfCorrectionContract) !==
        SELF_CORRECTION_INTENT_CONTRACT_HASH ||
      stableHash(trajectory) !== TRAJECTORY_ENVELOPE_HASH) {
    throw new Error("e2a35_frozen_protocol_file_sha_mismatch");
  }
  const expectedObjectives = new Map<number, string>([
    [1, "initial_misconception"],
    [2, "unsupported_self_correction"],
    [3, "valid_partial_self_correction"],
    [4, "regression_after_correction"],
    [5, "partial_repair_after_regression"],
    [6, "independent_self_selection_explanation"]
  ]);
  ...
  return {
    ...
    session_count: 1,
    session: {
      session_id: "E2A35-SAMPLING_BIAS_SELF_CORRECTION",
      design:
        "held_out_sampling_bias_self_correction_trajectory_envelope",
      academic_domain: frozen.held_out_domain,
      ...
      student_profile: {
        ...
        trajectory:
          "initial endorsement, unsupported correction claim, valid partial correction, regression, renewed repair, and independent self-selection explanation"
      },
      frozen_student_trajectory: [
        turn(1, {
          trajectory_role: "misconception",
          fixture_reasoning_quality: "misconception",
          no_live_fixture_message:
            "I agree with option D because anyone could volunteer, so the volunteers represent the whole university.",
          simulator_instruction:
            "Explicitly endorse option D and claim that open eligibility makes the volunteer sample representative. Do not mention self-selection bias.",
          semantic_envelope: ["misconception", "partial"],
          tutor_expected: true,
          required_anchor_application: "explicit",
          required_anchor_stance: "endorses_distractor",
          expected_self_correction_intent: "no_self_correction_intent",
          expected_self_correction_evidence_status: "not_applicable",
          expected_self_correction_disposition: "continue_normal_evaluation",
          latest_valid_evidence_eligible: false
        }),
        turn(2, {
          trajectory_role: "unsupported_self_correction",
          fixture_reasoning_quality: "insufficient",
          no_live_fixture_message:
            "I think my previous answer was wrong.",
          simulator_instruction:
            "Say naturally that the previous answer was wrong, but provide no conceptual reason, no sampling mechanism, and no anchor stance.",
          semantic_envelope: ["insufficient", "misconception", "partial"],
          tutor_expected: true,
          expected_self_correction_intent: "self_correction_intent",
          expected_self_correction_evidence_status:
            "copied_correction_language",
          expected_self_correction_disposition: "request_revision_evidence",
          latest_valid_evidence_eligible: false
        }),
        turn(3, {
          trajectory_role: "valid_partial_self_correction",
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "I think my previous answer was incomplete because volunteer students may already be more interested in online learning than other students. I reject option D.",
          simulator_instruction:
            "Make a natural self-correction. Identify that volunteers may already be more interested in online learning and explicitly reject option D, but do not fully explain how self-selection changes the observed preference or state a complete population-generalization conclusion.",
          semantic_envelope: ["misconception", "partial", "sound"],
          tutor_expected: true,
          required_anchor_application: "explicit",
          required_anchor_stance: "rejects_distractor",
          expected_self_correction_intent: "self_correction_intent",
          expected_self_correction_evidence_status: "revised_evidence_present",
          expected_self_correction_disposition: "evaluate_revised_evidence",
          latest_valid_evidence_eligible: true
        }),
        turn(4, {
          trajectory_role: "regression_after_correction",
          fixture_reasoning_quality: "misconception",
          no_live_fixture_message:
            "I need to change my previous answer: option D is still right because anyone could volunteer, so the volunteers represent the whole university.",
          simulator_instruction:
            "Naturally revise the prior response again, explicitly endorse option D, and restore the claim that open eligibility makes volunteers representative. Do not explain self-selection.",
          semantic_envelope: ["misconception", "partial"],
          tutor_expected: true,
          required_anchor_application: "explicit",
          required_anchor_stance: "endorses_distractor",
          ...
          expected_self_correction_disposition: "evaluate_revised_evidence",
          latest_valid_evidence_eligible: true,
          profile_reopening_expected: true,
          required_tutor_goal:
            "reopen support for the returned representation misconception and elicit the self-selection mechanism"
        }),
        turn(5, {
          trajectory_role: "partial_repair",
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "Students who volunteer might already prefer online learning, so the volunteer sample can differ from other students. Does that mean the result only describes the volunteers?",
          simulator_instruction:
            "Repair part of the reasoning by identifying a systematic volunteer difference and ask whether the result is limited to volunteers. Do not force an exact evaluator label; preserve sound evidence if independently produced.",
          semantic_envelope: ["partial", "sound"],
          tutor_expected: true,
          expected_self_correction_intent: "no_self_correction_intent",
          expected_self_correction_evidence_status: "not_applicable",
          expected_self_correction_disposition: "continue_normal_evaluation",
        ...
        turn(6, {
          trajectory_role: "sound",
          fixture_reasoning_quality: "sound",
          no_live_fixture_message:
            "I reject option D. Students who choose to volunteer may already prefer online learning, so self-selection can raise the preference observed in the sample. Open eligibility does not make the people who volunteer representative, and the result cannot be generalized to all university students without a more representative sample.",
          semantic_envelope: ["partial", "sound"],
          tutor_expected: false,
          simulator_instruction:
            "Independently explain how pre-existing preference affects both volunteering and observed preference, state the population-generalization limit, and explicitly reject option D. Do not copy the tutor's wording.",
          required_anchor_application: "explicit",
          required_anchor_stance: "rejects_distractor",
...
function frozenArtifactContract() {
  return readJson<JsonObject>(
    frozenArtifactPath("artifact-contract.json")
  );
}
...
function frozenCompositeRuntimeIdentity() {
  ...
  if (recorded !== FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      ...
      (identity.evaluator_v5 as JsonObject | undefined)?.evaluator_version !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
      identity.target_evidence_contract_hash !==
        TARGET_EVIDENCE_CONTRACT_HASH ||
      identity.canonical_anchor_contract_hash !==
        CANONICAL_ANCHOR_CONTRACT_HASH ||
      identity.anchor_stance_contract_hash !==
        STANCE_EVIDENCE_CONTRACT_HASH ||
      identity.self_correction_intent_contract_hash !==
        SELF_CORRECTION_INTENT_CONTRACT_HASH ||
      identity.trajectory_envelope_hash !== TRAJECTORY_ENVELOPE_HASH ||
      identity.execution_authorized !== false ||
      identity.live_execution_performed !== false) {
    throw new Error("e2a35_frozen_composite_runtime_identity_mismatch");
  }
  return identity;
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a35_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a35_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a35_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a35_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a35_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a35_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a35_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a35_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a35-live.ts",
    "prisma/formative-evaluation-e2a35.ts",
    ...
    path.relative(process.cwd(),
      frozenArtifactPath("self-correction-intent-contract.json")),
...
function protectedEvidenceIdentity() {
  ...
  const dataRoots = readdirSync(dataRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() &&
      /^e2a(?:1[2-9]|2[0-9]|3[0-4])[a-z]?(?:\D|$)/u.test(entry.name))
  ...
  return {
    snapshot_version: "e2a35-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    ...
    frozen_e2a35_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    frozen_e2a35_protocol_hash: PROTOCOL_HASH,
    frozen_e2a35_target_evidence_contract_hash:
      TARGET_EVIDENCE_CONTRACT_HASH,
    frozen_e2a35_alias_contract_hash:
      SAMPLING_BIAS_ALIAS_CONTRACT_HASH,
    frozen_e2a35_canonical_anchor_contract_hash:
      CANONICAL_ANCHOR_CONTRACT_HASH,
    ...
    frozen_e2a35_compiled_evaluator_v5_request_hash:
      COMPILED_EVALUATOR_V5_REQUEST_HASH,
    frozen_e2a35_overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    frozen_e2a35_artifact_contract_hash: ARTIFACT_CONTRACT_HASH,
    frozen_e2a35_budget_hash: BUDGET_HASH,
    frozen_e2a35_stance_evidence_contract_hash:
      STANCE_EVIDENCE_CONTRACT_HASH,
    frozen_e2a35_self_correction_intent_contract_hash:
      SELF_CORRECTION_INTENT_CONTRACT_HASH,
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a35-dispatch-checkpoint-v1";
  recorded_at: string;
...
/*
 * The E2A.31b scaffold below is retained as migration context only. E2A.33
 * uses the trajectory-envelope-aware preflight and authorization functions
 * that follow this block.
/*
 * Historical E2A.34 preflight retained only as migration context. E2A.35 uses
 * the self-correction-specific preflight below.
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  const blockers: string[] = [];
  const candidate = evaluateE2A24Candidate();
  const protocol = buildE2A35FrozenProtocol();
  ...
  const freezeSummary = readJson<{ status?: string;
    ...
    e2a35_live_execution_performed?: boolean;
  ...
  const overlap = readJson<{ passed?: boolean;
    ...
    broad_statistical-inference_domain_previously_seen_in_deterministic_calibration?: boolean;
  ...
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    ...
    correctionArtifactPath("e2a35-budget.json")
  );
  ...
  if (freezeSummary.status !==
      "e2a35_protocol_ready_for_separate_authorization" ||
  ...
  if (freezeValidation.passed !== true ||
    ...
    blockers.push("e2a35_artifact_validation_invalid");
  }
  ...
  if (overlap.passed !== true ||
      ...
      overlap.broad_statistical-inference_domain_previously_seen_in_deterministic_calibration !==
        true ||
  ...
  if (pids("[f]ormative-evaluation-e2a35").length > 0) {
    blockers.push("duplicate_e2a35_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a35_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a35_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A35 !== "1") blockers.push("live_opt_in_missing");
  ...
  return {
    version: "e2a35-live-preflight-v1",
    passed: blockers.length === 0,
    blockers,
    current_git_commit: commit,
    tracked_worktree_clean: trackedTreeClean(),
    application_build_info: buildInfo,
    authoritative_e2a35_freeze_run: E2A35_FREEZE_RUN,
    historical_e2a33_runtime_freeze_run: HISTORICAL_E2A33_FREEZE_RUN,
    authoritative_e2a35_protocol_run: E2A35_PROTOCOL_RUN,
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const candidate = evaluateE2A24Candidate();
  const protocol = buildE2A35FrozenProtocol();
  const summary = readJson<JsonObject>(frozenArtifactPath("summary.json"));
  const validation = readJson<JsonObject>(
    frozenArtifactPath("artifact-validation.json")
  );
  const artifactContract = frozenArtifactContract();
  const frozenIdentity = frozenCompositeRuntimeIdentity();
  const budget = readJson<JsonObject>(frozenArtifactPath("budget.json"));
  const overlap = readJson<JsonObject>(
    frozenArtifactPath("overlap-analysis.json")
  );
  ...
  const providerGuard = readJson<JsonObject>(
    frozenArtifactPath("provider-call-guard.json")
  );
  const targetContract = contractFor(protocol.session);
  const canonicalAnchor = readJson<JsonObject>(
    frozenArtifactPath("canonical-anchor-contract.json")
  );
  const stanceContract = readJson<JsonObject>(
  ...
  const protectedSources = readJson<{
    passed?: boolean;
    evaluator_v5_unchanged?: boolean;
    tutor_candidate_unchanged?: boolean;
    trajectory_envelope_unchanged?: boolean;
    profile_update_contract_unchanged?: boolean;
    results?: Array<{
      path?: string;
  ...
  if (
    ...
    summary.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    summary.self_correction_calibration_case_count !== 160 ||
    summary.self_correction_calibration_passed !== true ||
    summary.deterministic_self_correction_passed !== true ||
    summary.profile_update_regressions_passed !== true ||
    summary.regression_reopening_passed !== true ||
    summary.trajectory_envelope_passed !== true ||
    summary.held_out_overlap_passed !== true ||
    summary.candidate_integrity_passed !== true ||
    summary.protected_source_integrity_passed !== true ||
    summary.evaluator_v5_request_compiled !== true ||
    summary.latest_valid_evidence_has_precedence !== true ||
    summary.earlier_misconception_remains_historical !== true ||
    summary.regression_reopens_profile !== true ||
    summary.sound_authorizes_revision_immediately !== true ||
    summary.e2a35_live_execution_performed !== false ||
    summary.e2a35_execution_authorized !== false ||
  ...
  if (
    ...
    validation.expected_artifact_count_before_validation !== 23 ||
    validation.actual_artifact_count_before_validation !== 23 ||
    (validation.missing_artifacts as unknown[] | undefined)?.length !== 0 ||
    (validation.unexpected_artifacts as unknown[] | undefined)?.length !== 0 ||
    (validation.unsafe_artifacts as unknown[] | undefined)?.length !== 0
  ) {
    blockers.push("e2a35_artifact_validation_invalid");
  }
  const requiredFutureArtifacts =
    artifactContract.future_live_artifacts_required_after_separate_implementation;
  if (
    stableHash(artifactContract) !== ARTIFACT_CONTRACT_HASH ||
    artifactContract.artifact_contract_version !==
      "e2a35-artifact-contract-v1" ||
    !Array.isArray(requiredFutureArtifacts) ||
    ![
      "self-correction-intent-results.jsonl",
      "profile-update-dispositions.jsonl",
      "cumulative-profile-updates.jsonl",
      ...
      "progression-results.jsonl",
      "human-review-packet.json"
    ].every((name) => requiredFutureArtifacts.includes(name)) ||
    artifactContract.provider_outputs_must_be_preserved !== true ||
    artifactContract.self_correction_resolution_must_be_preserved !== true ||
    artifactContract.latest_evidence_precedence_trace_required !== true ||
    artifactContract.profile_reopening_trace_required !== true ||
    artifactContract.human_review_required !== true ||
    artifactContract.live_execution_in_this_phase !== false
  ) {
    blockers.push("frozen_artifact_contract_mismatch");
  }
  if (
    stableHash(budget) !== BUDGET_HASH ||
    budget.live_execution_authorized !== false ||
    budget.isolated_session_count !== BUDGET.sessions ||
    ...
    budget.maximum_usd_when_pricing_metadata_available !== BUDGET.cost_usd ||
    budget.provider_concurrency !== BUDGET.provider_concurrency ||
    budget.provider_transport_retry_policy_version !==
      PROVIDER_TRANSPORT_RETRY_POLICY_VERSION
  ) {
    blockers.push("frozen_budget_mismatch");
  }
  ...
  if (
    sha256(readFileSync(frozenArtifactPath(
      "compiled-evaluator-v5-request.json"
    ))) !== COMPILED_EVALUATOR_V5_ARTIFACT_SHA ||
    stableHash(compiledEvaluator) !== COMPILED_EVALUATOR_V5_REQUEST_HASH ||
    compiledEvaluator.schema_version !==
      PRODUCTION_TURN_EVIDENCE_EVALUATOR_INPUT_SCHEMA_VERSION_V5 ||
    (compiledEvaluator.required_structured_output as JsonObject | undefined)
      ?.evaluator_version !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
    (compiledEvaluator.legacy_evaluator_input as JsonObject | undefined)
      ?.self_correction_intent_resolution === undefined
  ) {
    blockers.push("frozen_evaluator_v5_request_invalid");
  }
  if (
    ...
    stableHash(stanceContract) !== STANCE_EVIDENCE_CONTRACT_HASH ||
    stableHash(selfCorrectionContract) !==
      SELF_CORRECTION_INTENT_CONTRACT_HASH ||
    stableHash(trajectory) !== TRAJECTORY_ENVELOPE_HASH ||
    stanceContract.resolver_version !==
      ANCHOR_STANCE_SCOPE_RESOLUTION_VERSION ||
    ...
    blockers.push("frozen_sampling_bias_contract_invalid");
  }
  if (
    stableHash(overlap) !== OVERLAP_ANALYSIS_HASH ||
    overlap.passed !== true ||
    ...
    overlap.scenario_or_domain_reuse_permitted !== false
  ) {
    blockers.push("held_out_overlap_invalid");
  }
  if (
    providerGuard.passed !== true ||
    providerGuard.provider_client_constructed !== false ||
    providerGuard.provider_dispatch_enabled !== false ||
    providerGuard.provider_calls_made !== 0 ||
    providerGuard.network_requests_made !== 0 ||
    providerGuard.e2a35_live_execution_performed !== false
  ) {
    blockers.push("frozen_provider_call_guard_invalid");
  }
  if (
  ...
  const protectedSourceChecks = [
    ...
    ["src/lib/services/student-assessment/production-turn-evidence-evaluator-v5.ts",
      EVALUATOR_V5_SHA],
    ["src/lib/services/student-assessment/target-evidence-contract-v5.ts",
      TARGET_EVIDENCE_V5_SHA],
    ["src/lib/services/student-assessment/anchor-stance-scope-resolution-v1.ts",
      ANCHOR_STANCE_SCOPE_SHA],
    ["src/lib/services/student-assessment/target-evidence-scoped-adjudication-v1.ts",
      SCOPED_ADJUDICATION_SHA],
    ["src/lib/services/student-assessment/turn-evidence-profile-update.ts",
      PROFILE_UPDATE_SHA],
    ["src/lib/evaluation/formative/self-correction-intent-v1.ts",
      SELF_CORRECTION_INTENT_SHA],
    ["src/lib/evaluation/formative/trajectory-envelope-v1.ts",
      TRAJECTORY_ENVELOPE_SOURCE_SHA]
  ] as const;
  if (
    protectedSources.passed !== true ||
    protectedSources.evaluator_v5_unchanged !== true ||
    protectedSources.tutor_candidate_unchanged !== true ||
    protectedSources.trajectory_envelope_unchanged !== true ||
    protectedSources.profile_update_contract_unchanged !== true ||
    protectedSources.results?.some((entry) =>
      entry.unchanged !== true ||
      entry.expected_sha256 !== entry.actual_sha256
    ) ||
    protectedSourceChecks.some(([file, expected]) =>
      sha256(readFileSync(path.join(process.cwd(), file))) !== expected
    )
  ) {
    blockers.push("frozen_source_integrity_mismatch");
  ...
  if (pids("[f]ormative-evaluation-e2a35-live").length > 0) {
    blockers.push("duplicate_e2a35_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a35_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) {
    blockers.push(`prior_e2a35_run_exists:${priorRuns.at(-1)}`);
  }
  ...
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A35 !== "1") {
  ...
  return {
    ...
    candidate_authorized_for_activation: false,
    candidate_active: false,
    frozen_evaluator_v5_request_valid:
      !blockers.includes("frozen_evaluator_v5_request_invalid"),
    frozen_canonical_anchor_contract_valid:
      !blockers.includes("frozen_sampling_bias_contract_invalid"),
    self_correction_intent_version: SELF_CORRECTION_INTENT_VERSION,
    self_correction_contract_valid:
      !blockers.includes("frozen_sampling_bias_contract_invalid"),
    latest_valid_evidence_precedence_verified:
      summary.latest_valid_evidence_has_precedence === true,
    profile_reopening_verified:
      summary.regression_reopens_profile === true,
    anchor_stance_scope_resolution_version:
      ANCHOR_STANCE_SCOPE_RESOLUTION_VERSION,
    target_evidence_scoped_adjudication_version:
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a35_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a35_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a35_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a35_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a35-one-session-authorization",
    ...
    "--confirm-stop-after-e2a35"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a35_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a35_dispatch_checkpoint_missing");
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const mapperPreservation = readJson<JsonObject>(
    frozenArtifactPath("mapper-preservation-prerequisite.json")
  );
  const targetContract = contractFor(protocol.session);
  const canonicalAnchor = readJson<JsonObject>(
    frozenArtifactPath("canonical-anchor-contract.json")
  );
  const trajectory = readJson<JsonObject>(
    frozenArtifactPath("trajectory-envelope-contract.json")
  );
  const stanceEvidenceContract = readJson<JsonObject>(
    frozenArtifactPath("anchor-stance-contract.json")
  );
  const runtimeTrajectory = TrajectoryEnvelopeContractSchema.parse(
    readJson(frozenArtifactPath(
  ...
  if (
    ...
    summary.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    summary.deterministic_regressions_passed !== true ||
    summary.required_contradiction_passed !== true ||
    summary.overlap_analysis_passed !== true ||
    summary.no_prior_scenario_reuse !== true ||
    summary.evaluator_v5_unchanged !== true ||
    summary.tutor_candidate_unchanged !== true ||
    summary.e2a35_ready_for_separate_authorization !== true ||
  ...
  if (
    validation.passed !== true ||
    validation.required_artifact_count !== 25 ||
    validation.actual_artifact_count !== 25 ||
    (validation.missing_artifacts as unknown[])?.length !== 0 ||
    (validation.unexpected_artifacts as unknown[])?.length !== 0 ||
    (validation.unsafe_artifacts as unknown[])?.length !== 0 ||
    (validation.semantic_issues as unknown[])?.length !== 0 ||
    validation.provider_calls_made !== 0 ||
    validation.network_requests_made !== 0
  ) {
    blockers.push("e2a35_artifact_validation_invalid");
  }
  if (
    mapperPreservation.passed !== true ||
    mapperPreservation.preservation_contract_passed !== true ||
    mapperPreservation.production_wiring_passed !== true ||
    mapperPreservation.source_artifact_validation_passed !== true ||
    mapperPreservation.mapper_version !==
      TURN_EVIDENCE_PROFILE_MAPPER_VERSION_V7 ||
    mapperPreservation.preservation_contract_version !==
      TARGET_EVIDENCE_MAPPER_PRESERVATION_VERSION ||
    mapperPreservation.finalization_version !==
      PRE_TUTOR_PROFILE_FINALIZATION_VERSION_V4 ||
    mapperPreservation.provider_calls_made !== 0
  ) {
    blockers.push("frozen_mapper_preservation_prerequisite_invalid");
  }
  if (
    stableHash(artifactContract) !== ARTIFACT_CONTRACT_HASH ||
    artifactContract.artifact_contract_version !==
      "e2a35-artifact-contract-v1" ||
    artifactContract.required_count !== 25 ||
    artifactContract
      .immutable_provider_outputs_required_after_future_live_execution !==
        true ||
    artifactContract
      .human_review_packet_required_after_future_live_execution !== true ||
    artifactContract.live_execution_in_this_phase !== false ||
    artifactContract.provider_calls_made !== 0
  ) {
    blockers.push("frozen_artifact_contract_mismatch");
  }
  if (
    stableHash(budget) !== BUDGET_HASH ||
    (budget.maximum as JsonObject | undefined)?.isolated_sessions !==
      BUDGET.sessions ||
    (budget.maximum as JsonObject | undefined)?.logical_generation_calls !==
      BUDGET.logical_generation_calls ||
    (budget.maximum as JsonObject | undefined)?.adapter_attempts !==
      BUDGET.adapter_attempts ||
    (budget.maximum as JsonObject | undefined)
      ?.transport_retries_per_logical_call !==
      BUDGET.transport_retries_per_logical_call ||
    (budget.maximum as JsonObject | undefined)?.input_tokens !==
      BUDGET.input_tokens ||
    (budget.maximum as JsonObject | undefined)?.output_tokens !==
      BUDGET.output_tokens ||
    (budget.maximum as JsonObject | undefined)?.total_tokens !==
      BUDGET.total_tokens ||
    (budget.maximum as JsonObject | undefined)
      ?.cost_usd_when_pricing_available !== BUDGET.cost_usd ||
    (budget.maximum as JsonObject | undefined)?.provider_concurrency !==
      BUDGET.provider_concurrency ||
  ...
  if (
    ...
    compiledEvaluator.canonical_request_hash !==
      COMPILED_EVALUATOR_V5_REQUEST_HASH ||
    compiledEvaluator.passed !== true ||
    compiledEvaluator.evaluator_version !==
      PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
    compiledEvaluator.input_schema_version !==
      PRODUCTION_TURN_EVIDENCE_EVALUATOR_INPUT_SCHEMA_VERSION_V5 ||
    compiledEvaluator.output_schema_version !==
      PRODUCTION_TURN_EVIDENCE_EVALUATOR_OUTPUT_SCHEMA_VERSION_V5 ||
    (compiledEvaluator.old_evaluator_contract_matches as unknown[])
      ?.length !== 0 ||
    compiledEvaluator.provider_dispatch_performed !== false ||
    compiledEvaluator.provider_calls_made !== 0
  ) {
    blockers.push("frozen_evaluator_v5_request_invalid");
  }
  if (
    stableHash(targetContract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
    stableHash(targetContract.active_anchor_alias_contract) !==
      STATISTICAL_INFERENCE_ALIAS_CONTRACT_HASH ||
    stableHash(canonicalAnchor) !== CANONICAL_ANCHOR_CONTRACT_HASH ||
    stableHash(trajectory) !== TRAJECTORY_ENVELOPE_HASH ||
    stableHash(stanceEvidenceContract) !== STANCE_EVIDENCE_CONTRACT_HASH ||
    stanceEvidenceContract.stance_contract_version !==
      "e2a35-p-value-anchor-stance-v1" ||
    stanceEvidenceContract.resolver_version !==
      ANCHOR_STANCE_EVIDENCE_RESOLUTION_VERSION ||
    stableHash(runtimeTrajectory) !== RUNTIME_TRAJECTORY_ENVELOPE_HASH ||
    runtimeTrajectory.authority_boundary
      .production_sound_gate_is_authoritative_for_progression !== true ||
    runtimeTrajectory.authority_boundary
      .exact_turn_by_turn_reasoning_labels_prohibited !== true
  ) {
    blockers.push("frozen_statistical_inference_contract_invalid");
  }
  if (
    stableHash(overlap) !== OVERLAP_ANALYSIS_HASH ||
    overlap.passed !== true ||
    overlap.no_prior_scenario_reuse !== true ||
    (overlap.prohibited_domain_alias_matches as unknown[])?.length !== 0
  ) {
    blockers.push("held_out_overlap_invalid");
  }
  if (
    providerGuard.passed !== true ||
    providerGuard.provider_executor_constructed !== false ||
    providerGuard.provider_calls_made !== 0 ||
    providerGuard.network_requests_made !== 0 ||
    providerGuard.execution_authorized !== false
  ) {
    blockers.push("frozen_provider_call_guard_invalid");
  }
  if (
  ...
  const protectedSourceChecks = [
    ...
    ["src/lib/services/student-assessment/active-anchor-alias-resolution-v3.ts",
      COMPOSED_RESOLVER_V3_SHA],
    ["src/lib/services/student-assessment/anchor-stance-evidence-resolution-v2.ts",
      STANCE_EVIDENCE_RESOLVER_V2_SHA],
    ["src/lib/services/student-assessment/active-anchor-alias-resolution-v4.ts",
      COMPOSED_RESOLVER_V4_SHA],
    ["src/lib/services/student-assessment/canonical-anchor-evidence.ts",
      CANONICAL_ANCHOR_EVIDENCE_SHA],
    ["src/lib/services/student-assessment/target-evidence-mapper-v7.ts",
      MAPPER_V7_SHA],
    ["src/lib/services/student-assessment/pre-tutor-profile-finalization-v4.ts",
      PRE_TUTOR_FINALIZATION_V4_SHA],
    ["src/lib/services/student-assessment/anchor-conclusion-consistency.ts",
      SOUND_GATE_SHA],
    ["src/lib/evaluation/formative/trajectory-envelope-v1.ts",
      TRAJECTORY_ENVELOPE_SOURCE_SHA]
  ...
  return {
    ...
    frozen_evaluator_v5_request_valid:
      !blockers.includes("frozen_evaluator_v5_request_invalid"),
    frozen_canonical_anchor_contract_valid:
      !blockers.includes("frozen_statistical_inference_contract_invalid"),
    anchor_reference_resolver_version: "active-anchor-alias-resolution-v1",
    anchor_stance_evidence_contract_version:
      ANCHOR_STANCE_EVIDENCE_CONTRACT_VERSION,
    anchor_stance_resolver_version:
...
function recordDispatchCheckpoint() {
  ...
  if (!trackedTreeClean()) {
    throw new Error("e2a35_checkpoint_tree_not_clean");
  }
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(
      `e2a35_checkpoint_preflight_failed:${check.blockers.join(",")}`
    );
  }
  const identity = compositeRuntimeIdentity();
  const checkpoint: DispatchCheckpoint = {
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  if (stableHash(contract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(contract.active_anchor_alias_contract) !==
        SAMPLING_BIAS_ALIAS_CONTRACT_HASH) {
    throw new Error("e2a35_target_evidence_contract_hash_mismatch");
  }
  return contract;
}

function selfCorrectionContract(): SelfCorrectionIntentContractV1 {
  const contract = SelfCorrectionIntentContractV1Schema.parse(
    readJson(frozenArtifactPath("self-correction-intent-contract.json"))
  );
  if (stableHash(contract) !== SELF_CORRECTION_INTENT_CONTRACT_HASH) {
    throw new Error("e2a35_self_correction_contract_hash_mismatch");
  }
  return contract;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a35_session_${session.session_id}`,
    scenario_version: "e2a35-trajectory-envelope-simulator-v1",
...
function validateSimulatorOutput(input: {
  ...
  const resolution = resolveActiveAnchorAliasV4({
    ...
    source_turn_id: `e2a35_simulator_validation_turn_${input.turn.turn}`,
  ...
  if (input.turn.expected_self_correction_intent &&
      selfCorrection.intent !==
        input.turn.expected_self_correction_intent) {
    issues.push("self_correction_intent_mismatch");
  }
  if (input.turn.expected_self_correction_evidence_status &&
      selfCorrection.evidence_status !==
        input.turn.expected_self_correction_evidence_status) {
    issues.push("self_correction_evidence_status_mismatch");
  }
  if (input.turn.expected_self_correction_disposition &&
      selfCorrection.downstream_disposition !==
        input.turn.expected_self_correction_disposition) {
    issues.push("self_correction_disposition_mismatch");
  }
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a35_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a35_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a35_${session.session_id}`,
    ...
    source_activity_context: {
      ...
      profile_condition: "synthetic_frozen_e2a35_trajectory",
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a35_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a35_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a35",
    source_diagnostic_purpose: "reasoning_boundary_repair",
    ...
    response_summary: contradictory
      ? `${session.session_id} turn ${turn.turn} explains self-selection but endorses the volunteer-representation claim, creating a reasoning/conclusion conflict.`
...
function noLiveTutorOutput(input: AutonomousPedagogyInput) {
  ...
  const strategies = [
    "contrast open eligibility with representativeness of the resulting volunteer sample",
    "trace how pre-existing online-learning preference can affect volunteering and the observed result",
    "ask for a one-sentence link between self-selection and the population-generalization limit",
    "test the representation claim with a concrete volunteer versus non-volunteer difference",
    "reconcile the stated option conclusion with the student's explanation of volunteer differences"
  ];
  const studentTasks = [
    ...
    "Set the wording aside and rebuild the sampling reasoning in one or two sentences, ending with its consequence for this option.",
    "Use a concrete counterexample to test the option, then explain the general rule your example reveals.",
    "Put the null-conditional explanation and your final judgment side by side. Do they support the same conclusion? Explain which part you would keep or change."
  ];
...
function buildE2A35NoLiveStructuredTurnEvidence(input: {
  ...
  return ProductionTurnEvidenceEvaluatorOutputV5Schema.shape
    .structured_turn_evidence.parse({
      ...
      evidence_limitations: [
        ...
        "e2a35_no_live_stance_evidence_v2_fixture"
      ]
    });
}
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a35_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a35_no_live_evaluator_case_missing");
      ...
      output = ProductionTurnEvidenceEvaluatorOutputV5Schema.parse({
        ...
        structured_turn_evidence:
          buildE2A35NoLiveStructuredTurnEvidence({
            source_student_turn_id:
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        ...
        throw new Error(
          `e2a35_no_live_tutor_fixture_invalid:turn_${turnNumber}:sequence_${tutorInput.latest_student_response.source_sequence_index}:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`
        );
      }
    } else {
      throw new Error(`e2a35_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a35_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a35_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a35_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a35_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a35:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a35_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a35_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a35_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a35_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a35_canary_failed_rate_limit_retry_exhausted"
          : "e2a35_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a35_canary_failed_provider_nonretryable_request"
        : `e2a35_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation?.passed) {
        throw new Error(
          `e2a35_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation?.issues.join("|") ?? "validation_not_run"}`
        );
      }
      ...
      appendJsonl(path.join(
        ...
        expected_evidence_status:
          turn.expected_self_correction_evidence_status ?? null,
        expected_downstream_disposition:
          turn.expected_self_correction_disposition ?? null,
        ...
        passed:
          ...
          (!turn.expected_self_correction_evidence_status ||
            selfCorrection.evidence_status ===
              turn.expected_self_correction_evidence_status) &&
          (!turn.expected_self_correction_disposition ||
            selfCorrection.downstream_disposition ===
              turn.expected_self_correction_disposition)
      });
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a35_canary_failed_evaluator_contract");
          }
          appendJsonl(path.join(
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a35_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a35_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a35_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a35_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a35_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
          ...
          appendJsonl(path.join(
            input.runDir, "sampling-bias-alias-resolution-results.jsonl"
          ), {
            session_id: input.session.session_id,
            turn: turn.turn,
            target_evidence_contract_hash: TARGET_EVIDENCE_CONTRACT_HASH,
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a35_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a35_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a35_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a35_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a35_idempotency_replay_failed");
      }
      ...
      if (!result.profile_update_record) {
        throw new Error("e2a35_profile_update_record_missing");
      }
      ...
      if (!latestValidEvidenceApplied) {
        throw new Error("e2a35_latest_valid_evidence_not_authoritative");
      }
      if (!unsupportedCorrectionPreserved) {
        throw new Error("e2a35_correction_language_promoted_to_evidence");
      }
      if (turn.profile_reopening_expected && !profileReopened) {
        throw new Error([
          "e2a35_regression_did_not_reopen_profile",
          updateDisposition,
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a35_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a35_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a35_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error(
          `e2a35_anchor_stance_misclassified:turn_${turn.turn}:expected_${turn.required_anchor_stance}:observed_${anchor.anchor_stance}`
        );
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a35_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a35_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
        ...
        !profileReopened
      )) throw new Error("e2a35_profile_reopening_failure");
      ...
      if (
        ...
        throw new Error("e2a35_unsupported_sound_promotion");
      }
      if (soundGatePassed) {
        if (
          ...
          throw new Error("e2a35_sound_gate_progression_failure");
        }
        endpoint = "passed_required_revision_endpoint";
      } else if (result.route.selected_mode === "request_revision") {
        throw new Error("e2a35_premature_revision_without_sound_gate");
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a35_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a35_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a35_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a35_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a35_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a35_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a35-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  const targetContract = readJson<JsonObject>(path.join(
    runDir, "sampling-bias-target-evidence-contract.json"
  ));
  const aliasContract = readJson<JsonObject>(path.join(
    runDir, "sampling-bias-alias-contract.json"
  ));
  ...
  if (frozenIdentity.composite_runtime_identity_hash !==
        FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      stableHash(targetContract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(aliasContract) !== SAMPLING_BIAS_ALIAS_CONTRACT_HASH ||
      stableHash(canonicalAnchor) !== CANONICAL_ANCHOR_CONTRACT_HASH ||
    ...
    failures.push("frozen_sampling_bias_binding_invalid");
  }
  ...
  if (burden.metric_version !== "e2a35-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  const frozenProtocol = frozenProtocolArtifact();
  const frozenIdentity = frozenCompositeRuntimeIdentity();
  ...
  const frozenSelfCorrectionContract = readJson<JsonObject>(
    frozenArtifactPath("self-correction-intent-contract.json")
  );
  ...
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a35-live-canary-manifest-v1",
    run_id: id,
    ...
    frozen_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    target_evidence_contract_hash: TARGET_EVIDENCE_CONTRACT_HASH,
    sampling_bias_alias_contract_hash: SAMPLING_BIAS_ALIAS_CONTRACT_HASH,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a35-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a35-sampling-bias-isolated-fixture-v1",
    synthetic_only: true,
  ...
  writeJson(path.join(runDir, "overlap-analysis-binding.json"), {
    binding_version: "e2a35-overlap-analysis-binding-v1",
    source_run: E2A35_FREEZE_RUN,
    overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    exact_overlap_passed: frozenOverlapAnalysis.exact_overlap_passed,
    normalized_overlap_passed:
      frozenOverlapAnalysis.normalized_overlap_passed,
    token_overlap_passed: frozenOverlapAnalysis.token_overlap_passed,
    semantic_tag_audit_passed:
      frozenOverlapAnalysis.semantic_tag_audit_passed,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a35_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "budget-ledger.json"), {
    ledger_version: "e2a35-budget-ledger-v1",
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a35-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a35-evidence-accuracy-v1",
    ...
    regressed_profile_reopened: readJsonl<JsonObject>(path.join(
      ...
      row.turn === 4 && row.profile_reopened === true &&
      row.passed === true
    ),
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a35-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a35-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a35-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a35-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a35-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a35-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a35-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a35-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a35_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a35_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a35-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A35FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a35_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a35_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a35_no_live_network_detected");
    ...
    if (stanceRows.length !== 6 ||
        ...
        stanceRows.find((row) => row.turn === 4)
          ?.observed_anchor_reference !== "explicit" ||
        stanceRows.find((row) => row.turn === 4)
          ?.observed_anchor_stance !== "endorses_distractor" ||
        stanceRows.find((row) => row.turn === 6)
          ?.observed_anchor_stance !== "rejects_distractor") {
      throw new Error("e2a35_no_live_stance_boundary_failed");
    }
    ...
    if (
      selfCorrectionRows.length !== 6 ||
      selfCorrectionRows.find((row) => row.turn === 2)?.intent !==
        "self_correction_intent" ||
      selfCorrectionRows.find((row) => row.turn === 2)?.evidence_status !==
        "copied_correction_language" ||
      selfCorrectionRows.find((row) => row.turn === 3)
        ?.latest_valid_evidence_eligible !== true ||
      latestEvidenceRows.find((row) => row.turn === 2)
        ?.unsupported_correction_preserved_prior_profile !== true ||
      latestEvidenceRows.find((row) => row.turn === 3)
        ?.latest_valid_evidence_applied !== true ||
      reopeningRows.find((row) => row.turn === 4)
        ?.profile_reopened !== true ||
      reopeningRows.find((row) => row.turn === 4)?.passed !== true
    ) {
      throw new Error("e2a35_no_live_self_correction_state_failed");
    }
    const trajectoryRows = readJsonl<JsonObject>(path.join(
      result.runDir, "trajectory-envelope-results.jsonl"
    )).filter((row) => typeof row.turn === "number");
    if (trajectoryRows.length !== 6 ||
        trajectoryRows.some((row) =>
          row.passed !== true ||
          row.trajectory_expectation_changed_evaluator_output !== false
        ) ||
    ...
    if ([...requiredCriterionIds].some((criterionId) =>
      ...
      throw new Error("e2a35_no_live_named_revision_criteria_failed");
    }
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a35_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
      ...
      turn_4_anchor_reference: "explicit",
      turn_4_anchor_stance: "endorses_distractor",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a35_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a35-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A35FrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a35_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a35_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a35_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a35_confirmation_missing:")) {
    throw new Error("e2a35_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a35_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a35_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a35_run_not_found");
...
function auditRun(run?: string) {
  ...
  const aliasResolutions = readJsonl<JsonObject>(path.join(
    runDir, "sampling-bias-alias-resolution-results.jsonl"
  )).filter((row) => typeof row.turn === "number");
  ...
  if (summary.passed === true) {
    ...
    if (completedTurnCount < 1 || completedTurnCount > 6 ||
    ...
    if (completedTurnCount >= 2 && (
      profile(2)?.reasoning_quality === "sound" ||
      selfCorrectionResults.find((row) => row.turn === 2)?.intent !==
        "self_correction_intent" ||
      selfCorrectionResults.find((row) => row.turn === 2)?.evidence_status !==
        "copied_correction_language" ||
      latestEvidenceResults.find((row) => row.turn === 2)
        ?.unsupported_correction_preserved_prior_profile !== true
    )) {
      failures.push("unsupported_self_correction_promoted_to_understanding");
    }
    if (completedTurnCount >= 3 &&
        selfCorrectionResults.find((row) => row.turn === 3)?.intent ===
          "self_correction_intent" &&
        selfCorrectionResults.find((row) => row.turn === 3)
          ?.latest_valid_evidence_eligible === true &&
        latestEvidenceResults.find((row) => row.turn === 3)
          ?.latest_valid_evidence_applied !== true) {
      failures.push("valid_self_correction_not_applied");
    }
    if (completedTurnCount >= 4 && (
      anchor(4)?.anchor_application !== "explicit" ||
      anchor(4)?.anchor_stance !== "endorses_distractor" ||
      profile(4)?.reasoning_quality !== "misconception" ||
      profile(4)?.revision_readiness !== false ||
      mode(4)?.selected_mode !== "remain_in_dialogue" ||
      profileReopeningResults.find((row) => row.turn === 4)
        ?.profile_reopened !== true ||
      profileReopeningResults.find((row) => row.turn === 4)
        ?.reopened_from_latest_regression !== true ||
      profileReopeningResults.find((row) => row.turn === 4)?.passed !== true
    )) failures.push("turn_4_profile_reopening_invalid");
    ...
    if (!trajectoryResults.every((row) =>
        ...
        !scopeResolutions.every((row) =>
          ...
          row.passed === true &&
          row.resolver_version === SELF_CORRECTION_INTENT_VERSION
  ...
  return {
    audit_version: "e2a35-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
    ...
    turn_counts: {
      ...
      sampling_bias_alias_resolutions: aliasResolutions.length,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a35.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a35_runner_failed");
  process.exitCode = 1;
});
