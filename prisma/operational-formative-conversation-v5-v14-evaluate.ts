...
function assertCanonicalLauncher(options: Options) {
  if (
    process.env
      ...
      "true" ||
    process.env.FORMATIVE_CONVERSATION_V5_V14_CANONICAL_LOADER_VERSION !==
      "formative-conversation-v5-canonical-node-import-tsx-v1"
  ) {
    throw new Error(
      "formative_conversation_v14_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    (process.env
      .FORMATIVE_CONVERSATION_V5_V14_CANONICAL_OUTER_LOADER_VALIDATED !==
      "true" ||
      process.env
        .FORMATIVE_CONVERSATION_V5_V14_CANONICAL_OUTER_LOADER_VERSION !==
        "formative-conversation-v5-canonical-node-import-tsx-v1")
  ) {
    throw new Error(
      "formative_conversation_v14_canonical_outer_loader_required"
    );
  }
}
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v14-evaluation-prisma",
