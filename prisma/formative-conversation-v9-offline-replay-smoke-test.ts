...
const runPublicId = `fcv5_v9_offline_replay_${Date.now()}`;
...
async function main() {
  ...
  const owner = createEvaluationDatabaseConnectionOwner({
    client: prisma,
    client_id: "v9-offline-replay-prisma",
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v9_offline_replay_smoke_failed",
      provider_calls: 0,
      network_requests: 0
    })
  );
