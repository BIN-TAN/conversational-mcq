...
function installIsolatedDatabaseEnvironment() {
  ...
  assert(
    ...
    "The V10 runtime smoke must use local PostgreSQL."
  );
...
async function main() {
  ...
  const childTests = [
    ...
    "prisma/formative-conversation-v10-secret-scanner-smoke-test.ts",
    "prisma/formative-conversation-v10-offline-replay-smoke-test.ts",
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v10_runtime_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0
    })
  );
