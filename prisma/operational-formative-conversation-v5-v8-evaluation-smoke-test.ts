...
async function main() {
  ...
  const v5Source = [
    ...
    "src/lib/operational/formative-conversation-v5-evaluation-v8/contracts.ts"
  ]
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v8/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v8_provider_smoke",
      derived_evaluation_id: "fcv5v8_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v8_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v8_derived_smoke_rerun",
  ...
  assert(
    ...
    "v8 must use fresh runtime and protocol identities while preserving the failed v5, v6, and v7 references."
  );
  assert(
  ...
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v8/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v8/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v8 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
