...
import {
  ...
  type FormativeConversationV18R2ControlPayload
...
const safe = scanFormativeConversationV18R2BufferedOutputs({
...
assert.throws(
  ...
  /formative_conversation_v18r2_secret_match_detected/
);
assert.throws(
  () =>
    scanFormativeConversationV18R2BufferedOutputs({
...
assert.throws(
  ...
  /formative_conversation_v18r2_scan_coverage_mismatch/
);
assert.doesNotThrow(() =>
  assertFormativeConversationV18IntendedArtifactCoverage({
...
async function releaseFixture(zipContent: string) {
  const workspaceRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "fcv5-v18r2-security-"))
  );
  const nonce = randomUUID().replaceAll("-", "");
  const packageId = `fcv5v18r2_provider_security_${nonce}`;
  ...
  ownerOnlyDirectory(path.join(workspaceRoot, FORMATIVE_CONVERSATION_V18R2_DATA_ROOT));
  ...
  const control: FormativeConversationV18R2ControlPayload = {
    ...
    derived_evaluation_id: `fcv5v18r2_derived_security_${nonce}`
  };
...
async function verifyPreventiveRelease() {
  ...
  try {
    const report = await releaseFormativeConversationV18R2Artifacts({
