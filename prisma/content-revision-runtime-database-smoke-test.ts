...
const childTests = ["prisma/content-revision-smoke-test.ts"] as const;

function installIsolatedEnvironment() {
  const parsed = new URL(isolatedDatabaseUrl);
  assert(
    parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1",
    "The content-revision smoke must use local PostgreSQL."
  );
  ...
  try {
    ...
    for (const childTest of childTests) {
      ...
      assert.equal(
        child.status,
        0,
        child.error?.message ?? `The content-revision database smoke failed: ${childTest}`
      );
    }
  } finally {
    await dropLiveCanaryDatabaseIfPresent();
...
main().catch((error) => {
  console.error(
    JSON.stringify({
      status: "failed",
      error_code:
        error instanceof Error ? error.message : "content_revision_runtime_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0,
      generation_network_requests: 0,
      dispatch_checkpoints: 0
