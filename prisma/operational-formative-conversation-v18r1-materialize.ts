...
import {
  FORMATIVE_CONVERSATION_V18R1_PRESERVED_ACTIVE_RUNTIME_HASH,
  FORMATIVE_CONVERSATION_V18R1_PRESERVED_ROLLBACK_RUNTIME_HASH,
  FORMATIVE_CONVERSATION_V18R1_REQUIRED_NO_PROVIDER_TESTS,
  FORMATIVE_CONVERSATION_V18R1_RUNTIME_SOURCE_PATHS,
  FORMATIVE_CONVERSATION_V18R1_VERIFICATION_SOURCE_PATHS,
  buildFormativeConversationV18R1RuntimeCandidateManifest
} from "../src/lib/operational/formative-conversation-v18r1/candidate";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/contracts";
...
import { FORMATIVE_CONVERSATION_V18R1_DISPATCH_CHECKPOINT_VERSION } from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/dispatch-checkpoint";
import {
  FORMATIVE_CONVERSATION_V18R1_SECURITY_WRAPPER_SOURCE_PATHS,
  formativeConversationV18SecurityWrapperFingerprint
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/security-release";
import {
  FORMATIVE_CONVERSATION_V18R1_COMMITTED_SOURCE_PATHS,
  FORMATIVE_CONVERSATION_V18R1_RUNNER_SOURCE_PATHS
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/provenance";
import { copyFile } from "node:fs/promises";

const EXPECTED_RUNTIME_HASH =
  "17ca582ac937be7f790a2841d3542a4d79ec9364c04703fecdbfc282134378ca";
const IMMUTABLE_V18_COMMIT =
  "9fd41e1139b7e80f433aff76a6985095333e6b7d";
const IMMUTABLE_V18_RUNTIME_HASH = EXPECTED_RUNTIME_HASH;
const IMMUTABLE_V18_PROTOCOL_HASH =
  "0709d61e2f681e5a73a8b488165bfeffb7a030b5e12bf5dd171eb15a06f4d5ef";
const IMMUTABLE_V18_FAILURE_ARTIFACT_SHA256 =
  "7dc167b68ff09abebb79c5c4e3e7da1ce67b1cb371831f1fa7b7e749126a6940";
const IMMUTABLE_V18_RUNNER_LOG_SHA256 =
  "9d5250f0bacc39c4318a84aa2502be879193c4447b1095c73ecb65fbf4a4d708";
...
function immutableV18Reference() {
  return {
    reference_version: "formative-conversation-v18r1-immutable-v18-reference-v1",
    source_revision: "formative-conversation-host-v5-executable-v18",
    source_commit: IMMUTABLE_V18_COMMIT,
    runtime_candidate_hash: IMMUTABLE_V18_RUNTIME_HASH,
    protocol_hash: IMMUTABLE_V18_PROTOCOL_HASH,
    failed_predispatch_attempt: {
      failure_class: "engineering_contract_integration",
      failure_code: "spawnSync_git_ENOENT",
      failure_artifact_sha256: IMMUTABLE_V18_FAILURE_ARTIFACT_SHA256,
      runner_log_sha256: IMMUTABLE_V18_RUNNER_LOG_SHA256,
    ...
    revision_reason:
      "deployed provenance used Git working-tree semantics unavailable in the canonical Render image",
    research_runtime_changed: false,
    pedagogy_changed: false,
    profiling_semantics_changed: false,
    formative_semantics_changed: false,
    case_meanings_changed: false,
    local_artifact_policy:
      "hash_reference_only_not_required_for_v18r1_live_readiness",
    required_for_v18r1_live_readiness: false,
    candidate_artifacts_mutated: false,
    run_artifacts_mutated: false
  } as const;
}

async function copyImmutableV18Fixtures() {
  ...
  for (const caseId of FORMATIVE_CONVERSATION_V5_CASE_ORDER) {
    await copyFile(
      path.resolve(
        process.cwd(),
        "config/operational-candidates/formative-conversation-host-v5-executable-v18/fixtures",
        `${caseId}.json`
      ),
...
async function main() {
  const runtimeManifest = buildFormativeConversationV18R1RuntimeCandidateManifest();
  if (runtimeManifest.runtime_candidate_hash !== EXPECTED_RUNTIME_HASH) {
    throw new Error("formative_conversation_v18r1_runtime_candidate_hash_drift");
  }
  const profilingPrompt = getPromptForAgent("student_profiling_agent");
  const immutableV18 = immutableV18Reference();
  await copyImmutableV18Fixtures();

  const runtimeSources = await Promise.all(
    FORMATIVE_CONVERSATION_V18R1_RUNTIME_SOURCE_PATHS.map(fileIdentity)
  );
  const runnerSources = await Promise.all(
    FORMATIVE_CONVERSATION_V18R1_RUNNER_SOURCE_PATHS.map(fileIdentity)
  );
  ...
  for (const caseId of FORMATIVE_CONVERSATION_V5_CASE_ORDER) {
    ...
    if (stableHash(hashable) !== fixture.fixture_hash) {
      throw new Error(`formative_conversation_v18r1_fixture_hash_invalid:${caseId}`);
    }
    fixtureRecords.push({
      fixture,
      reference: {
  ...
  const fixtureMaterial = {
    manifest_version: "formative-conversation-v18r1-fixture-manifest-v1",
    ...
    immutable_v18_reference_hash: stableHash(immutableV18),
  ...
  const securitySourceIdentities = await Promise.all(
    FORMATIVE_CONVERSATION_V18R1_SECURITY_WRAPPER_SOURCE_PATHS.map(fileIdentity)
  );
  ...
  const securityWrapperMaterial = {
    manifest_version: "formative-conversation-v18r1-security-wrapper-manifest-v1",
  ...
  const dispatchMaterial = {
    contract_version: "formative-conversation-v18r1-dispatch-contract-v1",
    checkpoint_version: FORMATIVE_CONVERSATION_V18R1_DISPATCH_CHECKPOINT_VERSION,
  ...
  const provenanceMaterial = {
    contract_version: "formative-conversation-v18r1-dual-boundary-provenance-contract-v1",
    ...
    immutable_v18_reference_hash: stableHash(immutableV18),
    committed_source_paths: [...FORMATIVE_CONVERSATION_V18R1_COMMITTED_SOURCE_PATHS],
  ...
  const deployedSourceClosureHash = stableHash({
    closure_version: "formative-conversation-v18r1-deployed-source-closure-v1",
    runtime_source_files: runtimeSources,
    runner_source_files: runnerSources,
    materializer_source: materializerIdentity,
    immutable_v18_reference_hash: stableHash(immutableV18)
  ...
  const protocolMaterial = {
    protocol_version: "formative-conversation-host-v5-executable-v18r1",
    status: "frozen_executable_not_run",
    runtime_candidate_hash: EXPECTED_RUNTIME_HASH,
    pre_freeze_candidate_identity: IMMUTABLE_V18_RUNTIME_HASH,
  ...
  const commandAuthorizationTemplate = authorizationTemplate.replace(
    ...
    "${FORMATIVE_CONVERSATION_V18R1_EXPECTED_DEPLOYED_GIT_SHA}"
  );
  const exactFutureLiveCommand = [
    "node --import tsx scripts/operational-formative-conversation-v5-v18r1-process-local-runner.mjs",
    '--env-fifo "$FORMATIVE_CONVERSATION_V18R1_ENV_FIFO" --',
    "--mode=live",
    `--runtime-candidate-hash=${EXPECTED_RUNTIME_HASH}`,
    `--evaluation-protocol-hash=${protocolHash}`,
    '--expected-deployed-git-sha="$FORMATIVE_CONVERSATION_V18R1_EXPECTED_DEPLOYED_GIT_SHA"',
  ...
  const authorizationPackage = {
    authorization_package_version:
      "formative-conversation-v18r1-future-live-authorization-v1",
    runtime_candidate_hash: EXPECTED_RUNTIME_HASH,
  ...
  const sourceConfigurationMaterial = {
    configuration_version: "formative-conversation-v18r1-source-configuration-v1",
    evaluated_roles: [
      "student_profiling_agent",
      "formative_conversation_agent"
    ],
  ...
  const candidateManifestMaterial = {
    manifest_version:
      "formative-conversation-host-v5-executable-candidate-revision-v18r1",
    ...
    runtime_change_scope:
      "deployment_provenance_execution_infrastructure_only",
    runtime_candidate_hash: EXPECTED_RUNTIME_HASH,
    pre_freeze_candidate_identity: IMMUTABLE_V18_RUNTIME_HASH,
    protocol_hash: protocolHash,
    runner_implementation_hash: runnerHash,
  ...
  const immutableV18Sha = await writeJson(
    `${FORMATIVE_CONVERSATION_V5_EVALUATION_ROOT}/immutable-v18-reference.json`,
    immutableV18
  );
  ...
  const approvalPlaceholder = {
    placeholder_version: "formative-conversation-v18r1-approval-placeholder-v1",
    runtime_candidate_hash: EXPECTED_RUNTIME_HASH,
    protocol_hash: protocolHash,
  ...
  await writeJson(
    ...
    {
      identity_version: "formative-conversation-v18r1-candidate-identity-v1",
      pre_freeze_candidate_identity: IMMUTABLE_V18_RUNTIME_HASH,
      runtime_candidate_hash: EXPECTED_RUNTIME_HASH,
      formative_prompt_hash: FORMATIVE_CONVERSATION_V18_PROMPT_HASH,
      profiling_prompt_hash: profilingPrompt.prompt_hash,
      ...
      file_sha256: {
        ...
        immutable_v18_reference: immutableV18Sha,
