...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/service";
import {
...
import {
  ...
  assertFormativeConversationV18R1LiveControlEnvironment,
  writeFormativeConversationV18R1ControlPayload
...
type Options = {
  ...
  expected_deployed_git_sha: string | null;
...
function parseOptions(args: string[]): Options {
  ...
  return {
    ...
    expected_deployed_git_sha:
      valueFor(args, "--expected-deployed-git-sha") ?? null,
...
function assertCanonicalLauncher(options: Options) {
  if (
    process.env
      ...
      "true" ||
    process.env.FORMATIVE_CONVERSATION_V5_V18R1_CANONICAL_LOADER_VERSION !==
      "formative-conversation-v18r1-canonical-node-import-tsx-v1"
  ) {
    throw new Error(
      "formative_conversation_v18r1_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    ...
    throw new Error(
      "formative_conversation_v18r1_canonical_outer_loader_required"
    );
  }
}
...
async function main(input: {
  ...
  if (options.mode === "preflight") {
    if (!options.expected_deployed_git_sha) {
      throw new Error(
        "formative_conversation_v18r1_expected_git_sha_missing"
      );
    }
    const result = await compileFormativeConversationV5PreDispatch(
      {},
  ...
  const controlEnvironment =
    await assertFormativeConversationV18R1LiveControlEnvironment({
  ...
  await writeFormativeConversationV18R1ControlPayload({
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v18r1-evaluation-prisma",
