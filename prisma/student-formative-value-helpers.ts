...
export function configureNoLiveFormativeValueRuntime() {
...
export function restoreEnvValue(name: string, value: string | undefined) {
  if (value === undefined) {
...
export async function addSyntheticFormativeValueProcessContext(
  prisma: PrismaClient,
      ...
      payload:
        index === 1
          ...
          : {
              ...
              enter_key_count: 1,
              duration_ms: 28_000 + index * 1000
            }
    });
  }
}

export async function createFormativeValueSampleSession(prisma: PrismaClient) {
  configureNoLiveFormativeValueRuntime();
  await ensureDemoStudentAssessment(prisma);
  await applyProvisionalItemDiagnosticMetadata(prisma);

  const prefix = `formative_value_${Date.now()}_${randomUUID().slice(0, 8)}`;
  ...
  await addSyntheticFormativeValueProcessContext(prisma, started.session.session_public_id);
