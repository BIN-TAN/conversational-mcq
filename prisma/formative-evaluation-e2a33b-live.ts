...
const VERSION = "e2a33b-causal-inference-anchor-stance-live-v1";
const E2A33B_FREEZE_RUN = "e2a33b_20260724T020933630_a1270ef0";
const E2A33B_FREEZE_ROOT = path.join(
  process.cwd(), ".data", "e2a33b-anchor-stance-resolution-protocol",
  E2A33B_FREEZE_RUN
);
const HISTORICAL_E2A33_FREEZE_RUN =
  "e2a33_20260723T231529905_eddc5657";
const HISTORICAL_E2A33_FREEZE_ROOT = path.join(
  process.cwd(), ".data", "e2a33-causal-inference-protocol-freeze",
  HISTORICAL_E2A33_FREEZE_RUN
);
const HISTORICAL_E2A33_RUN_ID = "e2a33_20260724014237_58099b2a";
const HISTORICAL_E2A33_RUN_ROOT = path.join(
  process.cwd(), ".data", "e2a33-causal-inference-held-out-canary",
  HISTORICAL_E2A33_RUN_ID
);
const HISTORICAL_E2A33_PROTOCOL_HASH =
  "c6536a9861c91692e9d5d26a6868f43d79c87d23dd2f9e7cf4dc744ef4ffa45b";
const HISTORICAL_E2A33_COMPOSITE_RUNTIME_IDENTITY_HASH =
  "a0df20358f1850c68e48404826d38d3480322ca6dc422b9be0a7bec75a97c443";
const HISTORICAL_E2A33_EVIDENCE_TREE_HASH =
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a33b-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a33b-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const ARTIFACT_CONTRACT_HASH =
  "aced8fb0f38055e58a39208621b4a2107746df73112cf7214a87761b35310fdf";
const TARGET_EVIDENCE_CONTRACT_HASH =
  "5cb4888fb06e69b5a61fb878d6ac7abd5c78fadb9bfed2f859363a856e72e5d1";
const CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH =
  "c310ed34913dcdbbd6d8eeab9f46bde0bd8fa6bebc2bbbb40b3b7646bc6fd4c7";
const CANONICAL_ANCHOR_CONTRACT_HASH =
  "01d98c32c949be9a0b9a960aceb27a4da37df13a7ba0231c96b26d6f5cc3adbd";
const TRAJECTORY_ENVELOPE_HASH =
  "e874902b289f07861a721f243ece494db0990df97c8412cdeefc04c091ccaad2";
const RUNTIME_TRAJECTORY_ENVELOPE_HASH =
  "47a052f759804e63dfd97948104211e929dab0871eeff13aeedda7064069ae02";
const COMPILED_EVALUATOR_V5_REQUEST_HASH =
  "d1d682d29ea49455aabb917b03ca809d30ce7ce5d0c22f190c7f1ff34fca7f2f";
const COMPILED_EVALUATOR_V5_ARTIFACT_SHA =
  "2f551b02c2e456ac5727e97381069045d49ceb55941a6cb8301ff9f8419406c8";
const OVERLAP_ANALYSIS_HASH =
  "a34deabd4843e659879437a06cc78fe226f1e6813b62eab8dcfc7014856c6c88";
const BUDGET_HASH =
  "1320890f049fc3dc98127c01b8e24c6238159e72bbf913d7b2b12b998486db84";
const STANCE_EVIDENCE_CONTRACT_HASH =
  "6b7a270adbeaa4313a9dd2b69c64e6c0824b788ec6b2e72b058a97a71275aae7";
...
const STANCE_EVIDENCE_RESOLVER_V2_SHA =
  "4cb954d19b1a975694ffaa277b13ec856284a0c6714fd74a9801bd29a33a927b";
const COMPOSED_RESOLVER_V4_SHA =
...
const ARTIFACT_NAMES = [
  ...
  "causal-inference-target-evidence-contract.json",
  "causal-inference-alias-contract.json",
  ...
  "causal-inference-alias-resolution-results.jsonl",
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.33 evaluation-only constraints:",
  ...
  "Keep the response inside the frozen educational-app correlation-versus-causation scenario.",
...
type FrozenProtocolArtifact = {
  ...
  trajectory_envelope_version: "trajectory-envelope-v1";
  trajectory_envelope_hash: string;
  runtime_trajectory_envelope_projection_hash: string;
  target_evidence_contract_hash: string;
  canonical_anchor_contract_hash: string;
  evaluator_version: string;
  evaluator_request_hash: string;
  reference_and_stance_resolver_version: string;
  stance_resolver_version: string;
  contradiction_propagation_version: string;
...
type E2A33BCorrectionProtocolArtifact = {
  protocol_version: "e2a33b-causal-inference-anchor-stance-canary-v1";
  correction_version: "e2a33a-anchor-stance-resolution-correction-v1";
  protocol_state: "frozen_protocol_only";
  execution_authorized: false;
  live_execution_performed: false;
  e2a33_rerun_performed: false;
  historical_e2a33_run_id: string;
  historical_e2a33_status: string;
  historical_e2a33_passed: false;
  historical_protocol_hash: string;
  historical_composite_runtime_identity: string;
  candidate_configuration_hash: string;
  evaluator_version: string;
  evaluator_prompt_version: string;
  evaluator_prompt_hash: string;
  evaluator_input_schema: string;
  evaluator_output_schema: string;
  reference_resolver_version: string;
  stance_evidence_contract_version: string;
  stance_evidence_resolver_version: string;
  composed_resolver_version: string;
  source_hashes: Record<string, string>;
  stance_evidence_contract_hash: string;
  calibration_hash: string;
  replay_hash: string;
  target_evidence_contract_hash: string;
  alias_contract_hash: string;
  canonical_anchor_contract_hash: string;
  trajectory_envelope_contract_hash: string;
  evaluator_request_hash: string;
  budget_hash: string;
  artifact_contract_hash: string;
  historical_evidence_tree_hash: string;
  protected_source_hashes: Record<string, string>;
  gates: Record<string, boolean>;
  protocol_hash: string;
  passed: true;
};
...
function frozenArtifactPath(name: string) {
  return path.join(E2A33B_FREEZE_ROOT, name);
}

function historicalRuntimeArtifactPath(name: string) {
  return path.join(HISTORICAL_E2A33_FREEZE_ROOT, name);
}

function frozenHistoricalRuntimeProtocol() {
  const protocol = readJson<FrozenProtocolArtifact>(
    historicalRuntimeArtifactPath("frozen-protocol.json")
  );
  const body: Partial<FrozenProtocolArtifact> = { ...protocol };
  delete body.protocol_hash;
  delete body.passed;
  const recordedHash = readFileSync(
    historicalRuntimeArtifactPath("frozen-protocol.sha256"), "utf8"
  ).trim();
  if (recordedHash !== HISTORICAL_E2A33_PROTOCOL_HASH ||
      stableHash(body) !== HISTORICAL_E2A33_PROTOCOL_HASH ||
      protocol.protocol_hash !== HISTORICAL_E2A33_PROTOCOL_HASH ||
      ...
      protocol.domain.topic !== "correlation_versus_causation" ||
      protocol.domain.exact_scenario_previously_used !== false ||
      protocol.candidate_configuration_hash !== CANDIDATE_HASH ||
      protocol.reference_and_stance_resolver_version !==
        "active-anchor-alias-resolution-v3" ||
      protocol.stance_resolver_version !== ANCHOR_STANCE_RESOLUTION_VERSION ||
      protocol.trajectory_envelope_version !== TRAJECTORY_ENVELOPE_VERSION ||
      protocol.trajectory_envelope_hash !== TRAJECTORY_ENVELOPE_HASH ||
      protocol.runtime_trajectory_envelope_projection_hash !==
        RUNTIME_TRAJECTORY_ENVELOPE_HASH ||
      protocol.passed !== true) {
    throw new Error("e2a33b_historical_runtime_protocol_mismatch");
  }
  return protocol;
}

function frozenCorrectionProtocol() {
  const protocol = readJson<E2A33BCorrectionProtocolArtifact>(
    frozenArtifactPath("e2a33b-protocol.json")
  );
  const body: Partial<E2A33BCorrectionProtocolArtifact> = { ...protocol };
  delete body.protocol_hash;
  delete body.passed;
  const recordedHash = readFileSync(
    frozenArtifactPath("e2a33b-protocol.sha256"), "utf8"
  ).trim();
  if (
    recordedHash !== PROTOCOL_HASH ||
    ...
    protocol.execution_authorized !== false ||
    protocol.live_execution_performed !== false ||
    protocol.e2a33_rerun_performed !== false ||
    protocol.historical_e2a33_run_id !== HISTORICAL_E2A33_RUN_ID ||
    protocol.historical_protocol_hash !== HISTORICAL_E2A33_PROTOCOL_HASH ||
    protocol.historical_composite_runtime_identity !==
      HISTORICAL_E2A33_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    protocol.candidate_configuration_hash !== CANDIDATE_HASH ||
    protocol.reference_resolver_version !==
      "active-anchor-alias-resolution-v1" ||
    protocol.stance_evidence_contract_version !==
      ANCHOR_STANCE_EVIDENCE_CONTRACT_VERSION ||
    protocol.stance_evidence_resolver_version !==
      ANCHOR_STANCE_EVIDENCE_RESOLUTION_VERSION ||
    protocol.composed_resolver_version !==
      ACTIVE_ANCHOR_ALIAS_RESOLUTION_VERSION_V4 ||
    protocol.target_evidence_contract_hash !==
      TARGET_EVIDENCE_CONTRACT_HASH ||
    protocol.alias_contract_hash !== CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH ||
    protocol.canonical_anchor_contract_hash !==
      CANONICAL_ANCHOR_CONTRACT_HASH ||
    protocol.trajectory_envelope_contract_hash !==
      TRAJECTORY_ENVELOPE_HASH ||
    protocol.evaluator_request_hash !==
      COMPILED_EVALUATOR_V5_REQUEST_HASH ||
    protocol.budget_hash !== BUDGET_HASH ||
    protocol.artifact_contract_hash !== ARTIFACT_CONTRACT_HASH ||
    protocol.stance_evidence_contract_hash !==
      STANCE_EVIDENCE_CONTRACT_HASH ||
    protocol.historical_evidence_tree_hash !==
      HISTORICAL_E2A33_EVIDENCE_TREE_HASH ||
    Object.values(protocol.gates).some((passed) => passed !== true) ||
    protocol.passed !== true
  ) {
    throw new Error("e2a33b_frozen_correction_protocol_mismatch");
  }
  return protocol;
}

function buildE2A33BFrozenProtocol(): Protocol {
  const correction = frozenCorrectionProtocol();
  const frozen = frozenHistoricalRuntimeProtocol();
  ...
  const aliasContract = ActiveAnchorAliasContractSchema.parse(
    readJson(frozenArtifactPath("alias-contract.json"))
  );
  const trajectory = TrajectoryEnvelopeContractSchema.parse(
    readJson(historicalRuntimeArtifactPath(
      "runtime-trajectory-envelope-projection.json"
    ))
  );
  if (correction.protocol_hash !== PROTOCOL_HASH ||
      frozen.protocol_hash !== HISTORICAL_E2A33_PROTOCOL_HASH ||
      frozen.domain.distractor_option !== "D" ||
      frozen.domain.item_id !== targetContract.item_id ||
      frozen.candidate_configuration_hash !== CANDIDATE_HASH ||
      frozen.candidate_approved !== false ||
      frozen.candidate_activated !== false ||
      stableHash(targetContract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(aliasContract) !== CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH ||
      stableHash(targetContract.active_anchor_alias_contract) !==
        CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH ||
      stableHash(trajectory) !== RUNTIME_TRAJECTORY_ENVELOPE_HASH ||
      targetContract.active_anchor_id !==
        frozen.domain.canonical_anchor_id ||
      targetContract.distractor_claim !==
        frozen.domain.distractor_claim) {
    throw new Error("e2a33b_frozen_protocol_file_sha_mismatch");
  }
  const expectedObjectives = new Map<number, string>([
    [1, "explicit_anchor_endorsement"],
    [2, "copied_wording_without_independent_application"],
    [3, "partial_confounder_improvement"],
    [4, "correct_confounder_reasoning_with_wrong_anchor_conclusion"],
    [5, "causal_inference_clarification"],
  ...
  return {
    ...frozen,
    protocol_version: correction.protocol_version,
    preparation_version: correction.correction_version,
    protocol_hash: correction.protocol_hash,
    session_count: 1,
    session: {
      session_id: "E2A33B-CAUSAL_INFERENCE",
      design:
        "held_out_causal_inference_educational_app_trajectory_envelope",
      academic_domain: frozen.domain.domain,
      concept: frozen.domain.concept_id,
      ...
      student_profile: {
        ...
        trajectory:
          "direct causal endorsement, copied wording, confounder reasoning, deliberate mechanism/conclusion contradiction, clarification, and coherent anchor rejection"
      },
      frozen_student_trajectory: [
        turn(1, {
          ...
          no_live_fixture_message:
            "I choose option D. Students who use the app more get skills from it, so the app causes the higher scores.",
          simulator_instruction:
            "Explicitly endorse option D and use the observed association as support for the direct app-effect claim. Do not add a confounder or stronger-design requirement.",
        ...
        turn(2, {
          trajectory_role: "copied_wording",
          fixture_reasoning_quality: "insufficient",
          copy_behavior: true,
          no_live_fixture_message:
            "An association alone does not prove causation. I think that is what happens here.",
          simulator_instruction:
            "Echo one distinctive causal-boundary phrase from the latest tutor message without independently explaining a confounder or applying it to option D.",
        ...
        turn(3, {
          ...
          no_live_fixture_message:
            "Motivation could make students use the app more and also study more, so that could help explain the higher scores. I still need to decide what that means for the app claim.",
          simulator_instruction:
            "Identify motivation or study habits as a plausible confounder and explain at least part of how it affects both variables. Do not force an exact evaluator label; if the evidence becomes independently sound, preserve it.",
          semantic_envelope: ["misconception", "partial", "sound"],
          tutor_expected: true
        }),
        turn(4, {
          trajectory_role: "contradiction",
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "Motivation could make students both use the app more and study more, and the extra studying could explain their higher scores. I agree with option D anyway because the app gives students skills.",
          simulator_instruction:
            "Identify a plausible confounder, explain why correlation alone does not prove causation, and then state unambiguous direct agreement with option D anyway so the conclusion contradicts the conceptual reasoning.",
          semantic_envelope: ["misconception", "partial"],
          tutor_expected: true,
          required_anchor_application: "explicit",
          required_anchor_stance: "endorses_distractor",
          ...
          required_tutor_goal:
            "reconcile the confounder explanation with the explicit endorsement of the unsupported direct app-effect claim"
        }),
        turn(5, {
          trajectory_role: "clarification",
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "If motivation affects both app use and exam scores, does that mean the study can only show an association unless it rules motivation out?",
          simulator_instruction:
            "Ask a natural clarification about how a common cause changes the interpretation and what stronger evidence would be needed.",
          semantic_envelope: ["insufficient", "partial", "sound"],
          tutor_expected: true
        }),
        turn(6, {
          ...
          no_live_fixture_message:
            "The study shows correlation, not causation. More motivated students could both use the app more and study more, producing higher scores without the app causing them. A randomized experiment or another design that rules out that confounder is needed, so I reject option D.",
          semantic_envelope: ["partial", "sound"],
          tutor_expected: false,
          simulator_instruction:
            "Independently distinguish correlation from causation, identify a plausible confounder, explain how it affects interpretation, require stronger causal evidence, and explicitly reject option D.",
          required_anchor_application: "explicit",
          required_anchor_stance: "rejects_distractor",
          required_anchor_consistency:
            "consistent_with_conceptual_reasoning"
      ...
      raw_history_truncation_allowed: false,
      summary_only_substitution_allowed: false,
      natural_initial_activity: frozen.domain.scenario
    }
  };
}
...
function frozenArtifactContract() {
  return readJson<JsonObject>(
    frozenArtifactPath("e2a33b-artifact-contract.json")
  );
}
...
function frozenCompositeRuntimeIdentity() {
  ...
  if (recorded !== FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      ...
      identity.reference_resolver_version !==
        "active-anchor-alias-resolution-v1" ||
      identity.stance_evidence_contract_version !==
        ANCHOR_STANCE_EVIDENCE_CONTRACT_VERSION ||
      identity.stance_evidence_resolver_version !==
        ANCHOR_STANCE_EVIDENCE_RESOLUTION_VERSION ||
      identity.composed_resolver_version !==
        ACTIVE_ANCHOR_ALIAS_RESOLUTION_VERSION_V4 ||
      identity.evaluator_version !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
      identity.historical_e2a33_run_id !== HISTORICAL_E2A33_RUN_ID ||
      identity.historical_evidence_tree_hash !==
        HISTORICAL_E2A33_EVIDENCE_TREE_HASH) {
    throw new Error("e2a33b_frozen_composite_runtime_identity_mismatch");
  }
  return identity;
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a33b_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a33b_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a33b_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a33b_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a33b_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a33b_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a33b_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a33b_"))
...
function frozenHistoricalEvidenceIdentity(root: string) {
  const files = filesRecursively(root).map((file) => ({
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a33b-live.ts",
    "prisma/formative-evaluation-e2a33b.ts",
    ...
    path.relative(process.cwd(), frozenArtifactPath("e2a33b-protocol.json")),
    path.relative(process.cwd(), frozenArtifactPath("e2a33b-protocol.sha256")),
    path.relative(process.cwd(), frozenArtifactPath("target-evidence-contract.json")),
    path.relative(process.cwd(), frozenArtifactPath("alias-contract.json")),
    path.relative(process.cwd(), frozenArtifactPath("stance-evidence-contract.json")),
    ...
    path.relative(process.cwd(),
      historicalRuntimeArtifactPath("runtime-trajectory-envelope-projection.json")),
    path.relative(process.cwd(), frozenArtifactPath("compiled-evaluator-v5-request.json")),
    path.relative(process.cwd(), historicalRuntimeArtifactPath("overlap-analysis.json")),
    path.relative(process.cwd(), frozenArtifactPath("e2a33b-budget.json")),
    path.relative(process.cwd(), frozenArtifactPath("e2a33b-artifact-contract.json")),
...
function protectedEvidenceIdentity() {
  ...
  return {
    snapshot_version: "e2a33b-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    ...
    frozen_e2a33b_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    frozen_e2a33b_protocol_hash: PROTOCOL_HASH,
    frozen_e2a33b_target_evidence_contract_hash:
      TARGET_EVIDENCE_CONTRACT_HASH,
    frozen_e2a33b_alias_contract_hash:
      CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH,
    frozen_e2a33b_canonical_anchor_contract_hash:
      CANONICAL_ANCHOR_CONTRACT_HASH,
    ...
    frozen_runtime_trajectory_envelope_hash:
      RUNTIME_TRAJECTORY_ENVELOPE_HASH,
    frozen_e2a33b_compiled_evaluator_v5_request_hash:
      COMPILED_EVALUATOR_V5_REQUEST_HASH,
    frozen_e2a33b_overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    frozen_e2a33b_artifact_contract_hash: ARTIFACT_CONTRACT_HASH,
    frozen_e2a33b_budget_hash: BUDGET_HASH,
    frozen_e2a33b_stance_evidence_contract_hash:
      STANCE_EVIDENCE_CONTRACT_HASH,
    historical_e2a33_protocol_hash: HISTORICAL_E2A33_PROTOCOL_HASH,
    historical_e2a33_composite_runtime_identity_hash:
      HISTORICAL_E2A33_COMPOSITE_RUNTIME_IDENTITY_HASH,
    historical_e2a33_evidence_tree_hash:
      HISTORICAL_E2A33_EVIDENCE_TREE_HASH,
    ...
    anchor_stance_resolver_source_file_sha256:
    ...
    active_anchor_alias_resolver_source_file_sha256:
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a33b-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A33BFrozenProtocol();
  ...
  const freezeSummary = readJson<{ status?: string;
    ...
    e2a33b_live_execution_performed?: boolean;
  ...
  const overlap = readJson<{ passed?: boolean;
    ...
    broad_causal-inference_domain_previously_seen_in_deterministic_calibration?: boolean;
  ...
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    ...
    correctionArtifactPath("e2a33b-budget.json")
  );
  ...
  if (freezeValidation.passed !== true ||
    ...
    blockers.push("e2a33b_artifact_validation_invalid");
  }
  ...
  if (overlap.passed !== true ||
      ...
      overlap.broad_causal-inference_domain_previously_seen_in_deterministic_calibration !==
        true ||
  ...
  if (pids("[f]ormative-evaluation-e2a33b").length > 0) {
    blockers.push("duplicate_e2a33b_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a33b_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a33b_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A33B !== "1") blockers.push("live_opt_in_missing");
  ...
  return {
    version: "e2a33b-live-preflight-v1",
    passed: blockers.length === 0,
    blockers,
    current_git_commit: commit,
    tracked_worktree_clean: trackedTreeClean(),
    application_build_info: buildInfo,
    authoritative_e2a33b_freeze_run: E2A33B_FREEZE_RUN,
    historical_e2a33_runtime_freeze_run: HISTORICAL_E2A33_FREEZE_RUN,
    historical_e2a33_run_id: HISTORICAL_E2A33_RUN_ID,
    historical_e2a33_evidence_tree_hash:
      HISTORICAL_E2A33_EVIDENCE_TREE_HASH,
    authoritative_e2a33b_protocol_run: E2A33B_PROTOCOL_RUN,
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a33b_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a33b_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a33b_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a33b_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a33b-one-session-authorization",
    ...
    "--confirm-stop-after-e2a33b"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a33b_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a33b_dispatch_checkpoint_missing");
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const summary = readJson<{
    ...
    minimum_calibration_case_count?: number;
    calibration_passed?: boolean;
    first_failure_replay_passed?: boolean;
    corrected_first_failure_reference?: string;
    corrected_first_failure_stance?: string;
    historical_e2a33_run_id?: string;
    historical_e2a33_status?: string;
    historical_e2a33_passed?: boolean;
    historical_evidence_unchanged?: boolean;
    evaluator_v5_unchanged?: boolean;
    tutor_candidate_unchanged?: boolean;
    e2a33_rerun_performed?: boolean;
    e2a33b_live_execution_performed?: boolean;
    execution_authorized?: boolean;
    provider_calls_made?: number;
    network_requests_made?: number;
  }>(frozenArtifactPath("summary.json"));
  const validation = readJson<{
    passed?: boolean;
    required_artifact_count?: number;
    missing_artifacts?: unknown[];
    unexpected_artifacts?: unknown[];
    unsafe_artifacts?: unknown[];
    semantic_issues?: unknown[];
    historical_evidence_unchanged?: boolean;
    historical_evidence_read_only?: boolean;
    protocol_hash_matches_file?: boolean;
    calibration_passed?: boolean;
  ...
  const budget = readJson<JsonObject>(
    frozenArtifactPath("e2a33b-budget.json")
  );
  const overlap = readJson<JsonObject>(
    historicalRuntimeArtifactPath("overlap-analysis.json")
  );
  ...
  const stanceEvidenceContract = readJson<JsonObject>(
    frozenArtifactPath("stance-evidence-contract.json")
  );
  ...
  const historicalE2A33 = frozenHistoricalEvidenceIdentity(
    HISTORICAL_E2A33_RUN_ROOT
  );
  const buildInfo = resolveApplicationBuildInfo();
  const protectedEvidence = protectedEvidenceIdentity();
  ...
  if (
    ...
    summary.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    summary.calibration_case_count !== 192 ||
    summary.minimum_calibration_case_count !== 150 ||
    summary.calibration_passed !== true ||
    summary.first_failure_replay_passed !== true ||
    summary.corrected_first_failure_reference !== "explicit" ||
    summary.corrected_first_failure_stance !== "endorses_distractor" ||
    summary.historical_e2a33_run_id !== HISTORICAL_E2A33_RUN_ID ||
    summary.historical_e2a33_status !==
      "e2a33_canary_failed_anchor_resolution" ||
    summary.historical_e2a33_passed !== false ||
    summary.historical_evidence_unchanged !== true ||
    summary.evaluator_v5_unchanged !== true ||
    summary.tutor_candidate_unchanged !== true ||
    summary.e2a33_rerun_performed !== false ||
    summary.e2a33b_live_execution_performed !== false ||
  ...
  if (
    ...
    validation.required_artifact_count !== 20 ||
    validation.missing_artifacts?.length !== 0 ||
    validation.historical_evidence_unchanged !== true ||
    validation.historical_evidence_read_only !== true ||
    validation.protocol_hash_matches_file !== true ||
    validation.calibration_passed !== true ||
    validation.replay_passed !== true ||
    validation.compiled_evaluator_request_passed !== true ||
    validation.protected_sources_passed !== true ||
    validation.provider_call_guard_passed !== true ||
    validation.execution_authorized !== false
  ) {
    blockers.push("e2a33b_artifact_validation_invalid");
  }
  if (
    ...
    artifactContract.artifact_contract_version !==
      "e2a33b-artifact-contract-v1" ||
    artifactContract.required_future_live_artifact_count !==
      ARTIFACT_NAMES.length ||
    stableHash(
      [...(artifactContract.required_future_live_artifacts as string[])]
        .sort()
    ) !== stableHash([...ARTIFACT_NAMES].sort()) ||
    artifactContract.execution_authorized !== false ||
    artifactContract.historical_e2a33_artifacts_are_read_only !== true ||
    !(artifactContract.required_future_live_artifacts as unknown[])
      ?.includes("anchor-reference-resolution-results.jsonl") ||
    !(artifactContract.required_future_live_artifacts as unknown[])
      ?.includes("anchor-stance-evidence-resolution-results.jsonl") ||
    artifactContract.passed !== true
  ) {
    blockers.push("frozen_artifact_contract_mismatch");
  }
  ...
  if (
    ...
    stanceEvidenceContract.contract_version !==
      ANCHOR_STANCE_EVIDENCE_CONTRACT_VERSION ||
    ...
    blockers.push("frozen_causal_inference_contract_invalid");
  }
  if (
    stableHash(overlap) !== OVERLAP_ANALYSIS_HASH ||
    overlap.passed !== true ||
  ...
  if (
    ...
    providerGuard.live_command_present !== false ||
  ...
  if (
    !historicalE2A33.exists ||
    historicalE2A33.file_count !== 88 ||
    historicalE2A33.sha256 !== HISTORICAL_E2A33_EVIDENCE_TREE_HASH
  ) {
    blockers.push("historical_e2a33_evidence_changed");
  }
  if (
    requireCheckpoint &&
  ...
  if (pids("[f]ormative-evaluation-e2a33b-live").length > 0) {
    blockers.push("duplicate_e2a33b_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a33b_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) {
    blockers.push(`prior_e2a33b_run_exists:${priorRuns.at(-1)}`);
  }
  ...
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A33B !== "1") {
  ...
  return {
    ...
    frozen_canonical_anchor_contract_valid:
      !blockers.includes("frozen_causal_inference_contract_invalid"),
    anchor_reference_resolver_version: "active-anchor-alias-resolution-v1",
    anchor_stance_evidence_contract_version:
      ANCHOR_STANCE_EVIDENCE_CONTRACT_VERSION,
    anchor_stance_resolver_version:
    ...
    source_identity: sourceIdentity(),
    protected_evidence_identity: protectedEvidence,
    historical_e2a33_identity: historicalE2A33,
    prior_live_runs: priorRuns,
    budget: BUDGET,
    network_request_count: 0
  };
...
function recordDispatchCheckpoint() {
  ...
  if (!trackedTreeClean()) {
    throw new Error("e2a33b_checkpoint_tree_not_clean");
  }
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(
      `e2a33b_checkpoint_preflight_failed:${check.blockers.join(",")}`
    );
  }
  const identity = compositeRuntimeIdentity();
  const checkpoint: DispatchCheckpoint = {
...
function assertLiveAuthorizationArguments() {
  for (const flag of [
    ...
    "--confirm-human-review-remains-pending",
    "--confirm-candidate-remains-unapproved",
    "--confirm-no-e2a33-rerun",
    "--confirm-no-e2a33b-rerun",
    "--confirm-bounded-transport-retry-policy",
    "--confirm-no-additional-live-session",
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  if (stableHash(contract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(contract.active_anchor_alias_contract) !==
        CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH) {
    throw new Error("e2a33b_target_evidence_contract_hash_mismatch");
  }
  return contract;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a33b_session_${session.session_id}`,
    scenario_version: "e2a33b-trajectory-envelope-simulator-v1",
...
function validateSimulatorOutput(input: {
  ...
  const resolution = resolveActiveAnchorAliasV4({
    ...
    source_turn_id: `e2a33b_simulator_validation_turn_${input.turn.turn}`,
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a33b_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a33b_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a33b_${session.session_id}`,
    ...
    source_activity_context: {
      ...
      profile_condition: "synthetic_frozen_e2a33b_trajectory",
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a33b_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a33b_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a33b",
    source_diagnostic_purpose: "reasoning_boundary_repair",
    ...
    response_summary: contradictory
      ? `${session.session_id} turn ${turn.turn} identifies a plausible confounder and the correlation limitation but explicitly endorses the direct app-effect claim, creating a reasoning/conclusion conflict.`
...
function noLiveTutorOutput(input: AutonomousPedagogyInput) {
  ...
  const strategies = [
    "contrast an observed association with a causal conclusion",
    "trace one plausible confounder through both observed variables",
    "ask for a one-sentence link between the common cause and the score pattern",
    "test what evidence would distinguish an app effect from self-selection",
    "reconcile the stated option conclusion with the confounder explanation"
  ];
  const studentTasks = [
    ...
    "Use a concrete counterexample to test the option, then explain the general rule your example reveals.",
      "Put the confounder explanation and your final judgment side by side. Do they support the same conclusion? Explain which part you would keep or change."
  ];
...
function buildE2A33BNoLiveStructuredTurnEvidence(input: {
  ...
  return ProductionTurnEvidenceEvaluatorOutputV5Schema.shape
    .structured_turn_evidence.parse({
      ...
      evidence_limitations: [
        ...
        "e2a33b_no_live_stance_evidence_v2_fixture"
      ]
    });
}
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a33b_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a33b_no_live_evaluator_case_missing");
      ...
      output = ProductionTurnEvidenceEvaluatorOutputV5Schema.parse({
        ...
        structured_turn_evidence:
          buildE2A33BNoLiveStructuredTurnEvidence({
            source_student_turn_id:
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        ...
        throw new Error(
          `e2a33b_no_live_tutor_fixture_invalid:turn_${turnNumber}:sequence_${tutorInput.latest_student_response.source_sequence_index}:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`
        );
      }
    } else {
      throw new Error(`e2a33b_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a33b_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a33b_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a33b_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a33b_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a33b:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a33b_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a33b_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a33b_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a33b_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a33b_canary_failed_rate_limit_retry_exhausted"
          : "e2a33b_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a33b_canary_failed_provider_nonretryable_request"
        : `e2a33b_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation?.passed) {
        throw new Error(
          `e2a33b_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation?.issues.join("|") ?? "validation_not_run"}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a33b_canary_failed_evaluator_contract");
          }
          appendJsonl(path.join(
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a33b_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a33b_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a33b_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a33b_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a33b_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
          ...
          appendJsonl(path.join(
            input.runDir, "causal-inference-alias-resolution-results.jsonl"
          ), {
            session_id: input.session.session_id,
            turn: turn.turn,
            target_evidence_contract_hash: TARGET_EVIDENCE_CONTRACT_HASH,
            causal_inference_alias_contract_hash: CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH,
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a33b_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a33b_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a33b_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a33b_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a33b_idempotency_replay_failed");
      }
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a33b_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a33b_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a33b_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error(
          `e2a33b_anchor_stance_misclassified:turn_${turn.turn}:expected_${turn.required_anchor_stance}:observed_${anchor.anchor_stance}`
        );
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a33b_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a33b_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
      ...
      )) throw new Error("e2a33b_anchor_interpretation_failure");
      ...
      if (
        ...
        throw new Error("e2a33b_unsupported_sound_promotion");
      }
      if (soundGatePassed) {
        if (
          ...
          throw new Error("e2a33b_sound_gate_progression_failure");
        }
        endpoint = "passed_required_revision_endpoint";
      } else if (result.route.selected_mode === "request_revision") {
        throw new Error("e2a33b_premature_revision_without_sound_gate");
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a33b_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a33b_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a33b_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a33b_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a33b_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a33b_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a33b-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  const targetContract = readJson<JsonObject>(path.join(
    runDir, "causal-inference-target-evidence-contract.json"
  ));
  const aliasContract = readJson<JsonObject>(path.join(
    runDir, "causal-inference-alias-contract.json"
  ));
  ...
  const runtimeTrajectoryEnvelope = readJson<JsonObject>(path.join(
    runDir, "runtime-trajectory-envelope-projection.json"
  ));
  ...
  if (frozenIdentity.composite_runtime_identity_hash !==
    ...
    failures.push("frozen_causal_inference_binding_invalid");
  }
  ...
  if (burden.metric_version !== "e2a33b-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  const frozenAliasContract = readJson<JsonObject>(
    frozenArtifactPath("alias-contract.json")
  );
  ...
  const frozenRuntimeTrajectoryEnvelope = readJson<JsonObject>(
    historicalRuntimeArtifactPath(
      "runtime-trajectory-envelope-projection.json"
    )
  );
  ...
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a33b-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a33b-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a33b-causal-inference-isolated-fixture-v1",
    synthetic_only: true,
  ...
  writeJson(path.join(
    runDir, "runtime-trajectory-envelope-projection.json"
  ), frozenRuntimeTrajectoryEnvelope);
  writeJson(path.join(
    runDir, "compiled-evaluator-v5-request.json"
  ), frozenCompiledEvaluatorRequest);
  writeJson(path.join(runDir, "overlap-analysis-binding.json"), {
    binding_version: "e2a33b-overlap-analysis-binding-v1",
    source_run: HISTORICAL_E2A33_FREEZE_RUN,
    overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    no_prior_scenario_reuse:
      frozenOverlapAnalysis.no_prior_scenario_reuse,
    novelty_classification:
      frozenOverlapAnalysis.novelty_classification,
    concept_level_novelty_claimed:
      frozenOverlapAnalysis.concept_level_novelty_claimed,
    ...
    passed: stableHash(frozenOverlapAnalysis) === OVERLAP_ANALYSIS_HASH &&
      ...
      frozenOverlapAnalysis.no_prior_scenario_reuse === true
  });
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a33b_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "budget-ledger.json"), {
    ledger_version: "e2a33b-budget-ledger-v1",
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a33b-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a33b-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a33b-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a33b-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a33b-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a33b-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a33b-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a33b-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a33b-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a33b-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a33b_protected_evidence_changed_during_execution";
  }
  ...
  const failureStatus = explicitProviderStatus && [
    ...
    : failure?.includes("evaluator_contract")
      ? "e2a33b_canary_failed_evaluator_contract"
      : failure?.includes("anchor")
        ? "e2a33b_canary_failed_anchor_resolution"
        : failure?.includes("contradiction")
          ? "e2a33b_canary_failed_contradiction_propagation"
          : failure?.includes("cross_artifact")
            ? "e2a33b_canary_failed_cross_artifact_consistency"
            : failure?.includes("profile_transition")
              ? "e2a33b_canary_failed_profile_transition"
              : failure?.includes("profile_finalization")
                ? "e2a33b_canary_failed_profile_finalization"
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a33b_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a33b-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A33BFrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a33b_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a33b_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a33b_no_live_network_detected");
    ...
    if (stanceRows.length !== 6 ||
      ...
      throw new Error("e2a33b_no_live_stance_boundary_failed");
    }
    const trajectoryRows = readJsonl<JsonObject>(path.join(
    ...
    if (trajectoryRows.length !== 6 ||
      ...
      throw new Error("e2a33b_no_live_trajectory_envelope_failed");
    }
    ...
    if ([...requiredCriterionIds].some((criterionId) =>
      ...
      throw new Error("e2a33b_no_live_named_revision_criteria_failed");
    }
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a33b_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a33b_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a33b-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A33BFrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a33b_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a33b_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a33b_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a33b_confirmation_missing:")) {
    throw new Error("e2a33b_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a33b_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a33b_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a33b_run_not_found");
...
function auditRun(run?: string) {
  ...
  const aliasResolutions = readJsonl<JsonObject>(path.join(
    runDir, "causal-inference-alias-resolution-results.jsonl"
  )).filter((row) => typeof row.turn === "number");
  ...
  if (summary.passed === true) {
    ...
    if (completedTurnCount >= 2 &&
      ...
      failures.push("copied_wording_promoted_to_sound");
    }
  ...
  return {
    audit_version: "e2a33b-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
    ...
    turn_counts: {
      ...
      causal_inference_alias_resolutions: aliasResolutions.length,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a33b.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a33b_runner_failed");
  process.exitCode = 1;
});
