...
const VERSION = "e2a33-causal-inference-trajectory-envelope-live-v1";
const E2A33_FREEZE_RUN = "e2a33_20260723T231529905_eddc5657";
const E2A33_FREEZE_ROOT = path.join(
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a33-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a33-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const ARTIFACT_CONTRACT_HASH =
  "2af91fe29a7e8538a5e61771b130b90648faf3e1da3ebdbae924d0678312d472";
...
const CANONICAL_ANCHOR_CONTRACT_HASH =
  "e4b6e83e5b1500221f6323a776962b076d0f990807dc7909ddbcde475e4cc900";
...
const COMPILED_EVALUATOR_V5_REQUEST_HASH =
  "ed63a41db63f705ed724f83b700ceaea3826ce736575ba9f09d199bbc179cb7b";
const COMPILED_EVALUATOR_V5_ARTIFACT_SHA =
  "a05017df4417fc619244980fcc7b3159fb3d429e58f0b38a753b46030dd25687";
const OVERLAP_ANALYSIS_HASH =
  "a34deabd4843e659879437a06cc78fe226f1e6813b62eab8dcfc7014856c6c88";
const BUDGET_HASH =
  "b38e0d7eeb5f67ee95e5ff6e2438deca642bd534e056aa92465eedbabc7cec4e";
...
function frozenArtifactPath(name: string) {
  return path.join(E2A33_FREEZE_ROOT, name);
}
...
function frozenCorrectionProtocol() {
  ...
  if (recordedHash !== PROTOCOL_HASH ||
    ...
    throw new Error("e2a33_frozen_correction_protocol_mismatch");
  }
  return protocol;
}

function buildE2A33FrozenProtocol(): Protocol {
  const frozen = frozenCorrectionProtocol();
  ...
  if (frozen.protocol_hash !== PROTOCOL_HASH ||
    ...
    throw new Error("e2a33_frozen_protocol_file_sha_mismatch");
  }
  ...
  return {
    ...
    session_count: 1,
    session: {
      session_id: "E2A33-CAUSAL_INFERENCE",
      design:
      ...
      frozen_student_trajectory: [
        ...
        turn(4, {
          ...
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "Motivation could make students both use the app more and study more, and the extra studying could explain their higher scores. But I choose option D anyway because the app gives students skills.",
          simulator_instruction:
            "Identify a plausible confounder, explain why correlation alone does not prove causation, and then explicitly endorse option D anyway so the conclusion contradicts the conceptual reasoning.",
          semantic_envelope: ["misconception", "partial"],
          tutor_expected: true,
          required_anchor_application: "explicit",
          required_anchor_stance: "endorses_distractor",
...
function frozenArtifactContract() {
  return readJson<JsonObject>(frozenArtifactPath("artifact-contract.json"));
}
...
function frozenCompositeRuntimeIdentity() {
  ...
  if (recorded !== FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      ...
      identity.evaluator_version !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
      identity.trajectory_envelope_version !== TRAJECTORY_ENVELOPE_VERSION ||
      identity.trajectory_envelope_hash !== TRAJECTORY_ENVELOPE_HASH ||
      identity.runtime_trajectory_envelope_projection_hash !==
        RUNTIME_TRAJECTORY_ENVELOPE_HASH) {
    throw new Error("e2a33_frozen_composite_runtime_identity_mismatch");
  }
  return identity;
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a33_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a33_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a33_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a33_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a33_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a33_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a33_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a33_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a33-live.ts",
    "prisma/formative-evaluation-e2a33.ts",
...
function protectedEvidenceIdentity() {
  ...
  return {
    snapshot_version: "e2a33-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    ...
    frozen_e2a33_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    frozen_e2a33_protocol_hash: PROTOCOL_HASH,
    frozen_e2a33_target_evidence_contract_hash:
      TARGET_EVIDENCE_CONTRACT_HASH,
    frozen_e2a33_alias_contract_hash:
      CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH,
    frozen_e2a33_canonical_anchor_contract_hash:
    ...
    frozen_runtime_trajectory_envelope_hash:
      RUNTIME_TRAJECTORY_ENVELOPE_HASH,
    frozen_e2a33_compiled_evaluator_v5_request_hash:
      COMPILED_EVALUATOR_V5_REQUEST_HASH,
    frozen_e2a33_overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    frozen_e2a33_artifact_contract_hash: ARTIFACT_CONTRACT_HASH,
    frozen_e2a33_budget_hash: BUDGET_HASH,
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a33-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A33FrozenProtocol();
  ...
  const freezeSummary = readJson<{ status?: string;
    ...
    e2a33_live_execution_performed?: boolean;
  ...
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    ...
    correctionArtifactPath("e2a33-budget.json")
  );
  ...
  if (freezeSummary.status !==
      "e2a33_protocol_ready_for_separate_authorization" ||
  ...
  if (freezeValidation.passed !== true ||
    ...
    blockers.push("e2a33_artifact_validation_invalid");
  }
  ...
  if (pids("[f]ormative-evaluation-e2a33").length > 0) {
    blockers.push("duplicate_e2a33_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a33_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a33_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A33 !== "1") blockers.push("live_opt_in_missing");
  ...
  return {
    version: "e2a33-live-preflight-v1",
    passed: blockers.length === 0,
    blockers,
    current_git_commit: commit,
    tracked_worktree_clean: trackedTreeClean(),
    application_build_info: buildInfo,
    authoritative_e2a33_freeze_run: E2A33_FREEZE_RUN,
    authoritative_e2a33_protocol_run: E2A33_PROTOCOL_RUN,
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a33_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a33_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a33_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a33_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a33-one-session-authorization",
    ...
    "--confirm-stop-after-e2a33"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a33_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a33_dispatch_checkpoint_missing");
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const summary = readJson<{
    ...
    protocol_hash?: string;
    composite_runtime_identity_hash?: string;
    trajectory_envelope_version?: string;
    deterministic_regressions_passed?: boolean;
    early_sound_immediate_revision?: boolean;
    evaluator_follows_evidence?: boolean;
    overlap_analysis_passed?: boolean;
    no_prior_scenario_reuse?: boolean;
    required_contradiction_passed?: boolean;
  ...
  if (
    ...
    summary.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    summary.trajectory_envelope_version !== TRAJECTORY_ENVELOPE_VERSION ||
    summary.deterministic_regressions_passed !== true ||
    summary.early_sound_immediate_revision !== true ||
    summary.evaluator_follows_evidence !== true ||
    ...
    summary.e2a33_ready_for_separate_authorization !== true
  ) {
    blockers.push("e2a33_authoritative_freeze_invalid");
  }
  if (
    validation.passed !== true ||
    validation.required_artifact_count !== 23 ||
    validation.actual_artifact_count !== 23 ||
    validation.missing_artifacts?.length !== 0 ||
    validation.unexpected_artifacts?.length !== 0 ||
    validation.unsafe_artifacts?.length !== 0 ||
    validation.semantic_issues?.length !== 0 ||
  ...
  if (
    ...
    artifactContract.artifact_contract_version !==
      "e2a33-artifact-contract-v1" ||
    artifactContract.required_count !== 23 ||
  ...
  if (pids("[f]ormative-evaluation-e2a33-live").length > 0) {
    blockers.push("duplicate_e2a33_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a33_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) {
    blockers.push(`prior_e2a33_run_exists:${priorRuns.at(-1)}`);
  }
  ...
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A33 !== "1") {
...
function recordDispatchCheckpoint() {
  ...
  if (!trackedTreeClean()) {
    throw new Error("e2a33_checkpoint_tree_not_clean");
  }
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(
      `e2a33_checkpoint_preflight_failed:${check.blockers.join(",")}`
    );
  }
  const identity = compositeRuntimeIdentity();
  const checkpoint: DispatchCheckpoint = {
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  if (stableHash(contract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(contract.active_anchor_alias_contract) !==
        CAUSAL_INFERENCE_ALIAS_CONTRACT_HASH) {
    throw new Error("e2a33_target_evidence_contract_hash_mismatch");
  }
  return contract;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a33_session_${session.session_id}`,
    scenario_version: "e2a33-trajectory-envelope-simulator-v1",
...
function validateSimulatorOutput(input: {
  ...
  const resolution = resolveActiveAnchorAliasV3({
    ...
    source_turn_id: `e2a33_simulator_validation_turn_${input.turn.turn}`,
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a33_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a33_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a33_${session.session_id}`,
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a33_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a33_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a33",
    source_diagnostic_purpose: "reasoning_boundary_repair",
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a33_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a33_no_live_evaluator_case_missing");
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        ...
        throw new Error(
          `e2a33_no_live_tutor_fixture_invalid:turn_${turnNumber}:sequence_${tutorInput.latest_student_response.source_sequence_index}:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`
        );
      }
    } else {
      throw new Error(`e2a33_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a33_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a33_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a33_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a33_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a33:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a33_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a33_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a33_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a33_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a33_canary_failed_rate_limit_retry_exhausted"
          : "e2a33_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a33_canary_failed_provider_nonretryable_request"
        : `e2a33_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation?.passed) {
        throw new Error(
          `e2a33_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation?.issues.join("|") ?? "validation_not_run"}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a33_canary_failed_evaluator_contract");
          }
          appendJsonl(path.join(
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a33_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a33_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a33_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a33_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a33_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a33_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a33_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a33_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a33_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a33_idempotency_replay_failed");
      }
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a33_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a33_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a33_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error(
          `e2a33_anchor_stance_misclassified:turn_${turn.turn}:expected_${turn.required_anchor_stance}:observed_${anchor.anchor_stance}`
        );
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a33_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a33_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
      ...
      )) throw new Error("e2a33_anchor_interpretation_failure");
      ...
      if (
        ...
        throw new Error("e2a33_unsupported_sound_promotion");
      }
      if (soundGatePassed) {
        if (
          ...
          throw new Error("e2a33_sound_gate_progression_failure");
        }
        endpoint = "passed_required_revision_endpoint";
      } else if (result.route.selected_mode === "request_revision") {
        throw new Error("e2a33_premature_revision_without_sound_gate");
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a33_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a33_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a33_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a33_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a33_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a33_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a33-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  if (burden.metric_version !== "e2a33-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a33-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a33-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a33-causal-inference-isolated-fixture-v1",
    synthetic_only: true,
  ...
  writeJson(path.join(runDir, "overlap-analysis-binding.json"), {
    binding_version: "e2a33-overlap-analysis-binding-v1",
    source_run: E2A33_FREEZE_RUN,
    overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a33_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "budget-ledger.json"), {
    ledger_version: "e2a33-budget-ledger-v1",
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a33-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a33-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a33-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a33-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a33-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a33-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a33-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a33-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a33-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a33-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a33_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a33_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a33-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A33FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a33_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a33_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a33_no_live_network_detected");
    ...
    if (stanceRows.length !== 6 ||
      ...
      throw new Error("e2a33_no_live_stance_boundary_failed");
    }
    const trajectoryRows = readJsonl<JsonObject>(path.join(
    ...
    if (trajectoryRows.length !== 6 ||
      ...
      throw new Error("e2a33_no_live_trajectory_envelope_failed");
    }
    ...
    if ([...requiredCriterionIds].some((criterionId) =>
      ...
      throw new Error("e2a33_no_live_named_revision_criteria_failed");
    }
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a33_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a33_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a33-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A33FrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a33_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a33_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a33_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a33_confirmation_missing:")) {
    throw new Error("e2a33_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a33_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a33_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a33_run_not_found");
...
function auditRun(run?: string) {
  ...
  return {
    audit_version: "e2a33-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a33.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a33_runner_failed");
  process.exitCode = 1;
});
