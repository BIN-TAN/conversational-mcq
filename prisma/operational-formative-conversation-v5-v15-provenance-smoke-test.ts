...
function createCommittedFixture(options?: { include_identity?: boolean }) {
  const root = mkdtempSync(
    path.join(os.tmpdir(), "formative-conversation-v15-provenance-")
  );
  ...
  git(root, ["init", "--quiet"]);
  git(root, ["config", "user.email", "v15-provenance@example.invalid"]);
  git(root, ["config", "user.name", "V15 Provenance Smoke"]);
  git(root, ["add", "freeze"]);
...
function verifyCleanCommittedPackage(input: {
  ...
  const verified = verifyFormativeConversationV15CommittedSource(input);
  ...
  if (workingTreeStatus !== "") {
    throw new Error(
      "formative_conversation_v15_protected_working_tree_not_clean"
    );
  }
  return verified;
}
...
function assertReproducibleMaterialization(before: string, after: string) {
  if (before !== after) {
    throw new Error(
      "formative_conversation_v15_materialization_not_reproducible"
    );
  }
}
...
try {
  ...
  expectProvenanceFailure(
    ...
    /formative_conversation_v15_tracked_source_changed/
  );
  ...
  expectProvenanceFailure(
    ...
    /formative_conversation_v15_tracked_package_missing/
  );
