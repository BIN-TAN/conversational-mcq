...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v6/candidate-runner";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v6/package";
import {
...
async function main() {
  ...
  const v5Source = [
    ...
    "src/lib/operational/formative-conversation-v5-evaluation-v6/contracts.ts"
  ]
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v6/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v6_provider_smoke",
      derived_evaluation_id: "fcv5v6_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v6_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v6_derived_smoke_rerun",
  ...
  assert(
    loaded.runtime_candidate_hash !==
      ...
      v6Authorization.includes(
        "formative-conversation-host-v5-executable-v6"
      ) &&
      v6Authorization.includes(loaded.runtime_candidate_hash) &&
      v6Authorization.includes(loaded.protocol_hash),
    "v6 must use fresh runtime and protocol identities while preserving the failed v5 reference."
  );
  assert(
  ...
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v6/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v6/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v6 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ...
          : "formative_conversation_v5_v6_smoke_failed",
      provider_calls: 0,
