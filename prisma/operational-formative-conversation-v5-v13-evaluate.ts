...
function assertCanonicalLauncher(options: Options) {
  if (
    process.env
      ...
      "true" ||
    process.env.FORMATIVE_CONVERSATION_V5_V13_CANONICAL_LOADER_VERSION !==
      "formative-conversation-v5-canonical-node-import-tsx-v1"
  ) {
    throw new Error(
      "formative_conversation_v13_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    ...
    throw new Error(
      "formative_conversation_v13_canonical_outer_loader_required"
    );
  }
}
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v13-evaluation-prisma",
