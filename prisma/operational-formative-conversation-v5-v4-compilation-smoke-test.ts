...
async function main() {
  ...
  const originalKey = process.env.RESEARCH_PSEUDONYMIZATION_KEY;
  let providerNetworkRequests = 0;

  if (!originalKey?.trim()) {
    process.env.RESEARCH_PSEUDONYMIZATION_KEY =
      "formative-conversation-v5-v4-no-provider-smoke-key";
  }
  globalThis.fetch = (async (input: RequestInfo | URL) => {
    ...
    if (/api\.openai\.com/iu.test(url)) {
      ...
      throw new Error(
        "formative_conversation_v5_v4_provider_call_forbidden"
      );
    }
  ...
  try {
    ...
    if (originalKey === undefined) {
      ...
      process.env.RESEARCH_PSEUDONYMIZATION_KEY = originalKey;
    }
  }
}
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ...
          : "formative_conversation_v5_v4_compilation_smoke_failed",
      provider_calls: 0,
