...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v17/service";
import {
...
import {
  ...
  assertFormativeConversationV17LiveControlEnvironment,
  writeFormativeConversationV17ControlPayload
...
function assertCanonicalLauncher(options: Options) {
  if (
    process.env
      ...
      "true" ||
    process.env.FORMATIVE_CONVERSATION_V5_V17_CANONICAL_LOADER_VERSION !==
      "formative-conversation-v17-canonical-node-import-tsx-v1"
  ) {
    throw new Error(
      "formative_conversation_v17_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    ...
    throw new Error(
      "formative_conversation_v17_canonical_outer_loader_required"
    );
  }
}
...
async function main(input: {
  ...
  const controlEnvironment =
    await assertFormativeConversationV17LiveControlEnvironment({
  ...
  await writeFormativeConversationV17ControlPayload({
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v17-evaluation-prisma",
