...
const VERSION = "e2a34-statistical-inference-live-v1";
const E2A34_FREEZE_RUN = "e2a34_20260724T153144381_ffc2d6a3";
const E2A34_FREEZE_ROOT = path.join(
  process.cwd(), ".data", "e2a34-statistical-inference-protocol-freeze",
  E2A34_FREEZE_RUN
);
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a34-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a34-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const ARTIFACT_CONTRACT_HASH =
  "fdbfa1baaee3770835bab0d5f4b6e3462102cdc8804d1df29420c088a1daf53b";
const TARGET_EVIDENCE_CONTRACT_HASH =
  "811afda34563feb4a9f1f20e38b1cfb0f50589a08e4b5fb3bad15d8c655a947c";
const STATISTICAL_INFERENCE_ALIAS_CONTRACT_HASH =
  "d089daceb528763825cd2e276b35f5831ed158800513fa462ffb82d4d653fac0";
const CANONICAL_ANCHOR_CONTRACT_HASH =
  "c1f70ffb7bd1d0050dd84abde3613dadcf5b0953a1462349f32bdd580bcb0406";
const TRAJECTORY_ENVELOPE_HASH =
  "5682f184c21a5fd487575e185c17dfd58d524a221af93d8acf496db942d7a767";
const RUNTIME_TRAJECTORY_ENVELOPE_HASH =
  "603c1c77b3fbe1ac0c2dca3f27551e4d82ff47b9360831ee14a7e908afb70f3d";
const COMPILED_EVALUATOR_V5_REQUEST_HASH =
  "a28084472c65c04b8921b584d8447523fca185909a39a183759137f2b1e45518";
const COMPILED_EVALUATOR_V5_ARTIFACT_SHA =
  "5aae0cb19c5937090ed2ba163b10671c649a9785d663816958446b5ee7f2374f";
const OVERLAP_ANALYSIS_HASH =
  "e229edcb3c343064f4bcc475eaac21d7691504b0862d60420cc5f348a3624a67";
const BUDGET_HASH =
  "62474b3ff43b5cbd53daf2f22511ed088990aef03e63fd6308972e9e514a0ae8";
const STANCE_EVIDENCE_CONTRACT_HASH =
  "d4f2f5751ec115a90e5360712a851b61777f86b2d508ff66b280a82dba12094d";
...
const CANONICAL_ANCHOR_EVIDENCE_SHA =
...
const ARTIFACT_NAMES = [
  ...
  "statistical-inference-target-evidence-contract.json",
  "statistical-inference-alias-contract.json",
  ...
  "statistical-inference-alias-resolution-results.jsonl",
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.34 evaluation-only constraints:",
  ...
  "Keep the response inside the frozen p-value interpretation scenario.",
...
type FrozenProtocolArtifact = {
  ...
  preparation_version:
    "e2a34-statistical-inference-held-out-protocol-freeze-v1";
  ...
  target_evidence_mapper_version: string;
  evidence_preservation_contract_version: string;
  pre_tutor_finalization_version: string;
...
function frozenArtifactPath(name: string) {
  return path.join(E2A34_FREEZE_ROOT, name);
}
...
function frozenProtocolArtifact() {
  ...
  if (recordedHash !== PROTOCOL_HASH ||
      ...
      protocol.reference_and_stance_resolver_version !==
        ACTIVE_ANCHOR_ALIAS_RESOLUTION_VERSION_V4 ||
      protocol.stance_resolver_version !==
        ANCHOR_STANCE_EVIDENCE_RESOLUTION_VERSION ||
      ...
      protocol.anchor_stance_contract_hash !==
        STANCE_EVIDENCE_CONTRACT_HASH ||
      protocol.evaluator_request_hash !==
        COMPILED_EVALUATOR_V5_REQUEST_HASH ||
      protocol.target_evidence_mapper_version !==
        TURN_EVIDENCE_PROFILE_MAPPER_VERSION_V7 ||
      protocol.evidence_preservation_contract_version !==
        TARGET_EVIDENCE_MAPPER_PRESERVATION_VERSION ||
      protocol.pre_tutor_finalization_version !==
        PRE_TUTOR_PROFILE_FINALIZATION_VERSION_V4 ||
      protocol.sound_gate_version !==
        SOUND_GATE_ANCHOR_CONSISTENCY_VERSION ||
    ...
    throw new Error("e2a34_frozen_protocol_mismatch");
  }
  return protocol;
}

function buildE2A34FrozenProtocol(): Protocol {
  ...
  if (frozen.protocol_hash !== PROTOCOL_HASH ||
      ...
      stableHash(aliasContract) !== STATISTICAL_INFERENCE_ALIAS_CONTRACT_HASH ||
    ...
    throw new Error("e2a34_frozen_protocol_file_sha_mismatch");
  }
  const expectedObjectives = new Map<number, string>([
    ...
    [3, "partial_null_condition_improvement"],
    [4, "null_condition_with_wrong_hypothesis_probability_conclusion"],
    [5, "conditional_probability_clarification"],
  ...
  return {
    ...
    session_count: 1,
    session: {
      session_id: "E2A34-P_VALUE_INFERENCE",
      design:
        "held_out_p_value_inverse_probability_trajectory_envelope",
      ...
      student_profile: {
        ...
        trajectory:
          "direct p-value inversion, copied wording, partial null-conditional reasoning, deliberate mechanism/conclusion contradiction, clarification, and coherent anchor rejection"
      },
      frozen_student_trajectory: [
        turn(1, {
          ...
          no_live_fixture_message:
            "I agree with option D. Since p=.03, there is a 97% chance that the research hypothesis is true.",
          simulator_instruction:
            "Explicitly endorse option D and interpret p=.03 as a 97% probability that the research hypothesis is true. Do not add the null-conditional interpretation.",
        ...
        turn(2, {
          ...
          copy_behavior: true,
          no_live_fixture_message:
            "A p-value is calculated assuming the null hypothesis. I think that is what happens here.",
          simulator_instruction:
            "Echo one distinctive null-conditional phrase from the latest tutor message without independently explaining what p=.03 means or applying it to option D.",
        ...
        turn(3, {
          ...
          no_live_fixture_message:
            "I know the p-value is calculated assuming the null is true and says the data would be unusual under that assumption, but I still think the alternative hypothesis is probably 97% true.",
          simulator_instruction:
            "Recognize that the p-value is calculated under the null and concerns how unusual the data are, but retain the 97% hypothesis-probability interpretation. Do not force an exact evaluator label; if the evidence becomes independently sound, preserve it.",
        ...
        turn(4, {
          ...
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "I understand that p-values are calculated assuming the null is true, but a very small p-value still means there is a 97% chance my hypothesis is correct, so I agree with option D.",
          simulator_instruction:
            "Explain that the p-value is calculated assuming the null and concerns data under that assumption, then state unambiguous agreement with option D and the 97% hypothesis-probability conclusion.",
          semantic_envelope: ["misconception", "partial"],
          tutor_expected: true,
          required_anchor_application: "explicit",
          required_anchor_stance: "endorses_distractor",
          ...
          required_tutor_goal:
            "reconcile the null-conditional explanation with the explicit endorsement of the unsupported 97% hypothesis-probability claim"
        }),
        turn(5, {
          trajectory_role: "clarification",
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "Does p=.03 tell me how unusual the data are if the null is true rather than how likely my research hypothesis is? What else would be needed for a hypothesis probability?",
          simulator_instruction:
            "Ask a natural clarification about the direction of the conditional probability and what additional reasoning or design support would be needed for a stronger inferential claim.",
        ...
        turn(6, {
          ...
          no_live_fixture_message:
            "A p-value of .03 means results this extreme or more would be unusual if the null hypothesis were true. It is P(data like these | null), not P(my hypothesis | data), so one minus .03 is not a 97% probability that my hypothesis is true. A direct hypothesis probability needs additional model assumptions and a design that supports the intended inference, so I reject option D.",
          semantic_envelope: ["partial", "sound"],
          tutor_expected: false,
          simulator_instruction:
            "Independently explain the null-conditional meaning of p=.03, distinguish P(data under the null) from P(hypothesis given data), reject the 97% interpretation, note that stronger claims need additional statistical and design support, and explicitly reject option D.",
          required_anchor_application: "explicit",
          required_anchor_stance: "rejects_distractor",
          required_anchor_consistency:
            "consistent_with_conceptual_reasoning"
...
function frozenCompositeRuntimeIdentity() {
  ...
  if (recorded !== FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      ...
      identity.resolver_version !==
        ACTIVE_ANCHOR_ALIAS_RESOLUTION_VERSION_V4 ||
      identity.stance_resolver_version !==
        ANCHOR_STANCE_EVIDENCE_RESOLUTION_VERSION ||
      identity.evaluator_version !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
      identity.target_evidence_mapper_version !==
        TURN_EVIDENCE_PROFILE_MAPPER_VERSION_V7 ||
      identity.evidence_preservation_contract_version !==
        TARGET_EVIDENCE_MAPPER_PRESERVATION_VERSION ||
      identity.pre_tutor_finalization_version !==
        PRE_TUTOR_PROFILE_FINALIZATION_VERSION_V4) {
    throw new Error("e2a34_frozen_composite_runtime_identity_mismatch");
  }
  return identity;
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a34_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a34_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a34_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a34_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a34_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a34_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a34_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a34_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a34-live.ts",
    "prisma/formative-evaluation-e2a34.ts",
    ...
    path.relative(process.cwd(),
      frozenArtifactPath("runtime-trajectory-envelope-projection.json")),
...
function protectedEvidenceIdentity() {
  ...
  const dataRoots = readdirSync(dataRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() &&
      /^e2a(?:1[2-9]|2[0-9]|3[0-3])[a-z]?(?:\D|$)/u.test(entry.name))
  ...
  return {
    snapshot_version: "e2a34-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    ...
    frozen_e2a34_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    frozen_e2a34_protocol_hash: PROTOCOL_HASH,
    frozen_e2a34_target_evidence_contract_hash:
      TARGET_EVIDENCE_CONTRACT_HASH,
    frozen_e2a34_alias_contract_hash:
      STATISTICAL_INFERENCE_ALIAS_CONTRACT_HASH,
    frozen_e2a34_canonical_anchor_contract_hash:
      CANONICAL_ANCHOR_CONTRACT_HASH,
    ...
    frozen_runtime_trajectory_envelope_hash:
      RUNTIME_TRAJECTORY_ENVELOPE_HASH,
    frozen_e2a34_compiled_evaluator_v5_request_hash:
      COMPILED_EVALUATOR_V5_REQUEST_HASH,
    frozen_e2a34_overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
    frozen_e2a34_artifact_contract_hash: ARTIFACT_CONTRACT_HASH,
    frozen_e2a34_budget_hash: BUDGET_HASH,
    frozen_e2a34_stance_evidence_contract_hash:
      STANCE_EVIDENCE_CONTRACT_HASH,
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a34-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A34FrozenProtocol();
  ...
  const freezeSummary = readJson<{ status?: string;
    ...
    e2a34_live_execution_performed?: boolean;
  ...
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    ...
    correctionArtifactPath("e2a34-budget.json")
  );
  ...
  if (freezeSummary.status !==
      "e2a34_protocol_ready_for_separate_authorization" ||
  ...
  if (freezeValidation.passed !== true ||
    ...
    blockers.push("e2a34_artifact_validation_invalid");
  }
  ...
  if (pids("[f]ormative-evaluation-e2a34").length > 0) {
    blockers.push("duplicate_e2a34_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a34_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a34_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A34 !== "1") blockers.push("live_opt_in_missing");
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a34_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a34_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a34_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a34_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a34-one-session-authorization",
    ...
    "--confirm-stop-after-e2a34"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a34_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a34_dispatch_checkpoint_missing");
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  if (
    ...
    summary.e2a34_ready_for_separate_authorization !== true ||
  ...
  if (pids("[f]ormative-evaluation-e2a34-live").length > 0) {
    blockers.push("duplicate_e2a34_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a34_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) {
    blockers.push(`prior_e2a34_run_exists:${priorRuns.at(-1)}`);
  }
  ...
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A34 !== "1") {
...
function recordDispatchCheckpoint() {
  ...
  if (!trackedTreeClean()) {
    throw new Error("e2a34_checkpoint_tree_not_clean");
  }
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(
      `e2a34_checkpoint_preflight_failed:${check.blockers.join(",")}`
    );
  }
  const identity = compositeRuntimeIdentity();
  const checkpoint: DispatchCheckpoint = {
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  if (stableHash(contract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(contract.active_anchor_alias_contract) !==
        STATISTICAL_INFERENCE_ALIAS_CONTRACT_HASH) {
    throw new Error("e2a34_target_evidence_contract_hash_mismatch");
  }
  return contract;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a34_session_${session.session_id}`,
    scenario_version: "e2a34-trajectory-envelope-simulator-v1",
...
function validateSimulatorOutput(input: {
  ...
  const resolution = resolveActiveAnchorAliasV4({
    ...
    source_turn_id: `e2a34_simulator_validation_turn_${input.turn.turn}`,
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a34_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a34_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a34_${session.session_id}`,
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a34_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a34_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a34",
    source_diagnostic_purpose: "reasoning_boundary_repair",
    ...
    response_summary: contradictory
      ? `${session.session_id} turn ${turn.turn} explains the null-conditional meaning of the p-value but explicitly endorses the unsupported 97% hypothesis-probability claim, creating a reasoning/conclusion conflict.`
...
function noLiveTutorOutput(input: AutonomousPedagogyInput) {
  ...
  const strategies = [
    "contrast the p-value's null-conditional meaning with a hypothesis-probability claim",
    "trace the direction of the conditional probability in the student's explanation",
    "ask for a one-sentence distinction between data under the null and a hypothesis given data",
    "test what additional assumptions or design evidence a stronger inferential claim would require",
    "reconcile the stated option conclusion with the null-conditional explanation"
  ];
  const studentTasks = [
    ...
    "Set the wording aside and rebuild the statistical reasoning in one or two sentences, ending with its consequence for this option.",
...
function buildE2A34NoLiveStructuredTurnEvidence(input: {
  ...
  return ProductionTurnEvidenceEvaluatorOutputV5Schema.shape
    .structured_turn_evidence.parse({
      ...
      evidence_limitations: [
        ...
        "e2a34_no_live_stance_evidence_v2_fixture"
      ]
    });
}
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a34_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a34_no_live_evaluator_case_missing");
      ...
      output = ProductionTurnEvidenceEvaluatorOutputV5Schema.parse({
        ...
        structured_turn_evidence:
          buildE2A34NoLiveStructuredTurnEvidence({
            source_student_turn_id:
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        ...
        throw new Error(
          `e2a34_no_live_tutor_fixture_invalid:turn_${turnNumber}:sequence_${tutorInput.latest_student_response.source_sequence_index}:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`
        );
      }
    } else {
      throw new Error(`e2a34_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a34_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a34_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a34_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a34_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a34:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a34_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a34_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a34_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a34_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a34_canary_failed_rate_limit_retry_exhausted"
          : "e2a34_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a34_canary_failed_provider_nonretryable_request"
        : `e2a34_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation?.passed) {
        throw new Error(
          `e2a34_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation?.issues.join("|") ?? "validation_not_run"}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a34_canary_failed_evaluator_contract");
          }
          appendJsonl(path.join(
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a34_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a34_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a34_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a34_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a34_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
          ...
          appendJsonl(path.join(
            input.runDir, "statistical-inference-alias-resolution-results.jsonl"
          ), {
            session_id: input.session.session_id,
            turn: turn.turn,
            target_evidence_contract_hash: TARGET_EVIDENCE_CONTRACT_HASH,
            statistical_inference_alias_contract_hash: STATISTICAL_INFERENCE_ALIAS_CONTRACT_HASH,
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a34_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a34_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a34_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a34_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a34_idempotency_replay_failed");
      }
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a34_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a34_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a34_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error(
          `e2a34_anchor_stance_misclassified:turn_${turn.turn}:expected_${turn.required_anchor_stance}:observed_${anchor.anchor_stance}`
        );
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a34_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a34_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
      ...
      )) throw new Error("e2a34_anchor_interpretation_failure");
      ...
      if (
        ...
        throw new Error("e2a34_unsupported_sound_promotion");
      }
      if (soundGatePassed) {
        if (
          ...
          throw new Error("e2a34_sound_gate_progression_failure");
        }
        endpoint = "passed_required_revision_endpoint";
      } else if (result.route.selected_mode === "request_revision") {
        throw new Error("e2a34_premature_revision_without_sound_gate");
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a34_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a34_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a34_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a34_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a34_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a34_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a34-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  const targetContract = readJson<JsonObject>(path.join(
    runDir, "statistical-inference-target-evidence-contract.json"
  ));
  const aliasContract = readJson<JsonObject>(path.join(
    runDir, "statistical-inference-alias-contract.json"
  ));
  ...
  if (frozenIdentity.composite_runtime_identity_hash !==
    ...
    failures.push("frozen_statistical_inference_binding_invalid");
  }
  ...
  if (burden.metric_version !== "e2a34-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a34-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a34-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a34-statistical-inference-isolated-fixture-v1",
    synthetic_only: true,
  ...
  writeJson(path.join(runDir, "overlap-analysis-binding.json"), {
    binding_version: "e2a34-overlap-analysis-binding-v1",
    source_run: E2A34_FREEZE_RUN,
    overlap_analysis_hash: OVERLAP_ANALYSIS_HASH,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a34_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "budget-ledger.json"), {
    ledger_version: "e2a34-budget-ledger-v1",
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a34-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a34-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a34-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a34-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a34-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a34-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a34-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a34-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a34-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a34-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a34_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a34_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a34-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A34FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a34_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a34_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a34_no_live_network_detected");
    ...
    if (stanceRows.length !== 6 ||
      ...
      throw new Error("e2a34_no_live_stance_boundary_failed");
    }
    const trajectoryRows = readJsonl<JsonObject>(path.join(
    ...
    if (trajectoryRows.length !== 6 ||
      ...
      throw new Error("e2a34_no_live_trajectory_envelope_failed");
    }
    ...
    if ([...requiredCriterionIds].some((criterionId) =>
      ...
      throw new Error("e2a34_no_live_named_revision_criteria_failed");
    }
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a34_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a34_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a34-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A34FrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a34_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a34_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a34_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a34_confirmation_missing:")) {
    throw new Error("e2a34_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a34_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a34_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a34_run_not_found");
...
function auditRun(run?: string) {
  ...
  const aliasResolutions = readJsonl<JsonObject>(path.join(
    runDir, "statistical-inference-alias-resolution-results.jsonl"
  )).filter((row) => typeof row.turn === "number");
  ...
  return {
    audit_version: "e2a34-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
    ...
    turn_counts: {
      ...
      statistical_inference_alias_resolutions: aliasResolutions.length,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a34.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a34_runner_failed");
  process.exitCode = 1;
});
