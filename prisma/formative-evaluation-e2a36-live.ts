...
const VERSION = "e2a36-measurement-reasoning-longitudinal-live-v1";
const E2A36_FREEZE_RUN = "e2a36_20260725T021228230Z_4eef5bb5";
const E2A36_FREEZE_ROOT = path.join(
  process.cwd(), ".data",
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a36-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a36-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const ARTIFACT_CONTRACT_HASH =
  "5063f8eccb075209d0ff8003448c8452f5dda05f75592990b8cf7f9c58ea5a47";
const TARGET_EVIDENCE_CONTRACT_HASH =
  "aa737095ccf703f2ed7f39ae21d18ab3e93c1ad24a02bc12aaf7fee77f267b63";
const MEASUREMENT_ALIAS_CONTRACT_HASH =
  "37120cd77e7ce1cf82c1d494b72749fe463d37dde54f0ca7089069c877c9160d";
const CANONICAL_ANCHOR_CONTRACT_HASH =
  "883c596f5b8d808e38db9f74c320d0bea27abb0dd89bfb856e312b13f32971be";
const TRAJECTORY_ENVELOPE_HASH =
  "13deef5f0c839e15ecac0c08ac7dc8c6edcff5cdbc3e53d37b638461edfad0b8";
const COMPILED_EVALUATOR_V5_REQUEST_HASH =
  "bd8a829d73903db7a69a4639adee15592aa9be5431d32e99a9e57d91b53c19af";
const COMPILED_EVALUATOR_V5_ARTIFACT_SHA =
  "41ed4f82b9f3a834ac6efb290708fb1e456a55a0ec2b53697a711eb02a83acbe";
const PROTECTED_SOURCE_SET_HASH =
  "c49c06c15e6a1ea6ff7e4102af40b88893cea1ad9de4cbee103aa6db3c8826b3";
const BUDGET_HASH =
  "c06ee3d47385b44a5a5b7a54f2f77530d5531c47aa332069cc676922c2725eb8";
const STANCE_EVIDENCE_CONTRACT_HASH =
  "d73511aef4ea13d642b6b68367d6370f471a958e9adcf4bf5b5f2d917e415a64";
const SELF_CORRECTION_INTEGRATION_CONTRACT_HASH =
  "f1340dd54844dd28d8342445324179d8b0e8b95cc55993c461a037a3d8912989";
...
const STUDENT_COMMUNICATION_CONTRACT_HASH =
  "ab7618f83e39445b0df74ef281d8adaf0ded71e7f5ef23d32e16e5891675baa1";
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.36 evaluation-only constraints:",
  ...
  "Respond as one longitudinal learner whose latest message may improve, self-correct, contradict, or regress from prior reasoning.",
...
type FrozenProtocolArtifact = {
  ...
  preparation_version:
    "e2a36-measurement-longitudinal-protocol-freeze-preparation-v1";
...
function frozenArtifactPath(name: string) {
  return path.join(E2A36_FREEZE_ROOT, name);
}
...
function frozenProtocolArtifact() {
  ...
  if (recordedHash !== PROTOCOL_HASH ||
      ...
      protocol.domain !== "educational_measurement_measurement_theory" ||
      protocol.execution_authorized !== false ||
      protocol.live_execution_performed !== false ||
      protocol.provider_dispatch_path_present !== false ||
      protocol.candidate_configuration_hash !== CANDIDATE_HASH ||
      protocol.evaluator_v5.evaluator_version !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V5 ||
      protocol.evaluator_v5.prompt_hash !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_PROMPT_HASH_V5 ||
      protocol.evaluator_v5.repair_prompt_hash !==
        PRODUCTION_TURN_EVIDENCE_EVALUATOR_REPAIR_PROMPT_HASH_V5 ||
      protocol.contract_versions.anchor_scope !==
        ANCHOR_STANCE_SCOPE_RESOLUTION_VERSION ||
      protocol.contract_versions.self_correction_evidence !==
        SELF_CORRECTION_EVIDENCE_CONTRACT_VERSION ||
      protocol.contract_versions.trajectory_envelope !==
        TRAJECTORY_ENVELOPE_VERSION ||
      protocol.contract_versions.learning_profile_evolution !==
        E2A36_LEARNING_PROFILE_EVOLUTION_VERSION ||
      protocol.contract_versions.engagement_profile_evolution !==
        E2A36_ENGAGEMENT_PROFILE_EVOLUTION_VERSION ||
      protocol.contract_versions.intervention_memory !==
        E2A36_INTERVENTION_MEMORY_VERSION ||
      protocol.contract_versions.adaptive_stopping_policy !==
        E2A36_ADAPTIVE_STOPPING_POLICY_VERSION ||
      protocol.contract_versions.instructor_escalation_policy !==
        E2A36_INSTRUCTOR_ESCALATION_POLICY_VERSION ||
      protocol.contract_versions.student_facing_communication !==
        E2A36_STUDENT_FACING_COMMUNICATION_VERSION ||
      protocol.contract_versions.metrics !==
        E2A36_LONGITUDINAL_METRICS_VERSION ||
      ...
      protocol.contract_hashes.self_correction_integration !==
        SELF_CORRECTION_INTEGRATION_CONTRACT_HASH ||
      protocol.contract_hashes.compiled_evaluator_request !==
      ...
      protocol.contract_hashes.escalation_policy !==
        ESCALATION_POLICY_CONTRACT_HASH ||
      protocol.contract_hashes.student_communication !==
        STUDENT_COMMUNICATION_CONTRACT_HASH ||
      protocol.contract_hashes.metrics !==
        LONGITUDINAL_METRICS_CONTRACT_HASH ||
      protocol.artifact_contract_hash !== ARTIFACT_CONTRACT_HASH ||
      protocol.budget.maximum_logical_generation_calls !==
        BUDGET.logical_generation_calls ||
      protocol.budget.maximum_adapter_attempts !== BUDGET.adapter_attempts ||
      protocol.budget.maximum_input_tokens !== BUDGET.input_tokens ||
      protocol.budget.maximum_output_tokens !== BUDGET.output_tokens ||
      protocol.budget.maximum_total_tokens !== BUDGET.total_tokens ||
      protocol.budget.maximum_cost_usd_when_pricing_metadata_available !==
        BUDGET.cost_usd ||
      protocol.budget.provider_concurrency !== BUDGET.provider_concurrency ||
      protocol.deterministic_gate_results.all_regressions_passed !== true ||
      protocol.deterministic_gate_results.protected_components_unchanged !==
        true ||
      protocol.deterministic_gate_results.provider_calls_zero !== true ||
      protocol.deterministic_gate_results.network_requests_zero !== true) {
    throw new Error("e2a36_frozen_protocol_mismatch");
  }
  return protocol;
}

function buildE2A36FrozenProtocol(): Protocol {
  ...
  if (frozen.protocol_hash !== PROTOCOL_HASH ||
      ...
      frozen.scenario.active_distractor_claim !==
        targetContract.distractor_claim ||
    ...
    throw new Error("e2a36_frozen_protocol_file_sha_mismatch");
  }
  const expectedObjectives = new Map<number, string>([
    [1, "initial_misconception"],
    [2, "partial_concept_distinction"],
    [3, "contradictory_anchor_endorsement"],
    [4, "unsupported_self_correction"],
    [5, "evidence_bearing_self_correction"],
    [6, "regression_to_reliability_overclaim"],
    [7, "counterexample_based_repair"],
    [8, "independent_sound_reconstruction"],
    [9, "bounded_longitudinal_endpoint"]
  ]);
  ...
  return {
    ...
    session: {
      session_id: "E2A36-MEASUREMENT_REASONING_LONGITUDINAL",
      design:
        "held_out_measurement_reasoning_longitudinal_adaptive_stopping",
      ...
      student_profile: {
        ...
        trajectory:
          "misconception, partial distinction, contradiction, unsupported correction, evidence-bearing correction, regression, renewed repair, and independent reconstruction"
      },
      frozen_student_trajectory: [
        ...
        turn(2, {
          trajectory_role: "partial_understanding",
        ...
        turn(4, {
          trajectory_role: "self_correction",
        ...
        turn(6, {
          ...
          required_tutor_goal:
            "reopen support and distinguish internal consistency evidence from validity evidence"
        }),
        turn(7, {
          trajectory_role: "partial_understanding",
          fixture_reasoning_quality: "partial",
        ...
        turn(8, {
          trajectory_role: "sound_understanding",
        ...
        turn(9, {
          trajectory_role: "bounded_endpoint",
          fixture_reasoning_quality: "partial",
          no_live_fixture_message:
            "I can tell consistency is not enough, but I am still stuck on what evidence would support the intended use.",
          simulator_instruction:
            "If the dialogue has not already stopped, give a relevant but still incomplete response that identifies consistency as insufficient and asks for help with validity evidence. Do not fabricate sound understanding.",
          semantic_envelope: [
            "insufficient",
            "misconception",
            "partial",
...
function frozenCompositeRuntimeIdentity() {
  ...
  if (recorded !== FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
      ...
      identity.candidate_file_sha256 !== CANDIDATE_FILE_SHA ||
      identity.evaluator_v5_source_sha256 !== EVALUATOR_V5_SHA ||
      identity.tutor_candidate_source_sha256 !== TUTOR_CANDIDATE_SHA ||
      identity.protected_source_set_hash !== PROTECTED_SOURCE_SET_HASH ||
      identity.budget_hash !== BUDGET_HASH ||
      identity.artifact_contract_hash !== ARTIFACT_CONTRACT_HASH) {
    throw new Error("e2a36_frozen_composite_runtime_identity_mismatch");
  }
  return identity;
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a36_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a36_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a36_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a36_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a36_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a36_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a36_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a36_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a36-live.ts",
...
function protectedEvidenceIdentity() {
  ...
  return {
    snapshot_version: "e2a36-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const identity = {
    ...
    frozen_e2a36_composite_runtime_identity_hash:
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH,
    frozen_e2a36_protocol_hash: PROTOCOL_HASH,
    frozen_e2a36_target_evidence_contract_hash:
      TARGET_EVIDENCE_CONTRACT_HASH,
    frozen_e2a36_alias_contract_hash:
      MEASUREMENT_ALIAS_CONTRACT_HASH,
    frozen_e2a36_canonical_anchor_contract_hash:
      CANONICAL_ANCHOR_CONTRACT_HASH,
    ...
    frozen_e2a36_compiled_evaluator_v5_request_hash:
      COMPILED_EVALUATOR_V5_REQUEST_HASH,
    frozen_e2a36_protected_source_set_hash: PROTECTED_SOURCE_SET_HASH,
    frozen_e2a36_artifact_contract_hash: ARTIFACT_CONTRACT_HASH,
    frozen_e2a36_budget_hash: BUDGET_HASH,
    frozen_e2a36_stance_evidence_contract_hash:
      STANCE_EVIDENCE_CONTRACT_HASH,
    frozen_e2a36_self_correction_integration_contract_hash:
      SELF_CORRECTION_INTEGRATION_CONTRACT_HASH,
    self_correction_evidence_contract_version:
      SELF_CORRECTION_EVIDENCE_CONTRACT_VERSION,
    self_correction_evidence_contract_hash:
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a36-dispatch-checkpoint-v1";
  recorded_at: string;
...
/*
 * Historical E2A.35 preflight retained as migration context. The active
 * E2A.36 preflight below binds directly to the longitudinal freeze artifacts.
/*
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  const blockers: string[] = [];
  const candidate = evaluateE2A24Candidate();
  const protocol = buildE2A36FrozenProtocol();
  ...
  const freezeSummary = readJson<{ status?: string;
    ...
    e2a36_live_execution_performed?: boolean;
  ...
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    ...
    correctionArtifactPath("e2a36-budget.json")
  );
  ...
  if (freezeSummary.status !==
      "e2a36_protocol_ready_for_separate_authorization" ||
  ...
  if (freezeValidation.passed !== true ||
    ...
    blockers.push("e2a36_artifact_validation_invalid");
  }
  ...
  if (pids("[f]ormative-evaluation-e2a36").length > 0) {
    blockers.push("duplicate_e2a36_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a36_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a36_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A36 !== "1") blockers.push("live_opt_in_missing");
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  if (
    summary.status !==
      "e2a36_protocol_frozen_for_separate_authorization" ||
    ...
    summary.e2a36_live_execution_performed !== false ||
    summary.e2a36_execution_authorized !== false ||
  ...
  if (
    ...
    blockers.push("frozen_measurement_contract_invalid");
  }
  if (
    stableHash(overlap) !== OVERLAP_ANALYSIS_HASH ||
    overlap.passed !== true ||
  ...
  if (
    ...
    providerGuard.e2a36_live_execution_performed !== false
  ) {
    blockers.push("frozen_provider_call_guard_invalid");
  }
  if (
  ...
  if (pids("[f]ormative-evaluation-e2a36-live").length > 0) {
    blockers.push("duplicate_e2a36_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a36_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) {
    blockers.push(`prior_e2a36_run_exists:${priorRuns.at(-1)}`);
  }
  ...
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A36 !== "1") {
  ...
  return {
    ...
    frozen_canonical_anchor_contract_valid:
      !blockers.includes("frozen_measurement_contract_invalid"),
    self_correction_intent_version: SELF_CORRECTION_INTENT_VERSION,
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  if (
    summary.status !== "e2a36_protocol_frozen_not_executed" ||
    summary.protocol_hash !== PROTOCOL_HASH ||
    summary.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    summary.deterministic_case_count !== 34 ||
    summary.deterministic_regressions_passed !== true ||
    summary.protected_components_unchanged !== true ||
  ...
  if (
    validation.passed !== true ||
    validation.expected_artifact_count !== 35 ||
    validation.actual_artifact_count_before_validation !== 34 ||
    (validation.missing_artifacts as unknown[] | undefined)?.length !== 0 ||
    (validation.unexpected_artifacts as unknown[] | undefined)?.length !== 0 ||
    (validation.checks as JsonObject | undefined)
      ?.deterministic_regressions_passed !== true ||
    (validation.checks as JsonObject | undefined)?.no_provider_calls !== true ||
    (validation.checks as JsonObject | undefined)?.protocol_hash_matches !==
      true ||
    (validation.checks as JsonObject | undefined)?.protocol_not_authorized !==
      true
  ) {
    blockers.push("e2a36_artifact_validation_invalid");
  }
  if (
    ...
    artifactContract.artifact_contract_version !==
      "e2a36-artifact-contract-v1" ||
    artifactContract.preserve_provider_outputs_if_separately_authorized_later !==
      true ||
    artifactContract.preparation_creates_live_artifacts !== false ||
    artifactContract.artifacts_must_be_read_only !== true
  ) {
    blockers.push("frozen_artifact_contract_mismatch");
  }
  if (
    stableHash(budget) !== BUDGET_HASH ||
    budget.exactly_one_isolated_session !== true ||
    budget.maximum_logical_generation_calls !==
      BUDGET.logical_generation_calls ||
    budget.maximum_adapter_attempts !== BUDGET.adapter_attempts ||
    ...
    budget.live_execution_performed !== false
  ) {
    blockers.push("frozen_budget_mismatch");
  }
  if (
  ...
  if (
    ...
    stableHash(selfCorrectionIntegration) !==
      SELF_CORRECTION_INTEGRATION_CONTRACT_HASH ||
    stableHash(buildSelfCorrectionEvidenceContractV1()) !==
      SELF_CORRECTION_EVIDENCE_CONTRACT_HASH ||
    stableHash(trajectory) !== TRAJECTORY_ENVELOPE_HASH ||
    runtimeContracts.some(([contract, expectedHash]) =>
      stableHash(contract) !== expectedHash
    )
  ) {
    blockers.push("frozen_measurement_longitudinal_contract_invalid");
  }
  if (
    providerGuard.passed !== true ||
  ...
  if (
    protectedSources.all_unchanged !== true ||
    protectedSources.evaluator_v5_unchanged !== true ||
    protectedSources.tutor_candidate_unchanged !== true ||
    protectedSources.files?.some((entry) =>
      entry.unchanged !== true ||
      entry.expected_sha256 !== entry.actual_sha256
    )
  ) {
  ...
  if (
    frozenIdentity.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    frozenIdentity.protected_source_set_hash !== PROTECTED_SOURCE_SET_HASH
  ) {
    blockers.push("frozen_composite_runtime_identity_mismatch");
  }
  if (
  ...
  if (
    ...
    (!checkpoint ||
      ...
      checkpoint.compiled_evaluator_v5_request_hash !==
        COMPILED_EVALUATOR_V5_REQUEST_HASH ||
      checkpoint.candidate_configuration_hash !== CANDIDATE_HASH ||
      checkpoint.candidate_file_sha256 !== CANDIDATE_FILE_SHA ||
      checkpoint.live_execution_started !== false)
  ) {
    blockers.push("dispatch_checkpoint_mismatch");
  }
  if (pids("[f]ormative-evaluation-e2a36-live").length > 0) {
  ...
  return {
    ...
    candidate_authorized_for_activation: false,
    candidate_active: false,
    evaluator_v5_verified:
      !blockers.includes("frozen_evaluator_v5_request_invalid"),
    learning_profile_evolution_verified:
      !blockers.includes("frozen_measurement_longitudinal_contract_invalid"),
    engagement_profile_evolution_verified:
      !blockers.includes("frozen_measurement_longitudinal_contract_invalid"),
    intervention_memory_verified:
      !blockers.includes("frozen_measurement_longitudinal_contract_invalid"),
    adaptive_stopping_policy_verified:
      !blockers.includes("frozen_measurement_longitudinal_contract_invalid"),
    student_facing_communication_policy_verified:
      !blockers.includes("frozen_measurement_longitudinal_contract_invalid"),
    provider_call_guard_verified:
      !blockers.includes("frozen_provider_call_guard_invalid"),
    composite_identity: identity,
    dispatch_checkpoint: checkpoint,
    provider_concurrency: 1,
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a36_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a36_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a36_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a36_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a36-one-session-authorization",
    ...
    "--confirm-stop-after-e2a36"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a36_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a36_dispatch_checkpoint_missing");
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  if (
    summary.status !==
      "e2a36_protocol_frozen_not_authorized_not_executed" ||
    summary.protocol_hash !== PROTOCOL_HASH ||
    summary.composite_runtime_identity_hash !==
      FROZEN_COMPOSITE_RUNTIME_IDENTITY_HASH ||
    summary.deterministic_regressions_passed !== true ||
    ...
    summary.e2a36_ready_for_separate_authorization !== true ||
    summary.execution_authorized !== false ||
    summary.live_execution_performed !== false ||
    summary.provider_calls_made !== 0 ||
    summary.network_requests_made !== 0
  ...
  if (
    ...
    stanceEvidenceContract.stance_contract_version !==
      "e2a36-p-value-anchor-stance-v1" ||
...
function recordDispatchCheckpoint() {
  ...
  if (!trackedTreeClean()) {
    throw new Error("e2a36_checkpoint_tree_not_clean");
  }
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(
      `e2a36_checkpoint_preflight_failed:${check.blockers.join(",")}`
    );
  }
  const identity = compositeRuntimeIdentity();
  const checkpoint: DispatchCheckpoint = {
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  if (stableHash(contract) !== TARGET_EVIDENCE_CONTRACT_HASH ||
      stableHash(contract.active_anchor_alias_contract) !==
        MEASUREMENT_ALIAS_CONTRACT_HASH) {
    throw new Error("e2a36_target_evidence_contract_hash_mismatch");
  }
  return contract;
}

function selfCorrectionContract(): SelfCorrectionIntentContractV1 {
  return buildE2A36FrozenProtocol().self_correction_intent_contract;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a36_session_${session.session_id}`,
    scenario_version: "e2a36-trajectory-envelope-simulator-v1",
...
function validateSimulatorOutput(input: {
  ...
  const resolution = resolveActiveAnchorAliasV4({
    ...
    source_turn_id: `e2a36_simulator_validation_turn_${input.turn.turn}`,
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a36_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a36_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a36_${session.session_id}`,
    ...
    source_activity_context: {
      ...
      profile_condition: "synthetic_frozen_e2a36_trajectory",
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a36_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a36_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a36",
    source_diagnostic_purpose: "reasoning_boundary_repair",
...
function buildE2A36NoLiveStructuredTurnEvidence(input: {
  ...
  return ProductionTurnEvidenceEvaluatorOutputV5Schema.shape
    .structured_turn_evidence.parse({
      ...
      evidence_limitations: [
        ...
        "e2a36_no_live_stance_evidence_v2_fixture"
      ]
    });
}
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return {
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a36_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a36_no_live_evaluator_case_missing");
      ...
      output = ProductionTurnEvidenceEvaluatorOutputV5Schema.parse({
        ...
        structured_turn_evidence:
          buildE2A36NoLiveStructuredTurnEvidence({
            source_student_turn_id:
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        ...
        throw new Error(
          `e2a36_no_live_tutor_fixture_invalid:turn_${turnNumber}:sequence_${tutorInput.latest_student_response.source_sequence_index}:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`
        );
      }
    } else {
      throw new Error(`e2a36_no_live_unknown_agent:${request.agent_name}`);
    }
      return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a36_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a36_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a36_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a36_composite_identity_changed_before_dispatch");
    }
  }
  ...
  const transport = await executeWithBoundedProviderTransportRetry({
    ...
    logical_idempotency_key: `e2a36:${logicalCallId}`,
  ...
  if (!result) {
    throw new Error(`e2a36_provider_dispatch_blocked:${transport.status}`);
  }
  recordCall(
  ...
  if (transport.accepted_result) {
    ...
    if (!acceptedTrace || acceptedTrace.retry_decision !== "accepted") {
      throw new Error("e2a36_accepted_transport_trace_missing");
    }
    ...
    if (receipt.status !== "committed") {
      throw new Error(`e2a36_duplicate_semantic_effect:${receipt.status}`);
    }
  } else {
    appendJsonl(path.join(input.runDir, "exactly-once-results.jsonl"), {
  ...
  if (transport.status !== "accepted" || result.status !== "completed" ||
    ...
    const failureCode = transport.status === "transport_failure_retry_exhausted"
      ? category === "network_timeout" || category === "upstream_timeout"
        ? "e2a36_canary_failed_provider_timeout_retry_exhausted"
        : category === "retryable_rate_limit"
          ? "e2a36_canary_failed_rate_limit_retry_exhausted"
          : "e2a36_canary_failed_provider_infrastructure_retry_exhausted"
      : transport.status === "transport_failure_nonretryable"
        ? "e2a36_canary_failed_provider_nonretryable_request"
        : `e2a36_provider_call_failed_${transport.status}`;
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation?.passed) {
        throw new Error(
          `e2a36_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation?.issues.join("|") ?? "validation_not_run"}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          if (!evaluatorContractIdentity.passed) {
            throw new Error("e2a36_canary_failed_evaluator_contract");
          }
          appendJsonl(path.join(
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a36_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a36_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a36_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a36_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a36_evaluator_effective_output_mismatch");
          }
          appendJsonl(path.join(
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a36_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a36_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a36_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a36_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a36_idempotency_replay_failed");
      }
      ...
      if (!result.profile_update_record) {
        throw new Error("e2a36_profile_update_record_missing");
      }
      ...
      if (!latestValidEvidenceApplied) {
        throw new Error("e2a36_latest_valid_evidence_not_authoritative");
      }
      if (!unsupportedCorrectionPreserved) {
        throw new Error("e2a36_correction_language_promoted_to_evidence");
      }
      if (turn.profile_reopening_expected && !profileReopened) {
        throw new Error([
          "e2a36_regression_did_not_reopen_profile",
          updateDisposition,
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a36_pre_tutor_profile_finalization_failed");
      }
      appendJsonl(path.join(
      ...
      if (!result.observation_record || !result.profile_update_record ||
        ...
        throw new Error("e2a36_canary_failed_profile_finalization");
      }
      appendJsonl(path.join(
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a36_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error(
          `e2a36_anchor_stance_misclassified:turn_${turn.turn}:expected_${turn.required_anchor_stance}:observed_${anchor.anchor_stance}`
        );
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a36_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a36_blocking_conflict_only_in_limitations");
      }
      ...
      if (
        ...
        throw new Error("e2a36_unsupported_sound_promotion");
      }
      ...
      if (
        ...
        throw new Error(
          "e2a36_self_correction_intent_promoted_without_evidence"
        );
      }
      appendJsonl(path.join(
      ...
      if (!stoppingRuntimeConsistent) {
        throw new Error([
          "e2a36_longitudinal_stopping_runtime_mismatch",
          stoppingDecision.internal_decision,
      ...
      const effectiveMessageValidation =
        validateStudentFacingCommunicationV1(result.effective_message);
      ...
      if (!effectiveMessageValidation.passed) {
        throw new Error(
          `e2a36_student_facing_internal_state_leak:${effectiveMessageValidation.issue_codes.join("|")}`
        );
      }
      appendJsonl(path.join(
      ...
      if (soundGatePassed) {
        if (
          ...
          throw new Error("e2a36_sound_gate_progression_failure");
        }
        endpoint = "passed_required_revision_endpoint";
      } else if (result.route.selected_mode === "request_revision") {
        throw new Error("e2a36_premature_revision_without_sound_gate");
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a36_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a36_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a36_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a36_session_endpoint_not_reached");
    if (!input.session.required_endpoints.includes(endpoint)) {
      throw new Error("e2a36_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a36_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a36-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  if (frozenIdentity.composite_runtime_identity_hash !==
      ...
      stableHash(selfCorrectionContractArtifact) !==
        stableHash(buildE2A36FrozenProtocol().self_correction_intent_contract) ||
      stableHash(selfCorrectionEvidenceContract) !==
        SELF_CORRECTION_EVIDENCE_CONTRACT_HASH ||
      stableHash(selfCorrectionIntegration) !==
        SELF_CORRECTION_INTEGRATION_CONTRACT_HASH ||
  ...
  if (burden.metric_version !== "e2a36-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a36-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a36-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a36-measurement-reasoning-isolated-fixture-v1",
    synthetic_only: true,
  ...
  if (input.live) {
    writeJson(path.join(runDir, "dispatch-checkpoint.json"), {
      ...checkpoint,
      checkpoint_version: "e2a36-live-dispatch-start-v1",
      run_id: id,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a36_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "budget-ledger.json"), {
    ledger_version: "e2a36-budget-ledger-v1",
  ...
  writeJson(path.join(runDir, "transport-handling-metrics.json"), {
    metric_version: "e2a36-transport-handling-v1",
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a36-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a36-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a36-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a36-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a36-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a36-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a36-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a36-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a36-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a36_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a36_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a36-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A36FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a36_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a36_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a36_no_live_network_detected");
    ...
    if (stanceRows.length !== 8 ||
      ...
      throw new Error("e2a36_no_live_stance_boundary_failed");
    }
    ...
    if (
      ...
      throw new Error("e2a36_no_live_self_correction_state_failed");
    }
    const trajectoryRows = readJsonl<JsonObject>(path.join(
    ...
    if (trajectoryRows.length !== 8 ||
      ...
      throw new Error("e2a36_no_live_trajectory_envelope_failed");
    }
    ...
    if ([...requiredCriterionIds].some((criterionId) =>
      ...
      throw new Error("e2a36_no_live_named_revision_criteria_failed");
    }
    ...
    if (
      ...
      throw new Error("e2a36_no_live_longitudinal_contract_failed");
    }
    ...
    if (usage.simulator_calls !== 8 || usage.evidence_evaluator_calls !== 8 ||
      ...
      throw new Error("e2a36_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
async function runTransportRetrySmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a36_transport_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(),
    `e2a36-transport-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A36FrozenProtocol();
    ...
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a36_transport_retry_smoke"
    });
    ...
    if (!result.summary.passed || !result.validation.passed ||
        ...
        new Set(retriedAttempts.map((row) => row.x_client_request_id))
          .size !== 2) {
      throw new Error("e2a36_transport_retry_fault_injection_failed");
    }
    if (networkRequests !== 0) {
      throw new Error("e2a36_transport_smoke_network_detected");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a36_confirmation_missing:")) {
    throw new Error("e2a36_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a36_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a36_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a36_run_not_found");
...
function auditRun(run?: string) {
  ...
  if (summary.passed === true) {
    ...
    if (completedTurnCount < 1 || completedTurnCount > 9 ||
    ...
    const correctionEvidence = (turn: number) =>
      selfCorrectionEvidenceResults.find((row) => row.turn === turn)
        ?.resolution as JsonObject | undefined;
    if (completedTurnCount >= 4 && (
      profile(4)?.reasoning_quality === "sound" ||
      correctionEvidence(4)?.self_correction_intent !== true ||
      correctionEvidence(4)?.conceptual_evidence_update !== false ||
      correctionEvidence(4)?.profile_update_eligible !== false ||
      latestEvidenceResults.find((row) => row.turn === 4)
        ?.unsupported_correction_preserved_prior_profile !== true
    )) {
      failures.push("unsupported_self_correction_promoted_to_understanding");
    }
    if (completedTurnCount >= 5 &&
        correctionEvidence(5)?.self_correction_intent === true &&
        correctionEvidence(5)?.latest_valid_evidence_eligible === true &&
        latestEvidenceResults.find((row) => row.turn === 5)
          ?.latest_valid_evidence_applied !== true) {
      failures.push("valid_self_correction_not_applied");
    }
    if (completedTurnCount >= 6 && (
      anchor(6)?.anchor_application !== "explicit" ||
      anchor(6)?.anchor_stance !== "endorses_distractor" ||
      profile(6)?.reasoning_quality !== "misconception" ||
      profile(6)?.revision_readiness !== false ||
      mode(6)?.selected_mode !== "remain_in_dialogue" ||
      profileReopeningResults.find((row) => row.turn === 6)
        ?.profile_reopened !== true ||
      profileReopeningResults.find((row) => row.turn === 6)
        ?.reopened_from_latest_regression !== true ||
      profileReopeningResults.find((row) => row.turn === 6)?.passed !== true
    )) failures.push("turn_6_profile_reopening_invalid");
  ...
  return {
    audit_version: "e2a36-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a36-live.ts preflight|checkpoint|smoke|transport-retry-smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a36_runner_failed");
  process.exitCode = 1;
});
