...
import {
  writeFormativeConversationV18R1DispatchCheckpoint
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/dispatch-checkpoint";

const h = (value: string) => value.repeat(64);
const commit = "a".repeat(40);

async function main() {
  const root = await mkdtemp(path.join(tmpdir(), "fcv18r1-checkpoint-"));
  try {
    const identity = {
      provider_run_id: "fcv5v18r1_provider_test",
      derived_evaluation_id: "fcv5v18r1_derived_test",
    ...
    const created = await writeFormativeConversationV18R1DispatchCheckpoint({
      dispatch_root: root,
      identity
    });
