...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v14/candidate-runner";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v14/package";
import {
  writeFormativeConversationV5DispatchCheckpoint
} from "../src/lib/operational/formative-conversation-v5-evaluation-v14/service";
import {
...
async function main() {
  ...
  assert(
    v5Source.includes("safe_invalid_output_evidence") &&
      v5Source.includes(
        "formative-conversation-safe-opening-invalid-output-evidence-v1"
      ) &&
      v5Source.includes("provider_execution_audit"),
    "V14 must preserve safe invalid opening and semantic-regeneration evidence without raw provider envelopes."
  );
  assert(
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v14/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  assert(
    ...
    "The exact immutable Case 7 opening must pass opening validator v3."
  );
  ...
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v14_provider_smoke",
      derived_evaluation_id: "fcv5v14_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v14_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v14_derived_smoke_rerun",
  ...
  const v14Authorization =
    exactFormativeConversationV5LiveAuthorization(loaded);
  assert(
    loaded.runtime_candidate_hash !==
      loaded.candidate_manifest.frozen_v10_runtime_candidate_hash &&
      loaded.candidate_manifest.frozen_v12_runtime_candidate_hash ===
        loaded.candidate_manifest.frozen_v10_runtime_candidate_hash &&
      loaded.candidate_manifest.runtime_behavior_changed &&
      loaded.candidate_manifest.instructional_behavior_changed === false &&
      loaded.candidate_manifest.evaluation_governance_changed &&
      loaded.candidate_manifest.runtime_candidate_hash_unchanged === false &&
      loaded.candidate_identity.runtime_candidate_hash_unchanged === false &&
      loaded.protocol_hash !==
        loaded.protocol.failed_v5_execution.protocol_hash &&
      loaded.protocol_hash !==
      loaded.protocol.failed_v6_execution.protocol_hash &&
      loaded.protocol_hash !==
        loaded.protocol.failed_v7_execution.protocol_hash &&
      v14Authorization.includes(
        "formative-conversation-host-v5-executable-v14"
      ) &&
      v14Authorization.includes(loaded.runtime_candidate_hash) &&
      v14Authorization.includes(loaded.protocol_hash),
    "V14 must preserve model and prompt behavior while binding the corrected validation runtime and protocol identities."
  );
  assert(
  ...
  const v7FailureAnalysis = JSON.parse(
    readFileSync(
      FORMATIVE_CONVERSATION_V5_V7_FAILURE_ANALYSIS_PATH,
      "utf8"
    )
    ...
    transaction_trace: {
      provider_wait_inside_interactive_transaction: boolean;
      default_interactive_transaction_timeout_ms: number;
      nested_service_disconnect_detected: boolean;
      unawaited_persistence_detected: boolean;
    };
    case_findings: Array<{
      case_id: string;
      validated_recommendation?: string;
      provider_calls: number;
    }>;
    candidate_output_defects: unknown[];
    teaching_prompt_change_required: boolean;
    v7_approval_eligible: boolean;
    v7_activation_permitted: boolean;
    v7_rerunnable: boolean;
    provider_calls_during_analysis: number;
    model_auth_requests_during_analysis: number;
  };
  const v7HumanReview = JSON.parse(
    readFileSync(
      FORMATIVE_CONVERSATION_V5_V7_HUMAN_REVIEW_ADJUDICATION_PATH,
      "utf8"
    )
    ...
    source_runtime_candidate_hash: string;
    source_protocol_hash: string;
    student_visible_output_count: number;
    outputs: unknown[];
    aggregate: {
      conceptually_appropriate: boolean;
      tone_appropriate: boolean;
      adaptive_across_turns: boolean;
      excessive_praise_detected: boolean;
      unjustified_mastery_claim_detected: boolean;
      unsupported_markdown_detected: boolean;
      privacy_issue_detected: boolean;
      unadministered_answer_leakage_detected: boolean;
      prompt_change_required: boolean;
    };
    approval_eligible: boolean;
    activation_permitted: boolean;
  };
  const case5Fixture = loaded.fixtures.find(
    (fixture) =>
      fixture.case_id === "fcv5_05_sound_profile_transition"
  );
  assert(
    loaded.protocol.failed_v7_execution.frozen_commit ===
      "c0c05b60755ab1b9b293c8e12a2ac5645a952c17" &&
      loaded.protocol.failed_v7_execution.execution_status ===
        "completed_failed" &&
      loaded.protocol.failed_v7_execution.passed === 3 &&
      loaded.protocol.failed_v7_execution.failed === 5 &&
      loaded.protocol.failed_v7_execution.invalid === 0 &&
      loaded.protocol.failed_v7_execution.not_exercised === 0 &&
      !loaded.protocol.failed_v7_execution.approval_eligible &&
      !loaded.protocol.failed_v7_execution.activation_permitted &&
      !loaded.protocol.failed_v7_execution.rerunnable &&
      loaded.protocol.failed_v7_execution.preserved_immutable &&
      v7FailureAnalysis.source_commit ===
        loaded.protocol.failed_v7_execution.frozen_commit &&
      v7FailureAnalysis.source_runtime_candidate_hash ===
        loaded.protocol.failed_v7_execution.runtime_candidate_hash &&
      v7FailureAnalysis.source_protocol_hash ===
        loaded.protocol.failed_v7_execution.protocol_hash &&
      v7FailureAnalysis.provider_run_id ===
        loaded.protocol.failed_v7_execution.provider_run_id &&
      v7FailureAnalysis.derived_evaluation_id ===
        loaded.protocol.failed_v7_execution.derived_evaluation_id &&
      v7FailureAnalysis.source_status === "completed_failed" &&
      v7FailureAnalysis.source_result.passed === 3 &&
      v7FailureAnalysis.source_result.failed === 5 &&
      v7FailureAnalysis.source_result.logical_calls === 16 &&
      v7FailureAnalysis.source_result.provider_attempts === 16 &&
      !v7FailureAnalysis.transaction_trace
        .provider_wait_inside_interactive_transaction &&
      v7FailureAnalysis.transaction_trace
        .default_interactive_transaction_timeout_ms === 5000 &&
      !v7FailureAnalysis.transaction_trace
        .nested_service_disconnect_detected &&
      !v7FailureAnalysis.transaction_trace
        .unawaited_persistence_detected &&
      v7FailureAnalysis.case_findings.length === 5 &&
      v7FailureAnalysis.case_findings.find(
        (entry) =>
          entry.case_id === "fcv5_05_sound_profile_transition"
      ...
      v7FailureAnalysis.case_findings.find(
        (entry) =>
          entry.case_id === "fcv5_08_mixed_resolved_evidence"
      )?.validated_recommendation ===
        "largely_improved_understanding" &&
      v7FailureAnalysis.candidate_output_defects.length === 0 &&
      !v7FailureAnalysis.teaching_prompt_change_required &&
      !v7FailureAnalysis.v7_approval_eligible &&
      !v7FailureAnalysis.v7_activation_permitted &&
      !v7FailureAnalysis.v7_rerunnable &&
      v7FailureAnalysis.provider_calls_during_analysis === 0 &&
      v7FailureAnalysis.model_auth_requests_during_analysis === 0 &&
      v7HumanReview.source_commit ===
        loaded.protocol.failed_v7_execution.frozen_commit &&
      v7HumanReview.source_runtime_candidate_hash ===
        loaded.protocol.failed_v7_execution.runtime_candidate_hash &&
      v7HumanReview.source_protocol_hash ===
        loaded.protocol.failed_v7_execution.protocol_hash &&
      v7HumanReview.student_visible_output_count === 16 &&
      v7HumanReview.outputs.length === 16 &&
      v7HumanReview.aggregate.conceptually_appropriate &&
      v7HumanReview.aggregate.tone_appropriate &&
      v7HumanReview.aggregate.adaptive_across_turns &&
      !v7HumanReview.aggregate.excessive_praise_detected &&
      !v7HumanReview.aggregate.unjustified_mastery_claim_detected &&
      !v7HumanReview.aggregate.unsupported_markdown_detected &&
      !v7HumanReview.aggregate.privacy_issue_detected &&
      !v7HumanReview.aggregate
        .unadministered_answer_leakage_detected &&
      !v7HumanReview.aggregate.prompt_change_required &&
      !v7HumanReview.approval_eligible &&
      !v7HumanReview.activation_permitted &&
      case5Fixture?.permitted_terminal_outcomes.includes(
        "largely_improved_understanding"
      ) &&
      !case5Fixture.permitted_terminal_outcomes.includes(
        "sound_understanding"
      ),
    "The V7 run, failure classification, human review, and Case 5 oracle correction must remain immutable and approval-ineligible."
  );
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v14/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v14/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v14 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
    ...
    JSON.stringify(
      {
        ...
        v7_execution_evidence: "artifact_verified",
        v7_failure_analysis: "artifact_verified",
        v7_outputs_reviewed:
          v7HumanReview.student_visible_output_count,
        v7_teaching_prompt_change_required:
          v7HumanReview.aggregate.prompt_change_required,
        v7_case5_oracle:
          "largely_improved_with_unresolved_sem",
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ...
          : "formative_conversation_v5_v8_smoke_failed",
      provider_calls: 0,
