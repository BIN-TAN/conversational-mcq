...
const missingExpectedSha = spawnSync(
  process.execPath,
  ["--import", "tsx", launcher, "--mode=preflight"],
  { cwd: process.cwd(), encoding: "utf8", timeout: 120_000 }
);
assert.notEqual(missingExpectedSha.status, 0);
assert.match(missingExpectedSha.stderr, /expected_git_sha_missing/);

const malformedExpectedSha = spawnSync(
  process.execPath,
  [
    "--import",
    "tsx",
    launcher,
    "--mode=preflight",
    "--expected-deployed-git-sha=bad"
  ],
  { cwd: process.cwd(), encoding: "utf8", timeout: 120_000 }
);
assert.notEqual(malformedExpectedSha.status, 0);
assert.match(malformedExpectedSha.stderr, /expected_git_sha_malformed/);

const duplicateExpectedSha = spawnSync(
  process.execPath,
  [
    "--import",
    "tsx",
    launcher,
    "--mode=preflight",
    `--expected-deployed-git-sha=${"a".repeat(40)}`,
    `--expected-deployed-git-sha=${"a".repeat(40)}`
  ],
  { cwd: process.cwd(), encoding: "utf8", timeout: 120_000 }
);
assert.notEqual(duplicateExpectedSha.status, 0);
assert.match(duplicateExpectedSha.stderr, /expected_git_sha_duplicate/);

const conflictingExpectedSha = spawnSync(
  process.execPath,
  [
    "--import",
    "tsx",
    ...
    "--mode=preflight",
    `--expected-deployed-git-sha=${"a".repeat(40)}`,
    `--expected-deployed-git-sha=${"b".repeat(40)}`
  ],
  { cwd: process.cwd(), encoding: "utf8", timeout: 120_000 }
);
assert.notEqual(conflictingExpectedSha.status, 0);
assert.match(conflictingExpectedSha.stderr, /expected_git_sha_duplicate/);

const separatedExpectedSha = spawnSync(
  process.execPath,
  [
    "--import",
    "tsx",
...
assert.notEqual(separatedExpectedSha.status, 0);
assert.match(separatedExpectedSha.stderr, /expected_git_sha_malformed/);
...
console.log(
  JSON.stringify({
    ...
    missing_expected_sha_blocked_before_cli: true,
    malformed_expected_sha_blocked_before_cli: true,
    duplicate_expected_sha_blocked_before_cli: true,
    conflicting_expected_sha_blocked_before_cli: true,
    separated_expected_sha_blocked_before_cli: true,
    provider_calls: 0,
