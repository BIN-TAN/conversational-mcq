...
import {
  ...
  createFormativeConversationV18R1FinalizedManifest,
  ...
  type FormativeConversationV18R1ControlPayload
...
const safe = scanFormativeConversationV18R1BufferedOutputs({
...
assert.throws(
  ...
  /formative_conversation_v18r1_secret_match_detected/
);
assert.throws(
  () =>
    scanFormativeConversationV18R1BufferedOutputs({
...
assert.throws(
  ...
  /formative_conversation_v18r1_scan_coverage_mismatch/
);
assert.doesNotThrow(() =>
  assertFormativeConversationV18IntendedArtifactCoverage({
...
async function releaseFixture(zipContent: string) {
  const workspaceRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "fcv5-v18r1-security-"))
  );
  const nonce = randomUUID().replaceAll("-", "");
  const packageId = `fcv5v18r1_provider_security_${nonce}`;
  ...
  ownerOnlyDirectory(path.join(workspaceRoot, FORMATIVE_CONVERSATION_V18R1_DATA_ROOT));
  ...
  const finalized = await createFormativeConversationV18R1FinalizedManifest({
    staging_root: stagingRoot,
    package_id: packageId
  });
  const control: FormativeConversationV18R1ControlPayload = {
    schema_version: FORMATIVE_CONVERSATION_V18R1_CONTROL_SCHEMA_VERSION,
    record_type: "finalized_artifact_package",
    evaluation_revision: "formative-conversation-host-v5-executable-v18r1",
    control_nonce: nonce,
    mode: "live",
    staging_root: stagingRoot,
    ...
    derived_evaluation_id: `fcv5v18r1_derived_security_${nonce}`
  };
...
async function verifyPreventiveRelease() {
  ...
  try {
    const report = await releaseFormativeConversationV18R1Artifacts({
