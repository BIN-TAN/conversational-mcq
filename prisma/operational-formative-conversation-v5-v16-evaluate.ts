...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v16/service";
import {
...
import {
  ...
  assertFormativeConversationV16LiveControlEnvironment,
  writeFormativeConversationV16ControlPayload
...
function assertCanonicalLauncher(options: Options) {
  if (
    process.env
      ...
      "true" ||
    process.env.FORMATIVE_CONVERSATION_V5_V16_CANONICAL_LOADER_VERSION !==
      "formative-conversation-v5-canonical-node-import-tsx-v1"
  ) {
    throw new Error(
      "formative_conversation_v16_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    ...
    throw new Error(
      "formative_conversation_v16_canonical_outer_loader_required"
    );
  }
}
...
async function main(input: {
  ...
  const controlEnvironment =
    await assertFormativeConversationV16LiveControlEnvironment({
  ...
  await writeFormativeConversationV16ControlPayload({
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v16-evaluation-prisma",
