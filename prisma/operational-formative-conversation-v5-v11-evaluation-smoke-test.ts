...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v11/candidate-runner";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v11/contracts";
import {
  assertFormativeConversationV5CompilationParity
} from "../src/lib/operational/formative-conversation-v5-evaluation-v11/compiler";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v11/package";
import {
  writeFormativeConversationV5DispatchCheckpoint
} from "../src/lib/operational/formative-conversation-v5-evaluation-v11/service";
import {
...
async function main() {
  ...
  const v5Source = [
    ...
    "src/lib/operational/formative-conversation-v5-evaluation-v11/compiler.ts",
    "src/lib/operational/formative-conversation-v5-evaluation-v11/contracts.ts"
  ]
  ...
  assert(
    ...
    "V11 must preserve safe invalid opening and semantic-regeneration evidence without raw provider envelopes."
  );
  assert(
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v11/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v11_provider_smoke",
      derived_evaluation_id: "fcv5v11_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v11_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v11_derived_smoke_rerun",
  ...
  assert(
    loaded.runtime_candidate_hash ===
      ...
      v8Authorization.includes(
        "formative-conversation-host-v5-executable-v11"
      ) &&
      v8Authorization.includes(loaded.runtime_candidate_hash) &&
      v8Authorization.includes(loaded.protocol_hash),
    "v11 must preserve the frozen V10 runtime while using a new protocol identity for evaluation-governance changes."
  );
  assert(
  ...
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v11/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v11/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v11 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
