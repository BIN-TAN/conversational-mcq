...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v9/candidate-runner";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v9/contracts";
import {
  assertFormativeConversationV5CompilationParity
} from "../src/lib/operational/formative-conversation-v5-evaluation-v9/compiler";
import {
...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v9/package";
import {
  writeFormativeConversationV5DispatchCheckpoint
} from "../src/lib/operational/formative-conversation-v5-evaluation-v9/service";
import {
...
async function main() {
  ...
  const v5Source = [
    "src/lib/operational/formative-conversation-v5-evaluation-v9/service.ts",
    "src/lib/operational/formative-conversation-v5-evaluation-v9/compiler.ts",
    "src/lib/operational/formative-conversation-v5-evaluation-v9/contracts.ts"
  ]
  ...
  const openingRegression = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v9/regressions/case7-opening-output.json",
      "utf8"
    )
  ...
  try {
    ...
    await writeFormativeConversationV5DispatchCheckpoint({
      ...
      provider_run_id: "fcv5v9_provider_smoke",
      derived_evaluation_id: "fcv5v9_derived_smoke",
    ...
    await assertRejects(
      () =>
        writeFormativeConversationV5DispatchCheckpoint({
          dispatch_root: tempRoot,
          provider_run_id: "fcv5v9_provider_smoke_rerun",
          derived_evaluation_id: "fcv5v9_derived_smoke_rerun",
  ...
  assert(
    loaded.runtime_candidate_hash !==
      ...
      v8Authorization.includes(
        "formative-conversation-host-v5-executable-v9"
      ) &&
      v8Authorization.includes(loaded.runtime_candidate_hash) &&
      v8Authorization.includes(loaded.protocol_hash),
    "v9 must use fresh runtime and protocol identities while preserving the failed v5, v6, and v7 references."
  );
  assert(
  ...
  const failureAnalysis = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v9/v3-failure-analysis.json",
      "utf8"
    )
  ) as {
  ...
  const reviewAdvisory = JSON.parse(
    readFileSync(
      "config/operational-candidates/formative-conversation-host-v5-executable-v9/v3-human-review-advisory.json",
      "utf8"
    )
  ) as {
    output_count: number;
  ...
  assert(
    ...
    "v9 must remain inactive, approval-ineligible, and activation-forbidden."
  );
  assert(
    plan.mode === "plan" &&
