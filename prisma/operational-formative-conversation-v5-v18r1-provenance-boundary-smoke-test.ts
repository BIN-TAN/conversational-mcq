...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r1/local-committed-source";
...
const root = mkdtempSync(path.join(os.tmpdir(), "fcv18r1-local-provenance-"));
try {
  mkdirSync(path.join(root, "freeze"));
  writeFileSync(path.join(root, "freeze", "identity.json"), "{}\n");
  git(root, ["init", "--quiet", "--initial-branch=main"]);
  git(root, ["config", "user.email", "v18r1@example.invalid"]);
  git(root, ["config", "user.name", "V18R1 Provenance"]);
  git(root, ["add", "freeze"]);
  git(root, ["commit", "--quiet", "-m", "freeze"]);
  git(root, ["remote", "add", "origin", root]);
  git(root, ["update-ref", "refs/remotes/origin/main", "HEAD"]);
  const clean = verifyFormativeConversationV18R1LocalCommittedSource({
    workspace_root: root,
    source_paths: ["freeze"]
  });
