...
const VERSION = "e2a27-geometrical-optics-anchor-consistency-live-v1";
const E2A26A_RUN = "e2a26a_20260722035845_e8280a5f";
const E2A26A_ROOT = path.join(
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a27-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a27-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.27 evaluation-only constraints:",
...
function frozenArtifactPath(name: string) {
  return path.join(E2A26A_ROOT, name);
}

function buildE2A27FrozenProtocol(): Protocol {
  const protocol = readJson<Protocol>(
    frozenArtifactPath("e2a27-frozen-protocol.json")
  );
  if (protocol.protocol_hash !== PROTOCOL_HASH ||
      protocol.session_count !== 1 ||
      protocol.session.academic_domain !== "geometrical_optics") {
    throw new Error("e2a27_frozen_protocol_invalid");
  }
  const expectedFileSha = readFileSync(
    frozenArtifactPath("e2a27-frozen-protocol.sha256"), "utf8"
  ).trim();
  const actualFileSha = sha256(readFileSync(
    frozenArtifactPath("e2a27-frozen-protocol.json")
  ));
  if (expectedFileSha !== actualFileSha) {
    throw new Error("e2a27_frozen_protocol_file_sha_mismatch");
  }
  return protocol;
}
...
function frozenArtifactContract() {
  return readJson<JsonObject>(
    frozenArtifactPath("e2a27-artifact-contract.json")
  );
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a27_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a27_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a27_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a27_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a27_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a27_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a27_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a27_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a27.ts",
...
function protectedEvidenceIdentity() {
  ...
  const dataRoots = readdirSync(dataRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() &&
      /^e2a(?:1[2-9]|2[0-6])a?(?:\D|$)/u.test(entry.name))
  ...
  return {
    snapshot_version: "e2a27-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const anchorSource =
    "src/lib/services/student-assessment/anchor-conclusion-consistency.ts";
  const identity = {
    identity_version: "e2a27-composite-runtime-identity-v1",
    ...
    evidence_evaluator_v3_source_hash: sourceHash(v3Source),
    evidence_evaluator_version:
      PRODUCTION_TURN_EVIDENCE_EVALUATOR_VERSION_V3,
    target_evidence_contract_implementation_hash: sourceHash(v3Source),
    profile_mapper_v3_source_hash: sourceHash(v3Source),
    profile_mapper_version: TURN_EVIDENCE_PROFILE_MAPPER_VERSION_V3,
    profile_consistency_v3_source_hash: sourceHash(v3Source),
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a27-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A27FrozenProtocol();
  const e2a26aSummary = readJson<{ status?: string; provider_call_count?: number;
    network_request_count?: number; protected_evidence_unchanged?: boolean }>(
    path.join(E2A26A_ROOT, "summary.json")
  );
  const overlap = readJson<{ passed?: boolean; protocol_hash?: string }>(
    frozenArtifactPath("e2a27-held-out-overlap-analysis.json")
  );
  const frozenBudget = readJson<{ arithmetic_valid?: boolean;
    maximum?: typeof BUDGET }>(frozenArtifactPath("e2a27-budget.json"));
  ...
  if (e2a26aSummary.status !==
      "e2a26a_anchor_contradiction_false_sound_corrected_e2a27_ready" ||
      e2a26aSummary.provider_call_count !== 0 ||
      e2a26aSummary.network_request_count !== 0 ||
      e2a26aSummary.protected_evidence_unchanged !== true) {
    blockers.push("e2a26a_authoritative_artifacts_invalid");
  }
  if (protocol.protocol_hash !== PROTOCOL_HASH) {
    blockers.push("protocol_hash_mismatch");
  }
  if (overlap.passed !== true || overlap.protocol_hash !== PROTOCOL_HASH) {
    blockers.push("held_out_overlap_invalid");
  }
  if (artifactContract.protocol_hash !== PROTOCOL_HASH ||
      artifactContract.failure_path_artifacts_required !== true ||
      artifactContract.provider_generated_but_not_displayed_output_must_be_retained !==
        true || artifactContract.secrets_and_raw_provider_payloads_prohibited !==
        true) {
  ...
  if (frozenBudget.arithmetic_valid !== true ||
      stableHash(frozenBudget.maximum) !==
  ...
  if (pids("[f]ormative-evaluation-e2a27").length > 0) {
    blockers.push("duplicate_e2a27_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a27_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a27_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A27 !== "1") blockers.push("live_opt_in_missing");
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a27_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a27_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a27_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a27_confirmation_mismatch:${name}`);
  }
}
...
function assertLiveAuthorizationArguments() {
  for (const flag of [
    ...
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a27_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a27_dispatch_checkpoint_missing");
...
function contractFor(session: Session): TargetEvidenceContractV3 {
  ...
  return TargetEvidenceContractV3Schema.parse({
    ...
    anchor_resolution_criteria: [
      "Apply the optical mechanism to option B and coherently reject the claim that a converging lens always forms a real image."
    ],
    anchor_contradiction_criteria: [
      "Endorsing option B conflicts with an explanation that an object inside the focal length produces a virtual image."
    ],
    ...
    contradiction_criteria: [{
      ...
      description:
        "The stated conclusion about option B conflicts with the conceptual explanation.",
      observable_patterns: []
    }]
  });
}

function plannedMessage(turn: PlannedTurn) {
  return "message" in turn && typeof turn.message === "string"
    ? turn.message : null;
}
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a27_session_${session.session_id}`,
    scenario_version: "e2a27-frozen-student-trajectory-v1",
    ...
    planned_evidence_ceiling: plannedMessage(turn),
...
function validateSimulatorOutput(input: {
  ...
  const planned = plannedMessage(input.turn);
  const plannedIncludesAnchor = planned !== null && new RegExp(
    `\\boption\\s+${input.session.target_evidence_contract.distractor_option}\\b`,
    "iu"
  ).test(planned);
  if (plannedIncludesAnchor) {
    if (!new RegExp(
          `\\boption\\s+${input.session.target_evidence_contract.distractor_option}\\b`,
          "iu"
...
function evaluatorProviderInput(
  ...
  return {
    ...
    case_id: `e2a27_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a27_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a27_${session.session_id}`,
    ...
    source_activity_context: {
      ...
      profile_condition: "synthetic_frozen_e2a27_trajectory",
      distractor_role: "selected_distractor",
      safe_activity_prompt:
        `A student claims that a converging lens always forms a real image because it bends rays inward. Explain whether that claim works for every object position.`,
    },
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a27_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a27_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a27",
    source_diagnostic_purpose: "reasoning_boundary_repair",
    ...
    evidence_flags: {
      ...
      student_explained_target_boundary:
        sound || contradictory ? "yes" : partial ? "partial" : "no",
...
function noLiveTutorOutput(input: AutonomousPedagogyInput) {
  ...
  const strategies = [
    "contrast object positions on opposite sides of the focal boundary",
    "trace what the outgoing rays do before using the image label",
    "ask for a one-sentence causal link tied to the active option",
    "separate lens type from the object's position",
    "reconcile the stated option conclusion with the ray behavior"
  ];
  const studentTasks = [
    ...
    "Put the ray behavior and your final judgment side by side. Do they support the same conclusion? Explain which part you would keep or change."
  ];
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return async <TInput, TOutput>(
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a27_no_live_simulator_case_missing");
      const input = request.input as ReturnType<typeof simulatorInput>;
      const message = plannedMessage(turn) ??
        input.latest_assistant_message.split(/(?<=[.!?])\s+/u)[0] ??
      ...
      if (!session || !turn) throw new Error("e2a27_no_live_evaluator_case_missing");
      output = noLivePacket(
        session, turn,
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        throw new Error(`e2a27_no_live_tutor_fixture_invalid:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`);
      }
    } else {
      throw new Error(`e2a27_no_live_unknown_agent:${request.agent_name}`);
    }
    return {
      provider: "mock",
...
class SessionStore {
  ...
  constructor(readonly runId: string, readonly session: Session) {
    ...
    this.turns = [{
      ...
      actor_type: "agent",
      message_text:
        "A student says, \"A converging lens always forms a real image because it bends rays inward.\" Think about the object's position and what the rays do after the lens. Do you agree with that claim? Explain your reasoning.",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a27_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a27_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a27_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a27_composite_identity_changed_before_dispatch");
    }
  }
  ...
  if (result.status !== "completed" || !result.parsed_output) {
    ...
    throw new Error(`e2a27_provider_call_failed:${input.role}:${reason}`);
  }
  return result;
}
...
async function runSession(input: {
  ...
  let priorAnchorResolution: TargetEvidenceAdjudicationV3[
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      if (endpoint) {
        ...
        throw new Error("e2a27_unnecessary_turn_after_endpoint");
      }
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation.passed) {
        throw new Error(
          `e2a27_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation.issues.join("|")}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a27_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a27_evaluator_hard_rejection:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a27_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a27_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          priorAnchorResolution =
            adjudication.anchor_interpretation.anchor_resolution_status;
          return adjudication;
        },
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a27_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a27_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a27_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a27_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a27_idempotency_replay_failed");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error("e2a27_anchor_stance_misclassified");
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a27_anchor_contradiction_not_structured");
      }
      if (turn.required_contradiction &&
          (!anchor.contradictions.includes(turn.required_contradiction) ||
           !result.latest_profile.contradictions.includes(
             turn.required_contradiction
           ))) {
        throw new Error("e2a27_anchor_contradiction_not_structured");
      }
      if (anchor.blocking_limitations.length > 0 &&
          !result.latest_profile.contradictions.includes(
            "anchor_conclusion_conceptual_explanation_conflict"
          )) {
        throw new Error("e2a27_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
      ...
      )) throw new Error("e2a27_anchor_interpretation_failure");
      ...
      )) throw new Error("e2a27_genuine_sound_false_negative");
      if (!humanProfileMatches(
        ...
        throw new Error(
          expectedProfile(turn).includes("sound")
            ? "e2a27_genuine_sound_false_negative"
            : result.latest_profile.reasoning_quality === "sound"
              ? "e2a27_genuine_false_sound"
              : `e2a27_profile_semantically_outside_allowed_envelope:turn_${turn.turn}:${result.latest_profile.reasoning_quality}:${turn.semantic_envelope.join("|")}`
        );
      }
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a27_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a27_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a27_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
      ...
      appendJsonl(path.join(input.runDir, "profile-consistency-results.jsonl"), {
        ...
        blocking_conflict_promoted: anchor.blocking_limitations.length === 0 ||
          result.latest_profile.contradictions.includes(
            "anchor_conclusion_conceptual_explanation_conflict"
          ),
        passed: true
      ...
      input.reviewRows.push({
        ...
        structured_contradictions: result.latest_profile.contradictions,
        revision_readiness: result.latest_profile.revision_readiness,
    ...
    if (!endpoint) throw new Error("e2a27_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a27_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a27_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a27-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
async function executeCanary(input: {
  ...
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a27-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a27-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  writeJson(path.join(runDir, "frozen-protocol.json"), protocol);
  writeJson(path.join(runDir, "frozen-protocol.sha256"), {
    protocol_hash: PROTOCOL_HASH,
    source_file_sha256: sha256(readFileSync(
      frozenArtifactPath("e2a27-frozen-protocol.json")
    )),
    verified: protocol.protocol_hash === PROTOCOL_HASH
  });
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a27-geometrical-optics-isolated-fixture-v1",
    synthetic_only: true,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a27_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a27-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a27-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a27-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a27-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a27-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a27-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a27-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a27_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a27_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a27-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A27FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a27_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a27_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a27_no_live_network_detected");
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a27_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a27_confirmation_missing:")) {
    throw new Error("e2a27_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a27_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a27_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a27_run_not_found");
...
function auditRun(run?: string) {
  ...
  return {
    audit_version: "e2a27-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a27.ts preflight|checkpoint|smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a27_runner_failed");
  process.exitCode = 1;
});
