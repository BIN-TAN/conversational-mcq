...
function installIsolatedEnvironment() {
  ...
  assert(
    ...
    "The V17 runtime smoke must use local PostgreSQL."
  );
  ...
  try {
    for (const childTest of childTests) {
      ...
      assert.equal(
        child.status,
        0,
        child.error?.message ?? `The database smoke ${childTest} failed.`
      );
    }
  } finally {
    await dropLiveCanaryDatabaseIfPresent();
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v17_runtime_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0,
      dispatch_checkpoints: 0
    })
