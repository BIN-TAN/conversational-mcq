...
import {
  executeE2A12Canary,
  analyzeE2A12ProtocolOverlap,
  cleanupE2A12Fixture,
  createE2A12Fixture,
  inspectE2A12Preflight,
  resolveE2A12Budget,
  temporaryE2A12ArtifactRoot,
  validateE2A12Artifacts,
  validateE2A12HeldOutProtocol
} from "@/lib/evaluation/formative/e2a12-v8-runtime-canary";
import {
  E2A12_PROTOCOL_HASH,
  assertE2A12ProtocolFrozen,
  e2a12HeldOutCases
} from "@/lib/evaluation/formative/e2a12-v8-held-out-protocol";
import {
...
class DeterministicNoLiveProvider implements LlmProvider {
  ...
  async executeStructured<TInput, TOutput>(
    ...
    this.callCount += 1;
    const caseId = e2a12HeldOutCases().find((entry) =>
      request.client_request_id.includes(entry.case_id)
    );
    if (!caseId) throw new Error("e2a12_mock_case_not_found");
...
function contextFor(testCase: ReturnType<typeof e2a12HeldOutCases>[number]) {
  return {
...
async function runRuntimeScenarioAssertions(suite: string) {
  ...
  const cases = e2a12HeldOutCases().slice(0, 4);
  const prisma = new PrismaClient();
  const runId = `smoke_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`;
  const fixture = await createE2A12Fixture(prisma, runId, cases);
  ...
  try {
    if (shouldRun("accepted")) {
      ...
      const output = {
        ...
        student_facing_message:
          "For Item 4 option B, explain how a smaller standard error narrows uncertainty without making the observed score exact."
      };
    ...
    if (shouldRun("flagged")) {
      ...
      const output = {
        ...
        student_facing_message:
          "Think about it: Item 6 option D concerns difficulty and discrimination. What response pattern separates those two?"
      };
      const execution = await executeTopicDialogueCandidateRuntimeV2({
        prisma,
        assessment_session_db_id:
    ...
    if (shouldRun("regeneration")) {
      ...
      const accepted = {
        schema_version:
          TOPIC_DIALOGUE_OPERATION_OUTPUT_SCHEMA_VERSIONS.clarify_task,
        student_facing_message:
          "For Item 8 option A, identify the claim about content coverage, then state what evidence would evaluate that claim."
      };
      const execution = await executeTopicDialogueCandidateRuntimeV2({
        prisma,
        assessment_session_db_id:
...
async function runArtifactAssertions() {
  const root = temporaryE2A12ArtifactRoot();
  const provider = new DeterministicNoLiveProvider();
  try {
    const canary = await executeE2A12Canary({
      live: false,
      provider,
      artifactRoot: root,
      runId: "e2a12_no_live_artifact_smoke"
    });
    const validation = validateE2A12Artifacts(canary.runDir);
    assert(validation.passed, "emitted E2A12 artifacts should validate");
    assert(provider.callCount === 12,
      "no-live artifact run should dispatch exactly 12 injected calls");
    assert(canary.summary.provider_usage.generation_provider_calls === 12,
      "no-live artifact run should report 12 generation calls");
    assert(canary.summary.initial_schema_valid_count === 12,
      "all injected no-live outputs should satisfy strict schemas");
    assert(canary.summary.fallback_count === 0,
      "accepted injected outputs should not use deterministic fallback");
    assert(canary.summary.final_status ===
...
async function main() {
  ...
  assert(known.has(suite), `unknown_e2a12_suite:${suite}`);
  if (["all", "protocol"].includes(suite)) {
    ...
    assert(assertE2A12ProtocolFrozen() === E2A12_PROTOCOL_HASH,
  ...
  if (["all", "shadow"].includes(suite)) {
    const testCase = e2a12HeldOutCases()[1]!;
    const output = {
      schema_version:
        TOPIC_DIALOGUE_OPERATION_OUTPUT_SCHEMA_VERSIONS
          .clarify_concept_with_new_strategy,
  ...
  if (["all", "budget"].includes(suite)) {
    ...
    assert(budget.maximum_cases === 12 &&
      budget.maximum_total_generation_calls === 24 &&
      budget.maximum_input_tokens === 350000 &&
      budget.maximum_output_tokens === 60000 &&
      budget.maximum_estimated_cost_usd === 15,
    "E2A12 hard budgets should match the bounded protocol");
    let failed = false;
    try {
      resolveE2A12Budget({ EVAL_E2A12_MAX_TOTAL_CALLS: "25" });
    } catch {
      failed = true;
    }
  ...
  if (["all", "resource"].includes(suite)) {
    const protocol = validateE2A12HeldOutProtocol();
    assert(protocol.checks.case_count_12,
      "resource policy should be bounded to 12 cases");
    assert(resolveE2A12Budget().provider_case_concurrency === 1,
      "provider concurrency must remain one");
  }
  if (["all", "compilation"].includes(suite)) {
    const root = mkdtempSync(path.join(os.tmpdir(), "e2a12-compile-"));
    try {
  ...
  if (["all", "preflight"].includes(suite)) {
    ...
    assert(preflight.passed,
      `no-live E2A12 preflight failed:${preflight.blockers.join(",")}`);
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a12_smoke_failed");
  process.exitCode = 1;
});
