...
const identity = {
  provider_run_id: "fcv5v15_provider_test",
  derived_evaluation_id: "fcv5v15_derived_test",
...
async function main() {
const root = await mkdtemp(path.join(tmpdir(), "fcv15-dispatch-"));
try {
  const first = await writeFormativeConversationV15DispatchCheckpoint({
    dispatch_root: root,
    identity
  });
