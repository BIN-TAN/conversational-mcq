...
const secret = "v16-deterministic-secret-value-123456789";
const safe = scanFormativeConversationV16BufferedOutputs({
...
assert.throws(
  ...
  /formative_conversation_v16_secret_match_detected/
);
assert.throws(
  () =>
    scanFormativeConversationV16BufferedOutputs({
...
assert.throws(
  ...
  /formative_conversation_v16_scan_coverage_mismatch/
);
console.log(
  JSON.stringify({
    status: "passed",
