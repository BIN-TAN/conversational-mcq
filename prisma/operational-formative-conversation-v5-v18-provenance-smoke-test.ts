...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18/provenance";
...
function createCommittedFixture(options?: { include_identity?: boolean }) {
  const root = mkdtempSync(
    path.join(os.tmpdir(), "formative-conversation-v18-provenance-")
  );
  ...
  git(root, ["init", "--quiet"]);
  git(root, ["config", "user.email", "v18-provenance@example.invalid"]);
  git(root, ["config", "user.name", "V18 Provenance Smoke"]);
  git(root, ["add", "freeze"]);
...
function verifyCleanCommittedPackage(input: {
  ...
  const verified = verifyFormativeConversationV18CommittedSource(input);
...
function assertReproducibleMaterialization(before: string, after: string) {
  if (before !== after) {
    throw new Error(
      "formative_conversation_v18_materialization_not_reproducible"
    );
  }
}
...
try {
  ...
  assert.ok(FORMATIVE_CONVERSATION_V18_COMMITTED_SOURCE_PATHS.length >= 8);
  ...
  expectProvenanceFailure(
    ...
    /formative_conversation_v18_tracked_source_changed/
  );
  ...
  expectProvenanceFailure(
    ...
    /formative_conversation_v18_tracked_package_missing/
  );
