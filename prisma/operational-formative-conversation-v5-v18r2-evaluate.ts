...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r2/service";
import {
...
function assertCanonicalLauncher(options: Options) {
  if (
    ...
    throw new Error(
      "formative_conversation_v18r2_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    ...
    throw new Error(
      "formative_conversation_v18r2_canonical_outer_loader_required"
    );
  }
}
...
async function main(input: {
  ...
  const controlEnvironment =
    await assertFormativeConversationV18R2LiveControlEnvironment({
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v18r2-evaluation-prisma",
