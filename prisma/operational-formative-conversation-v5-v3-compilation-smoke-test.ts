...
async function main() {
  ...
  if (!originalKey?.trim()) {
    process.env.RESEARCH_PSEUDONYMIZATION_KEY =
      "formative-conversation-v5-v3-no-provider-smoke-key";
  }
  globalThis.fetch = (async (input: RequestInfo | URL) => {
    ...
    if (/api\.openai\.com/iu.test(url)) {
      ...
      throw new Error(
        "formative_conversation_v5_v3_provider_call_forbidden"
      );
    }
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ...
          : "formative_conversation_v5_v3_compilation_smoke_failed",
      provider_calls: 0,
