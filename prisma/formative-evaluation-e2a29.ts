...
const VERSION = "e2a29-electrical-circuits-anchor-contradiction-live-v1";
const E2A28A_RUN = "e2a28a_20260722100440_2157dea0";
const E2A28A_ROOT = path.join(
...
const LOCK_PATH = path.join(
  process.cwd(), ".data", "locks", "e2a29-live-canary.lock"
);
const CHECKPOINT_PATH = path.join(
  ARTIFACT_ROOT, "e2a29-dispatch-checkpoint.json"
);
const PROTOCOL_HASH =
  "369535b52c909eb8bc3875d3a0cb6a97afb0acfa0dd1fb902eae9493e12d3a9b";
const ARTIFACT_CONTRACT_HASH =
  "08b63748218e7581eed77eb6dee37ced941c1440d2b7880b50c5caed88e9d887";
...
const SIMULATOR_INSTRUCTIONS = [
  ...
  "E2A.29 evaluation-only constraints:",
...
function frozenArtifactPath(name: string) {
  return path.join(E2A28A_ROOT, name);
}

function buildE2A29FrozenProtocol(): Protocol {
  const sourcePath = frozenArtifactPath("e2a29-frozen-protocol.json");
  const frozen = readJson<FrozenProtocolArtifact>(sourcePath);
  const expectedFileSha = readFileSync(
    frozenArtifactPath("e2a29-frozen-protocol.sha256"), "utf8"
  ).trim();
  const { protocol_hash: embeddedHash, ...frozenBody } = frozen;
  const actualProtocolHash = stableHash(frozenBody);
  if (expectedFileSha !== PROTOCOL_HASH ||
      actualProtocolHash !== PROTOCOL_HASH ||
      embeddedHash !== PROTOCOL_HASH ||
      frozen.protocol_version !==
        "e2a29-electrical-circuits-anchor-contradiction-canary-v1" ||
      frozen.execution_authorized !== false ||
      frozen.live_execution_performed !== false ||
      frozen.session_count !== 1 ||
      frozen.domain !== "introductory_electrical_circuits" ||
      frozen.active_distractor.option_label !== "C" ||
    ...
    throw new Error("e2a29_frozen_protocol_file_sha_mismatch");
  }
  ...
  return {
    ...
    session_count: 1,
    session: {
      session_id: "E2A29-CIRCUITS",
      design: "held_out_electrical_circuits_anchor_contradiction",
      academic_domain: frozen.domain,
      concept: frozen.concept,
      target_evidence_contract: {
        item_id: "series_circuit_current_item_1",
        distractor_option: frozen.active_distractor.option_label,
        distractor_claim: frozen.active_distractor.claim,
        required_relationship:
          "In a simple steady-state series circuit, the same current enters and leaves each component.",
        required_mechanism:
          "Charge flows continuously around the series circuit while the lamp transfers electrical energy to light and thermal energy; it does not consume current.",
      ...
      student_profile: {
        ...
        trajectory:
          "current-consumption misconception to conservation reasoning with a deliberate mechanism/conclusion contradiction"
      },
      frozen_student_trajectory: [
        turn(1, {
          no_live_fixture_message:
            "The bulb uses some of the current, so less should come out after it. I would choose option C.",
          semantic_envelope: ["misconception"], tutor_expected: true
        }),
        turn(2, {
          no_live_fixture_message:
            "The current goes all the way around the loop, so I think that is what happens.",
        ...
        turn(3, {
          no_live_fixture_message:
            "Maybe the charges keep moving around the whole circuit, but the bulb still takes some current away.",
          semantic_envelope: ["misconception", "partial"], tutor_expected: true
        }),
        turn(4, {
          no_live_fixture_message:
            "The same current enters and leaves the bulb because charge is conserved, and the bulb transfers electrical energy into light and heat. Option C still seems right because the bulb uses up current.",
          semantic_envelope: ["misconception", "partial"], tutor_expected: true,
          simulator_instruction:
            "Explain current conservation and energy transfer accurately, then end with the explicit conclusion that option C still seems right.",
          required_anchor_application: "explicit",
          ...
          required_tutor_goal:
            "reconcile current conservation and energy transfer with the explicit endorsement of option C"
        }),
        turn(5, {
          no_live_fixture_message:
            "So is the bulb using energy rather than using current?",
          semantic_envelope: ["insufficient", "partial"], tutor_expected: true
        }),
        turn(6, {
          no_live_fixture_message:
            "The same current enters and leaves the bulb in the series circuit. The bulb transfers electrical energy to light and heat, but it does not use up charge, so I reject option C.",
          semantic_envelope: ["sound"], tutor_expected: false,
          simulator_instruction:
            "Independently explain why current is conserved while energy is transferred, and explicitly reject option C.",
          required_anchor_application: "explicit",
          required_anchor_stance: "rejects_distractor",
          required_anchor_consistency:
            "consistent_with_conceptual_reasoning"
...
function frozenArtifactContract() {
  return readJson<JsonObject>(
    frozenArtifactPath("e2a29-artifact-contract.json")
  );
}
...
function safe(value: unknown) {
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    throw new Error("e2a29_artifact_secret_or_private_reasoning_detected");
  }
}
...
function initializeRun(runId: string, root = ARTIFACT_ROOT) {
  ...
  if (existsSync(runDir)) throw new Error("e2a29_run_directory_exists");
...
function runId() {
  const stamp = new Date().toISOString().replace(/[-:.TZ]/gu, "")
    .slice(0, 14);
  return `e2a29_${stamp}_${randomBytes(4).toString("hex")}`;
}
...
function assertBudgetBeforeCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (next[key as keyof typeof next] > limit) {
      throw new Error(`e2a29_pre_call_budget_block:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd >= BUDGET.cost_usd) {
    throw new Error("e2a29_pre_call_budget_block:cost_usd");
  }
}
...
function recordCall(
  ...
  for (const [key, limit] of Object.entries({
    ...
    if (actual[key as keyof typeof actual] > limit) {
      throw new Error(`e2a29_actual_budget_exceeded:${key}`);
    }
  }
  if (ledger.pricing_complete && ledger.estimated_cost_usd > BUDGET.cost_usd) {
    throw new Error("e2a29_actual_budget_exceeded:cost_usd");
  }
}
...
function existingLiveRuns() {
  ...
  return readdirSync(ARTIFACT_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("e2a29_"))
...
function sourceIdentity() {
  const files = [
    "prisma/formative-evaluation-e2a29.ts",
...
function protectedEvidenceIdentity() {
  ...
  const dataRoots = readdirSync(dataRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() &&
      /^e2a(?:1[2-9]|2[0-8])a?(?:\D|$)/u.test(entry.name))
  ...
  return {
    snapshot_version: "e2a29-protected-evidence-snapshot-v1",
    trees,
...
function compositeRuntimeIdentity() {
  ...
  const anchorSource =
    "src/lib/services/student-assessment/anchor-conclusion-consistency.ts";
  const identity = {
    identity_version: "e2a29-composite-runtime-identity-v1",
    ...
    active_anchor_alias_resolver_version:
      ACTIVE_ANCHOR_ALIAS_RESOLUTION_VERSION,
    profile_mapper_v5_source_hash: sourceHash(v5Source),
    profile_mapper_version: TURN_EVIDENCE_PROFILE_MAPPER_VERSION_V5,
    profile_consistency_v5_source_hash: sourceHash(v5Source),
    ...
    pre_tutor_finalization_version: PRE_TUTOR_PROFILE_FINALIZATION_VERSION_V2,
...
type DispatchCheckpoint = {
  checkpoint_version: "e2a29-dispatch-checkpoint-v1";
  recorded_at: string;
...
function preflight(requireLive: boolean, requireCheckpoint = requireLive) {
  ...
  const protocol = buildE2A29FrozenProtocol();
  const e2a28aSummary = readJson<{ status?: string;
    provider_calls_made?: number; network_requests_made?: number;
    protected_evidence_unchanged?: boolean;
    candidate_integrity_passed?: boolean; e2a29_executed?: boolean;
    e2a29_protocol_hash?: string; e2a29_artifact_contract_hash?: string }>(
    path.join(E2A28A_ROOT, "summary.json")
  );
  const overlap = readJson<{ passed?: boolean; exact_match_count?: number;
    normalized_match_count?: number; structural_template_match_count?: number;
    deterministic_semantic_overlap_passed?: boolean }>(
    frozenArtifactPath("e2a29-held-out-overlap-analysis.json")
  );
  const frozenBudget = readJson<{ execution_authorized?: boolean;
    maximum?: Record<string, number> }>(frozenArtifactPath("e2a29-budget.json"));
  ...
  if (e2a28aSummary.status !==
      "e2a28a_live_evaluator_wiring_corrected_e2a29_ready" ||
      e2a28aSummary.provider_calls_made !== 0 ||
      e2a28aSummary.network_requests_made !== 0 ||
      e2a28aSummary.protected_evidence_unchanged !== true ||
      e2a28aSummary.candidate_integrity_passed !== true ||
      e2a28aSummary.e2a29_executed !== false ||
      e2a28aSummary.e2a29_protocol_hash !== PROTOCOL_HASH ||
      e2a28aSummary.e2a29_artifact_contract_hash !==
        ARTIFACT_CONTRACT_HASH) {
    blockers.push("e2a28a_authoritative_artifacts_invalid");
  }
  if (protocol.protocol_hash !== PROTOCOL_HASH) {
    blockers.push("protocol_hash_mismatch");
  }
  if (overlap.passed !== true || overlap.exact_match_count !== 0 ||
      overlap.normalized_match_count !== 0 ||
      overlap.structural_template_match_count !== 0 ||
      overlap.deterministic_semantic_overlap_passed !== true) {
    blockers.push("held_out_overlap_invalid");
  }
  if (artifactContract.artifact_contract_version !==
      "e2a29-complete-turn-evidence-contract-v1" ||
      artifactContract.execution_authorized !== false ||
      !Array.isArray(artifactContract.every_attempted_student_turn_requires) ||
      artifactContract.failure_path_must_be_complete !== true ||
      artifactContract.no_chain_of_thought !== true ||
      artifactContract.no_secrets_or_environment_values !== true ||
      artifactContract.artifact_contract_hash !== ARTIFACT_CONTRACT_HASH) {
  ...
  if (pids("[f]ormative-evaluation-e2a29").length > 0) {
    blockers.push("duplicate_e2a29_process");
  }
  if (existsSync(LOCK_PATH)) blockers.push("e2a29_lock_present");
  const priorRuns = existingLiveRuns();
  if (priorRuns.length > 0) blockers.push(`prior_e2a29_run_exists:${priorRuns.at(-1)}`);
  let credential = null;
  const database = requireLive
    ? databaseReadiness() : { ready: null, code: "not_checked" };
  if (requireLive) {
    if (process.env.RUN_LIVE_E2A29 !== "1") blockers.push("live_opt_in_missing");
...
function recordDispatchCheckpoint() {
  if (existingLiveRuns().length > 0) {
    throw new Error("e2a29_prior_live_run_exists");
  }
  if (!trackedTreeClean()) throw new Error("e2a29_checkpoint_tree_not_clean");
  const check = preflight(false, false);
  if (!check.passed) {
    throw new Error(`e2a29_checkpoint_preflight_failed:${check.blockers.join(",")}`);
  }
  const identity = compositeRuntimeIdentity();
...
function requiredArgument(name: string, expected: string | number) {
  ...
  if (value !== String(expected)) {
    throw new Error(`e2a29_confirmation_mismatch:${name}`);
  }
}

function assertLiveAuthorizationArguments() {
  for (const flag of [
    "--confirm-e2a29-one-session-authorization",
    ...
    "--confirm-stop-after-e2a29"
  ]) {
    if (!process.argv.includes(flag)) {
      throw new Error(`e2a29_confirmation_missing:${flag}`);
    }
  }
  const checkpoint = readCheckpoint();
  if (!checkpoint) throw new Error("e2a29_dispatch_checkpoint_missing");
...
function contractFor(session: Session): TargetEvidenceContractV5 {
  ...
  return TargetEvidenceContractV5Schema.parse({
    ...
    anchor_resolution_criteria: [
      `Apply current conservation and energy transfer to option ${source.distractor_option} and coherently reject the current-consumption claim.`
    ],
    anchor_contradiction_criteria: [
      `Endorsing option ${source.distractor_option} conflicts with explaining that the same current enters and leaves the lamp while energy is transferred.`
    ],
...
function simulatorInput(
  ...
  return {
    scenario_id: `e2a29_session_${session.session_id}`,
    scenario_version: "e2a29-frozen-student-trajectory-v1",
...
function evaluatorProviderInput(
  ...
  const legacyEvaluatorInput = {
    ...
    case_id: `e2a29_${session.session_id}_${evaluatorInput.latest_student_message.source_sequence_index}`,
    session_public_id: evaluatorInput.complete_visible_formative_conversation.dialogue_public_id,
    student_public_id: `synthetic_student_e2a29_${session.session_id}`,
    assessment_public_id: `synthetic_assessment_e2a29_${session.session_id}`,
    ...
    source_activity_context: {
      ...
      profile_condition: "synthetic_frozen_e2a29_trajectory",
...
function auditFor(
  ...
  const usage = e2aUsageFor(result);
  return {
    agent_call_id: `e2a29_audit_${randomUUID()}`,
...
function noLivePacket(
  ...
  const fixture = buildNoLiveActivityMisconceptionEvidenceFixture({
    case_id: `e2a29_${session.session_id}_${turn.turn}`,
    activity_family: "reasoning_chain_repair",
    selected_formative_value: "reasoning_refinement",
    profile_condition: "frozen_e2a29",
    source_diagnostic_purpose: "reasoning_boundary_repair",
    ...
    response_summary: contradictory
      ? `${session.session_id} turn ${turn.turn} supplies an accurate current-conservation and energy-transfer mechanism but explicitly endorses the current-consumption claim, creating a mechanism/conclusion conflict.`
...
function noLiveTutorOutput(input: AutonomousPedagogyInput) {
  ...
  const strategies = [
    "contrast current conservation with electrical energy transfer",
    "trace charge flow before and after the lamp",
    "ask for a one-sentence conservation link tied to the active option",
    "separate charge flow from energy transfer",
    "reconcile the stated option conclusion with the series-circuit mechanism"
  ];
  const studentTasks = [
...
function makeNoLiveExecutor(protocol: Protocol): ProviderExecutor {
  return async <TInput, TOutput>(
    ...
    if (request.agent_name === "evaluation_llm_student_simulator") {
      if (!session || !turn) throw new Error("e2a29_no_live_simulator_case_missing");
      ...
      if (!session || !turn) throw new Error("e2a29_no_live_evaluator_case_missing");
      ...
      if (validation.runtime_acceptance === "hard_rejected") {
        throw new Error(`e2a29_no_live_tutor_fixture_invalid:${validation.hard_rejections.map((entry) => entry.rule_code).join("|")}`);
      }
    } else {
      throw new Error(`e2a29_no_live_unknown_agent:${request.agent_name}`);
    }
    return {
      provider: "mock",
  ...
  persistence(): AutonomousTurnPersistence {
    return {
      ...
      completePriorIntervention: async (
        ...
        if (index < 0) throw new Error("e2a29_prior_intervention_missing");
...
async function executeProviderCall<TInput, TOutput>(input: {
  ...
  if (input.live) {
    ...
    if (!checkpoint || currentCommit() !== checkpoint.application_git_commit ||
        !trackedTreeClean()) {
      throw new Error("e2a29_source_integrity_changed_before_dispatch");
    }
    if (sourceIdentity().aggregate_sha256 !== input.frozenSourceHash) {
      throw new Error("e2a29_source_hash_changed_before_dispatch");
    }
    if (compositeRuntimeIdentity().composite_runtime_identity_hash !==
        checkpoint.composite_runtime_identity_hash) {
      throw new Error("e2a29_composite_identity_changed_before_dispatch");
    }
  }
  ...
  if (result.status !== "completed" || !result.parsed_output) {
    ...
    throw new Error(`e2a29_provider_call_failed:${input.role}:${reason}`);
  }
  return result;
}
  ...
  try {
    ...
    for (const turn of input.session.frozen_student_trajectory) {
      if (endpoint) {
        ...
        throw new Error("e2a29_unnecessary_turn_after_endpoint");
      }
      ...
      if (!simulatorResult?.parsed_output || !simulatorValidation.passed) {
        throw new Error(
          `e2a29_student_simulator_validation_failed:${input.session.session_id}:${turn.turn}:${simulatorValidation.issues.join("|")}`
        );
      }
      ...
      const result = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async (autonomousInput) => {
          ...
          const firstAudit = input.live
            ...
            : makeActivityMisconceptionEvidenceAuditForTest({
                agent_call_id: `e2a29_eval_${input.session.session_id}_${turn.turn}`,
          ...
          if (pipeline.status === "rejected") {
            ...
            if (!activityMisconceptionEvidencePipelineIssuesAllowRepair(originalIssues)) {
              throw new Error(
                `e2a29_evaluator_hard_rejection:turn_${turn.turn}:${originalIssues.map((issue) =>
            ...
            const repairAudit = input.live
              ...
              : makeActivityMisconceptionEvidenceAuditForTest({
                  agent_call_id:
                    `e2a29_eval_repair_${input.session.session_id}_${turn.turn}`,
                  model_name: evaluatorModel.model_name
                });
          ...
          if (pipeline.status !== "accepted") {
            throw new Error(`e2a29_evaluator_rejected:${pipeline.blocked_reason}`);
          }
          ...
          if (!effectiveEvaluatorOutput ||
            ...
            throw new Error("e2a29_evaluator_effective_output_mismatch");
          }
        ...
        invokeAutonomousTutor: async (tutorInput, attempt, hardRejections) => {
          ...
          if ((tutorInput.latest_authoritative_turn_profile as
            ...
            throw new Error("e2a29_tutor_called_after_sound");
          }
          if (attempt === 2) {
            sessionRegenerations += 1;
            if (sessionRegenerations > 2) {
              throw new Error("e2a29_session_tutor_regeneration_limit_exceeded");
            }
          }
      ...
      const replay = await executeAutonomousFormativeTurn({
        ...
        evaluateEvidence: async () => {
          throw new Error("e2a29_idempotent_replay_evaluator_called");
        },
        invokeAutonomousTutor: async () => {
          throw new Error("e2a29_idempotent_replay_tutor_called");
        }
      });
      if (!replay.replayed || input.ledger.logical_generation_calls !== callsBeforeReplay) {
        throw new Error("e2a29_idempotency_replay_failed");
      }
      ...
      if (!preTutorFinalizationPassed) {
        throw new Error("e2a29_pre_tutor_profile_finalization_failed");
      }
      ...
      if (explicitOptionReference && anchor.anchor_application !== "explicit") {
        throw new Error("e2a29_anchor_application_false_absent");
      }
      ...
      if (turn.required_anchor_stance &&
        ...
        throw new Error("e2a29_anchor_stance_misclassified");
      }
      if (turn.required_anchor_consistency &&
          anchor.anchor_consistency !== turn.required_anchor_consistency) {
        throw new Error("e2a29_anchor_contradiction_not_structured");
      }
      ...
      if (propagation.blocking &&
        ...
        throw new Error("e2a29_blocking_conflict_only_in_limitations");
      }
      if (turn.turn === 4 && (
      ...
      )) throw new Error("e2a29_anchor_interpretation_failure");
      ...
      )) throw new Error("e2a29_genuine_sound_false_negative");
      if (!humanProfileMatches(
        ...
        throw new Error(
          expectedProfile(turn).includes("sound")
            ? "e2a29_genuine_sound_false_negative"
            : result.latest_profile.reasoning_quality === "sound"
              ? "e2a29_genuine_false_sound"
              : `e2a29_profile_semantically_outside_allowed_envelope:turn_${turn.turn}:${result.latest_profile.reasoning_quality}:${turn.semantic_envelope.join("|")}`
        );
      }
      ...
      if (result.intervention) {
        ...
        if (["misconception_persists", "no_new_evidence", "recurrence"]
          ...
          throw new Error("e2a29_strategy_not_changed_after_ineffective_intervention");
        }
        ...
        if ([2, 3, 4].includes(turn.turn)) {
          ...
          if (prior?.strategy_description ===
              result.intervention.strategy_description) {
            strategyChangeAfterIneffective = false;
            throw new Error("e2a29_strategy_adaptation_failure");
          }
        }
      }
      const visiblePrivacy = privacyAudit(result.effective_message);
      if (!visiblePrivacy.passed) throw new Error(
        `e2a29_privacy_or_safety_failure:${visiblePrivacy.findings.join("|")}`
      );
    ...
    if (!endpoint) throw new Error("e2a29_session_endpoint_not_reached");
    if (endpoint !== input.session.required_endpoint) {
      throw new Error("e2a29_required_revision_endpoint_missing");
    }
    if (store.turns.length > input.session.complete_visible_history_limit) {
      throw new Error("e2a29_complete_visible_history_limit_exceeded");
    }
...
function usageArtifact(ledger: BudgetLedger) {
  ...
  return {
    version: "e2a29-usage-and-cost-v1",
    budget: BUDGET,
    actual,
...
function artifactValidation(runDir: string) {
  ...
  if (burden.metric_version !== "e2a29-failed-session-burden-v1") {
...
async function executeCanary(input: {
  ...
  const frozenProtocol = readJson<FrozenProtocolArtifact>(
    frozenArtifactPath("e2a29-frozen-protocol.json")
  );
  writeJson(path.join(runDir, "canary-manifest.json"), {
    manifest_version: "e2a29-live-canary-manifest-v1",
    run_id: id,
  ...
  writeJson(path.join(runDir, "dispatch-checkpoint.json"),
    checkpoint ?? {
      checkpoint_version: "e2a29-no-live-checkpoint-not-required",
      application_git_commit: currentCommit(),
      composite_runtime_identity_hash:
        identity.composite_runtime_identity_hash
    });
  ...
  writeJson(path.join(runDir, "frozen-protocol.sha256"), {
    ...
    verified: (() => {
      const { protocol_hash: _hash, ...body } = frozenProtocol;
      void _hash;
  ...
  writeJson(path.join(runDir, "session-fixture.json"), {
    fixture_version: "e2a29-electrical-circuits-isolated-fixture-v1",
    synthetic_only: true,
  ...
  try {
    ...
    failure = error instanceof Error ? error.message : "e2a29_unknown_failure";
  }
  ...
  writeJson(path.join(runDir, "evidence-accuracy-metrics.json"), {
    metric_version: "e2a29-evidence-accuracy-v1",
  ...
  writeJson(path.join(runDir, "progression-efficiency-metrics.json"), {
    metric_version: "e2a29-progression-efficiency-v1",
    earliest_genuine_sound_turn: earliestSound,
  ...
  writeJson(path.join(runDir, "pedagogical-adaptation-metrics.json"), {
    metric_version: "e2a29-pedagogical-adaptation-v1",
  ...
  writeJson(path.join(runDir, "student-burden-metrics.json"), {
    metric_version: "e2a29-student-burden-v1",
  ...
  writeJson(path.join(runDir, "workflow-fidelity-metrics.json"), {
    metric_version: "e2a29-workflow-fidelity-v1",
  ...
  writeJson(path.join(runDir, "fixture-cleanup-results.json"), {
    cleanup_version: "e2a29-isolated-fixture-cleanup-v1",
  ...
  writeJson(path.join(runDir, "failure-path-completeness.json"), {
    completeness_version: "e2a29-failure-path-completeness-v1",
  ...
  writeJson(path.join(runDir, "failed-session-burden-metrics.json"), {
    metric_version: "e2a29-failed-session-burden-v1",
  ...
  writeJson(path.join(runDir, "human-review-packet.json"), {
    packet_version: "e2a29-human-review-packet-v1",
    run_id: id,
  ...
  if (!protectedUnchanged && failure === null) {
    failure = "e2a29_protected_evidence_changed_during_execution";
  }
...
async function runSmoke() {
  ...
  globalThis.fetch = (async () => {
    networkRequests += 1;
    throw new Error("e2a29_smoke_network_prohibited");
  }) as typeof fetch;
  const root = path.join(os.tmpdir(), `e2a29-smoke-${randomUUID()}`);
  try {
    const protocol = buildE2A29FrozenProtocol();
    const result = await executeCanary({
      ...
      live: false,
      artifactRoot: root,
      forcedRunId: "e2a29_no_live_smoke"
    });
    if (!result.summary.passed || !result.validation.passed) {
      throw new Error(
        `e2a29_no_live_smoke_failed:${result.summary.failure_reason ?? result.validation.failures.join("|")}`
      );
    }
    if (networkRequests !== 0) throw new Error("e2a29_no_live_network_detected");
    ...
    if (usage.simulator_calls !== 6 || usage.evidence_evaluator_calls !== 6 ||
      ...
      throw new Error("e2a29_no_live_expected_call_arithmetic_failed");
    }
    return {
      status: "passed",
...
function runAuthorizationGuardSmoke() {
  ...
  if (!blockedReason?.startsWith("e2a29_confirmation_missing:")) {
    throw new Error("e2a29_authorization_guard_did_not_fail_closed");
  }
  return {
    status: "passed",
...
async function executeLive() {
  ...
  if (!check.passed) {
    throw new Error(`e2a29_preflight_failed:${check.blockers.join(",")}`);
  }
  ...
  try {
    ...
    if (!credential.ok) throw new Error(`e2a29_credential_failed:${credential.code}`);
...
function report(run?: string) {
  ...
  if (!id) throw new Error("e2a29_run_not_found");
...
function auditRun(run?: string) {
  ...
  return {
    audit_version: "e2a29-post-run-audit-v1",
    run_id: id,
    run_directory: runDir,
...
async function main() {
  ...
  throw new Error(
    "usage: formative-evaluation-e2a29.ts preflight|checkpoint|smoke|authorization-guard-smoke|live|report|audit"
  );
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : "e2a29_runner_failed");
  process.exitCode = 1;
});
