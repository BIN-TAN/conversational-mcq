...
function installIsolatedDatabaseEnvironment() {
  ...
  assert(
    ...
    "The V8 transition smoke must use local PostgreSQL."
  );
...
async function main() {
  ...
  const childTests = [
    ...
    "prisma/formative-conversation-v8-transition-runtime-smoke-test.ts",
    "prisma/operational-formative-conversation-v5-v8-plan-smoke-test.ts",
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v8_transition_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0
    })
  );
