...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18r2/deployed-provenance";
import { compileFormativeConversationV5PreDispatch } from "../src/lib/operational/formative-conversation-v5-evaluation-v18r2/service";
import { installFormativeConversationV5TestEnvironment } from "./helpers/formative-conversation-v5-v18r2-test-environment";

const gitSha = "a".repeat(40);
const hash = (character: string) => character.repeat(64);
const identity: FormativeConversationV18R2DeployedIdentity = {
...
function expectFailure(input: Parameters<typeof verifyFormativeConversationV18R2DeployedProvenance>[0], code: RegExp) {
  assert.throws(
    () => verifyFormativeConversationV18R2DeployedProvenance(input),
    code
  );
}
...
const result = verifyFormativeConversationV18R2DeployedProvenance(passing);
...
const guardedBoundary = (
  ...
  const verified = verifyFormativeConversationV18R2DeployedProvenance(input);
  return {
    verified,
...
assert.equal(
  verifyFormativeConversationV18R2DeployedProvenance(passing).git_invocations,
  0
);
...
for (const field of Object.keys(
  identity
) as Array<keyof FormativeConversationV18R2DeployedIdentity>) {
