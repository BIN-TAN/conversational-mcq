...
const childTests = [
  "prisma/teacher-batch-cleanup-runtime-smoke-test.ts",
  "prisma/teacher-assessment-deletion-smoke-test.ts"
] as const;
...
function installIsolatedEnvironment() {
  ...
  assert(
    ...
    "The batch-cleanup runtime smoke must use local PostgreSQL."
  );
...
async function main() {
  ...
  try {
    ...
    for (const childTest of childTests) {
      const child = spawnSync(process.execPath, ["--import", "tsx", childTest], {
        cwd: process.cwd(),
      ...
      assert.equal(
        child.status,
        0,
        child.error?.message ?? `The batch-cleanup database smoke failed: ${childTest}`
      );
    }
  } finally {
    await dropLiveCanaryDatabaseIfPresent();
...
main().catch((error) => {
  console.error(
    JSON.stringify({
      status: "failed",
      error_code:
        error instanceof Error ? error.message : "teacher_batch_cleanup_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0,
      generation_network_requests: 0,
      dispatch_checkpoints: 0
