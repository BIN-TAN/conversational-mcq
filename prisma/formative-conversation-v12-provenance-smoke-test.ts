...
import {
  ...
  writeFormativeConversationV12RunScopedProvenance
...
function committedSourceFixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "fcv5-v12-provenance-"));
  const sources = [
    "config/operational-candidates/formative-conversation-host-v5-executable-v12/spec.json",
    "src/lib/operational/formative-conversation-v5-evaluation-v12/runtime.ts"
  ];
  ...
  git(root, ["init", "-q"]);
  git(root, ["config", "user.name", "V12 Provenance Test"]);
  git(root, ["config", "user.email", "v12-provenance@example.invalid"]);
  git(root, ["add", "."]);
...
function testCommittedSourceVerification() {
  ...
  try {
    ...
    const verified = verifyFormativeConversationV12CommittedSource({
    ...
    const written = writeFormativeConversationV12RunScopedProvenance({
      workspace_root: fixture.root,
    ...
    assert.throws(
      ...
      /formative_conversation_v12_tracked_source_changed/
    );
  } finally {
...
function testMaterializationIsVerificationOnly() {
  ...
  const v12Before = fileInventory(v12Root);
  ...
  assertInventoryEqual(v12Before, fileInventory(v12Root), "V12 candidate package");
...
function main() {
  ...
  console.log(
    JSON.stringify(
      {
        status: "passed",
        suite: "formative-conversation-v12-provenance",
