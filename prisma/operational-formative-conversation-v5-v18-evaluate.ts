...
} from "../src/lib/operational/formative-conversation-v5-evaluation-v18/service";
import {
...
import {
  ...
  assertFormativeConversationV18LiveControlEnvironment,
  writeFormativeConversationV18ControlPayload
...
function assertCanonicalLauncher(options: Options) {
  if (
    process.env
      ...
      "true" ||
    process.env.FORMATIVE_CONVERSATION_V5_V18_CANONICAL_LOADER_VERSION !==
      "formative-conversation-v18-canonical-node-import-tsx-v1"
  ) {
    throw new Error(
      "formative_conversation_v18_canonical_launcher_required"
    );
  }
  if (
    options.mode === "live" &&
    ...
    throw new Error(
      "formative_conversation_v18_canonical_outer_loader_required"
    );
  }
}
...
async function main(input: {
  ...
  const controlEnvironment =
    await assertFormativeConversationV18LiveControlEnvironment({
  ...
  await writeFormativeConversationV18ControlPayload({
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v18-evaluation-prisma",
