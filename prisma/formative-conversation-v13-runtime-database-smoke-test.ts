...
function installIsolatedDatabaseEnvironment() {
  ...
  assert(
    ...
    "The V13 runtime smoke must use local PostgreSQL."
  );
...
async function main() {
  ...
  const childTests = [
    ...
    "prisma/formative-conversation-v10-database-recovery-smoke-test.ts",
    "prisma/formative-conversation-v10-profile-semantics-smoke-test.ts",
    "prisma/formative-conversation-opening-v3-adversarial-smoke-test.ts",
    ...
    JSON.stringify(
      {
        ...
        opening_acceptance_boundary: "formative-conversation-opening-v3",
        semantic_regeneration_boundary:
          "formative-conversation-semantic-regeneration-policy-v2",
        legacy_non_regenerating_replay_treated_as_acceptable: false,
    ...
    JSON.stringify({
      ...
      error_code:
        error instanceof Error
          ? error.message
          : "formative_conversation_v13_runtime_database_smoke_failed",
      provider_calls: 0,
      model_auth_requests: 0
    })
  );
