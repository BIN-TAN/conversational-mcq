...
import {
  ...
  verifyFormativeConversationV16CommittedSource
} from "../src/lib/operational/formative-conversation-v5-evaluation-v16/provenance";
...
function createCommittedFixture(options?: { include_identity?: boolean }) {
  const root = mkdtempSync(
    path.join(os.tmpdir(), "formative-conversation-v16-provenance-")
  );
  ...
  git(root, ["init", "--quiet"]);
  git(root, ["config", "user.email", "v16-provenance@example.invalid"]);
  git(root, ["config", "user.name", "V16 Provenance Smoke"]);
  git(root, ["add", "freeze"]);
...
function verifyCleanCommittedPackage(input: {
  ...
  const verified = verifyFormativeConversationV16CommittedSource(input);
  ...
  if (workingTreeStatus !== "") {
    throw new Error(
      "formative_conversation_v16_protected_working_tree_not_clean"
    );
  }
  return verified;
}
...
function assertReproducibleMaterialization(before: string, after: string) {
  if (before !== after) {
    throw new Error(
      "formative_conversation_v16_materialization_not_reproducible"
    );
  }
}
...
try {
  ...
  assert.ok(FORMATIVE_CONVERSATION_V16_COMMITTED_SOURCE_PATHS.length >= 8);
  ...
  expectProvenanceFailure(
    ...
    /formative_conversation_v16_tracked_source_changed/
  );
  ...
  expectProvenanceFailure(
    ...
    /formative_conversation_v16_tracked_package_missing/
  );
