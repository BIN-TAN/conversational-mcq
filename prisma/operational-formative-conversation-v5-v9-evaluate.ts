...
type Options = {
  mode:
    ...
    | "preflight"
    | "plan"
    | "remote-database-canary-preflight"
    | "remote-database-canary"
    | "live";
...
function parseOptions(args: string[]): Options {
  ...
  if (
    ...
    mode !== "preflight" &&
    mode !== "plan" &&
    mode !== "remote-database-canary-preflight" &&
    mode !== "remote-database-canary" &&
    mode !== "live"
  ) {
    throw new Error(
...
async function main(input: {
  ...
  if (options.mode === "remote-database-canary-preflight") {
    if (!options.authorization) {
      throw new Error(
        "formative_conversation_v9_remote_database_canary_authorization_required"
      );
    }
    const result =
  ...
  if (options.mode === "remote-database-canary") {
    if (!options.authorization) {
      throw new Error(
        "formative_conversation_v9_remote_database_canary_authorization_required"
      );
    ...
    const result =
      await executeFormativeConversationV9RemoteDatabaseCanary({
        authorization: options.authorization,
        diagnostics: input.diagnostics,
        database_state: databaseOwner.state
      });
    console.log(JSON.stringify(result, null, 2));
    return;
  }
...
const databaseOwner = createEvaluationDatabaseConnectionOwner({
  client: prisma,
  client_id: "formative-conversation-v9-evaluation-prisma",
