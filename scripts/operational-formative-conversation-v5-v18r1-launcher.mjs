...
const LOCAL_PATH_PROJECTION = {
  OPERATIONAL_APPROVAL_BUNDLE_PATH:
    "FORMATIVE_CONVERSATION_V5_V18R1_LOCAL_APPROVAL_BUNDLE_PATH",
  OPERATIONAL_APPROVED_MANIFEST_PATH:
    "FORMATIVE_CONVERSATION_V5_V18R1_LOCAL_APPROVED_MANIFEST_PATH",
  OPERATIONAL_APPROVAL_EVIDENCE_PATH:
    "FORMATIVE_CONVERSATION_V5_V18R1_LOCAL_APPROVAL_EVIDENCE_PATH"
};
...
const LOCAL_DATABASE_URL =
  "FORMATIVE_CONVERSATION_V5_V18R1_LOCAL_DATABASE_URL";
...
function assertCanonicalLoader() {
  if (!canonicalLoaderActive()) {
    throw new Error(
      "formative_conversation_v18r1_canonical_loader_required"
    );
  }
}
...
function childEnvironment() {
  ...
  if (projectionEntries.length > 0) {
    ...
    child.FORMATIVE_CONVERSATION_V5_V18R1_APPROVAL_PATH_PROJECTION_USED =
      "true";
    child.FORMATIVE_CONVERSATION_V5_V18R1_ENVIRONMENT_SOURCE ??=
  ...
  if (configured(process.env[LOCAL_DATABASE_URL])) {
    ...
    child.FORMATIVE_CONVERSATION_V5_V18R1_DATABASE_CONNECTION_SOURCE =
      "render_external_process_local";
    child.FORMATIVE_CONVERSATION_V5_V18R1_DATABASE_IDENTITY_MATCHED =
      "true";
  ...
  child.FORMATIVE_CONVERSATION_V5_V18R1_LAUNCHER_VERSION =
    LAUNCHER_VERSION;
  child.FORMATIVE_CONVERSATION_V5_V18R1_LAUNCH_MECHANISM =
    LAUNCH_MECHANISM;
  child.FORMATIVE_CONVERSATION_V5_V18R1_CANONICAL_LAUNCHER_VALIDATED =
    "true";
  child.FORMATIVE_CONVERSATION_V5_V18R1_CANONICAL_LOADER_VERSION =
    CANONICAL_LOADER_VERSION;
  for (const name of [
    "FORMATIVE_CONVERSATION_V5_V18R1_ENVIRONMENT_SOURCE",
    "FORMATIVE_CONVERSATION_V5_V18R1_APPROVAL_PATH_PROJECTION_USED",
    ...
    "FORMATIVE_CONVERSATION_V5_V18R1_CANONICAL_OUTER_LOADER_VERSION",
    "FORMATIVE_CONVERSATION_V5_V18R1_CANONICAL_OUTER_LOADER_MECHANISM",
    "FORMATIVE_CONVERSATION_V5_V18R1_CANONICAL_OUTER_LOADER_VALIDATED"
  ]) {
    if (configured(child[name])) {
      injectedKeys.add(name);
    }
  }
  child.FORMATIVE_CONVERSATION_V5_V18R1_INJECTED_ENVIRONMENT_KEYS =
...
function main() {
  ...
  try {
    ...
    const modeArgument = process.argv.slice(2).find((argument) =>
      argument.startsWith("--mode=")
    );
    const mode = modeArgument?.slice("--mode=".length) ?? "plan";
    if (mode === "live" || mode === "preflight") {
      const rawArguments = process.argv.slice(2);
      const inlineExpectedArguments = rawArguments.filter((argument) =>
        argument.startsWith("--expected-deployed-git-sha=")
      );
      const separatedExpectedIndexes = rawArguments
        .map((argument, index) =>
          argument === "--expected-deployed-git-sha" ? index : -1
        )
        .filter((index) => index >= 0);
      const expectedArgumentCount =
        inlineExpectedArguments.length + separatedExpectedIndexes.length;
      if (expectedArgumentCount === 0) {
        throw new Error(
          "formative_conversation_v18r1_expected_git_sha_missing"
        );
      }
      if (separatedExpectedIndexes.length > 0) {
        throw new Error(
          "formative_conversation_v18r1_expected_git_sha_malformed"
        );
      }
      if (expectedArgumentCount !== 1) {
        throw new Error(
          "formative_conversation_v18r1_expected_git_sha_duplicate"
        );
      }
      const expectedSha = inlineExpectedArguments[0].slice(
        "--expected-deployed-git-sha=".length
      );
