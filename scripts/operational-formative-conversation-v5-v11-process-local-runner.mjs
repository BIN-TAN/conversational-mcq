...
import {
  ...
  scanFormativeConversationV11BufferedOutputs,
  writeFormativeConversationV11SafeFailureRecord
} from "../src/lib/operational/formative-conversation-v5-evaluation-v11/security-release.ts";
...
function parsedOptions(args) {
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v11_process_local_arguments_invalid"
    );
  }
  ...
  if (modeArguments.length !== 1) {
    throw new Error(
      "formative_conversation_v11_process_local_mode_required"
    );
  }
  const mode = modeArguments[0].slice("--mode=".length);
  if (mode !== "live" && mode !== "environment-preflight") {
    throw new Error(
      "formative_conversation_v11_process_local_mode_not_permitted"
    );
  }
  return {
...
function readEnvironmentOnce(fifoPath) {
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v11_environment_fifo_invalid"
    );
  }
  let raw = readFileSync(fifoPath, "utf8");
  if (Buffer.byteLength(raw, "utf8") > MAX_ENVIRONMENT_BYTES) {
    throw new Error(
      "formative_conversation_v11_environment_payload_too_large"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v11_environment_payload_invalid"
    );
  }
  return parsed;
}
...
function safeErrorCode(error) {
  const code =
    error instanceof Error
      ...
      : "formative_conversation_v11_process_local_runner_failed";
...
async function run() {
  ...
  const recordPreReleaseFailure = async (failureCode) => {
    ...
    return writeFormativeConversationV11SafeFailureRecord({
  ...
  try {
    ...
    controlChannel = await createFormativeConversationV11ControlChannel();
    ...
    childEnvironment = {
      ...safeHostEnvironment(),
      ...injectedEnvironment,
      FORMATIVE_CONVERSATION_V5_V11_CONTROL_PATH:
        controlChannel.control_path,
      FORMATIVE_CONVERSATION_V5_V11_CONTROL_NONCE:
        controlChannel.control_nonce,
      FORMATIVE_CONVERSATION_V5_V11_STAGING_BASE_ROOT: stagingBase
    };

    const child = spawn(
      process.execPath,
      [path.join(workspaceRoot, LAUNCHER), ...options.launcher_args],
      {
        cwd: workspaceRoot,
    ...
    if (result.code !== 0 || result.signal) {
      let failureCode =
        childErrorCode ??
        "formative_conversation_v11_process_local_child_failed";
      try {
        scanFormativeConversationV11BufferedOutputs({
    ...
    let control;
    try {
      control = await readFormativeConversationV11ControlPayload({
        control_path: controlChannel.control_path,
    ...
    const release = await releaseFormativeConversationV11Artifacts({
      workspace_root: workspaceRoot,
      control,
      exact_secret_values: secrets,
      buffered_outputs: [stdout, stderr],
    ...
    if (!secretsCleared) {
      throw new Error(
        "formative_conversation_v11_process_local_secret_clear_missing"
      );
    }
    await cleanupChannels();
    process.stdout.write(
