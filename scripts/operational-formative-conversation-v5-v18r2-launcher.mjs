...
function assertCanonicalLoader() {
  if (!canonicalLoaderActive()) {
    throw new Error(
      "formative_conversation_v18r2_canonical_loader_required"
    );
  }
}
...
function childEnvironment() {
  ...
  if (configured(process.env[LOCAL_DATABASE_URL])) {
    ...
    child.FORMATIVE_CONVERSATION_V5_V18R2_DATABASE_IDENTITY_MATCHED =
      "true";
  ...
  child.FORMATIVE_CONVERSATION_V5_V18R2_LAUNCHER_VERSION =
    LAUNCHER_VERSION;
  child.FORMATIVE_CONVERSATION_V5_V18R2_LAUNCH_MECHANISM =
    LAUNCH_MECHANISM;
  child.FORMATIVE_CONVERSATION_V5_V18R2_CANONICAL_LAUNCHER_VALIDATED =
    "true";
  child.FORMATIVE_CONVERSATION_V5_V18R2_CANONICAL_LOADER_VERSION =
    CANONICAL_LOADER_VERSION;
  for (const name of [
    "FORMATIVE_CONVERSATION_V5_V18R2_ENVIRONMENT_SOURCE",
    "FORMATIVE_CONVERSATION_V5_V18R2_APPROVAL_PATH_PROJECTION_USED",
    ...
    "FORMATIVE_CONVERSATION_V5_V18R2_CANONICAL_OUTER_LOADER_VALIDATED"
  ]) {
  ...
  try {
    ...
    if (mode === "live" || mode === "preflight") {
      ...
      if (expectedArgumentCount === 0) {
        throw new Error(
          "formative_conversation_v18r2_expected_git_sha_missing"
        );
      }
      if (separatedExpectedIndexes.length > 0) {
        throw new Error(
          "formative_conversation_v18r2_expected_git_sha_malformed"
        );
      }
      if (expectedArgumentCount !== 1) {
        throw new Error(
          "formative_conversation_v18r2_expected_git_sha_duplicate"
        );
      }
