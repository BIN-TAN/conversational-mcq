...
function parsedOptions(args) {
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v16_process_local_arguments_invalid"
    );
  }
  ...
  if (modeArguments.length !== 1) {
    throw new Error(
      "formative_conversation_v16_process_local_mode_required"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v16_process_local_mode_not_permitted"
    );
  }
  return {
...
function readEnvironmentOnce(fifoPath) {
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v16_environment_fifo_invalid"
    );
  }
  let raw = readFileSync(fifoPath, "utf8");
  if (Buffer.byteLength(raw, "utf8") > MAX_ENVIRONMENT_BYTES) {
    throw new Error(
      "formative_conversation_v16_environment_payload_too_large"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "formative_conversation_v16_environment_payload_invalid"
    );
  }
  return parsed;
}
...
function safeErrorCode(error) {
  const code =
    error instanceof Error
      ...
      : "formative_conversation_v16_process_local_runner_failed";
...
async function run() {
  ...
  const {
    createFormativeConversationV16ControlChannel,
    FORMATIVE_CONVERSATION_V16_DATA_ROOT,
    FORMATIVE_CONVERSATION_V16_STAGING_ROOT,
    readFormativeConversationV16ControlPayload,
    releaseFormativeConversationV16Artifacts,
    removeFormativeConversationV16ControlChannel,
    scanFormativeConversationV16BufferedOutputs,
    writeFormativeConversationV16SafeFailureRecord
  } = await import(
  ...
  const cleanupChannels = async () => {
    ...
    if (controlChannel) {
      await removeFormativeConversationV16ControlChannel({
  ...
  const recordPreReleaseFailure = async (failureCode) => {
    ...
    return writeFormativeConversationV16SafeFailureRecord({
  ...
  try {
    ...
    controlChannel = await createFormativeConversationV16ControlChannel();
    ...
    childEnvironment = {
      ...
      FORMATIVE_CONVERSATION_V5_V16_CANONICAL_OUTER_LOADER_VALIDATED:
        "true",
      FORMATIVE_CONVERSATION_V5_V16_CONTROL_PATH:
        controlChannel.control_path,
      FORMATIVE_CONVERSATION_V5_V16_CONTROL_NONCE:
        controlChannel.control_nonce,
      FORMATIVE_CONVERSATION_V5_V16_STAGING_BASE_ROOT: stagingBase
    };
    ...
    if (result.code !== 0 || result.signal) {
      let failureCode =
        childErrorCode ??
        "formative_conversation_v16_process_local_child_failed";
      try {
        scanFormativeConversationV16BufferedOutputs({
    ...
    let control;
    try {
      control = await readFormativeConversationV16ControlPayload({
        control_path: controlChannel.control_path,
    ...
    const release = await releaseFormativeConversationV16Artifacts({
      workspace_root: workspaceRoot,
      control,
      exact_secret_values: secrets,
      buffered_outputs: [stdout, stderr],
    ...
    if (!secretsCleared) {
      throw new Error(
        "formative_conversation_v16_process_local_secret_clear_missing"
      );
    }
    await cleanupChannels();
    process.stdout.write(
